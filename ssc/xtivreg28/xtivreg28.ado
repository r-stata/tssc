...
* 01.0.14 - Version specifically for ivreg28; prompted by ivreg28 use of partial rather than fwl.

program define xtivreg28, eclass byable(recall)
	version 8.2
	local lversion 01.0.14
	...
	if replay() {
		...
		if _rc != 0 {
di as err "Error - must have ivreg28 version 2.1.15 or greater installed"
				exit 601
		}
		...
		syntax [anything(name=0)] [if] [in] [aw fw pw iw/] , [ fe fd /*
			...
			*/	orthog(string) ENDOGtest(string) REDundant(string) FWL(string) /*
			*/	BW(string) SKIPCOLL * ]
...
* Do this even though ivreg2 currently won't accept TS operators with fwl()
			foreach vn of local fwl {
				local fwl_fd "`fwl_fd' d.`vn'"
			}
			...
			`ivreg2_cmd' `lhs_fd' `inexog_fd' (`endo_fd' = `exexog_fd') `wtexp' `if', /*
				...
				*/	 redundant(`redundant_fd') fwl(`fwl_fd') tvar(`tvar') bw(`bw') `options'
			preserve
			tempvar touse
...
* This guarantees that the demeaned variables are doubles
				...
				`ivreg2_cmd' `lhs_t' `inexog_t' (`endo_t' = `exexog_t') `wtexp' if `touse', /*
					...
					*/	redundant(`redundant_t') fwl(`fwl_t') tvar(`tvar') bw(`bw') `options'
...
* Macro varlists
				...
				local fwl     : subinstr local fwl       "`vn_t'" "`vn'"
				...
				local fwl1    : subinstr local fwl1      "`vn_t'" "`vn'"
			}
			mat colnames `b'       =`cnames'
			mat colnames `V'       =`cnames'
			mat rownames `V'       =`cnames'
