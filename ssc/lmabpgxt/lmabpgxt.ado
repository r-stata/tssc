...
tempvar D DE DF1 DW E TimeN EE Eo Es Ev Ew ht idv itv LE
tempvar LEo P Q SBB Sig2 SSE SST Time U U2 Ue wald weit Wi Wio WS X X0 Bo
tempvar Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z D DE DF DF1
tempvar E EE Eo LDE LE LEo P Q SSE Time U U2 Ue wald weit Wi Wio X X0 Yb Yh
tempname A B b Beta Bx Cov Cov1 D den DF Dim Dx E E1 Yh2 Yhb YY Z
tempname Ew F In IPhi J K kb kx llf N NC Nn NT P Phi Yh Yh_ML YYm YYv
tempname Pm Q Rho Sig2 Sig21 Sig2b Sig2o Sig2o1 SSE SSEo
tempname SST1 SST2 Ue Ue_ML v2 vh W W1 W2 Wald waldm We Wi V Wi1 Wio WS X X0 Y
qui xtset `id' `it'
...
local NC2=r(imax)
qui tsset `Time'
tempname Rho lmabpgk1 lmabpgk1p lmabpgd lmabpgdp lmabpgk lmabpgkp SSEu SSE SSE1 
qui tsset `Time'
tempvar Ue_ML1
qui gen `Ue_ML1'=L1.`Ue_ML' if `touse'
qui replace `Ue_ML1' = 0 in 1
tempvar EE1 SSE SSE1 E E2 En Obs E E2 
tempname E B EE1 SSE SSE1 Ro1 Ro2
local icc `id'
local i=`id'+1 
...
tempvar SSR1`i' SSR2`i' SSW1`i' SSW2`i'
 ...
 gen `ELd1'`i'=`EAL`i'' if `touse'
 gen `ELk1'`i'=`EAL`i'' if `touse' 
...
matrix `Etd' = vec(`Etd')
matrix `Etk' = vec(`Etk')
qui svmat `Etd' , name(`Etd')
qui svmat `Etk' , name(`Etk')
qui rename `Etd'1 `Etd'
qui rename `Etk'1 `Etk'
qui regress `Ue_ML' `Ue_ML1' `xvar' if `touse' , `noconstant'
scalar `lmabpgk1'=sqrt(e(N)*e(r2))
scalar `lmabpgk1p'=2*(1-normal(abs(`lmabpgk1')))
qui regress `Ue_ML' `Etd' `xvar' if `touse' , `noconstant'
scalar `lmabpgd'=sqrt(e(N)*e(r2))
scalar `lmabpgdp'=2*(1-normal(abs(`lmabpgd')))
qui regress `Ue_ML' `Etk' `xvar' if `touse' , `noconstant'
scalar `lmabpgk'=sqrt(e(N)*e(r2))
scalar `lmabpgkp'=2*(1-normal(abs(`lmabpgk')))
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Autocorrelation Breusch-Pagan-Godfrey Test}}"
di _dup(78) "{bf:{err:=}}"
di as txt _col(2) "{bf: Ho: No AR(1) Panel AutoCorrelation - Ha: AR(1) Panel AutoCorrelation}"
di
di as txt "- Panel Rho Value" _col(43) "=" as res %9.4f `Rho'
di as txt "- Breusch-Pagan-Godfrey Z (keep 1 nt obs)" _col(43) "=" as res %9.4f `lmabpgk1' _col(55) as txt "P-Value > Z(0,1)" _col(73) as res %5.4f `lmabpgk1p'
di as txt "- Breusch-Pagan-Godfrey Z (drop 1 cs obs)" _col(43) "=" as res %9.4f `lmabpgd' _col(55) as txt "P-Value > Z(0,1)" _col(73) as res %5.4f `lmabpgdp'
di as txt "- Breusch-Pagan-Godfrey Z (keep 1 cs obs)" _col(43) "=" as res %9.4f `lmabpgk' _col(55) as txt "P-Value > Z(0,1)" _col(73) as res %5.4f `lmabpgkp'
di _dup(78) "-"
