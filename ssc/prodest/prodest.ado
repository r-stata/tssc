...
*! authors Gabriele Rovigatti, University of Chicago Booth, Chicago, IL & EIEF, Rome, Italy. mailto: gabriele.rovigatti@gmail.com  |  grovigatti@chicagobooth.edu
...
program define prodest, sortpreserve eclass
	...
	syntax varlist(numeric min=1 max=1) [if] [in], free(varlist numeric min=1) /*
		  ...
		  */ VERbose ATTrition ACF INIT(string) FSRESiduals(name min=1 max=1) EVALuator(string) TRANSlog OVERidentification]  
			...
			if /*("`method'" == "wrdg" | "`method'" == "mr")*/ inlist("`method'","wrdg","mr","rob"){
				...
				if "`method'" == "wrdg"{ /* WRDG */
					if !mi("`overidentification'") | !mi("`gmm'") { // in case of overidentification, or in case the user specifies a preference for GMM, estimate a system GMM model, else go with the linear IV model
						...
						tempfile wrdg
						qui save `wrdg'
							qui reg `depvar' `state' `proxyGO' `interactionvars' `free' `lagfreeVars' // just to take the number of observations used in the estimation
							loc realObs = `e(N)'
							qui expand 2, gen(cons2) // double the dataset in order to stack dependent and regressors
							foreach var of local interactionvars{
								qui replace `var' = l_gen`var' if cons2 == 1 // change interactionvars to lagged varlues
							}
							qui ivregress gmm `depvar' `state' `proxyGO' `interactionvars' (`free' = `lagfreeVars') cons2, wmatrix(unadjusted) c // run the IV regression 
					...
					qui ivregress gmm `depvar' `state' `proxyGO' `control' `lagInteractionvars' (`free' = `lagfreeVars'), vce(cluster `id') 
					/// save locals for Hanses's J and its p-value
					loc hans_j: di %3.2f `e(J)'
					mat cV = colsof(e(V)) // find the number of instruments
					scalar cV = cV[1,1]
