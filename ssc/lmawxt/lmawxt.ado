...
tempvar D DE DF1 E TimeN EE Eo ht idv itv LE LEo P Q Sig2 SSE SST WS Bo
tempvar Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z D DE DF DF1
tempvar E E2 E3 E4 EE Eo LDE LE LEo P Q Sig2 SRho SSE
tempvar Time U U2 Ue wald weit Wi Wio X X0 Yb Yh Yh2 Yhb YY Z 
tempname A B b B1 b1 b2 BB2 Beta Bx Cov Cov1 D den DF Dim Dx E E1 Eo 
tempname F In IPhi J K kb kx llf N NC Nn NT P Phi V Wi1 Wio WS X X0 Y Yh Yh_ML
tempname Pm Q Rho Sig2 Sig21 Sig2o Sig2o1 SSE SSEo YYm YYv Z
...
qui tsset `Time'
tempvar UD
qui regress d.(`yvar') d.(`xvar')  if `touse' , noconstant cluster(`idv')
qui predict double `UD' if `touse' , res
qui regress `UD' L.`UD' if `touse' , noconstant cluster(`idv')
qui test L.`UD'
tempname lmawold1 lmawold1p lmawold2 lmawold2p woldf1 woldf2 Rho SSEu SSE SSE1
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Autocorrelation Wooldridge Test}}"
di _dup(78) "{bf:{err:=}}"
...
di _dup(78) "-"
di as txt "- Wooldridge  F Test" _col(43) "=" as res %9.4f `lmawold2' _col(55) as txt "P-Value > F(" `woldf1' ", " `woldf2' ")" _col(73) as res %5.4f `lmawold2p'
di as txt "- Wooldridge LM Test" _col(43) "=" as res %9.4f `lmawold1' _col(55) as txt "P-Value > Chi2(1)" _col(73) as res %5.4f `lmawold1p'
di _dup(78) "-"
