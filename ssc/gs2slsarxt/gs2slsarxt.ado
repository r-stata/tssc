...
program define gs2slsarxt, eclass 
 version 11.0
syntax varlist , nc(str) [WMFile(str) ORDer(int 1) tolog INV diag ///
 vce(passthru) stand MFX(str) INV2 LMHet LMSPac BE FE RE gmm(int 1) ///
 zero coll level(passthru) PREDict(str) RESid(str) TESTs NOCONStant LMNorm]
...
tempname Xb A B b Beta BetaSP Bx Sig21 Dim wmat xAx xAx2 TRa2
tempname Cov D den DVE DVNE Dx E E1 F wWw1 xBx Xo Sig2o
tempname h Hat IPhi J K M Q IN xMx Sig2 Sig2b
tempname SSE SST1 SST2 In WXb N DF kx kb WS D WW eVec eigw P Phi Pm
tempname Vec vh VM VP Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V NC NT mh Nmiss
tempname Y Yh Yi YYm YYv Z Z0 Z1 Zo X0 Sw Ue Sig2n kb llf
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
_rmcoll `vlistlog' , `coll' forcedrop
...
matrix `In'=I(`N')
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Generalized Spatial Panel Autoregressive Two Stage Least Squares (GS2SLSAR)}}"
di _dup(78) "{bf:{err:=}}"
ereturn scalar kb=`kb'
ereturn scalar DF=`DF'
ereturn scalar Nn=`N'
 gs2slsxt `yvar' `xvar' , nc(`nc') gmm(`gmm') `vce' order(`order') `coll' `be' `fe' `re' 
...
di as txt "{bf:* Xi = RHS Exogenous Vars:}"_col(33) " " `kx'-1 " = " "`xvar'"
...
program define gs2slsxt, eclass 
 version 11.0
syntax varlist , [nc(str) gmm(int 1) coll vce(passthru) ORDer(int 1) be fe re]
 tempvar `varlist'
 gettoken yvar xvar : varlist
tempvar Time tm idv itv
 qui {
qui cap count 
local N = r(N)
local NC=`nc'
...
local zvar w1x_* w2x_* w3x_* w4x_* 
 ...
 if "`coll'"=="" {
qui _rmcoll `varlist' , `noconstant' `coll' forcedrop
 local varlist "`r(varlist)'"
gettoken yvar xvar : varlist
qui _rmcoll `zvar' , `noconstant' `coll' forcedrop
local zvar "`r(varlist)'"
}

qui xtset `idv' `itv'
qui xtivreg `yvar' `xvar' (w1y_`yvar' = `zvar' ) , small `vce' `be' `fe' `re'  
matrix `BetaSP'=e(b)
matrix `Bx'=`BetaSP'[1,1..`kb']
qui predict `Yh_ML' , xb
qui gen `res' =`yvar'-`Yh_ML'  
qui gen `res2'=`res'^2
...
rename w`i'x_`var'1 w`i'x_`var'
 ...
 if "`order'"=="1" {
local zvar `xvar' w1x_* 
qui cap drop w2x_*
qui cap drop w3x_*
qui cap drop w4x_*
 }
 if "`order'"=="2" {
local zvar `xvar' w1x_* w2x_* 
qui cap drop w3x_*
qui cap drop w4x_*
 }
 if "`order'"=="3" {
local zvar `xvar' w1x_* w2x_* w3x_* 
qui cap drop w4x_*
 }
 if "`order'"=="4" {
local zvar `xvar' w1x_* w2x_* w3x_* w4x_* 
 }
 if "`coll'"=="" { 
qui _rmcoll `zvar' , `noconstant' `coll' forcedrop
local zvar "`r(varlist)'"
...
if `L'==1 {
di as txt "{bf:*** Initial GMM Model: 1 }"
 }
if `L'==2 {
di as txt "{bf:*** Partial Weighted GMM Model: 2 }"
 }
if `L'==3 {
di as txt "{bf:*** Full Weighted GMM Model: 3 }"
 }
qui xtset `idv' `itv'
qui xtivreg `yvar' `xvar' (w1y_`yvar' = `zvar' ) , small `vce' `be' `fe' `re' 
restore
