...
version 11.0
syntax varlist [aw] , nc(str) WMFile(str) [NWmat(int 1) LL(real 0) ///
 vce(passthru) diag INV stand MFX(str) NOLog INV2 LMHet LMSPac ROBust ///
 iter(int 100) zero coll level(passthru) PREDict(str) RESid(str) ///
 tech(str) tobit tolog TESTs DN NOCONStant LMNorm dist(str)]
...
tempvar Bw D DE DF1 DumE E e dcs XQX_ EE Eo Es Ev Hat ht idv itv LE LEo P Q
tempvar SST Time tm U U2 Ue wald weit Wi Wio WS X X0 eigw Bo
tempvar Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z SBB Sig2 SSE
tempname Xb A B Beta BetaSP Bx h Hat idv itv IPhi J K M
tempname Cov Cov1 Cov2s CovC D den DVE DVNE Dx E E1 F
tempname P Phi Pm Q IN xMx v V1 v1 V1s v2 In IRW IRWR WXb
tempname S11 S12 Sig2 Sig2o SSE SST1 SST2 Sig2n Rostar rRo
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V kb xBx
tempname Y Yh Yi YYm YYv Z X0 Ue WS D WW eVec eigw Xo mh olsin
...
local initopt init(`olsin', copy) 
 ml model lf spmstarxt01 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy)
 ml model lf spmstarxt02 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf spmstarxt03 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy)  
 ml model lf spmstarxt04 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR2e"
 matrix `olsin'=`Bo',0,0
local initopt init(`olsin', copy)
 ml model lf spmstarxt05 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
...
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstarxt06 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy) 
 ml model lf spmstarxt07 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)
 ml model lf spmstarxt08 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstarxt09 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy) 
 ml model lf spmstarxt10 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)
 ml model lf spmstarxt11 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstarxt12 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin ///
 `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
