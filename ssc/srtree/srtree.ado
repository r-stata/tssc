********************************************************************************
*! SRTREE: Stata command to perform a "regression tree using R called into Stata" 
********************************************************************************
* Version: 18
* Date: 06-05-2020
* Author: Giovanni Cerulli
********************************************************************************
program define srtree
...
********************************************************************************
	 ...
	 else if "`model'" == "boosting" & "`prediction'" == ""{
...
quietly: file write rcode ///
	...
	`"yhat=predict(tree.data)"' _newline ///
	...
	`"yhat_test=predict(tree.data,data.test)"' _newline ///
...
quietly: file write rcode ///
	...
	///	
	`"yhat=predict(prune.data)"' _newline ///
	`"y_train=y"' _newline ///
	`"yhat_train=yhat"' _newline ///
...
quietly: file write rcode ///
	...
	`"# Fit a Regression Tree over the whole training sample"' _newline ///
	`"tree.data=tree(y~.,data)"' _newline ///
	`"yhat_train=predict(tree.data)"' _newline ///
	`"# Cross-validation to find the optimal tree size"' _newline ///
	`"set.seed(`S')"' _newline ///
	`"cv.data=cv.tree(tree.data)"' _newline ///
	...
	`"prune.data.opt=prune.tree(tree.data,best=best.leaves)"' _newline ///
	...
	`"yhat_opt=predict(prune.data.opt)"' _newline ///
	`"data2 <- as.data.frame(cbind(yhat_train,yhat_opt,y,best.leaves,Test_MSE))"' _newline ///
	`"write.dta(data2, "`out_samp_data'.dta")"'
quietly: file close rcode
end
********************************************************************************
...
quietly: file write rcode ///
	...
	///
	`"set.seed(`S')"' _newline ///
	`"boost.data=gbm(y~.,data=data,distribution="gaussian",n.trees=`B',interaction.depth=`A',shrinkage=`C',verbose=F)"' _newline ///
	`"yhat.boost=predict(boost.data,n.trees=`B')"' _newline ///
	`"summary(boost.data)"' _newline ///
	///
	`"# Train-MSE "' _newline ///
	///
	`"# The training-MSE is:"' _newline ///
	`"mean((yhat.boost-y)^2)"' _newline ///
	`"y_train=y"' _newline ///
	`"yhat_train=yhat.boost"' _newline ///
		///
	`"data1 <- as.data.frame(cbind(y_train,yhat_train))"' _newline ///
    `"write.dta(data1,"`in_samp_data'.dta")"' _newline ///
	`"plot(y_train,yhat_train)"' _newline ///
	`"abline(0,1)"' _newline ///
	`"title(main="Actual Vs. Prediction on the training dataset")"' _newline ///
	///
	...
	`"boost.data=gbm(y~.,data=data[train,],distribution="gaussian",n.trees=`B',interaction.depth=`A',shrinkage=`C',verbose=F)"' _newline ///
	///
	`"# Partial dependent plot"' _newline ///
	///
	`"plot(boost.data,i="`D'")"' _newline ///
	`"title(main="Partial dependent plot for variable `D'")"' _newline ///
    `"yhat.boost=predict(boost.data,newdata=data[-train,],n.trees=`B')"' _newline ///
	`"# The testing-MSE is:"' _newline ///
    `"mean((yhat.boost-data.test)^2)"' _newline ///	
	`"y_test=data.test"' _newline ///	
	`"yhat_test=yhat.boost"' _newline ///	
	`"data2 <- as.data.frame(cbind(yhat_test,y_test))"' _newline ///
    `"write.dta(data2,"`out_samp_data'.dta")"' _newline ///
	`"plot(yhat_test,y_test)"' _newline ///
	`"abline(0,1)"' _newline ///
	`"title(main="Actual Vs. Prediction on the testing dataset")"'
quietly: file close rcode
end
********************************************************************************
...
quietly: file write rcode ///
	...
	`"yhat_new=predict(prune.data,my_data_new)"' _newline ///
...
quietly: file write rcode ///
	...
	`"yhat_new=predict(bag.data,data_new)"' _newline ///
	`"data1 <- as.data.frame(cbind(yhat_new))"' _newline ///
	`"write.dta(data1,"`out_samp_data'.dta")"'
quietly: file close rcode
end
...
quietly: file write rcode ///
    ...
    `"data_new <-cbind(y,x)"' _newline ///
	`"attach(data_new)"' _newline ///
	`"yhat_new = predict(boost.data,data_new,n.trees=`B')"' _newline ///		
	`"data1 <- as.data.frame(cbind(yhat_new))"' _newline ///
	`"write.dta(data1,"`out_samp_data'.dta")"'
quietly: file close rcode
end
