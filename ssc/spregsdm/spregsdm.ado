...
program define spregsdm , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsdm" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) [aux(str) INV INV2 ///
 ...
 tobit LL(real 0) tolog TESTs NOCONStant MHET(str) diag iter(int 100)]
gettoken yvar xvar : varlist
local sthlp spregsdm
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ EE Eo Es Ev Ew Hat ht LE LEo LYh2
tempvar SST Time U U2 wald weit Wi Wio WS X X0 P Q Sig2 SSE
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE Dx E E1 kz Sig21 sigox Dim
tempname IPhi J K M mh olsin Sig2n P Phi Pm Q In IRW IRWR kbm F N DF kx kb
tempname Sig2o Sn SSE SST1 SST2 D WW eVec eigw Xo WY X XB V Nmiss
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio Y Yh Yi YYm YYv Z
tempname minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
tempname waldx waldxp waldx_df waldj waldjp waldj_df Dim X0 llf rRo kb
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
local initopt init(`olsin', copy) 
 ml model lf spregsdm1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "SDM1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy)  
ml model lf spregsdm2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) (Rho:) `wgt' , ///
...
local MName "SDM1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf spregsdm3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
matrix `olsin'=`Bo',`olshet',0,`rmse'
 local initopt init(`olsin', copy) 
 ml model lf spregsdm4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
