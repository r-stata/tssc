...
{hline}
help for {hi:cureregr} (nov2013) {right:(see also:  {hi:[R] stset})}
{hline}

{title:cureregr}, cure model regression (split-population model) or, 
{title:parametric-cure model (PCM)}

{p 8 17}{cmd:cureregr} [{it:varlist}] [{cmd:if} {it:exp}] [{cmd:in} {it:range}] 
{it:,}
         [{cmdab:dist:ribution_option(}{it:weibull | lognormal | logistic | 
		 gamma | exponential |
		 lognv, lognormalv, normallnt | 
		 lognv1, lognormalv1, normallntv1 | 
		 logns1, lognormals1, normallnts1
		 }{cmd:)}
...
{p 8 17}{cmd:note} {it:varlist}, {cmdab:sc:ale_option(}{it:varlist}{cmd:)},
and {cmdab:sh:ape_option(}{it:varlist}{cmd:)} may contain factor variables; see 
{hi:fvvarlist}. stepwise is allowed; see {hi:prefix}.
          
{title:Description}

{p 4 4}{cmd:cureregr} fits a parametric cure model in either the non-mixture or mixture class.{p_end}
{p 8 8} important -- {cmd:cureregr} requires that the data be {cmd:stset} prior 
to use. {cmd:cureregr} will correctly estimate multiple records per subject, to 
include time-varying-covariates. Please note that for multiple records per 
subject the records must be in contiguous intervals defined in {cmd:stset}, 
[_t0, _t]; in other words, there may be no gaps (see: {cmd:stdes}).
{p_end}

{title:Options}

{p 4 8}{cmdab:dist:ribution_option(}{it:weibull | lognormal | logistic | gamma | exponential | lognv | lognv1 | logns1}{cmd:)}{p_end}
{p 8 12} {it:define Kernel:}{p_end}
{p 12 16} weibull: exponential in tt {p_end}
{p 12 16} lognormal: normal(ln(tt)) {p_end}
...
{p 12 16} lognv: lognormalv,  normal(ln(t)) fit variance {p_end}
{p 12 16} lognv1: lognormalv1, normal(ln(t)) variance=1 {p_end}
{p 12 16} logns1: lognormals1, normal(ln(scale*t)) shape=1 {p_end}
...
(or {res}overwrites{txt}) a variable called {it:{res}_leftd{txt}} and is 
indicated 
in e(depvar).{p_end}
{p 4 8} -- {p_end}
...
{p 8 12}{inp:. xi: cureregr  g0_ageyr i.g0_wbc i.g0_plt, sc(i.g0_wbc i.g0_plt) 
sh(i.g0_plt) dist(lognormal) class(non) link(logi) scale_noc shape_noc}{p_end}
...
number of covariate patterns displayed; An example can be found in the ancillary
file, cureregr_predict_eg.pdf. The {cmd:fmt()} option allows any acceptable 
numeric format specification for the displayed numeric results. Please note also
that the confidence limits are based on a ln(-ln(S)) transform. {p_end}
...
{p 8 12}{inp:. predict if study_r==1 & g_liver==1 & race2==2, 
at(30(30)365 2922)}{p_end}
{p 8 12}{inp:. predict ,  all gen(s_v2 se_v2 lci90_v2 uci90_v2 haz_v2 fd_v2) 
level(90)}{p_end}
...
{title:Author}
{p 8 12} Allen Buxton, Children's Oncology Group, Monrovia, CA{p_end}
...
{title:Files}
{p 8 12} main: cureregr.ado, lcureregr_lf.mlib {p_end}
{p 8 12} predict: cureregr_p.ado & cureregr_gen_p.ado{p_end}
{p 4 8} - ancillary - {p_end}
{p 8 12} pcm_key.pdf, a description of the models {cmd:cureregr} will fit. {p_end}
{p 8 12} wittelognormal_notes.pdf, comments on the lognormal distributions fit. {p_end}
{p 8 12} cureregr_predict_eg.pdf, on predicting. {p_end}
{p 8 12} example_stata.do, an example with factor variables. {p_end}
{p 8 12} example_stata.dta, dataset in the above example. {p_end}
{p 8 12} note regarding the Mata function library lcureregr_lf.mlib {p_end}
{p 8 12} lcureregr_lf.do refers to: PCM_evaluators_00.do and PCM_evaluators_01.do {p_end}
