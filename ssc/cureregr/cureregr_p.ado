...
*! main program cureregr.ado, called by predict after cureregr...run via predict switch 
* version 2.1 abuxton 23sep2007.

program define cureregr_p,rclass
version 13
...
/* bgn if At_points exists -atopt- then do the following and exit the cureregr_p */
if `"`timelst'"' ~= "" {
	preserve
        fvrevar `e(varlist)',list
	local efvarlist `"`r(varlist)'"'
		...
		tempfile dtasetA
			...
			qui keep if `touse'
				...
				keep `efvarlist' `e(depvar)'
			}
			qui save `dtasetA',replace all
			/*di `"contract `efvarlist'"'*/
                        if `flag_cons_only'==1 {
   			contract cons
   			}
   			else {
...
capture noisily {
	...
	foreach time of numlist `timelst' {
		...
		if `"`krn'"' == `"01"' {
			...
			local kr normal(((`tt' - (xb(#2)))/exp(xb(#3))))
			}
			...
			local kr normal(((`tt' - (xb(#2)))/1))
			}
                else if `"`krn'"' == `"08"' {
			local ttfunction `"((exp(xb(#2)) * `time'))"'
			local kr `"(norm(ln(`ttfunction')))"'
                        }
	...
	/* di `"qui cureregr_gen_p if `touse' `alt_argument'"' */
	qui cureregr_gen_p if `touse' `alt_argument'
	}
/* end if At_points exists -atopt- then do the following and exit the cureregr_p */
