...
program define diagreg2 , eclass
version 11.0
syntax [anything] [if] [in] , Model(str) [NOCONStant NOCONEXOG kc(real 0) ///
...
tempvar Yt e LDE DF1 Eo Lambda0 Yh2 LYh2 E3 E4 YYm DFF DFF1 DFF2 Yh
...
tempname Wio Wi Ew OM Omega hjm Wald We vy1 v kyi kzi Sig2n
...
tempname X X0 Vec Val J SST1 SST2 Sig2o1 CovC SSE Sig2 F K B Bx Zo V
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* 2SLS-IV Model Selection Diagnostic Criteria} - {bf:(Model= {err:`model'})}}"
di _dup(78) "{bf:{err:=}}"
