*! version 1.0.0  05may2017 Robert Picard, picard@netbox.com
*! Copied march 7th 2018 from rangerun_run, by Dennis Lund Hansen, Denmark
*! Used for the helpfile fore stlc and stltx.

program define svtxt_run
        ...
        if `"`requires'"' != "" {
                foreach f of local requires {
                        ...
                        if _rc {
                                ...
                                dis as err `"> {it:{stata `"net get `package', from("http://fmwww.bc.edu/repec/bocode/`p1'")"':click to inst
> all `package' example datasets from SSC}}"'
                                exit 601
                        }
                }
        }
