*! sptobitmstardhxt V1.0 25/11/2013
*! 
...
program define sptobitmstardhxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstardhxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) WMFile(str) LL(str) [mhet(str) diag ///
 vce(passthru) stand zero ROBust MFX(str) NOLog LMHet LMSPac aux(str) ///
 iter(int 100) level(passthru) PREDict(str) INV INV2 dist(str) ///
 coll RESid(str) tech(str) tolog NWmat(int 1) TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp sptobitmstardhxt
...
tempvar Bw D DE DF1 DumE E dcs XQX_ EE Eo Hat ht idv itv LE LEo P Q Sig2 SSE
...
tempname h Hat IPhi J K L M Sig2 Sig2o SSE SST1 SST2 In IRW IRWR WXb Sig2n
tempname P Phi Pm q Q kbm IN xMx Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1
tempname Y Yh Yi YYm YYv Z X0 WS D WW eVec eigw Xo mh olsin Wio WY X XB V
tempname N DF kx kb NC NT Nmiss kz llf Sig21 Dim wmat xAx xAx2 TRa2 wWw1 xBx
tempname Rostar rRo kb minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
local N=`N'
...
qui marksample touse
markout `touse' `varlist' `mhet' , strok
 if "`zero'"!="" {
tempvar zeromiss
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardhxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Hetero: `mhet', noconst) (Rho1:) (Sigma:) `wgt', `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons `mhet' Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression  (1 Weight Matrix)}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardhxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Hetero: `mhet', noconst) (Rho1:) (Rho2:) (Sigma:) `wgt', `mlopts' contin ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression (2 Weight Matrix)}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardhxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression (3 Weight Matrix)}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardhxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Hetero: `mhet', noconst) (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt', `mlopts' ///
 contin `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons `mhet' Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression (4 Weight Matrix)}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
tempname SSEo Sigo r2bu r2bu_a r2raw r2raw_a f fp wald waldp b
