...
program define sptobitmstar , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstar" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , WMFile(str) LL(str) [NWmat(int 1) coll NOCONStant ///
 INV INV2 vce(passthru) dist(str) stand MFX(str) noLOG ROBust tolog ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str)]
gettoken yvar xvar : varlist
local sthlp sptobitmstar
...
tempvar Bw D DE DF DF1 E P Q Sig2 SSE SST Time U U2 wald weit
tempvar Wi Wio Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname A B b B1 b1 B12 B1b B1t BB2 Beta BetaSP Bm Bt Bx WS 
tempname Cov D E F IPhi J K olsin N DF kx kb Nmiss 
tempname P Phi Pm Q Sig2 Sig2b Sig2o SSE SST1 SST2 D WW eVec kz
tempname In IRW Vec vh W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V Xo eigw
tempname Y Yh Yi YYm YYv Z Sw Ue llf Rostar rRo kb Sig21
tempname Dim minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstar1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di as txt "{bf:{err:* (m-STAR) Spatial Lag Normal Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf sptobitmstar2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Weibull Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstar3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Normal Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstar4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Weibull Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstar5 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Normal Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstar6 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Weibull Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstar7 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Normal Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstar8 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Weibull Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
