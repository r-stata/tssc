{smcl}
{hline}
{cmd:help: {helpb sptobitmstar}}{space 50} {cmd:dialog:} {bf:{dialog sptobitmstar}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:sptobitmstar: Tobit (m-STAR) Multiparametric Spatio Temporal AutoRegressive Regression}
{p 9 1 1}{bf:(Spatial Lag Cross Sections Models)}{p_end}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb sptobitmstar##01:Syntax}{p_end}
{p 5}{helpb sptobitmstar##02:Description}{p_end}
{p 5}{helpb sptobitmstar##03:Options}{p_end}
{p 5}{helpb sptobitmstar##04:Saved Results}{p_end}
{p 5}{helpb sptobitmstar##05:References}{p_end}

{p 1}*** {helpb sptobitmstar##06:Examples}{p_end}

{p 5}{helpb sptobitmstar##07:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt sptobitmstar} {depvar} {indepvars} {weight} , {opt wmf:ile(weight_file)} {opt ll(#)}{p_end} 
{p 3 5 6} 
{err: [}{opt nw:mat(#)} {opt dist(norm|weib)} {opt mfx(lin, log)} {opt stand inv inv2 tolog nolog robust coll zero}{p_end} 
{p 5 5 6} 
{opt nocons:tant} {opt pred:ict(new_var)} {opt res:id(new_var)} {opt iter(#)} {opt tech(name)} {p_end} 
{p 5 5 6} 
...
{p 2 2 2} {cmd:sptobitmstar} estimates Tobit Spatial econometric regression (MSTAR) "Multiparametric Spatio Temporal AutoRegressive Regression" for Spatial Lag Cross Sections Models.{p_end}

{p 2 4 2}{cmd:sptobitmstar} can generate:{p_end}
...
{p 2 4 2} {cmd:sptobitmstar} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:sptobitmstar} generates some variables names with prefix:
...
{p 2 2 2}{bf:sptobitmstar} is used with more than Weight Matrix: (Border, Language, Currency, Trade...){p_end}
...
{p 2 4 2 }{cmd:sptobitmstar} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog sptobitmstar}.
{hline}

{stata clear all}

{stata sysuse sptobitmstar.dta, clear}
...
{bf:* (1) *** Normal Distribution}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(norm) ll(3)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs2) nwmat(2) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs3) nwmat(3) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs4) nwmat(4) mfx(lin) dist(norm) ll(0)}
{hline}

{bf:* (2) *** Weibull Distribution}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(weib) ll(3)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs2) nwmat(2) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs3) nwmat(3) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstar ys x1 x2, wmfile(SPWmcs4) nwmat(4) mfx(lin) dist(weib) ll(0)}
{hline}

. clear all
. sysuse sptobitmstar.dta, clear
. sptobitmstar ys x1 x2 , wmfile(SPWmcs1) nwmat(1) dist(norm) mfx(lin) ll(0)
...
------------------------------------------------------------------------------
- Sum of Rho's =    0.0176958    Sum must be < 1 for Stability Condition
------------------------------------------------------------------------------
...
-------------+----------------------------------------------------------------
        Rho1 |   .0176958   .0268608     0.66   0.510    -.0349503     .070342
       Sigma |   10.70508   1.184352     9.04   0.000     8.383789    13.02636
------------------------------------------------------------------------------
 Wald Test [Rho1=0]:                 0.4340        P-Value > Chi2(1) 0.5100
...
{bf:{err:{dlgtab:SPTOBITMSTAR Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2013)}{p_end}
{p 1 10 1}{cmd:SPTOBITMSTAR: "Tobit (m-STAR) Spatial Multiparametric Spatio Temporal AutoRegressive Regression: Spatial Lag Cross Sections Models"}{p_end}
