...
//		local iv_e_o	`iv_e_o' `var_o'				//  no prefix
	...
	*********************** sup-score stat **********************************
	...
	if (`idstatsflag' | `sscsetflag' | `ssgridmatflag') & "`dendog_o'"~="" & "`dexog_o'"=="" & "`znotpen_o'`zpartial_o'`zaset_o'"=="" {
		...
		doSupScore,												/// 
						...
						consflag(`consflag')					/// needed for OLS
						dmflag(`dmflag')						///
						`qui'
	...
	*** Variable counts
	...
	if `consflag' {														//  adjust xnotpen_ct and xpartial_ct; won't enter if FE
	...
	*********************** Identification stats **********************************
	...
	if `idstatsflag' {
		...
		local inexog_ct		= `inexog_ct' + `xpartial_ct' + `N_g' + `consflag'
		doIDstats if `touse',											///
			endog(`dendog_t')											///
			endog_l(`rho_y_l')											///
			endog_pl(`rho_y_pl')										///
...
program define SelectControls, rclass
	...
	syntax ,						///
		...
		partial_o(string)			///
		partial_t(string)			///
		lsconsflag(int 0)			/// tell rlasso to include a constant
		dmflag(int 0)				/// tell rlasso data are zero-mean
		loptions(string)			///
		msg(string)					///
		startcol(int 15)			///
		qui							///
...
program define doregress, eclass
	...
	syntax varlist(numeric fv ts min=1 max=1) [if] [in],	/// dep var
		...
		wvar(varlist max=1)									/// weight variable
	...
	// pweights = aweights+rob so pw means robust appears in options
	if "`wvar'"~="" {
		local wexp [aw=`wvar']
	}
...
program define _wt, rclass
	...
	*
	...
	mata: s_weight("`varlist'","`tvarlist'","`touse'","`wvar'",`transform')
...
program define DispVars
	version 11.2
	syntax [anything] [, _col(integer 15) _lc1(integer 0) _lc2(integer 0) style(string) ]
...
// copied from lassoutils 1.0.09 03/08/2018
// Mata utility for weighting; called from Stata.
// Weight by sqrt of weighting variable.
void s_weight(		string scalar Xnames,				//  names of original Stata variables
...
//end program s_weight


// based on lassoutils 1.0.08
// partial out program
...
// FE transformation, based on version in lassoutils
