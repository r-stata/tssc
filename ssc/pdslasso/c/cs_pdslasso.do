...
cscript "rlasso/pdslasso/pdslasso" adofile rlasso pdslasso ivlasso lassoutils
clear all
capture log close
set more off
set rmsg on
program drop _all
log using cs_pdslasso,replace
about
which pdslasso
which ivlasso
which rlasso
...
//   as of lassoutils 1.1.01 08nov2018 need c0(0.55) option to replicate cbh
...
// nb: pnotpen(.) numerically unstable, commented out
...
foreach option3 in " " fe nocons										{
	di as text "Options: `option1' `option2' `option3'"
	if "`option1'"=="partial(i.year)" & "`option3'"=="nocons" {
		// this is illegal so display but don't exit with error
		local capnoi cap noi
	}
	else {
		local capnoi
	}
	`capnoi' pdslasso D.lpc_prop D.efaprop									///
			(															/// no trend
			c.prop0##c.prop0											///
			c.Dprop0##c.Dprop0											///
			c.(D.($xx))##c.(D.($xx))									/// note macro
...
// make sure that the same sample is used throughout.
cap drop touse
gen touse=(democ1<.)
...
global wtvar logpgp95
gen double wt=$wtvar
// assumes no missings for any regressors or dep var
sum wt if touse
replace wt = wt * 1/r(mean) if touse
...
foreach var of varlist logpgp95 avexpr lat_abst logem4 edes1975 avelf		///
	temp* humid* steplow-oilres euro1900-cons00a {
	qui sum `var' [aw=wt] if touse, meanonly
	qui gen double w_c_`var' = (`var'-r(mean))*sqrt(wt) if touse
}
...
foreach opt in "" "rob" {
	...
	mat V_pds=e(V_pds)
	mat b_plasso=e(beta_plasso)
	mat V_plasso=e(V_plasso)
	mat b_lasso=e(beta_lasso)
	mat V_lasso=e(V_lasso)
	// prestd rlasso option
	pdslasso logpgp95 avexpr												///
		(lat_abst edes1975 avelf temp* humid*)								///
		[aw=logpgp95] if touse												///
	...
	mat V_pds=V_pds[1..rowsof(V_pds)-1,1..colsof(V_pds)]
	assert mreldif(e(beta_pds),b_pds)<1e-8
	assert mreldif(e(V_pds),V_pds)<1e-8
	assert mreldif(e(beta_plasso),b_plasso)<1e-8
	assert mreldif(e(V_plasso),V_plasso)<1e-8
	assert mreldif(e(beta_lasso),b_lasso)<1e-8
	assert mreldif(e(V_lasso),V_lasso)<1e-8
}
...
foreach opt in "" "rob" {
	...
	assert mreldif(e(beta_lasso),b_lasso)<1e-8
	assert mreldif(e(V_lasso),V_lasso)<1e-8
	// compare first-stage regressions
	est restore _ivlasso_w_c_avexpr
	mat b_w_c_avexpr = e(b)
	assert mreldif(b_avexpr,b_w_c_avexpr)<1e-10
}
...
* weighted centering (so no intercept)
foreach var of varlist logpgp95 avexpr lat_abst logem4 edes1975 avelf		///
	temp* humid* euro1900-cons00a {
	qui reg `var' i.id [aw=wt] if touse
	qui predict double c_`var' if touse, resid
	qui gen double w_c_`var' = c_`var'*sqrt(wt) if touse
	// don't need and get in the way
	cap drop c_`var'
}
...
foreach var of varlist logpgp95 avexpr lat_abst logem4 edes1975 avelf		///
	temp* humid* euro1900-cons00a {
	qui reg `var' steplow i.id [aw=wt] if touse
	qui predict double c_`var' if touse, resid
	qui gen double w_c_`var' = c_`var'*sqrt(wt) if touse
	// don't need and get in the way
	cap drop c_`var'
