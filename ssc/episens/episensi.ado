...
capture program drop episensi
program  episensi, rclass
version 9.2
syntax anything  [ , /// 
...
if ("`grprior'" != "") {
	...
	if ("`probmie'"=="Yes") {
		...
		qui hist pseca , name(pseca, replace) frequency xtitle("Sensitivity Cases")
		qui hist psenc , name(psenc, replace) frequency xtitle("Sensitivity Noncases")
		qui hist pspca , name(pspca, replace) frequency xtitle("Specificity Cases")
		qui hist pspnc , name(pspnc, replace) frequency xtitle("Specificity Noncases")
		gr combine pseca  psenc  pspca  pspnc, name(priors_misclassification, replace)
		gr drop pseca  psenc  pspca  pspnc
      } 
