...
version 12
   ...
   ***************************************************************
   ...
   restore
   return clear
   return matrix margeff = margeff
   if ("`type'"=="binning") {
      return matrix estBin = MEbin
      }
   if ("`type'"=="binning" & `nbins'>1){
       return scalar pwald = `pwald'   
   }
   if ("`type'"=="kernel"){
       return scalar bandwidth = `bw'
       if(`CV'==1){
           return matrix CVout = cv   
       }
   }
