#delim ;
prog def listtex_vars, rclass;
version 10.0;
/*
 Input a varlist and a listtex rstyle() option
...
syntax [varlist (min=1)] [, SUbstitute(string) LOcal(name) Begin(passthru) Delimiter(passthru) End(passthru) Missnum(passthru) RStyle(passthru) ];
/*
...
*;
listtex_rstyle, `begin' `delimiter' `end' `missnum' `rstyle';
