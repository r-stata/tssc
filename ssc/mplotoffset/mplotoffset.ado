*! version 1.1.1  14mar2015
*! adds options OFFset(real 0.05) & ovar(varname)	
...
program _marginsplot
	syntax [using/] [, Level(passthru) MCOMPare(passthru) mformat(string) *]
	local 0 `", `options'"'
	...
	if `"`using'"' != "" {
		...
		capture ///
		_marg_save , saving(`filenm') `eclass' `level' `mcompare'
		if _rc == 301 {
			...
			_marg_save , saving(`"`filenm'"') `level'
		}
	}
...
program Rest
	...
	local list
	foreach v in _atopt _term _deriv _equation _outcome _predict	///
		     _pw _pw0 _pw1 {
...
program SetupMV
	...
	qui clonevar _outcome  = _predict
	...
	if (`sep'[1] == 0) {					// outcome | eqn
		...
		if "`subopt'" == "equation" {
			...
			qui drop _outcome
			qui gen _outcome = real(`eo1')
			capture assert _outcome != .
			if _rc {
				drop _outcome
				qui encode `eo1', gen(_outcome) label(_outcome)
			}
			char _outcome[varname]  "_outcome"
		}
	...
	qui gen _outcome = real(`eo2')
	capture assert _outcome != .
	if _rc {
		drop _outcome
		qui encode `eo2', gen(_outcome)  label(_outcome)
	}
	char _outcome[varname]  "_outcome"
	char _equation[varname] "_equation"
...
// Order preference:  
	...
	_userlist_ _at#s _m#s _by#s _atopt _term 			///
		   _predict|_outcome|_equation _deriv
