...
program define spglsxt, eclass 
version 11.0
syntax varlist , nc(str) [WMFile(str) stand vce(passthru) MFX(str) ///
 INV2 LMHet LMSPac ROBust gmm(int 1) coll INV level(passthru) PREDict(str) ///
 zero diag RESid(str) tech(str) tolog TESTs DN NOCONStant LMNorm]
...
tempvar Bw D DE DF1 DumE E dcs XQX_ EE Hat ht idv itv LE LEo
tempvar SST Time U U2 wald weit Wi Wio WS X X0 eigw Bo P Q Sig2 SSE
...
tempname h Hat idv itv IPhi J K L M P Phi Pm Q Sig2 Sig2o Sn SSE SST1 SST2
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V IN xMx
tempname xq Xx Y Yh Yi YYm YYv Z Sig2n kb WXb WS D WW eVec eigw Xo mh TRa2
tempname N DF kx kb NC NT Nmiss llf Sig21 Dim wmat xAx xAx2 wWw1 xBx In
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
matrix `In'=I(`N')
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Spatial Panel Autoregressive Generalized Least Squares Regression (SPGLS)}}"
di _dup(78) "{bf:{err:=}}"
qui cap drop Time
ereturn scalar kb=`kb'
ereturn scalar DF=`DF'
spglsxts `yvar' `SPXvar' , nc(`nc') gmm(`gmm') `noconstant' `vce' `robust' `dn'
scalar `llf'=e(llf)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
...
program define spglsxts , eclass 
 version 11.0
syntax varlist , [nc(str) gmm(int 1) NOCONStant vce(passthru) DN ROBust coll]
tempvar `varlist'
gettoken yvar xvar : varlist
 if "`coll'"=="" {
