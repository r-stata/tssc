{smcl}
{hline}
{cmd:help: {helpb spglsxt}}{space 50} {cmd:dialog:} {bf:{dialog spglsxt}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:spglsxt: Spatial Panel Autoregressive Generalized Least Squares Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb spglsxt##01:Syntax}{p_end}
{p 5}{helpb spglsxt##02:Description}{p_end}
{p 5}{helpb spglsxt##03:Options}{p_end}
{p 5}{helpb spglsxt##04:Spatial Panel Aautocorrelation Tests}{p_end}
{p 5}{helpb spglsxt##05:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb spglsxt##06:Heteroscedasticity Tests}{p_end}
{p 5}{helpb spglsxt##07:Non Normality Tests}{p_end}
{p 5}{helpb spglsxt##08:Saved Results}{p_end}
{p 5}{helpb spglsxt##09:References}{p_end}

{p 1}*** {helpb spglsxt##10:Examples}{p_end}

{p 5}{helpb spglsxt##11:Acknowledgments}{p_end}
{p 5}{helpb spglsxt##12:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt spglsxt} {depvar} {indepvars} , {bf:{err:nc(#)}} {p_end} 
{p 3 5 6} 
{err: [} {opt wmf:ile(weight_file)} {opt lmsp:ac} {opt lmh:et} {opt lmn:orm} {opt diag} {opt test:s}{p_end} 
{p 5 5 6} 
{opt dn coll zero tolog robust stand inv inv2} {opt mfx(lin, log)} {opt gmm(#)} {opt nocons:tant}{p_end} 
{p 5 5 6} 
...
{marker 02}{bf:{err:{dlgtab:Description}}}
 ...
 {cmd:spglsxt} estimates Spatial Panel Autoregressive Generalized Least Squares Regression
{p 5 5 2}When panel data model with error components are both spatially and time-wise correlated.
Generalized Method of Moments (GMM) that suggested in Kelejian-Prucha (1999), and Kapoor-Kelejian-Prucha (2007) is used in the estimation of {bf:({err:{it:spglsxt}})}{p_end}

{p 2 4 2}{cmd:spglsxt} can generate:{p_end}
...
{cmd:spglsxt} generates some variables names with prefix:
...
{col 20}{cmd:spglsxt} will convert automatically spatial cross section weight matrix
...
{synopt :{opt gmm(1, 2, 3)} GMM Estimators for {bf:({err:{it:spglsxt}})}}{p_end} 
...
{p 2 4 2 }Depending on the model estimated, {cmd:spglsxt} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog spglsxt}.
{bf:Note 4:} xtset is included automatically in {cmd:spglsxt} models.

{hline}

  {stata clear all}

  {stata sysuse spglsxt.dta, clear}

  {stata db spglsxt}

{bf:{err:* (1) Spatial Panel Autoregressive Generalized Least Squares Regression}}
 {stata spglsxt y  x1 x2 , nc(7) wmfile(SPWxt) gmm(1) mfx(lin) test}
 {stata spglsxt y  x1 x2 , nc(7) wmfile(SPWxt) gmm(1) mfx(log) test tolog}
 
 {stata spglsxt y  x1 x2 , nc(7) wmfile(SPWxt) gmm(2) mfx(lin) test}
 {stata spglsxt y  x1 x2 , nc(7) wmfile(SPWxt) gmm(3) mfx(lin) test}
{hline}

{bf:{err:* (2) Spatial Panel Autoregressive Generalized Least Squares Regression} (Cont.)}
 ...
 Results of {bf:({err:{it:spglsxt}})} with {cmd:gmm(3) option} is identical to:
 {browse "http://econweb.umd.edu/~prucha/STATPROG/PANOLS/PROGRAM3(L3).log"}

  {stata clear all}
  {stata sysuse spglsxt1.dta, clear}
  {stata spglsxt y x1 , wmfile(SPWxt1) nc(100) gmm(1) stand}
  {stata spglsxt y x1 , wmfile(SPWxt1) nc(100) gmm(2) stand}
  {stata spglsxt y x1 , wmfile(SPWxt1) nc(100) gmm(3) stand}
{hline}

. clear all
. sysuse spglsxt.dta, clear
. spglsxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test

==============================================================================
*** Binary (0/1) Weight Matrix: 49x49 - NC=7 NT=7 (Non Normalized)
==============================================================================
==============================================================================
* Spatial Panel Autoregressive Generalized Least Squares Regression (SPGLS)
==============================================================================
*** Initial GMM Model: 1 
  y = x1 + x2
------------------------------------------------------------------------------
...
- Akaike Final Prediction Error AIC               =   347.0999
- Schwarz Criterion             SC                =   352.7754
- Akaike Information Criterion  ln AIC            =     4.2458
- Schwarz Criterion             ln SC             =     4.3616
- Amemiya Prediction Criterion  FPE               =    80.2952
- Hannan-Quinn Criterion        HQ                =    72.9474
- Rice Criterion                Rice              =    70.3840
- Shibata Criterion             Shibata           =    69.3287
- Craven-Wahba Generalized Cross Validation-GCV   =    70.0846
------------------------------------------------------------------------------
...
{bf:{err:{dlgtab:SPGLSXT Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:SPGLSXT: "Spatial Panel Autoregressive Generalized Least Squares Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457421.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457421.htm"}
