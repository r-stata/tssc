{smcl}
{hline}
{cmd:help: {helpb sptobitsemxt}}{space 50} {cmd:dialog:} {bf:{dialog sptobitsemxt}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:sptobitsemxt: Tobit MLE Spatial Error Panel Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb sptobitsemxt##01:Syntax}{p_end}
{p 5}{helpb sptobitsemxt##02:Description}{p_end}
{p 5}{helpb sptobitsemxt##03:Options}{p_end}
{p 5}{helpb sptobitsemxt##04:Spatial Panel Aautocorrelation Tests}{p_end}
{p 5}{helpb sptobitsemxt##05:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb sptobitsemxt##06:Heteroscedasticity Tests}{p_end}
{p 5}{helpb sptobitsemxt##07:Non Normality Tests}{p_end}
{p 5}{helpb sptobitsemxt##08:Saved Results}{p_end}
{p 5}{helpb sptobitsemxt##09:References}{p_end}

{p 1}*** {helpb sptobitsemxt##10:Examples}{p_end}

{p 5}{helpb sptobitsemxt##11:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt sptobitsemxt} {depvar} {indepvars} {weight} , {bf:{err:nc(#)}} {opt wmf:ile(weight_file)} {opt ll(#)}{p_end} 
{p 3 5 6} 
{err: [}{opt lmsp:ac} {opt lmh:et} {opt lmn:orm} {opt test:s} {opt stand inv inv2 coll zero tolog nolog robust diag}{p_end} 
{p 5 5 6} 
{opt dist(norm|exp|weib)} {opt mfx(lin, log)} {opt pred:ict(new_var)} {opt res:id(new_var)} {opt iter(#)}{p_end} 
{p 5 5 6} 
{opt tech(name)} {opt l:evel(#)} {opth vce(vcetype)} {helpb maximize} {it:other maximization options} {err:]}{p_end}

{p2colreset}{...}
{marker 02}{bf:{err:{dlgtab:Description}}}

 {cmd:sptobitsemxt} estimates Tobit MLE Spatial Error Panel Regression Models

 {cmd:sptobitsemxt} can estimate the following models:
   1- Heteroscedastic Regression Models in disturbance term.
   2- Non Normal Regression Models in disturbance term.

{p 2 4 2}{cmd:sptobitsemxt} can generate:{p_end}
...
{p 2 4 2} {cmd:sptobitsemxt} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:sptobitsemxt} generates some variables names with prefix:
...
{col 20}{cmd:sptobitsemxt} will convert automatically spatial cross section weight matrix
...
{p 2 4 2 }Depending on the model estimated, {cmd:sptobitsemxt} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog sptobitsemxt}.
{bf:Note 4:} xtset is included automatically in {cmd:sptobitsemxt} models.
{hline}

  {stata clear all}

  {stata sysuse sptobitsemxt.dta, clear}

  {stata db sptobitsemxt}

{bf:{err:* (1) Tobit MLE Spatial SAC Panel Normal Regression Model}}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(norm) predict(Yh) resid(Ue) ll(3)}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(norm) mfx(lin) test ll(3)}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(norm) mfx(log) test ll(3) tolog}
{hline}

{bf:{err:* (2) Tobit MLE Spatial SAC Panel Exponential Regression Model}}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(exp) mfx(lin) test ll(3)}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(exp) mfx(log) test ll(3) tolog}
{hline}

{bf:{err:* (3) Tobit MLE Spatial SAC Panel Weibull Regression Model}}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(weib) mfx(lin) test ll(3)}
{stata sptobitsemxt y x1 x2 , nc(7) wmfile(SPWxt) dist(weib) mfx(log) test ll(3) tolog}
{hline}

{bf:{err:* (4) Tobit MLE Weighted Spatial SAC Panel Regression Model}}
{stata sptobitsemxt y x1 x2  [weight = x1], nc(7) wmfile(SPWxt) mfx(lin) test ll(3)}
{stata sptobitsemxt y x1 x2 [aweight = x1], nc(7) wmfile(SPWxt) mfx(lin) test ll(3)}
{stata sptobitsemxt y x1 x2 [iweight = x1], nc(7) wmfile(SPWxt) mfx(lin) test ll(3)}
{hline}
...
------------------------------------------------------------------------------
  ...
  Wald Test         =     43.4952   |   P-Value > Chi2(2)       =      0.0000
  F-Test            =     21.7476   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.4860   |   Raw Moments R2          =      0.8249
 (Buse 1973) R2 Adj =      0.3832   |   Raw Moments R2 Adj      =      0.7898
  Root MSE (Sigma)  =     16.5476   |   Log Likelihood Function =   -152.4694
------------------------------------------------------------------------------
- R2h= 0.4347   R2h Adj= 0.3216  F-Test =   17.68 P-Value > F(2 , 40)  0.0000
- R2v= 0.4142   R2v Adj= 0.2971  F-Test =   16.26 P-Value > F(2 , 40)  0.0000
------------------------------------------------------------------------------
             |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
ys           |
          x1 |  -.4443557   .1177555    -3.77   0.000    -.6751522   -.2135592
          x2 |  -1.305814   .3377474    -3.87   0.000    -1.967787   -.6438415
       _cons |   69.42618   4.994364    13.90   0.000      59.6374    79.21495
-------------+----------------------------------------------------------------
      Lambda |   .0073434   .0239708     0.31   0.759    -.0396385    .0543253
       Sigma |      10.73   1.186894     9.04   0.000     8.403725    13.05627
------------------------------------------------------------------------------
 LR Test SEM vs. OLS (Lambda=0):   0.0938   P-Value > Chi2(1)   0.7593
...
==============================================================================
- Log Likelihood Function       LLF               =  -152.4694
- Akaike Final Prediction Error AIC               =   310.9387
- Schwarz Criterion             SC                =   316.6142
- Akaike Information Criterion  ln AIC            =     5.5320
- Schwarz Criterion             ln SC             =     5.6478
- Amemiya Prediction Criterion  FPE               =   290.5895
- Hannan-Quinn Criterion        HQ                =   263.9977
- Rice Criterion                Rice              =   254.7206
- Shibata Criterion             Shibata           =   250.9014
- Craven-Wahba Generalized Cross Validation-GCV   =   253.6372
------------------------------------------------------------------------------
...
- GLOBAL Moran MI            =  -0.0995     P-Value > Z(-0.693)   0.4883
- GLOBAL Geary GC            =   1.0465     P-Value > Z(0.326)    0.7442
- GLOBAL Getis-Ords GO       =   0.2843     P-Value > Z(0.693)    0.4883
------------------------------------------------------------------------------
- Moran MI Error Test        =  -0.1053     P-Value > Z(-0.744)   0.9162
------------------------------------------------------------------------------
- LM Error (Burridge)        =   0.4620     P-Value > Chi2(1)     0.4967
- LM Error (Robust)          =   0.1846     P-Value > Chi2(1)     0.6675
------------------------------------------------------------------------------
  Ho: Spatial Lagged Dependent Variable has No Spatial AutoCorrelation
  Ha: Spatial Lagged Dependent Variable has    Spatial AutoCorrelation

- LM Lag (Anselin)           =   0.2796     P-Value > Chi2(1)     0.5970
- LM Lag (Robust)            =   0.0022     P-Value > Chi2(1)     0.9624
------------------------------------------------------------------------------
  Ho: No General Spatial AutoCorrelation
  Ha:    General Spatial AutoCorrelation

- LM SAC (LMErr+LMLag_R)     =   0.4642     P-Value > Chi2(2)     0.7929
- LM SAC (LMLag+LMErr_R)     =   0.4642     P-Value > Chi2(2)     0.7929
------------------------------------------------------------------------------
...
- Engle LM ARCH Test AR(1): E2 = E2_1   =   0.0338   P-Value > Chi2(1)  0.8541
------------------------------------------------------------------------------
- Hall-Pagan LM Test:   E2 = Yh         =   0.1783   P-Value > Chi2(1)  0.6729
- Hall-Pagan LM Test:   E2 = Yh2        =   0.1458   P-Value > Chi2(1)  0.7026
- Hall-Pagan LM Test:   E2 = LYh2       =   0.1281   P-Value > Chi2(1)  0.7204
------------------------------------------------------------------------------
- Harvey LM Test:    LogE2 = X          =   0.7406   P-Value > Chi2(2)  0.6905
- Wald Test:         LogE2 = X          =   1.8274   P-Value > Chi2(1)  0.1764
- Glejser LM Test:     |E| = X          =   0.7796   P-Value > Chi2(2)  0.6772
- Breusch-Godfrey Test:  E = E_1 X      =   0.5459   P-Value > Chi2(1)  0.4600
------------------------------------------------------------------------------
- Machado-Santos-Silva Test: Ev=Yh Yh2  =   0.2029   P-Value > Chi2(2)  0.9035
- Machado-Santos-Silva Test: Ev=X       =   0.8612   P-Value > Chi2(2)  0.6501
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X      =   0.6416   P-Value > Chi2(2)  0.7256
- White Test - B-P-G (SSR): E2 = X      =   0.9823   P-Value > Chi2(2)  0.6119
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2   =   2.1214   P-Value > Chi2(4)  0.7134
- White Test - B-P-G (SSR): E2 = X X2   =   3.2478   P-Value > Chi2(4)  0.5172
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2 XX=   2.3805   P-Value > Chi2(5)  0.7944
- White Test - B-P-G (SSR): E2 = X X2 XX=   3.6445   P-Value > Chi2(5)  0.6016
------------------------------------------------------------------------------
- Cook-Weisberg LM Test: E2/S2n = Yh    =   0.2729   P-Value > Chi2(1)  0.6014
- Cook-Weisberg LM Test: E2/S2n = X     =   0.9823   P-Value > Chi2(2)  0.6119
------------------------------------------------------------------------------
*** Single Variable Tests (E2/Sig2):
- Cook-Weisberg LM Test: x1                =   0.0035 P-Value > Chi2(1) 0.9530
- Cook-Weisberg LM Test: x2                =   0.7856 P-Value > Chi2(1) 0.3754
------------------------------------------------------------------------------
*** Single Variable Tests:
- King LM Test: x1                         =   0.1737 P-Value > Chi2(1) 0.6769
- King LM Test: x2                         =   1.2443 P-Value > Chi2(1) 0.2646
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
*** Non Normality Tests:
- Jarque-Bera LM Test                  =  11.9267     P-Value > Chi2(2) 0.0026
- White IM Test                        =  12.0792     P-Value > Chi2(2) 0.0024
- Doornik-Hansen LM Test               =  11.8797     P-Value > Chi2(2) 0.0026
- Geary LM Test                        =   0.0976     P-Value > Chi2(2) 0.9523
- Anderson-Darling Z Test              =   1.5326     P > Z( 3.226)     0.9994
- D'Agostino-Pearson LM Test           =  11.8985     P-Value > Chi2(2) 0.0026
------------------------------------------------------------------------------
*** Skewness Tests:
- Srivastava LM Skewness Test          =   9.6243     P-Value > Chi2(1) 0.0019
- Small LM Skewness Test               =   9.0129     P-Value > Chi2(1) 0.0027
- Skewness Z Test                      =  -3.0022     P-Value > Chi2(1) 0.0027
------------------------------------------------------------------------------
*** Kurtosis Tests:
- Srivastava  Z Kurtosis Test          =   1.5174     P-Value > Z(0,1)  0.1292
- Small LM Kurtosis Test               =   2.8855     P-Value > Chi2(1) 0.0894
- Kurtosis Z Test                      =   1.6987     P-Value > Chi2(1) 0.0894
------------------------------------------------------------------------------
    Skewness Coefficient = -1.0856     - Standard Deviation =  0.3398
    Kurtosis Coefficient =  4.0619     - Standard Deviation =  0.6681
------------------------------------------------------------------------------
    Runs Test: (25) Runs -  (29) Positives - (20) Negatives
    Standard Deviation Runs Sig(k) =  3.3440 , Mean Runs E(k) = 24.6735
    95% Conf. Interval [E(k)+/- 1.96* Sig(k)] = (18.1192 , 31.2278 )
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|ys          |                    |                    |                    |
|         x1 |            -0.4444 |            -0.5888 |            38.4362 |
|         x2 |            -1.3058 |            -0.6471 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:SPTOBITSEMXT Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2012)}{p_end}
{p 1 10 1}{cmd:SPTOBITSEMXT: "Tobit MLE Spatial Error Panel Regression"}{p_end}
