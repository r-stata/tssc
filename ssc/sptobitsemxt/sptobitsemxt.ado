...
program define sptobitsemxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsemxt" {
 error 301
 }
 Display `0'
 }
...
syntax varlist [aw] , nc(str) LL(str) WMFile(str) [diag vce(passthru) ///
 dist(str) stand MFX(str) NOLog INV2 LMHet LMSPac iter(int 100) INV zero coll ///
 level(passthru) PREDict(str) RESid(str) tech(str) tolog TESTs NOCONStant LMNorm ]
gettoken yvar xvar : varlist
local sthlp sptobitsemxt
...
tempname Xb A B Beta BetaSP Bm Bx Cov D den DVE DVNE Dx E F
tempname P Phi Pm Q Sig2 Sig2o SSE In SST1 SST2 WXb
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname Y Yh YYm YYv Z Dim wmat xAx xAx2 TRa2 wWw1 xBx
tempname WS D WW eVec eigw Xo mh olsin h Hat hjm IPhi J K lmhs M
tempname N DF kx kb NC NT Nmiss llf Sig21 sigox Sig2n
...
qui global spat_ARGS "`spat_ARGS' beta0 Lambda Sigma"
 ml model lf sptobitsemxt1 `MODEL' `wgt' , ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Error Panel Normal Model (SEM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
qui global spat_ARGS "`spat_ARGS' beta0 Lambda"
 ml model lf sptobitsemxt2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Lambda:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Error Panel Exponential Model (SEM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MODEL "`MODEL'(_cons:) (Lambda:) (Sigma:)"
qui global spat_ARGS "`spat_ARGS' beta0 Lambda Sigma"
 ml model lf sptobitsemxt3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Error Panel Weibull Model (SEM)}}"
di _dup(78) "{bf:{err:=}}"
 }
scalar `llf'=e(ll)
matrix `BetaSP'=e(b)
