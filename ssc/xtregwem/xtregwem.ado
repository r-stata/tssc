...
program define xtregwem, eclass 
version 11.0
...
tempname A B b B1 b1 B12 B1b B1t b2 BB2 Beta Bm BOLS BOLS1 Bt Bv Bv1 Bx Cov 
tempname D den DF dfab Dim DVE DVNE Dx E E1 EE1 Eg Eo eVec 
tempname Ew F f1 f13 f13d gam gam2 Go GoRY IDRmk In IPhi J K kb 
tempname kbm Ko Koi Kr kx L lf llf Ls M N NC NE Nn NT Omega P Phi Pm Q Qr
tempname rid Rmk RX RY s sd Sig2 Sig21 Sig2b Sig2o Sig2n
...
tempname v V v1 V1 V1s Wi1 Wio WMTD WS WW WY X X0 XB Xg Xo xq NC1 NC2 NT1 NT2
tempname Xx Y Yh Yh_ML YYm YYv Z Z0 Z1 Zo Zr Zz CovC
...
di 
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Within-Effects Panel Data: Ridge and Weighted Regression}}"
di _dup(78) "{bf:{err:=}}"
matrix `Omega'=`Wi''*`Q'*`Wi'
matrix `Beta'=inv(`X''*`Omega'*`X'+`Zz')*`X''*`Omega'*`Y'
matrix `E'=`Omega'*(`Y'-`X'*`Beta')
matrix `Sig2'=`E''*`E'/(`DF'-`NC')
matrix `E'=`Wi'*(`Y'-`X'*`Beta')
if "`ridge'"!="" & `kr' > 0 {
matrix `Cov'=`Sig2'*inv(`Xx'+`Zz')*`Xx'*inv(`Xx'+`Zz')
 } 
 else { 
