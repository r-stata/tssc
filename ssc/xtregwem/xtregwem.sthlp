{smcl}
{hline}
{cmd:help: {helpb xtregwem}}{space 50} {cmd:dialog:} {bf:{dialog xtregwem}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregwem: Within-Effects Panel Data Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregwem##01:Syntax}{p_end}
{p 5}{helpb xtregwem##02:Description}{p_end}
{p 5}{helpb xtregwem##03:Options}{p_end}
{p 5}{helpb xtregwem##04:Ridge Options}{p_end}
{p 5}{helpb xtregwem##05:Weight Options}{p_end}
{p 5}{helpb xtregwem##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregwem##07:Other Options}{p_end}
{p 5}{helpb xtregwem##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregwem##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregwem##10:Saved Results}{p_end}
{p 5}{helpb xtregwem##11:References}{p_end}

{p 1}*** {helpb xtregwem##12:Examples}{p_end}

{p 5}{helpb xtregwem##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregwem} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 2 2 2} {cmd:xtregwem} estimates Within-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregwem} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregwem} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregwem} can use large types of weighted regression options.
...
{col 20}{cmd:xtregwem} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregwem} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregwem} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregwem.dta, clear}

  {stata db xtregwem}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregwem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregwem.dta, clear
. xtregwem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh

==============================================================================
* Within-Effects Panel Data: Ridge and Weighted Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
 ...
 Wald Test          =      0.0078   |   P-Value > Chi2(2)       =      0.9961
 F-Test             =      0.0039   |   P-Value > F(2 , 40)     =      0.9961
 (Buse 1973) R2     =      0.6821   |   Raw Moments R2          =      0.9413
 (Buse 1973) R2 Adj =      0.6185   |   Raw Moments R2 Adj      =      0.9295
  Root MSE (Sigma)  =      9.8522   |   Log Likelihood Function =   -176.6531
------------------------------------------------------------------------------
- R2h= 0.5096   R2h Adj= 0.4115  F-Test =   23.90 P-Value > F(2 , 40)  0.0000
- R2v= 0.1267   R2v Adj=-0.0479  F-Test =    3.34 P-Value > F(2 , 40)  0.0457
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
          x1 |  -.0289468   4.204467    -0.01   0.995    -8.526491    8.468597
          x2 |  -.9943432   13.61298    -0.07   0.942     -28.5072    26.51852
       _cons |   25.46819   192.9307     0.13   0.896    -364.4593    415.3957
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -176.6531
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     89.5595
- Akaike Information Criterion              (1973) Log AIC =      4.4949
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =    100.5574
- Schwarz Criterion                         (1978) Log SC  =      4.6107
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =    103.0092
- Hannan-Quinn Criterion                    (1979) HQ      =     93.5829
- Rice Criterion                            (1984) Rice    =     90.2943
- Shibata Criterion                         (1981) Shibata =     88.9405
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     89.9103
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|         x1 |            -0.0289 |            -0.0317 |            38.4362 |
|         x2 |            -0.9943 |            -0.4069 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:XTREGWEM Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGWEM: "Within-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457461.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457461.htm"}
