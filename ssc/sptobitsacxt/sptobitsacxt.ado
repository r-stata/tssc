...
program define sptobitsacxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsacxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) LL(str) WMFile(str) [diag vce(passthru) ///
 INV dist(str) stand MFX(str) NOLog INV2 LMHet LMSPac spar(str) tolog ///
 iter(int 100) zero coll level(passthru) PREDict(str) RESid(str) tech(str) ///
 TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp sptobitsacxt
...
if inlist("`model'", "sac") & "`noconstant'"!="" {
di
...
di in gr _c " (sptobitsacxt Help):"
 exit
 }
 }
tempvar D DE DF1 DumE E dcs XQX_ EE Hat ht idv itv LE LEo P Q Sig2 SSE
...
tempname h Hat IPhi J K M P Phi Pm Q Sig2 Sig2o SSE In wWw1 xBx
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V Sig21 Dim wmat
tempname xq Y Yh YYm YYv Z X0 N DF kx kb NC NT Nmiss llf WXb
tempname WS D WW eVec eigw Xo mh IN xMx IRW IRWR IRWL SST1 SST2 xAx xAx2 TRa2
...
local initopt init(spat_ols, copy)  
 ml model lf sptobitsacxt1 `MODEL' `wgt' , ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial AutoCorrelation (SAC) Panel Normal Model}}"
di _dup(78) "{bf:{err:=}}"
 }
if inlist("`dist'", "exp") {
local MName "SAC1e"
...
local initopt init(spat_ols, copy) 
 ml model lf sptobitsacxt2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Lambda:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial AutoCorrelation (SAC) Panel Exponential Model}}"
di _dup(78) "{bf:{err:=}}"
 }
if inlist("`dist'", "weib") {
local MName "SAC1w"
...
qui global spat_ARGS "`spat_ARGS' beta0 Rho Lambda Sigma"
 matrix spat_ols=`Bo',0,0,`rmse'
local initopt init(spat_ols, copy)  
 ml model lf sptobitsacxt3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial AutoCorrelation (SAC) Panel Weibull Model}}"
di _dup(78) "{bf:{err:=}}"
 }
