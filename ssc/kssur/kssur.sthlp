...
{p2col :{hi:kssur} {hline 2}} Calculate Kapetanios, Shin & Snell unit root test statistic along with 1, 5 and 10% finite-sample critical values, and associated p-values{p_end}
{p2colreset}{...}


{title:Syntax}
{p 8 17 2}
{cmd:kssur} {varname} {ifin} [{cmd:,} {cmd:noprint}
{cmdab:maxlag(}{it:integer}{cmd:)}
{cmdab:noconstant} {cmdab:trend}]

{p 4 6 2}
{cmd:by} is not allowed. The routine can be applied to a single unit of a panel.{p_end}
{p 4 6 2}
Before using {opt kssur} you must {opt tsset} your data; see {manhelp tsset TS}.{p_end}
{p 4 6 2}
...
{pstd}{cmd:kssur} computes Kapetanios, Shin & Snell KSS (2003) OLS-detrending based unit root tests against the alternative of a globally stationary exponential smooth transition autoregressive (ESTAR) process.
...
If not specified, {hi:kssur} sets the number of lags following Schwert (1989) with the formula maxlag=int(12*(T/100)^0.25), where T is the total number of observations.

{phang} {opt noconstant} specifies that the test will be applied to the raw data. Use {hi:noconstant} when {it:varname} is a zero mean stochastic process.

{phang} {opt trend} specifies that OLS detrending is to be applied. Use {hi:trend} when {it:varname} is a nonzero trend stochastic process, in which case KSS recommend detrending the data using OLS.
...
Given that brco has a nonzero mean, the relevant KSS statistic is that based on OLS demeaned data, which is implemented by default.
Initially, the number of lags is set by the user as p=3:

{phang2}{inp:.} {stata "kssur brco, maxlag(3)":kssur brco, maxlag(3)}{p_end}

{pstd}This second illustration is the same as above, but using a subsample of the data that starts in January 1992:

{phang2}{inp:.} {stata "kssur brco if tin(1992m1,), maxlag(3)":kssur brco if tin(1992m1,), maxlag(3)}{p_end}
...
{phang2}{inp:.} {stata "kssur brco":kssur brco}{p_end}


{title:Stored results}

{synopt:{cmd:r(varname)}}Variable name{p_end}
{synopt:{cmd:r(treat)}}Raw, demeaned or detrended data, depending on the {cmd:noconstant} and {cmd:trend} option{p_end}
...
the KSS statistic; its p-value; and the critical values at 1%, 5%, and 10%, respectively.{p_end}
...
{phang}Kapetanios, G., Y. Shin, and A. Snell (2003). Testing for a unit root in the nonlinear STAR framework. Journal of Econometrics 112, 359-379.
