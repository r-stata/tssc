...
*! _cureregr8_gen, version 8.2 cure model regression or parametric cure model PCM
* version 1 abuxton based on post cureregr 'postcureregtask2'
* test 15sep2007 _cureregr_gen 16jan2005, draft
* post cureregr8 calculate hazard estimate and fail density for 
* any model fit by cureregr8, 
*! main program cureregr8.ado, called by _cureregr8.ado which is run via predict switch 

program define _cureregr8_gen
version 8.2
...
tokenize 'levelopt0'
	...
	if lower(substr(`"`svopt'"',1,1))==`"s"'  { 
		...
		di `"{err}please, check _cureregr8_gen {S, ses, lcs, ucs, h, fd, or all} only {txt}"'
		exit 
	} 
