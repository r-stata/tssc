* Creates specific source or target contagion spatial effect variable from a directed dyadic dataset
...
di _newline "   the generated spatial effect variable will represent specific {bf}`form'{sf} contagion."
...
if "`merge'"=="nomerge" { 
	...
	if "`filename'"=="" {
		di _newline "   Warning: file {bf}SE_file_sp_`form'_`rowst'{sf} will be replaced in current working directory, if already existant."
	}
	else {
	...
	if "`sename'"=="" {
		di _newline "   Warning: variable {bf}SE_var_sp_`form'_`rowst'{sf} will be dropped from dataset before merging, if already existant."
		capture drop SE_var_sp_`form'_`rowst'
	}
	else {
...
forvalues i = 1(1)`source_max'{
	...
	quietly {	
		...
		ge `spdyadcont'=`spatialvar'*`weightvar' 
		tempvar allmissing
		gen `allmissing'=`spdyadcont'
		collapse (sum) `spdyadcont' `weightvar' (mean) `allmissing', by(`sourceidinf' `targetidinf' `time')
		replace `spdyadcont'=. if `allmissing'==.
		if "`rowst'"=="norowst" { 
			ge SE_var_sp_`form'_`rowst'= `spdyadcont'
			replace SE_var_sp_`form'_`rowst'=. if `weightvar'==0
		}
		else {
			ge SE_var_sp_`form'_`rowst'= `spdyadcont'/`weightvar'
		}
		keep `sourceidinf' `targetidinf' SE_var_sp_`form'_`rowst' `time'
		tempfile SE_file_sp_`form'_`rowst'_`i'
		save "`SE_file_sp_`form'_`rowst'_`i''", replace
	}
}
...
quietly use "`SE_file_sp_`form'_`rowst'_1'", replace
forvalues i = 2(1)`source_max'{
	quietly append using "`SE_file_sp_`form'_`rowst'_`i''"
	if `i'/10==int(`i'/10) {
		di "."
	}
	else {
...
if "`sename'"=="" {
	local sename "SE_var_sp_`form'_`rowst'"
}
else {
	ren SE_var_sp_`form'_`rowst' `sename'
}
if "`filename'"=="" {
	local filename "SE_file_sp_`form'_`rowst'"
}
quietly sort `sourceid' `targetid' `time'
...
quietly {
	...
	if "`labelname'"=="" {
		if "`form'"=="source" {
			label var `sename' "Specific source contagion spatial effect variable" 
		}
		else {
			label var `sename' "Specific target contagion spatial effect variable" 
		}
	}
	else {
