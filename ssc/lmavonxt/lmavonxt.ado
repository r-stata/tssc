...
tempvar DW E e E2 E3 E4 Ea Ea1 EE Eo Es Es1 LDE LE LEo P Q Sig2 SRho SSE
tempvar Time tm U U2 Ue wald weit Wi Wio X X0 Yb Yh Yh2 Yhb Yt YY Z 
tempname A B b Beta Bx Cov D den DF Dim Dx E E1 F In IPhi J K kb kx llf N NC Nn
tempname Pm Q Rho Sig2 Sig21 Sig2o Sig2o1 SSE SSEo NT P Phi YYm YYv
tempname SST1 SST2 Ue_ML W W1 W2 Wald waldm We Wi V Wi1 Wio WS X X0 Y Yh Yh_ML
qui xtset `id' `it'
...
local NC2=r(imax)
qui tsset `Time'
tempname Rho Rhosq Rw1 Rw2 lmadw lmavon
qui tsset `Time'
tempvar Ue_ML1
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Autocorrelation Von Neumann Ratio Test}}"
di _dup(78) "{bf:{err:=}}"
...
di as txt "- Von Neumann Ratio Test" _col(35) "=" as res %9.4f `lmavon' _col(50) "df: ("  `kb'  " , " `N' ")
di _dup(78) "-"
