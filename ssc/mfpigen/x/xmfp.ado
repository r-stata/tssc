*! version 1.1.1 PR 31oct2012
program define xmfp, eclass 
...
*noi di in red _n `"Initial: qui FracSel `cmd' `lhs' `ni' `wgt' if `touse', df(1) `fixed' select(1) `regopt'"'
			`VV' ///
...
*noi di in red `"Proper: FracSel `cmd' `lhs' `ni' `wgt' if `touse', `dfi' `pw' `zero`r'' `catz`r'' `fixed' `h' `fpo' `regopt' `pvalopt' `n' `sequential' `testlinear'"'
			`VV' ///
...
*/
`VV' ///
quietly `cmd' `lhs' `finalvl' `linadj' `wgt' if e(sample), `regopt'
/*
...
if "`nn'"!="" | `df'==1 {
	* test linear for single or group of predictors, adjusting for base
	...
	local n `n' `nn'
*noi di in red `"qui TestVars `cmd' `lhs' `n' `if' `in' `weight', `base' `options'"'
	`VV' ///
...
*noi di in red "in TestVars: df for `rhs' = " `df' " base=`base' model df=" `df_m'
	frac_eqmodel k
	local df = `df' * `k'
	// PR 22mar2009 -end-
