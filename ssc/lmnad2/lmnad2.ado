...
program define lmnad2 , eclass byable(onecall)
version 11.2
...
tempvar wald SSE SST weit EE Ea Ea1 Es Es1 DW WS LE DE LEo Yho2
tempvar Yt LDE DF1 Eo Lambda0 Yh2 LYh2 E3 E4 YYm Yh
tempvar LE xq ht Hat Yhat1 Yho Yh_ML Ue_ML Wio Wi DFF DFF1 DFF2
tempname X Y Z M2 XgXg Xg M1 W1 W2 W1W Lambda Vec Yi E Cov b h mh kliml
tempname Wio Wi Ew OM Omega hjm Wald We vy1 v kyi kzi J D
tempname Bv1 Bv lamp1 lamp2 lamp M W b1 v1 q lmhs b2 v2 Beta P Pm IPhi Phi
tempname Hat Wi1 Z0 Z1 Eo E1 EE1 Sw Sn nw S11 S12 WY Xgw Eg Sig2 Sig2o
tempname X X0 Vec J Ds SST1 SST2 SSE Sig2 F K B Bx Zo qhiv V
tempname Nn kx kb DF N ky kyi kzi llf rhsendog Kz biv2 viv2 kmelo
tempname SSEo Sigo r2bu r2bu_a r2raw r2raw_a f fp wald waldp Q L
...
tempname B2 B3 LA Zp Rn Skn Ku1 Kun S1 S2 S3 S4
qui tsset `Time'
...
scalar `Zp'=abs(`B0'+`LA'*(`B1'+`LA'*(`B2'+`LA'*`B3')))
ereturn scalar lmnadp= normal(abs(-`Zp'))
qui sort `Time'
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Non Normality Anderson-Darling Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf: Ho: Normality - Ha: Non Normality}"
di _dup(78) "-"
di as txt "- Anderson-Darling Z Test" _col(40) "=" as res %9.4f e(lmnad) _col(55) as txt "P > Z(" %6.3f `Zp' ")" _col(73) as res %5.4f e(lmnadp)
di _dup(78) "-"
qui tsset `TimeN'
end
