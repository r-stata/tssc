{smcl}
{hline}
{cmd:help: {helpb spregrext}}{space 50} {cmd:dialog:} {bf:{dialog spregrext}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:spregrext: Spatial Panel Random Effects Regression: Lag and Durbin Models}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb spregrext##01:Syntax}{p_end}
{p 5}{helpb spregrext##02:Description}{p_end}
{p 5}{helpb spregrext##03:Model Options}{p_end}
{p 5}{helpb spregrext##04:Options}{p_end}
{p 5}{helpb spregrext##05:Spatial Panel Aautocorrelation Tests}{p_end}
{p 5}{helpb spregrext##06:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb spregrext##07:Heteroscedasticity Tests}{p_end}
{p 5}{helpb spregrext##08:Non Normality Tests}{p_end}
{p 5}{helpb spregrext##09:Saved Results}{p_end}
{p 5}{helpb spregrext##10:References}{p_end}

{p 1}*** {helpb spregrext##11:Examples}{p_end}

{p 5}{helpb spregrext##12:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt spregrext} {depvar} {indepvars} , {bf:{err:nc(#)}} {opt wmf:ile(weight_file)} {opt m:odel(sar, sdm)}{p_end} 
{p 3 5 6} 
{err: [} {opt lmsp:ac} {opt lmh:et} {opt lmn:orm} {opt diag} {opt test:s} {opt stand inv inv2} {opt mfx(lin, log)}  {opt aux(varlist)}{p_end} 
{p 5 5 6} 
{opt pred:ict(new_var)} {opt res:id(new_var)} {opt l:evel(#)} {opt nocons:tant} {opt dn coll zero tolog} {opth vce(vcetype)} {err:]}{p_end}

{p2colreset}{...}
{marker 02}{bf:{err:{dlgtab:Description}}}

{cmd:spregrext} estimates Spatial Panel MLE Random-Effects Regression
 with both Lag and Durbin Models

{p 2 4 2}{cmd:spregrext} can generate:{p_end}
...
{cmd:spregrext} generates some variables names with prefix:
...
{col 20}{cmd:spregrext} will convert automatically spatial cross section weight matrix
...
{p 2 4 2 }Depending on the model estimated, {cmd:spregrext} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog spregrext}.
{bf:Note 4:} xtset is included automatically in {cmd:spregrext} models.
{hline}

 {stata clear all}

 {stata sysuse spregrext.dta, clear}

 {stata db spregrext}

{bf:{err:* (1) Spatial Panel Random Effects Lag Model}}
 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sar) mfx(lin) test}
 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sar) mfx(log) test tolog}

{bf:{err:* (2) Spatial Panel Random Effects Durbin Model}}
 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sdm) mfx(lin) test}

 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sdm) aux(dcs1 dcs2 dcs3) test}

 {stata spregrext y x1 x2 dcs1 dcs2 dcs3 , nc(7) wmfile(SPWxt) model(sdm) mfx(lin) test}
{hline}

. clear all
. sysuse spregrext.dta, clear
. spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sar) mfx(lin) tests

==============================================================================
*** Binary (0/1) Weight Matrix: 49x49 - NC=7 NT=7 (Non Normalized)
==============================================================================
==============================================================================
* Spatial Panel Random-Effects Lag Regression (SAR)
==============================================================================
  y = w1y_y + x1 + x2
------------------------------------------------------------------------------
  Sample Size       =          49   |   Cross Sections Number   =           7
  Wald Test         =     59.6853   |   P-Value > Chi2(3)       =      0.0000
  F-Test            =     19.8951   |   P-Value > F(3 , 39)     =      0.0000
 (Buse 1973) R2     =      0.7981   |   Raw Moments R2          =      0.9633
 (Buse 1973) R2 Adj =      0.7515   |   Raw Moments R2 Adj      =      0.9548
  Root MSE (Sigma)  =      8.3417   |   Log Likelihood Function =   -167.8776
------------------------------------------------------------------------------
- R2h= 0.4981   R2h Adj= 0.3822  F-Test =   14.88 P-Value > F(3 , 39)  0.0000
- R2v= 0.4324   R2v Adj= 0.3014  F-Test =   11.43 P-Value > F(3 , 39)  0.0000
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
       w1y_y |  -.0970908   .0624423    -1.55   0.128    -.2233922    .0292106
          x1 |  -.2627951   .0800613    -3.28   0.002    -.4247343   -.1008559
          x2 |  -1.065928   .2860726    -3.73   0.001    -1.644564   -.4872913
       _cons |   70.26203   7.203419     9.75   0.000     55.69174    84.83232
------------------------------------------------------------------------------
  Rho Value  = -0.0971       Chi2 Test =     2.418   P-Value > Chi2(1)  0.1281
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function       LLF               =  -167.8776
- Akaike Final Prediction Error AIC               =   349.7552
- Schwarz Criterion             SC                =   362.9979
- Akaike Information Criterion  ln AIC            =     4.3000
- Schwarz Criterion             ln SC             =     4.5702
- Amemiya Prediction Criterion  FPE               =    79.5240
- Hannan-Quinn Criterion        HQ                =    81.6563
- Rice Criterion                Rice              =    77.5359
- Shibata Criterion             Shibata           =    71.2064
- Craven-Wahba Generalized Cross Validation-GCV   =    75.3821
------------------------------------------------------------------------------
...
- GLOBAL Moran MI            =   0.1581     P-Value > Z( 1.573)   0.1158
- GLOBAL Geary GC            =   0.7908     P-Value > Z(-1.482)   0.1384
- GLOBAL Getis-Ords GO       =  -0.4517     P-Value > Z(-1.573)   0.1158
------------------------------------------------------------------------------
- Moran MI Error Test        =   0.7232     P-Value > Z(6.540)    0.4695
------------------------------------------------------------------------------
- LM Error (Burridge)        =   1.1082     P-Value > Chi2(1)     0.2925
- LM Error (Robust)          =   4.1311     P-Value > Chi2(1)     0.0421
------------------------------------------------------------------------------
  Ho: Spatial Lagged Dependent Variable has No Spatial AutoCorrelation
  Ha: Spatial Lagged Dependent Variable has    Spatial AutoCorrelation

- LM Lag (Anselin)           =   0.1502     P-Value > Chi2(1)     0.6984
- LM Lag (Robust)            =   3.1731     P-Value > Chi2(1)     0.0749
------------------------------------------------------------------------------
  Ho: No General Spatial AutoCorrelation
  Ha:    General Spatial AutoCorrelation

- LM SAC (LMErr+LMLag_R)     =   4.2813     P-Value > Chi2(2)     0.1176
- LM SAC (LMLag+LMErr_R)     =   4.2813     P-Value > Chi2(2)     0.1176
------------------------------------------------------------------------------
...
- Engle LM ARCH Test AR(1): E2 = E2_1   =   0.2763   P-Value > Chi2(1)  0.5992
------------------------------------------------------------------------------
- Hall-Pagan LM Test:   E2 = Yh         =   0.0280   P-Value > Chi2(1)  0.8672
- Hall-Pagan LM Test:   E2 = Yh2        =   0.0148   P-Value > Chi2(1)  0.9030
- Hall-Pagan LM Test:   E2 = LYh2       =   0.0725   P-Value > Chi2(1)  0.7878
------------------------------------------------------------------------------
- Harvey LM Test:    LogE2 = X          =   1.6048   P-Value > Chi2(2)  0.4483
- Wald Test:         LogE2 = X          =   3.9596   P-Value > Chi2(1)  0.0466
- Glejser LM Test:     |E| = X          =   7.0870   P-Value > Chi2(2)  0.0289
- Breusch-Godfrey Test:  E = E_1 X      =   9.1582   P-Value > Chi2(1)  0.0025
------------------------------------------------------------------------------
- Machado-Santos-Silva Test: Ev=Yh Yh2  =   0.2813   P-Value > Chi2(2)  0.8688
- Machado-Santos-Silva Test: Ev=X       =   6.3181   P-Value > Chi2(3)  0.0971
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X      =   8.1716   P-Value > Chi2(3)  0.0426
- White Test - B-P-G (SSR): E2 = X      =  11.7901   P-Value > Chi2(3)  0.0081
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2   =   9.8556   P-Value > Chi2(6)  0.1309
- White Test - B-P-G (SSR): E2 = X X2   =  14.2198   P-Value > Chi2(6)  0.0273
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2 XX=  27.4331   P-Value > Chi2(9)  0.0012
- White Test - B-P-G (SSR): E2 = X X2 XX=  39.5809   P-Value > Chi2(9)  0.0000
------------------------------------------------------------------------------
- Cook-Weisberg LM Test: E2/S2n = Yh    =   0.0404   P-Value > Chi2(1)  0.8408
- Cook-Weisberg LM Test: E2/S2n = X     =  11.7901   P-Value > Chi2(3)  0.0081
------------------------------------------------------------------------------
*** Single Variable Tests (E2/Sig2):
- Cook-Weisberg LM Test: w1y_y             =   0.0011 P-Value > Chi2(1) 0.9733
- Cook-Weisberg LM Test: x1                =   2.8006 P-Value > Chi2(1) 0.0942
- Cook-Weisberg LM Test: x2                =   2.9608 P-Value > Chi2(1) 0.0853
------------------------------------------------------------------------------
*** Single Variable Tests:
- King LM Test: w1y_y                      =   0.0166 P-Value > Chi2(1) 0.8975
- King LM Test: x1                         =   0.0815 P-Value > Chi2(1) 0.7752
- King LM Test: x2                         =   3.0773 P-Value > Chi2(1) 0.0794
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
*** Non Normality Tests:
- Jarque-Bera LM Test                  =   1.9767     P-Value > Chi2(2) 0.3722
- White IM Test                        =   8.3237     P-Value > Chi2(2) 0.0156
- Doornik-Hansen LM Test               =   4.6284     P-Value > Chi2(2) 0.0988
- Geary LM Test                        =  -1.0081     P-Value > Chi2(2) 0.6041
- Anderson-Darling Z Test              =   0.3602     P > Z( 0.112)     0.5447
- D'Agostino-Pearson LM Test           =   2.8163     P-Value > Chi2(2) 0.2446
------------------------------------------------------------------------------
*** Skewness Tests:
- Srivastava LM Skewness Test          =   0.3753     P-Value > Chi2(1) 0.5401
- Small LM Skewness Test               =   0.4612     P-Value > Chi2(1) 0.4971
- Skewness Z Test                      =  -0.6791     P-Value > Chi2(1) 0.4971
------------------------------------------------------------------------------
*** Kurtosis Tests:
- Srivastava  Z Kurtosis Test          =   1.2654     P-Value > Z(0,1)  0.2057
- Small LM Kurtosis Test               =   2.3551     P-Value > Chi2(1) 0.1249
- Kurtosis Z Test                      =   1.5346     P-Value > Chi2(1) 0.1249
------------------------------------------------------------------------------
    Skewness Coefficient = -0.2144     - Standard Deviation =  0.3398
    Kurtosis Coefficient =  3.8856     - Standard Deviation =  0.6681
------------------------------------------------------------------------------
    Runs Test: (22) Runs -  (24) Positives - (25) Negatives
...
|------------+--------------------+--------------------+--------------------|
|      w1y_y |            -0.0971 |            -0.2764 |           100.0064 |
|         x1 |            -0.2628 |            -0.2875 |            38.4362 |
|         x2 |            -1.0659 |            -0.4362 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:SPREGREXT Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2012)}{p_end}
{p 1 10 1}{cmd:SPREGREXT: "Spatial Panel Random Effects Regression: Lag and Durbin Models"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/S457523.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/S457523.htm"}
