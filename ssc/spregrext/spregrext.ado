...
program define spregrext, eclass 
 version 11.0
syntax varlist , nc(str) Model(str) [WMFile(str) diag aux(str) INV INV2 ///
 ...
 coll PREDict(str) RESid(str) tolog TESTs DN NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spregrext
...
tempname Xb A B b Beta BetaSP Bm Bx Cov D den DVE DVNE Dx E E1
tempname J K M mh P Phi Pm Q In WS D WW eVec Xo kbm SSE SST1 SST2
tempname Sig2 Sig2b Sig2o Sig2u We Wi Wi1 Wio WY X X0 XB V F h Hat
tempname Vec vh VM VP Vs W W1 W2 Wald Y Yh YYm YYv Z Dim kb IPhi
tempname N DF kx kb NC NT Nmiss llf wmat xAx xAx2 TRa2 wWw1 xBx X0 Sig21
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
di _dup(78) "{bf:{err:=}}"
if inlist("`model'", "sar") {
di as txt "{bf:{err:* Spatial Panel Random-Effects Lag Regression (SAR)}}"
 }
if inlist("`model'", "sdm") {
di as txt "{bf:{err:* Spatial Panel Random-Effects Durbin Regression (SDM)}}"
 }
di _dup(78) "{bf:{err:=}}"
...
matrix `In'=I(`N')
qui xtset `idv' `itv'
qui xtreg `yvar' `SPXvar' , mle `vce' `level' `noconstant'
scalar `llf'=e(ll)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
