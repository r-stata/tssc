*! version 1.01 2020-03-10

program define stpp, rclass
  version 16.0
...
************
  ...
  if "`standstrata'" != "" {
    ...
    if "`standweights'" == "" {
      di "You need to specify weights using the standweights() option when you standardize"
      exit 198
    }
    else {
  ...
  for(k=1;k<=Nbylevels;k++) {
    for(s=1;s<=Nstandlevels;s++) {
      ...
      for(j=1;j<=Nuniq;j++) {
        ...
        atrisk2  = (t_by:>=tj)
	      atrisk_index = selectindex(atrisk2)

    	  attage  = rowmin((floor(agediag_by[atrisk_index] :+ (tstart_j)), pmmaxage_vec[atrisk_index])) :+ ageplus
      	attyear = rowmin((year(datediag_by[atrisk_index] :+ (tstart_j)*365.24), pmmaxyear_vec[atrisk_index])) :+ yearplus

        exprates = J(rows(atrisk_index),1,.)
        for(p=1;p<=Npmoth_levels;p++) {
          pmotherselect = rowsum(pmothervars_by[atrisk_index,] :== pmoth_levels[p,]):==Npmother :& atrisk2[atrisk_index]
          zzz = cols_pmother[p]
          for(b=zzz;b;b--) {
        ...
        expcumhaz[atrisk_index] = expcumhaz[atrisk_index] :+ exprates
        expsurv_tj = exp(-(expcumhaz[atrisk_index]))

        if(!ederer2) wt_t = indweights_by[atrisk_index]:/expsurv_tj
        else wt_t = indweights_by[atrisk_index]

        died_tj = (t_by[atrisk_index]:==tj) :& d_by[atrisk_index]
    	  wt_atrisk = atrisk[atrisk_index] :* wt_t
  ...
  if(haslist) {
    ...
    for(k=1;k<=Nbylevels;k++) {
      ...
      for(i=1;i<=Nlist;i++) {
        ...
        tmplist[i,] = (list[i],asarray(RS_PP, k)[tminindex,])
      }
