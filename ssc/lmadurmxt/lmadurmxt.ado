...
tempvar D DE DF1 DW E e TimeN EE Eo ht idv itv LE
tempvar LEo P Q Sig2 SSE SST Time wald weit Wi Wio WS X X0 Bo
tempvar Yb Yh Yh2 Yhb Yho Yt YY YYm YYv Yh_ML Ue_ML Z D DE DF DF1
tempvar E LDE LE LEo P Q Sig2 SSE Time wald weit Wi Wio X X0 Yb Yh Yh2 Yhb Yho Yt YY
tempname A B b Beta Bx Cov Cov1 D den DF Dim Dx E E1 Sig2o Sig2o1 SSE SSEo
tempname Ew F In IPhi J K kb kx llf N NC Nn NT P Phi Pm Q Rho Sig2 Sig21
tempname SST1 SST2 Ue Ue_ML v2 vh W W1 W2 Wald waldm We Wi 
...
local NC2=r(imax)
qui tsset `Time'
tempname Rho Rhosq Rw1 Rw2 lmadmd lmadmdp dmdf1 dmdf2 lmadmk lmadmkp dmkf1 dmkf2
tempname SSEu SSE SSE1 
tempvar Ue_ML1
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Autocorrelation Dynamic Durbin m Test}}"
di _dup(78) "{bf:{err:=}}"
...
di _dup(78) "-"
di as txt "- Durbin m Test (drop 1 cs obs)" _col(43) "=" as res %9.4f `lmadmd' _col(55) as txt "P-Value > F(" `dmdf1' "," `dmdf2' ")" _col(73) as res %5.4f `lmadmdp'
di as txt "- Durbin m Test (keep 1 cs obs)" _col(43) "=" as res %9.4f `lmadmk' _col(55) as txt "P-Value > F(" `dmkf1' "," `dmkf2' ")" _col(73) as res %5.4f `lmadmkp'
di _dup(78) "-"
