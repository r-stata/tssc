...
{title:Generate quantile envelopes for beta quantile-quantile plot}
...
{pstd}
{cmd:qenvbeta} generates two numeric variables for later plotting on a
quantile-quantile plot that define an envelope of low and high
expectations for each quantile given repeated sampling from a beta
distribution with the same number of values and parameters  
...
{pstd}
1. Either fit beta distribution using {stata findit betafit:betafit}, which must be 
installed separately, or specify the parameters of the beta distribution in the 
{cmd:alpha()} and {cmd:beta()} options. 

{pstd}
2. Take {cmd:reps} samples of the same size from a beta distribution
...
{phang}
{cmd:alpha()} specifies the alpha parameter of the beta distribution. If
the {cmd:alpha()} option is specified, than the {cmd:beta()} option must
also be specified.

{phang}
{cmd:beta()} specifies the alpha parameter of the beta distribution. If
the {cmd:beta()} option is specified, than the {cmd:alpha()} option must
also be specified.

{pmore}
The parameters alpha and beta refer to the parameterization of the beta 
distribution as implemented in {help betaden:betaden}.


{title:Examples}

{pstd}
In the example below we check the result that if we repeatedly draw B 
observations from a continuous standard uniform distribution and record 
the {it:k}th smallest value, then that will follow a beta distribution 
with parameters {it:k} and B + 1 - {it:k}.

{phang}{cmd:. clear all}{p_end}
{phang}{cmd:. program define sim, rclass}{p_end}
{phang}{cmd:.     drop _all}{p_end}
{phang}{cmd:.     set obs 10}{p_end}
{phang}{cmd:.     gen x = runiform()}{p_end}
{phang}{cmd:.     sort x}{p_end}
{phang}{cmd:.     return scalar x = x[2]}{p_end}
{phang}{cmd:. end}{p_end}
{phang}{cmd:. }{p_end}
{phang}{cmd:. simulate x=r(x), reps(1000) : sim}{p_end}
{phang}{cmd:. qenvbeta x, gen(lb ub) alpha(2) beta(9) overall reps(5000)}{p_end}
{phang}{cmd:. qplot x lb ub, ms(oh none ..) c(. l l) lc(gs10 ..) legend(off) } 
       {cmd:title("2{sup:nd} smallest value out of 10 random draws" "from a continuous standard uniform distribution")}
	   {cmd:trscale(invibeta(2,9,@)) xtitle("beta(2,9) quantiles")}{p_end}
{p 4 4 2}({stata `"qenv_ex "beta""':click to run}){p_end}
