*! sptobitmstardh V1.0 25/11/2013
*! 
...
program define sptobitmstardh , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstardh" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , WMFile(str) LL(str) [NWmat(int 1) NOCONStant MHET(str) ///
 INV INV2 vce(passthru) stand MFX(str) noLOG ROBust aux(str) coll ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str) tolog]
gettoken yvar xvar : varlist
local sthlp sptobitmstardh
...
tempvar Wi Wio Xb XB Xo XQ Yb Yh Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname A B b Beta BetaSP Bx WS Cov D E F IPhi J K olsin N DF kx kb Nmiss
tempname P Phi Pm Q Sig2 Sig2b Sig2o SSE SST1 SST2 D WW eVec eigw Xo
tempname In IRW Vec vh W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V kz
tempname Y Yh Yi YYm YYv Z llf Sig21 Dim Rostar rRo kb
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardh1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardh2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardh3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardh4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
