...
program define lmhmss2 , eclass byable(onecall)
version 11.2
...
tempvar Yt LDE DF1 Eo Lambda0 Yh2 LYh2 E3 E4 YYm DumE EDumE Yh
...
tempname Hat Sig2n Wi1 Z0 Z1 Eo E1 EE1 Sw Sn nw S11 S12 WY Xgw Eg Sig2 Sig2o
tempname X X0 Vec J SST1 SST2 Sig2o1 CovC SSE Sig2 F K B Bx Zo qhiv V
...
tempname lmhmss1 lmhmss1p mssdf1 lmhmss2 lmhmss2p mssdf2
local N=`Nn'
qui tsset `Time'
...
qui gen `DumE'=0 if `touse'
qui replace `DumE'=1 if `E' >= 0
qui summ `DumE' if `touse'
qui gen `EDumE'=`E'*(`DumE'-r(mean)) if `touse'
qui regress `EDumE' `Yh' `Yh2' if `touse' `wgt' , `noconstant' `vce'
scalar `lmhmss1'=e(N)*e(r2)
scalar `lmhmss1p'=chi2tail(e(df_m), abs(`lmhmss1'))
scalar `mssdf1'=e(df_m)
qui regress `EDumE' `xvar' if `touse' `wgt' , `noconstant' `vce'
scalar `lmhmss2'=e(N)*e(r2)
scalar `lmhmss2p'=chi2tail(e(df_m), abs(`lmhmss2'))
scalar `mssdf2'=e(df_m)
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Heteroscedasticity Tests - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
