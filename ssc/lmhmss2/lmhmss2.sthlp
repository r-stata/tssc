{smcl}
{hline}
{cmd:help: {helpb lmhmss2}}{space 50} {cmd:dialog:} {bf:{dialog lmhmss2}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:lmhmss2: 2SLS-IV Heteroscedasticity Machado-Santos-Silva Test}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb lmhmss2##01:Syntax}{p_end}
{p 5}{helpb lmhmss2##02:Description}{p_end}
{p 5}{helpb lmhmss2##03:Model}{p_end}
{p 5}{helpb lmhmss2##04:GMM Options}{p_end}
{p 5}{helpb lmhmss2##05:Other Options}{p_end}
{p 5}{helpb lmhmss2##06:Saved Results}{p_end}
{p 5}{helpb lmhmss2##07:References}{p_end}

{p 1}*** {helpb lmhmss2##08:Examples}{p_end}

{p 5}{helpb lmhmss2##09:Authors}{p_end}

{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 2 4 2} 
{cmd:lmhmss2} {depvar} {it:{help varlist:indepvars}} {cmd:({it:{help varlist:endog}} = {it:{help varlist:inst}})} {ifin} , {p_end} 
{p 6 6 2}
...
{pstd}
{cmd:lmhmss2} computes 2SLS-IV Heteroscedasticity Machado-Santos-Silva Test for instrumental variables regression models, via 2sls, liml, melo, gmm, and kclass.{p_end}
...
{synopt:{bf:noconexog}}Exclude Constant Term from all Equations (both RHS and Instrumental Equations). Results of using {cmd:noconexog} option are identical to Stata {helpb ivregress}.
 The default of {cmd:lmhmss2} is including Constant Term in both RHS and Instrumental Equations{p_end}

{marker 06}{bf:{err:{dlgtab:Saved Results}}}

{cmd:lmhmss2} saves the following in {cmd:e()}:
...
{marker 08}{bf:{err:{dlgtab:Examples}}}
 ...
 {stata sysuse lmhmss2.dta , clear}

 {stata db lmhmss2}

 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(2sls)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(melo)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(liml)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(fuller) kf(0.5)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(kclass) kc(0.5)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(white)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(bart)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(dan)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(nwest)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(parzen)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(quad)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(tent)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(trunc)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(tukeym)}
 {stata lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(tukeyn)}
{hline}

. clear all
. sysuse lmhmss2.dta , clear
. lmhmss2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(2sls)
...
------------------------------------------------------------------------------
 ...
 (Buse 1973) R2     =      0.9467   |   Raw Moments R2          =      0.9985
 (Buse 1973) R2 Adj =      0.9344   |   Raw Moments R2 Adj      =      0.9982
...
==============================================================================
* 2SLS-IV Heteroscedasticity Machado-Santos-Silva Test - Model= (2sls)
==============================================================================
 Ho: Homoscedasticity - Ha: Heteroscedasticity
------------------------------------------------------------------------------
- Machado-Santos-Silva Test: Ev=Yh Yh2 =   0.7335    P-Value > Chi2()   0.6930
- Machado-Santos-Silva Test: Ev=X      =   3.7662    P-Value > Chi2(3)  0.2878
------------------------------------------------------------------------------
...
{bf:{err:{dlgtab:LMHMSS2 Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2014)}{p_end}
{p 1 10 1}{cmd:LMHMSS2: "Stata Module to Compute 2SLS-IV Heteroscedasticity Machado-Santos-Silva Test"}{p_end}
