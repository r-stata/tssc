...
program define xtregrem, eclass 
version 11.0
...
local sthlp xtregrem
qui marksample touse
qui markout `touse' `varlist' `wvar' , strok
...
tempvar _X _Y absE Bw D DE DF1 DW DX_ DY_ E dcs Time TimeN
...
tempname Xb A B b B1 b1 b2 Beta Bx Cov D den DF dfab Dim DVE DVNE Dx E
tempname Ew F f1 f13 f13d gam gam2 Go GoRY IDRmk In IPhi J K kb BOLS BOLS1
tempname kbm Ko Koi Kr kx kxd L lf llf lmhs lmiab lmiabp Ls LSC M M1 Sig2n
tempname M2 mh N n NC Nmiss Nn NT nw eVec E1 Eg We Wi Qrq Phi Pm q Q q1 q2 Qr
tempname olsin Om Om1 Om2 Omega Omm Omm1 Omm2 P Sig2 Sig21 Sig2b Sig2o CovC
tempname rid Rmk RX RY s sd Sig2o1 Sig2u Sig2w sigox SLS Sn sqN SSE SSEo 
tempname v2 VaL VaL1 VaL21 VaLv1 Vec vh VM VN VP VQ Vs W W1 W2 Wald waldm
tempname v V v1 V1 V1s Wi1 Wio WMTD WS WW WY X X0 XB Xfe Xg XMB Xo xq Xre
tempname Xx Y Yfe Yh Yh_ML Yre YYm YYv Z Zo Zr Zz SST1 SST2 Sw Ue Ue_ML
...
di 
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Fuller-Battese GLS Random-Effects Panel Data: Ridge and Weighted Regression}}"
di _dup(78) "{bf:{err:=}}"
preserve
tempname Sig2b Sig2u Sig2w Dx Sigw Sigb Sigu Rho ss
tempvar dumxcs
qui tabulate `idv' if `touse' , generate(`dumxcs')
qui tsset `Time'
qui regress `yvar' `xvar' `dumxcs'* if `touse' , noconstant
scalar `Sigw'=e(rmse)
matrix `Beta'=invsym(`X''*`P'*`X')*(`X''*`P'*`Y')
matrix `E'=`P'*(`Y'-`X'*`Beta')
matrix `Sig2b'=`E''*`E'/(`NC'-`kb')
...
local id `idv'
qui tab `idv' if `touse'
local vlist "`yvar' `xvar'"
qui levels `idv' , local(levels)
qui foreach l of local levels {
qui foreach i of local vlist {
qui summ `i' if `idv' == `l', meanonly
scalar `ss' =r(mean)
 replace `i' = `i'-`Rho'*`ss' if `idv' == `l'
 }
 }
tempvar cons
qui gen double `cons' = 1-`Rho' if `touse'
tokenize `vlist'
local yvarre `1'
macro shift
local xvarre "`*'"
qui mkmat `yvarre' if `touse' , matrix(`Yre')
if "`noconstant'"!="" {
qui mkmat `xvarre' if `touse' , matrix(`Xre')
 }
 else { 
qui mkmat `xvarre' `cons' if `touse' , matrix(`Xre')
 }
if "`weights'"!="" & "`wvar'"!="" {
tempvar Wis ws
tempname Wis
qui gen `Wis' = 0 if `touse'
qui levels `idv' , local(levels)
qui foreach l of local levels {
qui summ `Wi' if `idv' == `l', meanonly
scalar `ss' =r(mean)
qui replace `Wis' = `Wi'-`Rho'*`ss' if `idv' == `l'
 }
mkmat `Wis' if `touse' , matrix(`Wis')
matrix `Wis'=diag(`Wis')
matrix `Omega'=`Wis''*`Wis'
 }
 else {
matrix `Omega'=`Wi''*`Wi'
 }
matrix Beta=invsym(`Xre''*`Omega'*`Xre'+`Zz')*`Xre''*`Omega'*`Yre'
matrix `E'=(`Yre'-`Xre'*Beta)
matrix Sig2=`E''*`E'/(`N'-`kb')
matrix E=`Wi'*(`Y'-`X'*Beta)
if "`ridge'"!="" & `kr' > 0 {
matrix `Xx'=`Xre''*`Omega'*`Xre'
matrix Cov=Sig2*invsym(`Xx'+`Zz')*`Xx'*invsym(`Xx'+`Zz')
 } 
 else { 
matrix Cov=Sig2*invsym(`Xre''*`Xre')
 }
restore 
matrix `Cov'=Cov
matrix `Beta'=Beta
qui cap matrix drop Beta Sig2 Cov E
