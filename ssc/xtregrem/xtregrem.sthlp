{smcl}
{hline}
{cmd:help: {helpb xtregrem}}{space 50} {cmd:dialog:} {bf:{dialog xtregrem}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregrem: Fuller-Battese GLS Random-Effects Panel Data Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregrem##01:Syntax}{p_end}
{p 5}{helpb xtregrem##02:Description}{p_end}
{p 5}{helpb xtregrem##03:Options}{p_end}
{p 5}{helpb xtregrem##04:Ridge Options}{p_end}
{p 5}{helpb xtregrem##05:Weight Options}{p_end}
{p 5}{helpb xtregrem##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregrem##07:Other Options}{p_end}
{p 5}{helpb xtregrem##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregrem##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregrem##10:Saved Results}{p_end}
{p 5}{helpb xtregrem##11:References}{p_end}

{p 1}*** {helpb xtregrem##12:Examples}{p_end}

{p 5}{helpb xtregrem##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregrem} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 2 2 2} {cmd:xtregrem} estimates Fixed-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregrem} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregrem} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregrem} can use large types of weighted regression options.
...
{col 20}{cmd:xtregrem} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregrem} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregrem} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregrem.dta, clear}

  {stata db xtregrem}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregrem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregrem.dta, clear
. xtregrem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh

==============================================================================
* Fuller-Battese GLS Random-Effects Panel Data: Ridge and Weighted Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
 ...
 Wald Test          =     33.1623   |   P-Value > Chi2(2)       =      0.0000
 F-Test             =     16.5812   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.7468   |   Raw Moments R2          =      0.9532
 (Buse 1973) R2 Adj =      0.6961   |   Raw Moments R2 Adj      =      0.9439
  Root MSE (Sigma)  =      8.7723   |   Log Likelihood Function =   -170.9642
------------------------------------------------------------------------------
- R2h= 0.5522   R2h Adj= 0.4626  F-Test =   28.36 P-Value > F(2 , 40)  0.0000
- R2v= 0.2521   R2v Adj= 0.1025  F-Test =    7.75 P-Value > F(2 , 40)  0.0014
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
          x1 |  -.1749944    .086516    -2.02   0.050    -.3498497    -.000139
          x2 |  -1.105569   .2960547    -3.73   0.001    -1.703918   -.5072206
       _cons |   53.45387   5.152151    10.38   0.000     43.04098    63.86675
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -170.9642
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     71.0015
- Akaike Information Criterion              (1973) Log AIC =      4.2627
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     79.7205
- Schwarz Criterion                         (1978) Log SC  =      4.3785
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     81.6642
- Hannan-Quinn Criterion                    (1979) HQ      =     74.1911
- Rice Criterion                            (1984) Rice    =     71.5840
- Shibata Criterion                         (1981) Shibata =     70.5107
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     71.2795
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|         x1 |            -0.1750 |            -0.1915 |            38.4362 |
|         x2 |            -1.1056 |            -0.4524 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:XTREGREM Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGREM: "Fuller-Battese GLS Random-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457459.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457459.htm"}
