...
program define xtregsam, eclass 
version 11.0
...
tempname rid Rmk RX RY s sd Sig2 Sig21 Sig2b Sig2o Sig2n CovC
...
tempname Xx Y Yh Yh_ML YYm YYv Z Z0 Z1 Zo Zr Zz Om Om1 Om2 Omm Omm1 Omm2 
...
di 
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Swamy-Arora Random-Effects Panel Data: Ridge and Weighted Regression}}"
di _dup(78) "{bf:{err:=}}"
matrix `VQ'=(`Y''*`Q'*`Y'-`Y''*`Q'*`X'*inv(`X''*`Q'*`X')*`X''*`Q'*`Y')/(`N'-`NC'-`kx')
matrix `VP'=(`Y''*`P'*`Y'-`Y''*`P'*`X'*inv(`X''*`P'*`X')*`X''*`P'*`Y')/(`N'-`NC'-1)
matrix `VM'=(`VP'-`VQ')/`NT'
tempname VMo VPo VQo VQos VPos VPo THw
scalar `VMo'=`VM'[1,1]
scalar `VPo'=`VP'[1,1]
scalar `VQo'=`VQ'[1,1]
