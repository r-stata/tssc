{smcl}
{hline}
{cmd:help: {helpb xtregsam}}{space 50} {cmd:dialog:} {bf:{dialog xtregsam}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregsam: Swamy-Arora Random-Effects Panel Data Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregsam##01:Syntax}{p_end}
{p 5}{helpb xtregsam##02:Description}{p_end}
{p 5}{helpb xtregsam##03:Options}{p_end}
{p 5}{helpb xtregsam##04:Ridge Options}{p_end}
{p 5}{helpb xtregsam##05:Weight Options}{p_end}
{p 5}{helpb xtregsam##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregsam##07:Other Options}{p_end}
{p 5}{helpb xtregsam##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregsam##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregsam##10:Saved Results}{p_end}
{p 5}{helpb xtregsam##11:References}{p_end}

{p 1}*** {helpb xtregsam##12:Examples}{p_end}

{p 5}{helpb xtregsam##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregsam} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 2 2 2} {cmd:xtregsam} estimates Swamy-Arora Random-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregsam} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregsam} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregsam} can use large types of weighted regression options.
...
{col 20}{cmd:xtregsam} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregsam} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregsam} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregsam.dta, clear}

  {stata db xtregsam}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregsam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregsam.dta, clear
. xtregsam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh

==============================================================================
* Swamy-Arora Random-Effects Panel Data: Ridge and Weighted Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
 ...
 Wald Test          =   4101.4361   |   P-Value > Chi2(2)       =      0.0000
 F-Test             =   2050.7181   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.7394   |   Raw Moments R2          =      0.9519
 (Buse 1973) R2 Adj =      0.6873   |   Raw Moments R2 Adj      =      0.9422
  Root MSE (Sigma)  =      8.6794   |   Log Likelihood Function =   -170.4428
------------------------------------------------------------------------------
- R2h= 0.5516   R2h Adj= 0.4620  F-Test =   28.30 P-Value > F(2 , 40)  0.0000
- R2v= 0.4338   R2v Adj= 0.3206  F-Test =   17.62 P-Value > F(2 , 40)  0.0000
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
          x1 |  -.2667096   .0095986   -27.79   0.000    -.2861092     -.24731
          x2 |  -1.350062   .0327156   -41.27   0.000    -1.416183   -1.283941
       _cons |   64.33976   .5427087   118.55   0.000     63.24291    65.43662
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -170.4428
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     69.5064
- Akaike Information Criterion              (1973) Log AIC =      4.2414
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     78.0418
- Schwarz Criterion                         (1978) Log SC  =      4.3572
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     79.9447
- Hannan-Quinn Criterion                    (1979) HQ      =     72.6289
- Rice Criterion                            (1984) Rice    =     70.0767
- Shibata Criterion                         (1981) Shibata =     69.0260
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     69.7787
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|         x1 |            -0.2667 |            -0.2918 |            38.4362 |
|         x2 |            -1.3501 |            -0.5525 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:XTREGSAM Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGSAM: "Swamy-Arora Random-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457460.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457460.htm"}
