*! version 1.0.0  09apr2015. Edited version of mfpmi_10.ado.
program define mfpmi_wald_10, eclass sortpreserve
...
if "`1'" == "" | "`1'"=="," {
	...
	`VV' FracRep "fractional polynomial" "  df  " "Powers" "`level'" mim
	exit
}
...
syntax [anything(name=xlist)] [if] [in] [aw fw pw iw] , ///
 [ ADDpowers(str) ADJust(str) CENTer(str) AIC ALpha(str) ALL ///
  CATzero(str) DF(str) DFDefault(int 4) CYCles(int 5) FIXpowers(str) ///
  FP01 noSCAling POwers(str) SELect(str) SEQuential noTESTLinear ///
  XOrder(str) XPowers(str) ZERo(str) LEVel(cilevel) * ]

// use -mim- to do fitting
global mimprog mim
global mifit "$mimprog, category(fit) storebv noclear: "
global replay "$mimprog"

* Disentangle
GetVL `xlist'
local options `options' `cmdopts'
...
quietly {
	...
	count if `touse' & (`impid' == 1)
	local nobs = r(N)

	// Transfer mark for estimation sample for `impid'=1 to `impid'=0
	qui sum `obsid' if `impid' == 1, meanonly
	local N = r(max)	// max sample size
	tempvar touse0 J
	sort `impid' `obsid'
	gen byte `touse0' = `touse'[_n + `N'] if `impid' == 0
	replace `touse' = 0 if `impid' == 0
	egen int `J' = group(`impid') if `touse'
	qui sum `J', meanonly
	local M = r(max)	// # imputations
}
/*
...
/*
// Compute product of non-missingness proportion and 1/M
qui forvalues i = 1 / `nx' {
	if `nx`i'' == 1 {
		count if !missing(`n`i'') & `touse0' == 1
		local vwt`i' = (r(N) / `nobs') / `M'
	}
	else {
		local avwt 0
		forvalues j = 1 / `nx`i'' {
			local x: word `j' of `n`i''
			count if !missing(`x') & `touse0' == 1
			local avwt = `avwt' + r(N) / `nobs'
		}
		local vwt`i' = (`avwt' / `nx`i'') / `M'
	}
}
*/
/*
...
*/
	...
	if !`stable' {
		di as txt "{hline 70}" _n "End of Cycle " ///
		 as res `it' as txt _n as txt "{hline 70}"
	}
}
if `nx'>1 {
	local s
...
quietly {
	if "`finalvl'"!="" {
		// Store if/in variable for later use in fracpred_mim
		cap drop _mim_e
		gen byte _mim_e = `touse'
		`VV' $mifit `cmd' `lhs' `finalvl' `wgt' if _mim_e, `regopt'
		local nterms: word count `finalvl'
		$mimprog: testparm `finalvl'
		local chi2 = `nterms'*invFtail(`nterms', `r(df_r)', `r(p)')
	}
	else local chi2 0
	if "`cmd'"=="stcox" local EST estimate
	* Check if outcome has missing values on original sample
	tempname tmp
	_estimates hold `tmp'
	cap `cmd' `lhs' if `touse0' == 1, `regopt' `EST'
	if (c(rc) == 0) {
		count if e(sample)
		local NN = r(N)
	}
	...
	if `NN' < `nobs' {
		* Has missing - average the null model deviances over imputations
		local d0 0
		forvalues j = 1 / `M' {
			`cmd' `lhs' if (`touse' == 1) & (`J' == `j'), `regopt' `EST'
			local d0 = `d0' - 2 * e(ll) / `M'
		}
	}
	else local d0 = -2 * e(ll)
	local dev = `d0' - `chi2'
...
`VV' FracRep "fractional polynomial" "  df  " "Powers" "`level'" "$replay"

* Store command to reproduce model using fracpoly.ado (new syntax)
local fracpoly `"$mifit fracpoly: `cmd' `lhs'"'
forvalues i=1/`nx2' {
...
ereturn local replay $replay
ereturn local cmdline `"mfpmi_wald `cmdline'"'
...
quietly {
	...
	if "`order'"=="n" {
		* variable order as given
		...
		if "`order'"=="+" | "`order'"=="-" {
			`VV' ///
			$mifit `cmd' $MFP_dv $MFP_cur `if' `in' [`weight' `exp'], `options'
			* micombine `cmd' $MFP_dv $MFP_cur `if' `in' [`weight' `exp'], `options'
		}
		tempvar c n
...
local nn `*'
if "`head'"=="" {
	di as txt "Variable" _col(14) "Model" _col(20) "(vs.)" /*
...
if "`nn'"!="" | `df'==1 {
	* test linear for single or group of predictors, adjusting for base
	...
	di as txt "`vname'" _col(14) "`vs1'" /*
	 */ _col(21) "`vs'" _col(27) %9.3f as res `dfirst' /*
	 */ _col(38) %8.3f `d' %7.3f `P' "`star'" /*
	 */ _col(57) "`pwrs2'" _col(67) "`pwrs'"
	if "`n'"=="" {
		local pwrs .
	}
	di as txt _col(14) "Final" _col(27) %9.3f as res `dev' _col(57) "`pwrs'"
	di
	ret local rhs `n'
	ret local pwrs `pwrs'
	ret scalar dev=`dev'
...
local xvar `n'
`VV' ///
capture $mifit frac_154_wald `cmd' `lhs' `n' `fixpowers' `nn' `base' `if' `in' /*
 */ `weight', `degree' `powers' `options' `catzero'
local rc=_rc
if `rc' {
	noi di as err "failure encountered when estimating fractional polynomial model"
 	noi di as err `"$mifit frac_154_wald `cmd' `lhs' `n' `fixpowers' `nn' `base' `if' `in' `weight', `degree' `powers' `options' `catzero'"'
	error `rc'
}
...
// PR 22mar2009 -end-
		...
		di as txt "`hed'" _col(14) "`vs'" /*
		 */ _col(21) "`degmax'" _col(26) %10.3f as res `dev`j'' /*
		 */ _col(38) %8.3f `d' %7.3f `P' "`star'" /*
	...
	if `alpha'==1 {			/* do not optimize function */
		...
		* compute AICs
		forvalues j=0/1 {
			...
			di as txt "`hed'" _col(14) "`vs_`j''" /*
			 */ _col(26) %10.3f as res `dev_`j'' /*
			 */ _col(57) "`pwrs_`j''"
			 local hed
		}
		...
		forvalues j=`m0'/`m' {
			if "`testlin'"!="notestlinear" | `j'!=0 {
				...
				di as txt "`hed'" _col(14) "`s(desc)'" /*
				 */ _col(26) %10.3f as res `dev`s(j)'' /*
				 */ _col(57) "`s(pwrs)'"
				if `Aic'<`aicopt' {
					local jopt `j'
					local aicopt `Aic'
...
*/
	...
	while !`done' & `j0'>`m0' {
		...
		di as txt "`hed'" _col(14) "`vs1'" /*
		 */ _col(21) "`vs'" _col(26) %10.3f as res `dev`j1'' /*
		 */ _col(38) %8.3f `d' %7.3f `P' "`star'" /*
...
di as txt _col(14) "Final" _col(28) as res %8.3f `dev' _col(57) "`pwrs'"
local rhs "`rhs' `n'"
di
ret local rhs `rhs'
ret local pwrs `pwrs'
...
version 10
local VV : di "version " string(_caller()) ", missing:"
args desc param paramv level replay
if "`level'" == "" {
	local level = c(level)
}
local i 1
...
/*
if "`e(cmd2)'"=="stpm" `VV' ml display, level(`level')
else `VV' `e(cmd)', level(`level')
*/
if "`e(fp_fvl)'" != "" {
	$replay
}
...
program define TestVars, rclass /* LR-blocktests variables in varlist, adj base */
	...
	*/
	/* ****************************************************************************
		!! ?? mods needed here for mfpmi_wald.
	...
	*/
	`VV' ///
	$mifit `cmd' `y' `rhs' `base' `wgt' if `touse', `options'
*	frac_dv `normal' "`wgt'" `nobs' `mnlnwt' `dist' `glm' /*
* 	*/ `xtgee' `qreg' "`scale'"

	local nterms: word count `rhs'
	$mimprog: testparm `rhs'
	local dev1 = -`nterms'*invFtail(`nterms', `r(df_r)', `r(p)')
