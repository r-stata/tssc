*! version 2.0.2  22dec2010
program define mfpmi, eclass 
	local VV : di "version " string(_caller()) ", missing:"
	version 10
	vercheck mfpmi_10 1.0.1
	vercheck mim 2.1.8
...
/*
	...
	before presenting it to mfpmi_10.
*/
	local 0 `statacmd'
	syntax [anything] [if] [in] [aw fw pw iw], [*]
	if `"`weight'"' != "" local wgt [`weight'`exp']
	local options `options' hascolon
	`VV' mfpmi_10 `anything' `if' `in' `wgt', mfpopts(`mfpopts') `options'
end
