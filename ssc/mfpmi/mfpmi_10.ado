*! version 1.0.1  22dec2010
program define mfpmi_10, eclass sortpreserve
...
local mimprog mim
local mifit "`mimprog', category(fit) storebv noclear: "
local replay "`mimprog'"
...
/*
			 ...
			 */ df(1) `fixed' select(1) vwt(`vwt`r'') `regopt'
			local dev=r(dev)
			di in gr _n /*
...
*/
		...
		if `df`r''==1 & `sel`r''==1 {	/* var is included anyway */
			 ...
			 */ `fpo' `regopt' `pvalopt' `n' `sequential' `testlinear' vwt(`vwt`r'') 
			local h`r' `r(rhs)'
			local dev=r(dev)
			local p `r(pwrs)'
			if "`p'"!="`po`r''" {
	...
	if !`stable' {
		di as txt "{hline 60}" _n "End of Cycle " ///
		 as res `it' as txt _n as txt "{hline 60}"
	}
}
if `nx'>1 {
	local s
...
ereturn local cmdline `"mfpmi `cmdline'"'
...
*/ POwers(string) BAse(string) noTIDy CATzero SEQuential noTESTLinear VWt(real 1) * ]
...
if "`nn'"!="" | `df'==1 {
	* test linear for single or group of predictors, adjusting for base
	...
	local n `n' `nn'
	`VV' ///
	qui TestVars `cmd' `lhs' `n' `if' `in' `weight', `base' `options' vwt(`vwt')
	local P = r(P)
	local dev1 = r(dev1)
	local dev0 = r(dev0)
	local d = r(devdif)
	 ...
	 */ _col(21) "`vs'" as res /*
	 */ _col(28) %8.3f `d' %7.3f `P' "`star'" /*
	 */ _col(47) "`pwrs2'" _col(57) "`pwrs'"
	if "`n'"=="" {
		local pwrs .
	}
	di as txt _col(14) "Final" as res _col(47) "`pwrs'"
	di
	ret local rhs `n'
	ret local pwrs `pwrs'
	ret scalar dev=`dev'
...
while `j' <= (`m' + 1) {
	if `j' == 0 {
		local dev0 = e(fp_d0) * `vwt'
	}
	else if `j' == 1 {
		local dev1 = e(fp_dlin) * `vwt'
	}
	else local dev`j' = e(fp_d`j0') * `vwt'
	local j0 `j'
	local j = `j' + 1
}
/*
...
// PR 22mar2009 -end-
		 ...
		 */ _col(21) "`degmax'" as res /*
		 */ _col(28) %8.3f `d' %7.3f `P' "`star'" /*
		 */ _col(47) "`pwrs'" _col(57) "`bigp'"
		local hed
		local degmax
		local bigp
		if `j0'<`m' & `P'>`pnom' {
	...
	if `alpha'==1 {			/* do not optimize function */
		...
		* compute AICs
		forvalues j=0/1 {
			 ...
			 */ _col(26) %10.3f as res `dev0'-`dev_`j'' /*
			 */ _col(47) as res "`pwrs_`j''"
			 local hed
		}
		...
		forvalues j=`m0'/`m' {
			if "`testlin'"!="notestlinear" | `j'!=0 {
				 ...
				 */ _col(26) %10.3f as res `dev0'-`dev`s(j)'' /*
				 */ _col(47)as res "`s(pwrs)'"
				if `Aic'<`aicopt' {
					local jopt `j'
					local aicopt `Aic'
				}
...
di as txt _col(14) "Final" _col(47) as res "`pwrs'"
local rhs "`rhs' `n'"
di
ret local rhs `rhs'
ret local pwrs `pwrs'
...
program define TestVars, rclass /* LR-blocktests variables in varlist, adj base */
	...
	syntax `vv' [if] [in] [aw fw pw iw], /*
 	 */ [, DEAD(string) noCONStant BASE(varlist) vwt(real 1) * ]
	frac_cox "`dead'" `dist'
