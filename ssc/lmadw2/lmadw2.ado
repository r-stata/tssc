...
program define lmadw2 , eclass byable(onecall)
version 11.0
...
tempvar wald SSE SST weit EE Ea Ea1 Es Es1 DW WS LE DE LEo xq
tempvar Yt e LDE DF1 Eo Lambda0 Yh2 LYh2 E3 E4 Yhr YYm
tempvar LnE2 absE LE XQ ht Hat Yhat1 Yho Yh_ML Ue_ML Wio Wi DFF Yh   
tempname X Y Z M2 XgXg Xg M1 W1 W2 W1W Lambda Vec Yi E Cov b h mh kliml
tempname Wio Wi Ew OM Omega hjm Wald We v kyi kzi J D
tempname Bv1 Bv lamp1 lamp2 lamp M W b1 v1 q lmhs b2 v2 Beta P Pm IPhi Phi
tempname Hat Sig2n Wi1 Z0 Z1 xq Eo E1 EE1 Sw Sn nw S11 S12 WY Xgw Eg
tempname DCor X X0 Vec Val J Ds Cr Dr S SST1 SST2 Sig2 Sig2o
tempname Sig2o1 SSE Sig2 F K B Bx Zo V Nn kx kb DF N ky kyi kzi llf Kz kmelo
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Autocorrelation Durbin-Watson Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
...
di as txt "- Durbin-Watson Test" _col(40) "AR(" `i' ")=" as res %8.4f `lmadw'`i' _col(56) "df: ("  `kx'  " , " `N' ")
