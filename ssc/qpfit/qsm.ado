...
program qsm, sort
	...
	if "`param'" != "" { 
		...
		cap assert "`e(ba)'" != "" & "`e(bb)'" != "" & "`e(bq)'" != ""
		if _rc {
                	di as err "needs parameter estimates for a, b and q" 
	                exit 198
        	}
		else { 
			local A = e(ba) 
	...
	quietly {
		...
		replace `Psubi' = ///
	        ...
	        gen double `Z' = ///
        	       	`B' * (( 1 / (1 - `Psubi'))^(1 / `Q') - 1)^(1 / `A') ///
			if `touse' 
		label var `Z' "Inverse Singh-Maddala"
	...
	qui if "`grid'" != "" {
		foreach p in 5 10 25 50 75 90 95 { 
                        local smq`p' : di %4.3f ///
	                `B' * (( 1 / (1 - `p' / 100))^(1 / `Q') - 1)^(1 / `A')
                }
		
                local xtl "`smq50' `smq5' `smq95'"
                local xn  "`xtl' `smq25' `smq75' `smq10' `smq90'"
	...
	if "`generate'" != "" { 
		gen `generate' = `Z' 
		label var `generate' "Singh-Maddala quantiles for `varlist'" 
	}	
end
