{smcl}
{hline}
{cmd:help: {helpb xtregam}}{space 50} {cmd:dialog:} {bf:{dialog xtregam}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregam: Amemiya Random-Effects Panel Data: Ridge and Weighted Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregam##01:Syntax}{p_end}
{p 5}{helpb xtregam##02:Description}{p_end}
{p 5}{helpb xtregam##03:Options}{p_end}
{p 5}{helpb xtregam##04:Ridge Options}{p_end}
{p 5}{helpb xtregam##05:Weight Options}{p_end}
{p 5}{helpb xtregam##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregam##07:Other Options}{p_end}
{p 5}{helpb xtregam##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregam##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregam##10:Saved Results}{p_end}
{p 5}{helpb xtregam##11:References}{p_end}

{p 1}*** {helpb xtregam##12:Examples}{p_end}

{p 5}{helpb xtregam##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregam} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 4 5 6} 
{opt nocons:tant} {opt coll dn tolog} {opt l:evel(#)} {opth vce(vcetype)} {err:]}{p_end}

{p2colreset}{...}
{marker 02}{bf:{err:{dlgtab:Description}}}

{p 2 2 2} {cmd:xtregam} estimates Amemiya Random-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregam} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregam} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregam} can use large types of weighted regression options.
...
{col 20}{cmd:xtregam} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregam} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregam} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregam.dta, clear}

  {stata db xtregam}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregam y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregam.dta, clear
. xtregam y x1 x2 , id(id) it(t) ridge(grr1) weights(x) wvar(x1) mfx(lin) lmhet diag

==============================================================================
* Amemiya Random-Effects Panel Data: Ridge and Weighted Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  Wald Test         =     35.5355   |   P-Value > Chi2(2)       =      0.0000
  F-Test            =     17.7678   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.7003   |   Raw Moments R2          =      0.9446
 (Buse 1973) R2 Adj =      0.6404   |   Raw Moments R2 Adj      =      0.9336
  Root MSE (Sigma)  =      9.6270   |   Log Likelihood Function =   -175.5200
------------------------------------------------------------------------------
- R2h= 0.5494   R2h Adj= 0.4593  F-Test =   28.04 P-Value > F(2 , 40)  0.0000
- R2v= 0.1029   R2v Adj=-0.0765  F-Test =    2.64 P-Value > F(2 , 40)  0.0839
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
          x1 |  -.1411531   .0603246    -2.34   0.024    -.2630736   -.0192325
          x2 |  -.6253377   .1723796    -3.63   0.001    -.9737298   -.2769456
       _cons |   47.71979   2.706059    17.63   0.000     42.25064    53.18894
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -175.5200
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     85.5117
- Akaike Information Criterion              (1973) Log AIC =      4.4487
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     96.0125
- Schwarz Criterion                         (1978) Log SC  =      4.5645
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     98.3535
- Hannan-Quinn Criterion                    (1979) HQ      =     89.3532
- Rice Criterion                            (1984) Rice    =     86.2133
- Shibata Criterion                         (1981) Shibata =     84.9206
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     85.8466
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|         x1 |            -0.1412 |            -0.1544 |            38.4362 |
|         x2 |            -0.6253 |            -0.2559 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:XTREGAM Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGAM: "Amemiya Random-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457453.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457453.htm"}
