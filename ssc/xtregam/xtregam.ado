...
tempname A B b B1 b1 B12 B1b B1t b2 BB2 Beta Bm BOLS BOLS1 Bx Cov 
tempname D den DF Dim DVE DVNE Dx E E1 eVec Pm Q Qr XMB
tempname Ew F f1 f13 f13d gam gam2 Go GoRY HQ IDRmk In IPhi J K kb 
tempname kbm Ko Koi Kr kx L lf llf Ls LSC M CovC N NC NE Nn NT Omega P Phi
tempname rid Rmk RX RY s sd Sig2 Sig21 Sig2b Sig2o Om Om1 Om2 Omm Omm1 Omm2
tempname Sig2o1 sigox SLS Sn sqN SSE SSEo SST1 SST2 Sw Ue Ue_ML 
...
tempname Xx Y Yh Yh_ML YYm YYv Z Z0 Z1 Zo Zr Zz Sig2n
...
di 
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Amemiya Random-Effects Panel Data: Ridge and Weighted Regression}}"
di _dup(78) "{bf:{err:=}}"
matrix `Beta'=inv(`X''*`Q'*`X'+`Zz')*`X''*`Q'*`Y'
matrix `A'=`Yb'-`XMB'*`Beta'
matrix `DVE'=`Y'-`A'*`X0'-`X'*`Beta'
matrix `VQ'=(`DVE''*`Omega'*`DVE')/(`N'-`NC')
matrix `VP'=(`DVE''*`P'*`DVE')/`NC'
matrix `VM'=(`VP'-`VQ')/`NT'
tempname VMo VPo VQo VQos VPos THw
scalar `VMo'=`VM'[1,1]
scalar `VPo'=`VP'[1,1]
scalar `VQo'=`VQ'[1,1]
