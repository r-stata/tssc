{smcl}
{hline}
{cmd:help: {helpb logithetm}}{space 55} {cmd:dialog:} {bf:{dialog logithetm}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf: logithetm: Logit Multiplicative Heteroscedasticity Regression}

{bf:{err:{dlgtab:Syntax}}}

{p 8 16 2}
{cmd:logithetm} {depvar} [{indepvars}] {ifin} {weight} {cmd:,} 
...
{synopt :{it:{help logithetm##display_options:display_options}}}control spacing
           and display of omitted variables and base and empty cells{p_end}

{syntab :Maximization}
{synopt :{it:{help logithetm##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}
...
{pstd}
{cmd:logithetm} fits a maximum-likelihood for Logit Multiplicative Heteroscedasticity Regression.
...
maximization; see {manhelp logit R}.
...
{pstd}
The following option is available with {opt logithetm} but is not shown in the
dialog box:
...
{pstd}
{cmd:logithetm} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(cmd)}}{cmd:logithetm}{p_end}
...
{synopt:{cmd:e(offset1)}}offset for Logit equation{p_end}
...
{bf:{err:{dlgtab:Examples}}}
	...
	{stata sysuse logithetm.dta, clear}

	{stata db logithetm}

	{stata logithetm y x1 , het(x1)}
...
{bf:{err:{dlgtab:logithetm Citation}}}

{phang}Shehata, Emad Abd Elmessih (2011){p_end}
{phang}{cmd:logithetm: "Stata Module to Estimate Logit Multiplicative Heteroscedasticity Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457324.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457324.htm"}
