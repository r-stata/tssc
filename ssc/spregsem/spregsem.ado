...
program define spregsem , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsem" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) [LL(real 0) INV INV2 vce(passthru) ///
 dist(str) stand MFX(str) NOLog LMHet LMSPac tobit iter(int 100) LMNorm ///
 zero level(passthru) coll PREDict(str) ///
 RESid(str) tech(str) tolog TESTs diag]
gettoken yvar xvar : varlist
local sthlp spregsem
...
tempname IPhi J K M mh olsin Sig2n P Phi Pm Q In IRW IRWL
tempname Sig2 Sig2o SSE SST1 SST2 D WW eVec eigw Xo Wio WY X XB V Vec rLm kb
tempname vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Y Yh Yi YYm YYv Z X0 llf
tempname minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
tempname waldl waldlp waldl_df Dim N DF kx Nmiss kz Sig21 Dim
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
qui global spat_ARGS "`spat_ARGS' beta0 Lambda Sigma"
 ml model lf spregsem1 `MODEL' `wgt' , ///
...
qui global spat_ARGS "`spat_ARGS' beta0 Lambda"
 ml model lf spregsem2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
...
local MODEL "`MODEL'(_cons:) (Lambda:) (Sigma:)"
qui global spat_ARGS "`spat_ARGS' beta0 Lambda Sigma"
 ml model lf spregsem3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
tempname ImR D0 D1 DB DS U3 U4 M1 M2 DB0 XBX SigV SigV2 Yh SigM H
