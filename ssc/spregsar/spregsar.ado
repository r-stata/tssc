...
program define spregsar , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsar" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) [ll(real 0) vce(passthru) ///
 dist(str) stand MFX(str) NOLog LMHet LMSPac ROBust iter(int 100) ///
 zero level(passthru) coll PREDict(str) RESid(str) tech(str) ///
 INV INV2 tolog TESTs NOCONStant MHET(str) diag LMNorm tobit]
gettoken yvar xvar : varlist
local sthlp spregsar
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ Hat ht LE LEo LYh2 P Q Sig2
tempvar SST Time U U2 wald weit Wi Wio WS X X0 SSE
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE Dx E F 
tempname K M mh n olsin Sig2n P Phi Pm Q In IRW IRWR kbm
tempname Sig2 Sig2o SSE SST1 SST2 D WW eVec eigw Xo IPhi J
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V
tempname Y Yh Yi YYm YYv Z X0 Ue llf rRo df Dim Sig21 N DF kx kb
tempname kz minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df Nmiss
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
local initopt init(`olsin', copy)  
 ml model lf spregsar1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "SAR1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy)  
 ml model lf spregsar2 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
 (Rho:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
...
local MName "SAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spregsar3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf spregsar4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
scalar `kxt' = `kx'
qui tsset `Time'
qui tobit `yvar' `SPXvar' , nolog ll(`llt') `noconstant'
qui predict `XBX' , xb
local k = `kxt'
