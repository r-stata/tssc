...
program define sptobitsdmxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsdmxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) LL(str) WMFile(str) [aux(str) ///
 INV mhet(str) vce(passthru) dist(str) stand MFX(str) NOLog INV2 LMHet ///
 LMSPac ROBust iter(int 100) zero coll level(passthru) PREDict(str) ///
 diag RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
local sthlp sptobitsdmxt
...
if "`mhet'"!="" & inlist("`dist'", "exp", "weib") {
di 
di as err " {bf:mhet({it:varlist})} {cmd:cannot be used with} {bf:dist({it:exp, weib})}"
 exit
 }
 if "`mfx'"!="" {
...
tempvar D DE DF1 DumE E dcs XQX_ EE Hat ht idv itv LE LEo P Q
tempvar SST Time U U2 Ue wald weit Wi Wio WS X X0 eigw Bo Sig2 SSE
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B Beta BetaSP Bx Cov D den DVE DVNE Dx E E1 F
tempname h Hat IPhi J K M P Phi Pm Q Sig2 Sig2o SSE Sig2n rRo kb SST1 SST2
tempname In IRW IRWR kbm Wald We Wi Wi1 Wio WY X X0 XB V
tempname Vec vh VM VP VQ W W1 W2 Y Yh YYm YYv Z WS D WW eVec eigw
tempname Xo mh olsin N DF kx kb NC NT Nmiss llf
tempname Sig21 sigox Dim wmat xAx xAx2 TRa2 wWw1 xBx minEig maxEig
tempname waldm waldmp waldm_df waldr waldrp waldr_df waldx waldxp waldx_df
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
if !inlist("`dist'", "exp", "weib") & "`mhet'"=="" {
local MName "SDM1n"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf sptobitsdmxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Durbin Panel Normal Model (SDM)}}"
di _dup(78) "{bf:{err:=}}"
 }

 if inlist("`dist'", "exp") & "`mhet'"=="" {
local MName "SDM1e"
 matrix `olsin'=`Bo',0
 local initopt init(`olsin', copy) 
 ml model lf sptobitsdmxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) `wgt' , `mlopts' `nolog' `diparm' `initopt' maximize ///
 title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Durbin Panel Exponential Model (SDM)}}"
di _dup(78) "{bf:{err:=}}"
 }

 if inlist("`dist'", "weib") & "`mhet'"=="" {
local MName "SDM1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf sptobitsdmxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Durbin Panel Weibull Model (SDM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
matrix `olsin'=`Bo',`olshet',0,`rmse'
 local initopt init(`olsin', copy) 
 ml model lf sptobitsdmxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Hetero: `mhet', noconst) (Rho:) (Sigma:) `wgt', `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons `mhet' Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Durbin Panel Multiplicative Heteroscedasticity}}"
di _dup(78) "{bf:{err:=}}"
 }
...
tempname I m1k m2k m3k m4k MEAN NUM NUM1 NUM2 NUM3 RJ IN xMx WXb
