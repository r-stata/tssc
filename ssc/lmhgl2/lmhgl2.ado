...
program define lmhgl2 , eclass byable(onecall)
version 11.2
...
tempvar Yt e LDE DF1 Eo Lambda0 YYm absE LE XQ ht Hat Yhat1 Yho
tempvar DFF DFF1 DFF2 Yh Yh_ML Ue_ML Wio Wi
...
tempname lmhgl lmhglp
local N=`Nn'
qui tsset `Time'
tempvar E
qui gen `E' =`Ue_ML' if `touse'
qui gen `absE'=abs(`E') if `touse'
qui regress `absE' `xvar' if `touse'
scalar `lmhgl'=e(mss)/((1-2/_pi)*`Sig2n')
scalar `lmhglp'= chi2tail(2, abs(`lmhgl'))
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Heteroscedasticity Glejser Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
