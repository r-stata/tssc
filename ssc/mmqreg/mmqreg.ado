...
program define mmqreg1, eclass sortpreserve
	syntax varlist(fv) [in] [if] [aw pw iw fw], [Quantile(str)  denopt(str) robust cluster(varname) dfadj]
	...
	mata:mmqreg_vce1`x'("`y'","`xvar' `cns'","`lfb'","`sfb'", ///
		...
		*qui:gen `idd'=_n
		...
		mata:mmqreg_vce1c("`y'","`xvar' `cns'","`lfb'","`sfb'", ///
					 "`qval'","`qth'","`fden'","`cluster'", "`touse'","`dfadj'")
		*sort `idd'
	}
...
program define mmqreg2, eclass sortpreserve
	syntax varlist(fv) [in] [if] [aw pw iw fw], [Quantile(str) Absorb(varlist) cluster(varname) denopt(str) dfadj robust]
	** start with sample checks
	capture drop _i_* 
	capture drop ___zero___
	marksample touse
	...
	** this code does all the rest. 
	if "`cluster'"=="" {
		mata:mmqreg_vce2`x'("`y'","`xvar' `cns'","`st_res'","`ares_hat'","`lfb'","`sfb'", ///
					 "`qval'","`qth'","`fden'", `df_a', "`touse'","`dfadj'")
	}
	else {
	    sort `cluster'
	    mata:mmqreg_vce2c("`y'","`xvar' `cns'","`st_res'","`ares_hat'","`lfb'","`sfb'", ///
					 "`qval'","`qth'","`fden'", "`cluster'", `df_a', "`touse'","`dfadj'")
	}
	* After the mata code is run, I get vq bq
	* but need extra info. eq name and betas
	* This should capture all EQ names
...
void mmqreg_vce1x(string scalar yvar_, string scalar xvar_,	
				 ...
				 string scalar fden_,  string scalar touse,  string scalar dfadj ) {
		...
		qxx=quadcross(xvar,xvar)
	   iqxx=invsym(qxx)
		
		// E(U_s) E(U_s^2)
		us1=quadcross(u_hat,J(rows(u_hat),1,1))
		...
		//		( iqxx    , qval[2]*iqxx , J(k,1,0), 1/us1*gama)
 		...
 		omg = quadcross((xvar:*u_hat:*u,xvar:*u_hat:*v,u_hat:*w),(xvar:*u_hat:*u,xvar:*u_hat:*v,u_hat:*w))	
...
void mmqreg_vce1c(string scalar yvar_, string scalar xvar_,	
				 string scalar beta_,  string scalar gama_,
				 string scalar qval_,  string scalar qth_, 
				 string scalar fden_, string scalar cvar_,  string scalar touse,  string scalar dfadj) {
real matrix xvar, yvar
		real vector beta, gama , betaq
	   ...
	   iqxx=invsym(qxx)
		...
		real scalar nc, dfr,one
		  ...
		  // lxvar=(xvar:*u_hat:*u,xvar:*u_hat:*v,u_hat:*w)
 		  omg  = J(cols(xvar)*2+1,cols(xvar)*2+1,0)
        ...
        for(i=1; i<=nc; i++) {
			...
			one   = J(rows(xi),1,1)
            uhatu = panelsubmatrix(u_hat:*u,i,info)
			uhatv = panelsubmatrix(u_hat:*v,i,info)
			uhatw = panelsubmatrix(u_hat:*w,i,info)
			omgs= ( (xi'*uhatu)*(uhatu'*xi) , (xi'*uhatu)*(uhatv'*xi) , (xi'*uhatu)*(uhatw'*one) \ ///
			        (xi'*uhatv)*(uhatu'*xi) , (xi'*uhatv)*(uhatv'*xi) , (xi'*uhatv)*(uhatw'*one) \ ///
				   (one'*uhatw)*(uhatu'*xi) ,(one'*uhatw)*(uhatv'*xi) ,(one'*uhatw)*(uhatw'*one) )
            omg  =omg + omgs
        }
		...
		//n=(nobs-(k-diag0cnt(qxx)))
		vcvq = xi2*omg*xi2'*(nobs-1)/nn*(nc/(nc-1))
			//bq="b"+strofreal(i)
			//vq="v"+strofreal(i)
		st_matrix("__bq",betaq)
		st_matrix("__vq",makesymmetric(vcvq) )	
...
void mmqreg_vce1(string scalar yvar_, string scalar xvar_,	
	   ...
	   iqxx=invsym(qxx)
		...
		pxx=quadcross(xvar,u_hat:^2,xvar)
		px =quadcross(xvar,u_hat:^2)
	
		// E(U_s) E(U_s^2)
		us1=mean(u_hat)*nobs
		us2=mean(u_hat:^2)*nobs
		...
		// This residual changes by quantile
		w=J(nobs,qs,0)
///%%%% IMPORTANT. NEED TO FIGURE OUT THIS COMPOMENT
		for(i=1;i<=qs;i++) {
			w[.,i]=1/fden[i]*(qth[i]:-((u:-qval[i]):<=0))-(u:+(qval[i]*(v:-1)))
		}	
		
		euw=quadcross(u,w)/nobs
		evw=quadcross(v,w)/nobs
		ew2=quadcross(w,w)/nobs
...
void mmqreg_vce2(string scalar yvar_, string scalar xvar_,	
				  ...
				  string scalar beta_,  string scalar gama_,
				  string scalar qval_,  string scalar qth_, 
				  string scalar fden_, real scalar df_a, string scalar touse,  string scalar dfadj) {
		real matrix xvar, yvar
		real vector beta, gama , betaq
		real vector qval, qth, fden
		real matrix u_hat, u, v, w,  euv, euw , evw, ew2
		...
		euv=mean(u:*v)
		// elements that do not depend on w
		qxx=quadcross(xvar,xvar)/nobs
		
	   iqxx=invsym(qxx)
	   
		pxx=quadcross(xvar,u_hat:^2,xvar)/nobs
		px =quadcross(xvar,u_hat:^2)/nobs
...
void mmqreg_vce2c(string scalar yvar_, string scalar xvar_,	
				  ...
				  real scalar df_a, string scalar touse,  string scalar dfadj) {
		real matrix xvar, yvar, cvar
		real vector beta, gama , betaq
		real vector qval, qth, fden
		real matrix u_hat, u, v, w  
		real matrix qxx, iqxx, xi, xi2, omg 
