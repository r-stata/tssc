{smcl}
{hline}
{cmd:help: {helpb xtregdhp}}{space 50} {cmd:dialog:} {bf:{dialog xtregdhp}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregdhp: Han-Philips (2010) Linear Dynamic Panel Data Regression"}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregdhp##01:Syntax}{p_end}
{p 5}{helpb xtregdhp##02:Description}{p_end}
{p 5}{helpb xtregdhp##03:Options}{p_end}
{p 5}{helpb xtregdhp##04:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregdhp##05:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregdhp##06:Saved Results}{p_end}
{p 5}{helpb xtregdhp##07:References}{p_end}

{p 1}*** {helpb xtregdhp##08:Examples}{p_end}

{p 5}{helpb xtregdhp##09:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregdhp} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
{err: [} {opt lmh:et} {opt diag be fe re} {opt mfx(lin|log)} {opt pred:ict(new_var)} {opt res:id(new_var)}{p_end} 
{p 4 5 6} 
{opt iter(#)} {opt nocons:tant} {opt coll tolog} {opt l:evel(#)} {err:]}{p_end}

{p2colreset}{...}
{marker 02}{bf:{err:{dlgtab:Description}}}

{p 2 2 2} {cmd:xtregdhp} estimates Han-Philips (2010) Linear Dynamic Panel Data Regression{p_end}
{p 2 2 2} with be, fe, re Effects{p_end}
...
{col 3}{opt be}  Between Effects {col 33}(BE)
{col 3}{opt fe}  Fixed-Effects {col 33}(FE)
{col 3}{opt re}  Random-Effects {col 33}(RE)
...
{p2colreset}{...}
{marker 04}{bf:{err:{dlgtab:Model Selection Diagnostic Criteria}}}
...
{p2colreset}{...}
{marker 05}{bf:{err:{dlgtab:Groupwise Panel Heteroscedasticity Tests}}}
...
{p 2 4 2 }{cmd:xtregdhp} saves the following results in {cmd:e()}:
...
{marker 08}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregdhp.dta, clear}

  {stata db xtregdhp}

  {stata xtset id t}

  {stata xtregdhp y x1 x2 , id(id) it(t) mfx(lin) diag lmh predict(Yh) resid(Eu)}

  {stata xtregdhp y x1 x2 , id(id) it(t) mfx(log) diag lmh predict(Yh) resid(Eu) tolog}

  {stata xtregdhp y x1 x2 , id(id) it(t) mfx(lin) diag lmh be}

  {stata xtregdhp y x1 x2 , id(id) it(t) mfx(lin) diag lmh fe}

  {stata xtregdhp y x1 x2 , id(id) it(t) mfx(lin) diag lmh re}
{hline}
...
==============================================================================
* Han-Philips (2010) Linear Dynamic Panel Data Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
  Sample Size       =          42   |   Cross Sections Number   =           7
  Wald Test         =     52.9984   |   P-Value > Chi2(3)       =      0.0000
  F-Test            =     17.6661   |   P-Value > F(3 , 39)     =      0.0000
 (Buse 1973) R2     =      0.5761   |   Raw Moments R2          =      0.9579
 (Buse 1973) R2 Adj =      0.5543   |   Raw Moments R2 Adj      =      0.9558
  Root MSE (Sigma)  =     14.4009   |   Log Likelihood Function =   -147.0867
------------------------------------------------------------------------------
- R2h= 0.4053   R2h Adj= 0.3748  F-Test =    8.63 P-Value > F(3 , 39)  0.0002
- R2v= 0.3197   R2v Adj= 0.2848  F-Test =    5.95 P-Value > F(3 , 39)  0.0019
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
           y |
         L1. |  -.1309041   .2819316    -0.46   0.642    -.6834798    .4216717
             |
          x1 |  -.2835875   .0871885    -3.25   0.001    -.4544739   -.1127011
          x2 |  -1.186583   .3002919    -3.95   0.000    -1.775144   -.5980212
       _cons |   71.78851    6.02257    11.92   0.000     59.98449    83.59253
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -147.0867
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     75.9077
- Akaike Information Criterion              (1973) Log AIC =      4.3295
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     88.5841
- Schwarz Criterion                         (1978) Log SC  =      4.4840
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     75.1008
- Hannan-Quinn Criterion                    (1979) HQ      =     80.4881
- Rice Criterion                            (1984) Rice    =     77.0535
- Shibata Criterion                         (1981) Shibata =     74.9996
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     76.4447
------------------------------------------------------------------------------
...
- Lagrange Multiplier LM Test     =   8.4465     P-Value > Chi2(6)   0.2072
- Likelihood Ratio LR Test        =  38.0427     P-Value > Chi2(6)   0.0000
- Wald Test                       =  14.7346     P-Value > Chi2(7)   0.0396
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|        L.y |            -0.1309 |            -0.1306 |            35.0262 |
|         x1 |            -0.2836 |            -0.3229 |            39.9877 |
|         x2 |            -1.1866 |            -0.4879 |            14.4425 |
+---------------------------------------------------------------------------+
 Mean of Dependent Variable =     35.1209
...
{bf:{err:{dlgtab:XTREGDHP Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGDHP: "Han-Philips (2010) Linear Dynamic Panel Data Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457456.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457456.htm"}
