{smcl}
{hline}
{cmd:help: {helpb spmstardxt}}{space 50} {cmd:dialog:} {bf:{dialog spmstardxt}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:spmstardxt: (m-STAR) Multiparametric Spatio Temporal AutoRegressive Regression}
{p 9 1 1}{bf:(Spatial Durbin Panel Models)}{p_end}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb spmstardxt##01:Syntax}{p_end}
{p 5}{helpb spmstardxt##02:Description}{p_end}
{p 5}{helpb spmstardxt##03:Options}{p_end}
{p 5}{helpb spmstardxt##04:Spatial Panel Aautocorrelation Tests}{p_end}
{p 5}{helpb spmstardxt##05:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb spmstardxt##06:Heteroscedasticity Tests}{p_end}
{p 5}{helpb spmstardxt##07:Non Normality Tests}{p_end}
{p 5}{helpb spmstardxt##08:Saved Results}{p_end}
{p 5}{helpb spmstardxt##09:References}{p_end}

{p 1}*** {helpb spmstardxt##10:Examples}{p_end}

{p 5}{helpb spmstardxt##11:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt spmstardxt} {depvar} {indepvars} {weight} , {bf:{err:nc(#)}} {opt wmf:ile(weight_file)} {opt nw:mat(#)} {p_end} 
{p 3 5 6} 
{err: [} {opt lmsp:ac} {opt lmh:et} {opt lmn:orm} {opt diag} {opt test:s} {opt nocons:tant} {opt coll zero tolog nolog robust}{p_end} 
{p 5 5 6} 
{opt aux(varlist)} {opt tobit stand inv inv2} {opt dist(norm|exp|weib)} {opt mfx(lin, log)}{p_end} 
{p 5 5 6} 
...
{p 2 2 2} {cmd:spmstardxt} estimates Spatial econometric regression (mSTAR) "Multiparametric Spatio Temporal AutoRegressive Regression" for Spatial Durbin Panel Models.{p_end}

 {cmd:spmstardxt} can estimate the following models:
   1- Heteroscedastic Regression Models in disturbance term.
   2- Non Normal Regression Models in disturbance term.

 {cmd:spmstardxt} estimates Continuous and Truncated Dependent Variables models {cmd:tobit}.

{p 2 2 2} {cmd:spmstardxt} deals with data either continuous or truncated dependent variable. If {depvar} has missing values or lower limits,
so in this case {cmd:spmstardxt} will fit spatial panel model via {cmd: tobit} model,
and thus {cmd:spmstardxt} can resolve the problem of missing values that exist in many kinds of data. Otherwise, in the case of continuous data, the normal estimation will be used.{p_end}

{p 2 4 2}{cmd:spmstardxt} can generate:{p_end}
...
{p 2 4 2} {cmd:spmstardxt} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:spmstardxt} generates some variables names with prefix:
...
{col 20}{cmd:spmstardxt} will convert automatically spatial cross section weight matrix
...
{p 2 4 2 }Depending on the model estimated, {cmd:spmstardxt} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog spmstardxt}.
{bf:Note 4:} xtset is included automatically in {cmd:spmstardxt} models.

{hline}

  {stata clear all}

  {stata sysuse spmstardxt.dta, clear}
...
{bf:* (1) *** Normal Distribution}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(norm)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt2) nwmat(2) mfx(lin) dist(norm)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt3) nwmat(3) mfx(lin) dist(norm)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(norm)}

 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(norm) aux(x3 x4)}
 
 {stata spmstardxt ys x1 x2, nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(norm) tobit ll(0)}
{hline}

{bf:* (2) *** Weibull Distribution}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(weib)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt2) nwmat(2) mfx(lin) dist(weib)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt3) nwmat(3) mfx(lin) dist(weib)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(weib)}

 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(weib) aux(x3 x4)}

 {stata spmstardxt ys x1 x2, nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(weib) tobit ll(0)}
{hline}

{bf:* (3) *** Exponential Distribution}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(exp)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt2) nwmat(2) mfx(lin) dist(exp)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt3) nwmat(3) mfx(lin) dist(exp)}
 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(exp)}

 {stata spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(exp) aux(x3 x4)}

 {stata spmstardxt ys x1 x2, nc(7) wmfile(SPWmxt4) nwmat(4) mfx(lin) dist(exp) tobit ll(0)}
{hline}

{bf:* (4) Weighted mSTAR Normal Distribution:}
 {stata spmstardxt y x1 x2  [weight = x1], nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(norm)}
 {stata spmstardxt y x1 x2 [aweight = x1], nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(norm)}
{hline}

{bf:* (5) Weighted mSTAR Weibull Distribution:}
 {stata spmstardxt y x1 x2  [weight = x1], nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(weib)}
 {stata spmstardxt y x1 x2 [aweight = x1], nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(weib)}
{hline}

{bf:* (6) Weighted mSTAR Exponential Distribution:}
 {stata spmstardxt y x1 x2  [weight = x1], nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(exp)}
 {stata spmstardxt y x1 x2 [aweight = x1], nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) dist(exp)}
{hline}

. clear all
. sysuse spmstardxt.dta, clear
. spmstardxt y x1 x2 , nc(7) wmfile(SPWmxt1) nwmat(1) mfx(lin) test

==============================================================================
*** Binary (0/1) Weight Matrix: 49x49 - NC=7 NT=7 (Non Normalized)
==============================================================================

Iteration 4:   log likelihood = -186.54165  
==============================================================================
...
------------------------------------------------------------------------------
  ...
  Wald Test         =    162.4755   |   P-Value > Chi2(4)       =      0.0000
  F-Test            =     40.6189   |   P-Value > F(4 , 38)     =      0.0000
 (Buse 1973) R2     =      0.7869   |   Raw Moments R2          =      0.9613
 (Buse 1973) R2 Adj =      0.7308   |   Raw Moments R2 Adj      =      0.9511
  Root MSE (Sigma)  =      8.6810   |   Log Likelihood Function =   -186.5417
------------------------------------------------------------------------------
- R2h= 0.5700   R2h Adj= 0.4568  F-Test =   14.58 P-Value > F(4 , 38)  0.0000
- R2v= 0.5606   R2v Adj= 0.4449  F-Test =   14.03 P-Value > F(4 , 38)  0.0000
------------------------------------------------------------------------------
- Sum of Rho's =   -0.0494776    Sum must be < 1 for Stability Condition
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
y            |
          x1 |  -.2579315   .1010545    -2.55   0.011    -.4559948   -.0598683
          x2 |  -1.559436   .3205661    -4.86   0.000    -2.187734   -.9311377
      w1x_x1 |  -.0189976   .0495416    -0.38   0.701    -.1160974    .0781022
      w1x_x2 |   .1155217   .1548836     0.75   0.456    -.1880445     .419088
       _cons |    69.8722   5.814107    12.02   0.000     58.47676    81.26764
-------------+----------------------------------------------------------------
        Rho1 |  -.0494776   .0417626    -1.18   0.236    -.1313308    .0323756
       Sigma |   10.85209   1.098217     9.88   0.000     8.699629    13.00456
------------------------------------------------------------------------------
 Wald Test [Rho1=0]:                 1.4036        P-Value > Chi2(1) 0.2361
...
==============================================================================
- Log Likelihood Function                   LLF            =   -186.5417
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     71.6739
- Akaike Information Criterion              (1973) Log AIC =      4.2721
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     86.9358
- Schwarz Criterion                         (1978) Log SC  =      4.4652
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     83.0501
- Hannan-Quinn Criterion                    (1979) HQ      =     77.1204
- Rice Criterion                            (1984) Rice    =     73.4279
- Shibata Criterion                         (1981) Shibata =     70.3697
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     72.4797
------------------------------------------------------------------------------
...
- GLOBAL Moran MI            =   0.0771     P-Value > Z( 0.910)   0.3630
- GLOBAL Geary GC            =   0.9225     P-Value > Z(-0.548)   0.5835
- GLOBAL Getis-Ords GO       =  -0.2422     P-Value > Z(-0.910)   0.3630
------------------------------------------------------------------------------
- Moran MI Error Test        =   0.5543     P-Value > Z(5.344)    0.5794
------------------------------------------------------------------------------
- LM Error (Burridge)        =   0.2750     P-Value > Chi2(1)     0.6000
- LM Error (Robust)          =   0.4097     P-Value > Chi2(1)     0.5221
------------------------------------------------------------------------------
  Ho: Spatial Lagged Dependent Variable has No Spatial AutoCorrelation
  Ha: Spatial Lagged Dependent Variable has    Spatial AutoCorrelation

- LM Lag (Anselin)           =   0.0019     P-Value > Chi2(1)     0.9650
- LM Lag (Robust)            =   0.1366     P-Value > Chi2(1)     0.7116
------------------------------------------------------------------------------
  Ho: No General Spatial AutoCorrelation
  Ha:    General Spatial AutoCorrelation

- LM SAC (LMErr+LMLag_R)     =   0.4116     P-Value > Chi2(2)     0.8140
- LM SAC (LMLag+LMErr_R)     =   0.4116     P-Value > Chi2(2)     0.8140
------------------------------------------------------------------------------
...
- Engle LM ARCH Test AR(1): E2 = E2_1   =   0.0405   P-Value > Chi2(1)  0.8404
------------------------------------------------------------------------------
- Hall-Pagan LM Test:   E2 = Yh         =   0.0262   P-Value > Chi2(1)  0.8714
- Hall-Pagan LM Test:   E2 = Yh2        =   0.0122   P-Value > Chi2(1)  0.9122
- Hall-Pagan LM Test:   E2 = LYh2       =   0.0138   P-Value > Chi2(1)  0.9066
------------------------------------------------------------------------------
- Harvey LM Test:    LogE2 = X          =   3.9325   P-Value > Chi2(2)  0.1400
- Wald Test:         LogE2 = X          =   9.7031   P-Value > Chi2(1)  0.0018
- Glejser LM Test:     |E| = X          =   8.0097   P-Value > Chi2(2)  0.0182
- Breusch-Godfrey Test:  E = E_1 X      =   8.5605   P-Value > Chi2(1)  0.0034
------------------------------------------------------------------------------
- Machado-Santos-Silva Test: Ev=Yh Yh2  =   0.2390   P-Value > Chi2(2)  0.8874
- Machado-Santos-Silva Test: Ev=X       =   6.5229   P-Value > Chi2(4)  0.1634
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X      =   8.1063   P-Value > Chi2(4)  0.0878
- White Test - B-P-G (SSR): E2 = X      =  12.9045   P-Value > Chi2(4)  0.0118
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2   =  12.1779   P-Value > Chi2(8)  0.1434
- White Test - B-P-G (SSR): E2 = X X2   =  19.3862   P-Value > Chi2(8)  0.0129
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2 XX=  28.1237   P-Value > Chi2(14) 0.0137
- White Test - B-P-G (SSR): E2 = X X2 XX=  44.7704   P-Value > Chi2(14) 0.0000
------------------------------------------------------------------------------
- Cook-Weisberg LM Test: E2/S2n = Yh    =   0.0417   P-Value > Chi2(1)  0.8381
- Cook-Weisberg LM Test: E2/S2n = X     =  12.9045   P-Value > Chi2(4)  0.0118
------------------------------------------------------------------------------
*** Single Variable Tests (E2/Sig2):
- Cook-Weisberg LM Test: x1                =   3.8608 P-Value > Chi2(1) 0.0494
- Cook-Weisberg LM Test: x2                =   1.8478 P-Value > Chi2(1) 0.1740
- Cook-Weisberg LM Test: w1x_x1            =   0.3121 P-Value > Chi2(1) 0.5764
- Cook-Weisberg LM Test: w1x_x2            =   0.0055 P-Value > Chi2(1) 0.9407
------------------------------------------------------------------------------
*** Single Variable Tests:
- King LM Test: x1                         =   0.5181 P-Value > Chi2(1) 0.4717
- King LM Test: x2                         =   2.6015 P-Value > Chi2(1) 0.1068
- King LM Test: w1x_x1                     =   0.2726 P-Value > Chi2(1) 0.6016
- King LM Test: w1x_x2                     =   0.0641 P-Value > Chi2(1) 0.8002
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
*** Non Normality Tests:
- Jarque-Bera LM Test                  =   5.2358     P-Value > Chi2(2) 0.0730
- White IM Test                        =   9.8581     P-Value > Chi2(2) 0.0072
- Doornik-Hansen LM Test               =   5.4699     P-Value > Chi2(2) 0.0649
- Geary LM Test                        =  -0.0715     P-Value > Chi2(2) 0.9649
- Anderson-Darling Z Test              =   0.4626     P > Z( 0.640)     0.7389
- D'Agostino-Pearson LM Test           =   5.9690     P-Value > Chi2(2) 0.0506
------------------------------------------------------------------------------
*** Skewness Tests:
- Srivastava LM Skewness Test          =   2.3746     P-Value > Chi2(1) 0.1233
- Small LM Skewness Test               =   2.7126     P-Value > Chi2(1) 0.0996
- Skewness Z Test                      =  -1.6470     P-Value > Chi2(1) 0.0996
------------------------------------------------------------------------------
*** Kurtosis Tests:
- Srivastava  Z Kurtosis Test          =   1.6915     P-Value > Z(0,1)  0.0907
- Small LM Kurtosis Test               =   3.2564     P-Value > Chi2(1) 0.0711
- Kurtosis Z Test                      =   1.8045     P-Value > Chi2(1) 0.0711
------------------------------------------------------------------------------
    Skewness Coefficient = -0.5392     - Standard Deviation =  0.3398
    Kurtosis Coefficient =  4.1838     - Standard Deviation =  0.6681
------------------------------------------------------------------------------
...
|--------------+------------+------------+------------+------------+------------|
|y             |            |            |            |            |            |
|           x1 |    -0.2579 |    -0.2561 |    -0.2973 |     0.0412 |    38.4362 |
|           x2 |    -1.5594 |    -1.5483 |    -1.7972 |     0.2490 |    14.3749 |
|       w1x_x1 |    -0.0190 |    -0.0189 |    -0.0219 |     0.0030 |   126.1844 |
|       w1x_x2 |     0.1155 |     0.1147 |     0.1331 |    -0.0184 |    45.6271 |
+-------------------------------------------------------------------------------+
...
|--------------+------------+------------+------------+------------+------------|
|           x1 |    -0.2822 |    -0.2802 |    -0.3252 |     0.0451 |    38.4362 |
|           x2 |    -0.6381 |    -0.6336 |    -0.7354 |     0.1019 |    14.3749 |
|       w1x_x1 |    -0.0682 |    -0.0678 |    -0.0786 |     0.0109 |   126.1844 |
|       w1x_x2 |     0.1500 |     0.1490 |     0.1729 |    -0.0240 |    45.6271 |
+-------------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:SPMSTARDXT Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2013)}{p_end}
{p 1 10 1}{cmd:SPMSTARDXT: "Stata Module to Estimate (m-STAR) Spatial Multiparametric Spatio Temporal AutoRegressive Regression: Spatial Durbin Panel Models"}{p_end}
