...
program define spmstardxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spmstardxt" {
 error 301
...
version 11.0
syntax varlist [aw] , nc(str) WMFile(str) [NWmat(int 1) dist(str) aux(str) ///
 diag INV INV2 vce(passthru) stand zero ROBust MFX(str) coll LL(real 0) ///
 NOLog LMHet LMSPac iter(int 100) level(passthru) PREDict(str) tobit ///
 RESid(str) tech(str) tolog TESTs DN NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spmstardxt
...
tempvar Bw D DE DF1 DumE E e dcs XQX_ EE Es Ev Hat ht idv itv LE P Q
tempvar SST Time tm U U2 Ue wald weit Wi Wio WS X X0 eigw Bo Sig2 SSE
tempvar  Xo Yb Yh Yh2 Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bm Bx Cov Cov1 CovC D den DVE DVNE Dx E E1 F
tempname h Hat hjm IPhi J K M P Phi Pm Q S11 S12 sd Sig2 Sig2o SSE SST1 SST2
tempname v V1 v1 V1s v2 In IRW IRWR WXb IN xMx Y Yh Yi YYm YYv Z X0 Ue xBx
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V
tempname WS D WW eVec eigw Xo mh olsin Rostar kb Sig21 wWw1
tempname N DF kx kb NC NT Nmiss kz llf Dim wmat xAx xAx2 TRa2 Sig2n
...
local initopt init(`olsin', copy) 
 ml model lf spmstardxt01 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Normal Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "MSTAR1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy) 
 ml model lf spmstardxt02 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Exponential Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf spmstardxt03 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Weibull Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf spmstardxt04 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Normal Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "exp") {
local MName "MSTAR2e"
 matrix `olsin'=`Bo',0,0
local initopt init(`olsin', copy)
 ml model lf spmstardxt05 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
 (Rho1:) (Rho2:) `wgt' , contin `mlopts' `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Exponential Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstardxt06 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Weibull Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy) 
 ml model lf spmstardxt07 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Normal Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "exp") {
local MName "MSTAR3e"
...
local initopt init(`olsin', copy)
 ml model lf spmstardxt08 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) `wgt' , contin `mlopts' `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Exponential Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstardxt09 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Weibull Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy) 
 ml model lf spmstardxt10 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Normal Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "exp") {
local MName "MSTAR4e"
...
local initopt init(`olsin', copy)
 ml model lf spmstardxt11 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) `wgt' , contin `mlopts' `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Exponential Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstardxt12 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin ///
 `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Durbin Panel Weibull Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
di _dup(78) "-"
 ...
 if "`tests'"!="" {
tempname K B XBM E E2 Sig XBs Es Eg Gz Eg3 Eg4
