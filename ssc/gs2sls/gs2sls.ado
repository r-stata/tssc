...
program define gs2sls , eclass
version 11.0
syntax varlist [aw] , WMFile(str) [Model(str) ORDer(int 1) ///
 INV INV2 vce(passthru) stand MFX(str) LMHet LMSPac LMNorm 2sls liml gmm ///
 zero level(passthru) coll PREDict(str) RESid(str) tolog TESTs NOCONStant diag]
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ EE Eo Hat ht LE LEo LYh2 P Q Sig2 SSE
tempvar SST Time U U2 wald weit Wi Wio WS X X0
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z DVE Dx E E1 F M In
tempname Xb A B b B1 b1 b2 Beta Bx IPhi J K Cov D den kb vh VM VP Ue Pm Phi
tempname Sig2 Sig2o SSE SST1 SST2 D eVec eigw Xo mh n Q Vec
tempname Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V Y Yh Yi YYm YYv X0
tempname df Dim Sig2o1 N DF kx kb NC NT Nmiss kz llf Sig21 Sig2n P
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
qui ivregress 2sls `yvar' `xvar' (w1y_`yvar'=`xvar' `zvar') `wgt' , ///
`noconstant' `vce' `level' 
 }
if inlist("`model'", "liml") {
di as txt "{bf:{err:* Generalized Spatial Limited Information Maximum Likelihood (GSLIML)}}"
qui ivregress liml `yvar' `xvar' (w1y_`yvar'=`xvar' `zvar') `wgt' , ///
`noconstant' `vce' `level' 
 }
if inlist("`model'", "gmm") {
di as txt "{bf:{err:* Generalized Spatial Generalized Method of Moments (GSGMM)}}"
qui ivregress gmm `yvar' `xvar' (w1y_`yvar'=`xvar' `zvar') `wgt' , ///
`noconstant' `vce' `level' 
 }
di _dup(78) "{bf:{err:=}}"
matrix `Beta'=e(b)
