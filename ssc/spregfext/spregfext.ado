...
program define spregfext, eclass 
 version 11.0
syntax varlist , nc(str) Model(str) [WMFile(str) diag aux(str) INV INV2 ///
 vce(passthru) stand MFX(str) LMHet LMSPac zero level(passthru) ///
 coll PREDict(str) RESid(str) tolog TESTs DN LMNorm]
gettoken yvar xvar : varlist
local sthlp spregfext
...
tempname Xb A B b B1 Beta BetaSP Bx mh J K IPhi Z0 wWw1
tempname Cov D den DVE DVNE Dx E E1 F h Hat M eVec Xo kbm kb
tempname P Phi Pm q Q In WS D WW Wi1 Wio WY X X0 XB V Dim Y Yh YYm
tempname Sig2 Sig2b Sig2o SSE SST1 SST2 YYv Z Wald We Wi Z1 Zo xBx
tempname Vec vh VM VP Vs W W1 W2 N DF kx kb NC NT Nmiss llf wmat xAx xAx2 TRa2
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
di _dup(78) "{bf:{err:=}}"
if inlist("`model'", "sar") {
di as txt "{bf:{err:* Spatial Panel Fixed-Effects Lag Regression (SAR)}}"
 }
if inlist("`model'", "sdm") {
di as txt "{bf:{err:* Spatial Panel Fixed-Effects Durbin Regression (SDM)}}"
 }
di _dup(78) "{bf:{err:=}}"
...
matrix `In'=I(`N')
qui xtset `idv' `itv'
qui xtreg `yvar' `SPXvar' , fe `vce' `level'
scalar `llf'=e(ll)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
