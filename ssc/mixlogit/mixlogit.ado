...
*  1.3.0:	a new -coll- option has been added to override
*			checks for multicollinearity. iter(0) option added to
*			clogit if constraints or coll are specified.

*  1.3.1:	a new -userdraws- option has been added to use a
*			matrix of draws specified by the user (mixl_USERDRAWS)
*			The matrix should have number of rows equal to the number 
*			of random coefficients in the model and number of columns 
*			equal to the number of respondents times the number of draws 
*			per respondent

*  1.4.0:	a bug that could affect the estimation results in the case of 
...
program Estimate, eclass
	...
	** Set Mata matrices and scalars to be used in optimisation routine **
	...
	if ("`userdraws'" != "") mata: mixl_user = 1	
	else mata: mixl_user = 0
