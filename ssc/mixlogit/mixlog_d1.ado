...
void mixl_ll(string scalar B_s)
{
	...
	for (n=1; n<=np; n++) {
		...
		if (user == 1) {
			ERR = invnormal(mixl_USERDRAWS[|1,(1+nrep*(n-1))\krnd,(nrep*n)|])
		}
		else {
		...
		for (t=1; t<=nc; t++) {
			...
			for (j=1; j<=kall; j++) {
				if (j <= (kall-krln)) {
					M[j,.] = M[j,.] :+ colsum(PMAT :* XMAT[.,j],1)
				}
				else {
					M[j,.] = M[j,.] :+ colsum(PMAT :* XMAT[.,j],1) :* BETA[j,.]
				}
			}
			if (corr == 1) {
				num = 1
				for (j=1; j<=krnd; j++) {
					for (k=j; k<=krnd; k++) {
						if (k <= (krnd-krln)) {
							M[(kall+num),.] = M[(kall+num),.] :+ colsum(PMAT :* XMAT[.,(kfix+k)],1) :* ERR[j,.]
						}
						else {
							M[(kall+num),.] = M[(kall+num),.] :+ colsum(PMAT :* XMAT[.,(kfix+k)],1) :* BETA[(kfix+k),.] :* ERR[j,.]
						}
						num = num + 1
					}
				}
				...
				for (j=1; j<=krnd; j++) {
					if (j <= (krnd-krln)) {
					 	M[(kall+j),.] = M[(kall+j),.] :+ colsum(PMAT :* XMAT[.,(kfix+j)],1) :* ERR[j,.]
					}
					else {
					 	M[(kall+j),.] = M[(kall+j),.] :+ colsum(PMAT :* XMAT[.,(kfix+j)],1) :* BETA[(kfix+j),.] :* ERR[j,.]
					}
				}
			}
			i = i + mixl_CSID[i,1]
	...
	st_numscalar("r(ll)", colsum(mixl_WGT:*ln(P),1))
	st_matrix("r(gradient)", colsum(mixl_WGT:*G,1))	
