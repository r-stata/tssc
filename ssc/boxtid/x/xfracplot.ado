...
program define xfracplot, sortpreserve
	version 8
	if _caller() < 8 {
		fracplot_7 `0'
		exit
	...
	*local y "`e(fp_depv)'"
	...
	syntax [varlist(default=none fv)] [if] [in] [, nopts * ]
	...
	if "`varlist'"=="" {
		...
		if "`x'"=="" local x `rhs'
		// Check if `x' is a factor variable; if so strip its prefix
		fvexpand `x'
		if "`r(fvops)'" == "true" {
			local x = substr("`x'", 1 + strpos("`x'", "."), .)
		}
		local i 0
		while `i'<e(fp_nx) {
			local i=`i'+1
...
quietly {
	...
	tempvar etahat
	xfracpred `etahat', for(`rhs')
	lab var `etahat' "Fitted component"
	tempvar se low high
	xfracpred `se', for(`rhs') stdp
	tempname z
