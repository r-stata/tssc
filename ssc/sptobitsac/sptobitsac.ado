...
program define sptobitsac , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsac" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) LL(str) [INV INV2 ///
 ...
 RESid(str) tech(str) tolog TESTs LMNorm diag]
gettoken yvar xvar : varlist
local sthlp sptobitsac
...
tempname IPhi J K M mh Sig2n P Phi Pm Q In IRW IRWR IRWL
tempname Sig2 Sig2o Sn SSE SST1 SST2 D WW eVec eigw Xo
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname Y Yh Yi YYm YYv Z rRo rLm kb N DF kx kb Nmiss kz llf Sig21 Dim
tempname minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
tempname waldl waldlp waldl_df waldj waldjp waldj_df
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
local initopt init(spat_ols, copy)  
 ml model lf sptobitsac1 `MODEL' `wgt' , ///
...
local initopt init(spat_ols, copy)
 ml model lf sptobitsac2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName') 
...
qui global spat_ARGS "`spat_ARGS' beta0 Rho Lambda Sigma"
 matrix spat_ols=`Bo',0,0,`rmse'
local initopt init(spat_ols, copy)  
 ml model lf sptobitsac3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
