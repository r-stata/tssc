...
version 12.0
	if replay() {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _stevaluator (beta: `depvar' = `regs') ${t3} ${g3} /lambda /q  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _stevaluator (beta: `depvar' = `regs') /sigma /lambda /q  , `technique' `vce'
		}
