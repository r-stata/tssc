...
version 12.0
	if replay() {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _scauchyevaluator (beta: `depvar' = `regs') ${t3} ${g3} /lambda  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _scauchyevaluator (beta: `depvar' = `regs') /sigma /lambda , `technique' `vce'
		}
