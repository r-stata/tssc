...
version 12.0
	if replay() {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _egb2evaluator (beta: `depvar' = `regs') ${t3} ${g3} /delta /p /q  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _egb2evaluator (beta: `depvar' = `regs') /sigma /delta /p /q  , `technique' `vce'
		}
