...
version 12.0
	if replay() {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _sgedevaluator (beta: `depvar' = `regs') ${t3} ${g3} /lambda /p  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _sgedevaluator (beta: `depvar' = `regs') /sigma /lambda /p  , `technique' `vce'
		}
