...
version 12.0
	if replay() {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _cauchyevaluator (beta: `depvar' = `regs') ${t3} ${g3}  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _cauchyevaluator (beta: `depvar' = `regs') /sigma , `technique' `vce'
		}
