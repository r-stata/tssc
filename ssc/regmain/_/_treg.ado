...
version 12.0
	if replay() {
		...
		if (`initiallen' != `nvars'+2 + `arch' + `garch') {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _tevaluator (beta: `depvar' = `regs') ${t3} ${g3} /q  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _tevaluator (beta: `depvar' = `regs') /sigma /q  , `technique' `vce'
		}

		ml init `initial', copy
		//ml check
		ml maximize, showeqns `difficult' `iterate' `log' `trace' `gradient' `showstep' `hessian' `nrtolerance' `showtolerance' `tolerance' `ltolerance' `nrtolerance' 
