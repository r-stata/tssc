program _segb2evaluator
version 12.0
	args lnf beta $sigma $t1 $g1 delta p
	tempvar x
	...
	qui replace `lnf' = `p'*(`x' - `delta')/`sigma' - ln(abs(`sigma')) - 2*lngamma(`p') + lngamma(2*`p') -2*`p'*ln(1+exp((`x' - `delta')/`sigma'))
end
