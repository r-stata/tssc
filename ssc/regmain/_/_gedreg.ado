...
if replay() {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _gedevaluator (beta: `depvar' = `regs') ${t3} ${g3} /p  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _gedevaluator (beta: `depvar' = `regs') /sigma /p  , `technique' `vce'
		}
