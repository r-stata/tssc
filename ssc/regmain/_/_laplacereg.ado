program _laplacereg, eclass
version 12.0
	if replay() {
		display "Replay not implemented"
	}
	else {
		syntax varlist [,ARCH(integer 0) GARCH(integer 0) INITial(numlist) DIFficult TECHnique(passthru) ITERate(passthru) nolog TRace GRADient showstep HESSian SHOWTOLerance NONRtolerance TOLerance(passthru) LTOLerance(passthru) NRTOLerance(passthru) VCE(passthru)] 
		...
		if (`initiallen' != `nvars'+1 + `arch' + `garch') {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _laplaceevaluator (beta: `depvar' = `regs') ${t3} ${g3}  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _laplaceevaluator (beta: `depvar' = `regs') /sigma , `technique' `vce'
		}
