...
version 12.0
	if replay() {
		...
		if "`in'" == ""{
			local initial `initial' 0 2
		}
		local initiallen: word count `initial'
		if (`initiallen' != `nvars'+3 + `arch' + `garch') {
		...
		if "`arch'" != "0" | "`garch'" != "0"{
			global sigma = ""
			ml model lf _segb2evaluator (beta: `depvar' = `regs') ${t3} ${g3} /delta /p  , `technique' `vce'
		}
		else{
			global sigma = "sigma"
			ml model lf _segb2evaluator (beta: `depvar' = `regs') /sigma /delta /p  , `technique' `vce'
		}
