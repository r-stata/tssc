...
* estimation with PISA data

cap program drop pisacmd

program define pisacmd, rclass
syntax [namelist(min=1)] [if] [in] , cmd(string) [cmdops(string) cons ///
pvindep1(string) pvindep2(string) pvindep3(string) cycle(integer 2018) ///
over(varname numeric) cnt(string) save(string) fast round(integer 2) ///
pw keepvar(string) r2(string)]
...
if "`keepvar'"!="" local tokeep="`tokeep' `keepvar'"
			 ...
			 if `pv'==1 | `pvindep'==1 {
			 	if "`fast'"=="" {
					forvalues ip=1(1)`ilepv' {	
						...
						if `pv'==1 _cmdbrr pv`ip'`outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2') `pw' repweight(`repwt')
						else _cmdbrr `outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons'  r2(`r2') `pw' repweight(`repwt')
						matrix b`ip'=r(coef)
						matrix se`ip'=r(se)
						local r2`ip'=r(r2)
						}
					...
					forvalues ip=1(1)`ilepv' {
						...
						if "`pvindep3'"!="" local indeplist="`indeplist' pv`ip'`pvindep3'"
						local indeplist="`indeplist' `varlist'"
						if `pv'==1 _cmdfast pv`ip'`outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2') `pw'
						else _cmdfast `outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2') `pw'
						matrix b`ip'=r(coef)
						matrix se`ip'=r(se)
						local r2`ip'=r(r2)
						}
				...
				* MATRICES WITH MEANS AND SEs FROM PVs
				...
				if "`fast'"=="" {
					_cmdbrr `outcome' `varlist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2') `pw'  repweight(`repwt')
					matrix b=r(coef)
					matrix se=r(se)
					local temp_r2=r(r2)
					}
				else {
					_cmdfast `outcome' `varlist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2') `pw'
					matrix b=r(coef)
					matrix se=r(se)
					local temp_r2=r(r2)
					}
...
cap program drop _cmdbrr
program define _cmdbrr, rclass
						
syntax [namelist(min=1)], cmd(string) [cmdops(string) cons r2(string) pw] repweight(string)
...
qui `cmd' `outcome' `varlist' [`waga'=w_fstuwt], `cmdops'
...
forvalues j=1(1)80 {
	qui `cmd' `outcome' `varlist' [`waga'=`repweight'`j'], `cmdops'
	foreach var in `varlist' `stala' {
...
******************************************************
cap program drop _cmdfast
program define _cmdfast, rclass
						
syntax [namelist(min=1)], cmd(string) [cmdops(string) cons r2(string) pw]
