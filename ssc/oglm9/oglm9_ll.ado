...
* oglm9_ll.ado is called by oglm9 written by Richard Williams
...
program oglm9_ll
	...
	gettoken lnf rest: 0
	// The calling program should set the values of $oglm9x and $oglm9h. 
	// But if that has not been done, it will be assumed that there is a location
	// equation but no scale equation.
	local oglm9x $oglm9x
	if "`oglm9x'"=="" local oglm9x 1
	local oglm9h $oglm9h
	if "`oglm9h'"=="" local oglm9h 0
	...
	// calling program unless you are using oglm9_ll interactively.
	local link $Link
