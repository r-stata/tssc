...
program define oglm9_p 
	version 9, missing
	// link gives link fnc used.  oglm9x = 1 if there are x vars,
	// 0 if not.  oglm9h = 1 if there arw hetero vars, 0 otherwise.  
	// This info is needed for computing probabilities and/or scores.
	macro drop oglm9x oglm9h Link dv_*
	local link `e(link)'
	local oglm9x = "`e(xvars)'" != ""
	local oglm9h = "`e(hetero)'" != ""
...
// scores - this section modified for oglm9. Other score-related code was dropped.

	if `"`type'"' == "scores" {
		// Need to recreate global macros
		global Link `link'
		global oglm9h `oglm9h'
		global oglm9x `oglm9x'
		tempname ycat
		matrix `ycat' = e(cat)
		forval i = 1/`e(k_cat)' {
	        ...
	        ml_p `0'
	        macro drop Link dv_* oglm9h oglm9x
		sret clear
		exit
	}

// sigma - this code adapted from hetpr_p to use for oglm9
...
// Index, XB, or STDP - has oglm9 modifications
...
// If here we compute probabilities.  Do general preliminaries. Modified for oglm9
...
// Probability with outcome() specified: create one variable. modified for oglm9
...
// Probabilities with outcome() not specified: create e(k_cat) variables. modified for oglm9
