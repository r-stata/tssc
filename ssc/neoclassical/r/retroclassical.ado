...
program define retroclassical, eclass ;
 version 10.1 ;
 ...
 gen xtwo2=1 ;
 local rho = 0 ;
 local athrho = 0.5*log((1+`rho')/(1-`rho')) /* Eq 1 */ ;
 ...
 quietly heckprob `outcome2' `timeconstant2' `timevarying2',
         ...
         cluster(`clustvar') const(1) ;
 scalar n1eq=e(N) ;
 scalar n2eq=e(N)-e(N_cens) ;
 matrix accum `out1a'=`timeconstant1' `timevarying1' `identifiers' xone2,
          noconstant deviations ;
 matrix `out1b'=(`out1a'/n1eq) ;
 matrix accum `out2a'=`timeconstant2' `timevarying2' xtwo2 if `outcome1'==1,
          noconstant deviations ;
 ...
 matrix MyVCVBeta=VARMULT ;
   ...
   display "   ><Retro-Classical Education Transitions Model Results, Rho=0><" ;
   display "   " ;
