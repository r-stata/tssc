...
#delimit;
program define svytabform, rclass byable(recall) sort;
	version 9.0;
...
* Compute mean, sd, and ci for each of varlist and each of categories of by;
	local i = 1;
	if "`by'" != "" 
		{;
      ...
      while `by2' < `ntouse'  
      	...
      	* range `iby1'/`iby2' refer to obs in the current by-group;
      	...
      	local by2 = `by1' + r(N) - 1;
      	quiet gen `subpop' = 1 in `by1'/`by2';
      	quiet recode `subpop' . = 0;
      	
      	* loop over all variables;
      	forvalues k = 1/`nvars' 
      		{;
      		qui svy, subpop(`subpop'): mean `var`k'' if `touse';
      		mat B = e(b);
      		mat V = e(V);
      		local m`i'`k' = B[1,1];
      		local sd`i'`k' = sqrt(e(N) * el(e(V_srs),1,1));
      		if "`se'" != "" 
      			{;
      			local sd`i'`k' = sqrt(V[1,1]);
      			};
      		* Compute confidence interval;
      		scalar mean = `m`i'`k'';
      		scalar se = sqrt(V[1,1]);
      		local invt = invttail(e(N)-1, (100-`level')/200);
      		local lb`i'`k' = mean - `invt'*se;
      		local ub`i'`k' = mean + `invt'*se;
      		
      		* Mean comparison test;
      		qui svy: regress `var`k'' `subpop' if `touse';
      		local tstat = abs(_b[`subpop']/_se[`subpop']);
      		local mse = _se[`subpop'];
      		local df_r = e(df_r);
      		if `df_r'==. 
      			{;
