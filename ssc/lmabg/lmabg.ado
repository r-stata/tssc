...
tempvar DE DF1 DW DX E LDE LE LEo SSE Time wald weit Wi
tempvar Wio WS X0 Yb Yh Yhb Yho YY YYm YYv Yh_ML Ue_ML TimeN
tempname b B b1 b2 Beta Bx Cov D Ds DX E E1 F
tempname h Hat HT ICOR IPhi J K L M M1 M2 n NY OM P Phi Pm Q 
tempname Sig2 Sig2o Sig2o1 W W1 Wald We Wi Wi1 Wio
tempname X X0 Y Yh Nn N kx DF kb SSEo llf SST1 SST2 V v1
...
matrix `V'=e(V)
matrix `Bx'=e(b)
tempname SSE Rho BB lmabgd lmabgk
local N=`Nn'
qui tsset `Time'
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** OLS Autocorrelation Breusch-Godfrey Test}}"
di _dup(78) "{bf:{err:=}}"
