*! 0.0.0.9000 TidyFriday 2017年12月22日
program drop _all
program define stkd, rclass
	version 12.0
	syntax anything(name = firms),[ path(string) Store Iterm(string) Fmt(string)]
	if index("`firms'", " "){
		local store = "store"
	}
	if "`iterm'" == ""{
	...
	* 我要永远地为你设置set more off
	...
	foreach stkcode in `firms'{
		...
		qui cap copy "http://stockdata.stock.hexun.com/gszl/s`stkcode'.shtml" `tempcsvfile', replace
		local times = 0
		while _rc != 0 {
			local times = `times' + 1
			sleep 1000
		...
		qui infix strL v 1-20000 using `tempcsvfile', clear
		if `=_N' == 0 {
