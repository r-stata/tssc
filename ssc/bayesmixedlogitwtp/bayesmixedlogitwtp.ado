...
program bayesmixedlogitwtp
	version 11.2
	if replay() {
		if (`"`e(cmd)'"' != "bayesmixedlogitwtp") error 301
		Replay `0'
	}
	else Estimate `0'
end
...
program Estimate, eclass
	syntax varlist [if] [in], 				///
			...
			IDentifier(varname)				///
			RAND(varlist)					///
			PRICE(varname)					/// /* The sole new variable here...everything else is as before */	
			[DRAWs(int 1000)				///
			DRAWRandom(int 1)				///
			DRAWFixed(int 1)				///
			BURN(int 0)						///
		...
		local rand `price' `rand'				/* Just add the price variable as the first of the random coefficients */
	...
	/* Modtype zero has no fixed coefficients */
	/* Modtype one has fixed coefficients */
	tempname modtype
	if `fixeds'==0 {
		scalar `modtype'=1
	}
...
/* This option doesn't have much of a role right now but might be useful */
	...
	if ("`from'"!="") {
		...
		if (`modtype'==0) {
			...
			mata: b =from[1,nf+1::nr+nf]				/* Might be worth mentioning somewhere that b[1]=-exp(b[1]) is actually used */
		}
		else {
			mata: b =from
			mata: beta_fn=0
		...
		if (`modtype'==0)	{ 
			...
			mata: if (b[1,1]<0) b[1,1]=ln(-b[1,1]); else b[1,1]=0	/* Log-transform if possible, otherwise start at 0 */
		}
		else {
			mata:  b=from
			mata: beta_fn=0
	...
	tempname b V price_coef
	mata: st_matrix("`b'",eb)
	mata: pcoef=-exp(eb[1,1])
	mata: st_matrix("`V'",eV)
	mata: st_numscalar("`price_coef'",pcoef) 
	...
	ereturn local title "Bayesian Mixed Logit Model - WTP Form"
	ereturn local cmd "bayesmixedlogitwtp"
	ereturn local indepvars `rhs'
	ereturn local pricevar `price'
	...
	ereturn scalar price_coef=`price_coef'
	...
	di as txt "The price variable is `price' with transformed coef (-exp(b)): " as res %6.3f `e(price_coef)'   
	di _newline
	di as err "   Attention!"
					...
					real matrix cid)
{
	...
	if (cols(beta_rn)>1) {
		beta_rnp=beta_rn
		beta_rnp[1,1]=-exp(beta_rnp[1,1])
		beta_rnp[2::cols(beta_rn)]=beta_rnp[2::cols(beta_rn)]*(-beta_rnp[1,1])
	}
	else {
		beta_rnp=-exp(beta_rn[1,1])
	}
	for (i=1;i<=rows(z);i++) {
