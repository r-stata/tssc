...
tempvar D DE DF1 DW E TimeN EE Eo Ew ht idv itv LE
tempvar LEo P Q Sig2 SSE SST Time U U2 Ue wald weit Wi Wio WS X X0 Bo
tempvar Yb Yh Yh2 Yhb YY YYm YYv Yh_ML Ue_ML Z D DE DF DF1
tempvar DW E EE Eo LDE LE LEo P Q Sig2 SSE Yb Yh Yh2 Yhb YY 
tempname A B b Beta Bx Cov D den DF Dim Dx E E1 EE1 Eg Eo 
tempname Ew F In IPhi J K kb kx llf N NC Nn NT P Phi
tempname Pm Q Rho Sig2 Sig21 Sig2o Sig2o1 SSE SSEo Yh_ML YYm YYv
tempname SST1 SST2 Ue Ue_ML W Wald waldm We Wi V Wi1 Wio WS X X0 Y Yh
qui xtset `id' `it'
...
local NC2=r(imax)
qui tsset `Time'
tempname Rho Rhosq Rw1 Rw2 lmabgd lmabgdp lmabgk lmabgkp SSEu SSE SSE1
tempvar Ue_ML1
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Autocorrelation Breusch-Godfrey Test}}"
di _dup(78) "{bf:{err:=}}"
...
di as txt "- Breusch-Godfrey LM Test (drop 1 cs obs)" _col(43) "=" as res %9.4f `lmabgd' _col(55) as txt "P-Value > Chi2(1)" _col(73) as res %5.4f `lmabgdp'
di as txt "- Breusch-Godfrey LM Test (keep 1 cs obs)" _col(43) "=" as res %9.4f `lmabgk' _col(55) as txt "P-Value > Chi2(1)" _col(73) as res %5.4f `lmabgkp'
di _dup(78) "-"
