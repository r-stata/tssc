...
#delimit ;
gpscore `varlist' `if' `in'  [`weight' `exp'], 
t(`t')
gpscore(`gpscore') 
nq_gps(`nq_gps') 
predict(`predict') 
...
if(`delta'==0){
order treatment_level treatment_level_plus `dose_response' se_*_bs
}
if(`delta'>0){
...
if ("`cmd'"== "mlogit" | "`cmd'"=="mprobit" | "`cmd'"=="ologit" | "`cmd'"=="oprobit"){
drop dose_response_*  dose_response_plus_*
}
