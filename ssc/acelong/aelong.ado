*! aelong: Multilevel mixed-effects AE variance decomposition models for "long" - one person per data row - formatted twin data (wrapper for 'gsem')
*! version 1.4 03/13/19
program define aelong, rclass
	version 14.1
	...
	* Guo/Wang(2002)-model
	if "`gw'" == "gw" {
		...
		gsem `one' <- J[`three']@1 IJ[`three'>`ij']@1 `*' if `touse' [`weight' `exp'], `options' notable var(J[`three']@a IJ[`three'>`ij']@a)
		local ae = _b[var(J[`three']):_cons] + _b[var(IJ[`three'>`ij']):_cons]
		local as = _se[var(J[`three']):_cons] + _se[var(IJ[`three'>`ij']):_cons]
	...
	* McArdle/Prescott(2005)-model
		...
		if "`iwc'" == "iwc" {
			...
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three']@1 c.`av'#AV[`three']@1 `*' if `touse' [`weight' `exp'], notable noheader `options' ///
			cov(A[`three']*AU[`three']@0 A[`three']*AV[`three']@0 AU[`three']*AV[`three']@0)
			matrix `start' = e(b)
			display as text "With constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three']@1 c.`av'#AV[`three']@1 `*' if `touse' [`weight' `exp'], from(`start') notable `options' ///
			var(A[`three']@a AU[`three']@a AV[`three']@a) ///
			cov(A[`three']*AU[`three']@0 A[`three']*AV[`three']@0 AU[`three']*AV[`three']@0)
		}
		else {
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three']@1 c.`av'#AV[`three']@1 `*' if `touse' [`weight' `exp'], notable `options' ///
			var(A[`three']@a AU[`three']@a AV[`three']@a) ///
			cov(A[`three']*AU[`three']@0 A[`three']*AV[`three']@0 AU[`three']*AV[`three']@0)
		}
	...
	* Rabe-Hesketh/Skrondal/Gjessing(2008)-model
		...
		if "`iwc'" == "iwc" {
			display as text "Without constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three'>`four']@1 `*' if `touse' [`weight' `exp'], notable noheader `options'
			matrix `start' = e(b)
			display as text "With constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three'>`four']@1 `*' if `touse' [`weight' `exp'], from(`start') notable `options' ///
			var(A[`three']@a AU[`three'>`four']@a)
		}
		else {
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three'>`four']@1 `*' if `touse' [`weight' `exp'], notable `options' ///
			var(A[`three']@a AU[`three'>`four']@a)
		}
		local ae = _b[var(A[`three']):_cons]
		local as = _se[var(A[`three']):_cons]
	...
	* define additional locals for quantities estimated
	...
	forvalues x = 1/`k' {
		...
		if "``x''" != "J[`three']" & "``x''" != "IJ[`three'>`ij']" & "``x''" != "c.`a'#A[`three']" & "``x''" != "c.`au'#AU[`three']" & "``x''" != "c.`av'#AV[`three']" & "``x''" != "c.`au'#AU[`three'>`four']" {
			local vl`l' "``x''"
	...
	* define labels for quantities estimated
	...
	label define `n_val' `=`k'+2' "variance:" `=`k'+3' "A" `=`k'+4' "E" `=`k'+5' "A+E" `=`k'+6' "A %" `=`k'+7' "E %", add 
	if "`tcr'" == "tcr" {
		label define `n_val' `=`k'+8' "obs.cor.:" `=`k'+9' "MZr" `=`k'+10' "DZr", add
	}
	if "`total'" == "total" {
		label define `n_val' `=`k'+5' "total", modify
	}
	...
	* store point estimates
	...
	quietly: replace `bv' = `ee' if `n' == `=`k'+4'
	quietly: replace `bv' = `te' if `n' == `=`k'+5'
	quietly: replace `bv' = `ae'/`te'*100 if `n' == `=`k'+6'
	quietly: replace `bv' = `ee'/`te'*100 if `n' == `=`k'+7'
	if "`tcr'" == "tcr" {
		quietly: replace `bv' = `e1' if `n' == `=`k'+9'
		quietly: replace `bv' = `e2' if `n' == `=`k'+10'
	}
	...
	* store standard errors
	...
	quietly: replace `sev' = `es' if `n' == `=`k'+4'
	quietly: replace `sev' = `ts' if `n' == `=`k'+5'
	quietly: replace `sev' = `as'/`te'*100 if `n' == `=`k'+6' 
	quietly: replace `sev' = `es'/`te'*100 if `n' == `=`k'+7'
	if "`tcr'" == "tcr" {
		quietly: replace `sev' = `s1' if `n' == `=`k'+9'
		quietly: replace `sev' = `s2' if `n' == `=`k'+10'
	}
	...
	* calculate confidence intervals
	quietly: generate double `lb' = `bv' - invnormal(`lim')*`sev' if _n <= `=`k'+1' | _n >= `=`k'+8'
	quietly: replace `lb' = exp(ln(`bv') - invnormal(`lim')*`sev'/`bv') if _n > `=`k'+1' & _n <= `=`k'+5'
	quietly: replace `lb' = invlogit(logit(`bv'/100) - invnormal(`lim')*`sev'/`bv')*100 if _n > `=`k'+5' & _n <= `=`k'+7'
	label variable `lb' "[`lev'% Conf."
	quietly: generate double `ub' = `bv' + invnormal(`lim')*`sev' if _n <= `=`k'+1' | _n >= `=`k'+8'
	quietly: replace `ub' = exp(ln(`bv') + invnormal(`lim')*`sev'/`bv') if _n > `=`k'+1' & _n <= `=`k'+5'
	quietly: replace `ub' = invlogit(logit(`bv'/100) + invnormal(`lim')*`sev'/`bv')*100 if _n > `=`k'+5' & _n <= `=`k'+7'
	...
	* display results
	...
	if "`tcr'" == "tcr" {
		tabdisp `n' in 1/`=`k'+10', cell(`bv' `sev' `zp' `lb' `ub')
	}
	else {
		tabdisp `n' in 1/`=`k'+7', cell(`bv' `sev' `zp' `lb' `ub')
	}
	* store results of variance decomposition in return list
	...
	mkmat `bv' in `=`k'+3'/`=`k'+7', matrix(`b') rownames(`nval')
	if "`tcr'" == "tcr" {
		mkmat `bv' in `=`k'+9'/`=`k'+10', matrix(`b2') rownames(`nval')
		matrix `b' = `b' \ `b2'
	}																				
	matrix colnames `b' = b
	mkmat `sev' in `=`k'+3'/`=`k'+7', matrix(`se') rownames(`nval')
	if "`tcr'" == "tcr" {
		mkmat `sev' in `=`k'+9'/`=`k'+10', matrix(`se2') rownames(`nval')
		matrix `se' = `se' \ `se2'
	}
	matrix colnames `se' = se
	mkmat `lb' `ub' in `=`k'+3'/`=`k'+7', matrix(`ci') rownames(`nval')
	if "`tcr'" == "tcr" {
		mkmat `lb' `ub' in `=`k'+9'/`=`k'+10', matrix(`ci2') rownames(`nval')
		matrix `ci' = `ci' \ `ci2'
	}
