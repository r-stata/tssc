*! adelong: Multilevel mixed-effects ADE variance decomposition models for "long" - one person per data row - formatted twin data (wrapper for 'gsem')
*! version 1.4 03/13/19
program define adelong, rclass
	version 14.1
	syntax varlist(min=4) [if] [in] [fweight pweight iweight] [, rsg mp gw dzc(real .75) mzc(real 1) iwc tcr total ese *]
	marksample touse
	tokenize `varlist'
	...
	* check if syntax is correct
	...
	if `dzc' <= .5 | `dzc' >= 1 {
		display as error "The genetic correlation for dizygotic twin pairs ('dzc') in an ADE model has to be larger than .5 and smaller than 1."
	}
	quietly: assert `dzc' > .5 & `dzc' < 1
	if `mzc' <= `dzc' | `mzc' > 1 {
	...
	if "`gw'" == "gw" & `dzc' != .75 {
		display as error "No option 'dzc' for Gou/Wang(2002)-model ('gw'). For 'gw' the additive genetic correlation for dizygotic twin pairs ('dzc') is 0.75."
	}
	quietly: assert `dzc' == .75 if "`gw'" == "gw"
	if "`gw'" == "gw" & `mzc' != 1 {
	...
	display "Assumed genetic correlation of dizygotic twins: " `dzc' " (.5 additive and " `=`dzc'-.5' " dominant)"
	if `mzc' != 1 {
		display "Assumed genetic correlation of monozygotic twins: " `mzc'
	}
	* define temporary variables and names
	tempvar i res c0 c1 a au av d du dv ij n nval bv sev z p zp lb ub b b2 se se2 ci ci2
	...
	* Guo/Wang(2002)-model
	if "`gw'" == "gw" {
		...
		quietly: lincom 3*_b[var(J[`three']):_cons] - _b[var(IJ[`three'>`ij']):_cons]
		local ae = r(estimate)
		local as = r(se)
		quietly: lincom 2*(_b[var(IJ[`three'>`ij']):_cons] - _b[var(J[`three']):_cons]) 
		local de = r(estimate)
		local ds = r(se)
	...
	* McArdle/Prescott(2005)-model
		...
		quietly: replace `a' = sqrt(.5) if `two' == 2
		quietly: generate double `au' = 0
		quietly: replace `au' = sqrt(.5) if `two' == 2 & `i' == 1
		quietly: generate double `av' = 0
		quietly: replace `av' = sqrt(.5) if `two' == 2 & `i' == 2
		quietly: generate double `d' = 1 
		quietly: replace `d' = sqrt(`dzc' - .5) if `two' == 2
		quietly: generate double `du' = 0
		quietly: replace `du' = sqrt(`mzc' - (`dzc' - .5)) if `two' == 2 & `i' == 1
		quietly: generate double `dv' = 0
		quietly: replace `dv' = sqrt(`mzc' - (`dzc' - .5)) if `two' == 2 & `i' == 2
		if "`iwc'" == "iwc" {
			display as text "Without constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three']@1 c.`av'#AV[`three']@1 c.`d'#D[`three']@1 c.`du'#DU[`three']@1 c.`dv'#DV[`three']@1 `*' if `touse' [`weight' `exp'], notable noheader `options' ///
			cov(A[`three']*AU[`three']@0 A[`three']*AV[`three']@0 A[`three']*D[`three']@0 A[`three']*DU[`three']@0 A[`three']*DV[`three']@0 AU[`three']*AV[`three']@0 AU[`three']*D[`three']@0 AU[`three']*DU[`three']@0 AU[`three']*DV[`three']@0 AV[`three']*D[`three']@0 AV[`three']*DU[`three']@0 AV[`three']*DV[`three']@0 D[`three']*DU[`three']@0 D[`three']*DV[`three']@0 DU[`three']*DV[`three']@0)
			matrix `start' = e(b)
			display as text "With constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three']@1 c.`av'#AV[`three']@1 c.`d'#D[`three']@1 c.`du'#DU[`three']@1 c.`dv'#DV[`three']@1 `*' if `touse' [`weight' `exp'], from(`start') notable `options' ///
			var(A[`three']@a AU[`three']@a AV[`three']@a D[`three']@d DU[`three']@d DV[`three']@d) ///
			cov(A[`three']*AU[`three']@0 A[`three']*AV[`three']@0 A[`three']*D[`three']@0 A[`three']*DU[`three']@0 A[`three']*DV[`three']@0 AU[`three']*AV[`three']@0 AU[`three']*D[`three']@0 AU[`three']*DU[`three']@0 AU[`three']*DV[`three']@0 AV[`three']*D[`three']@0 AV[`three']*DU[`three']@0 AV[`three']*DV[`three']@0 D[`three']*DU[`three']@0 D[`three']*DV[`three']@0 DU[`three']*DV[`three']@0)
		}
		else {
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three']@1 c.`av'#AV[`three']@1 c.`d'#D[`three']@1 c.`du'#DU[`three']@1 c.`dv'#DV[`three']@1 `*' if `touse' [`weight' `exp'], notable `options' ///
			var(A[`three']@a AU[`three']@a AV[`three']@a D[`three']@d DU[`three']@d DV[`three']@d) ///
			cov(A[`three']*AU[`three']@0 A[`three']*AV[`three']@0 A[`three']*D[`three']@0 A[`three']*DU[`three']@0 A[`three']*DV[`three']@0 AU[`three']*AV[`three']@0 AU[`three']*D[`three']@0 AU[`three']*DU[`three']@0 AU[`three']*DV[`three']@0 AV[`three']*D[`three']@0 AV[`three']*DU[`three']@0 AV[`three']*DV[`three']@0 D[`three']*DU[`three']@0 D[`three']*DV[`three']@0 DU[`three']*DV[`three']@0)
		}
		local ae = _b[var(A[`three']):_cons]
		local as = _se[var(A[`three']):_cons]
		local de = _b[var(D[`three']):_cons]
		local ds = _se[var(D[`three']):_cons]
		if ("`f'" == "gaussian" & "`total'" != "total") | "`f'" == "gaussian ," { 
			quietly: lincom _b[var(A[`three']):_cons] + _b[var(e.`one'):_cons]
			local te = r(estimate)
			local ts = r(se)
		}
		if "`f'" == "bernoulli" | "`f'" == "ordinal" { 
			quietly: lincom _b[var(A[`three']):_cons] + `ee'
			local te = r(estimate)
			local ts = r(se)
			if "`ese'" == "ese" {
				local es = r(se)
	...
	* Rabe-Hesketh/Skrondal/Gjessing(2008)-model
		...
		quietly: replace `a' = sqrt(.5) if `two' == 2
		quietly: generate double `au' = 0
		quietly: replace `au' = sqrt(.5) if `two' == 2
		quietly: generate double `d' = 1 
		quietly: replace `d' = sqrt(`dzc' - .5) if `two' == 2
		quietly: generate double `du' = 0
		quietly: replace `du' = sqrt(`mzc' - (`dzc' - .5)) if `two' == 2
		if "`iwc'" == "iwc" {
			display as text "Without constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three'>`four']@1 c.`d'#D[`three']@1 c.`du'#DU[`three'>`four']@1 `*' if `touse' [`weight' `exp'], notable noheader `options' ///
			cov(A[`three']*D[`three']@0 AU[`three'>`four']*DU[`three'>`four']@0)
			matrix `start' = e(b)
			display as text "With constraints:"
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three'>`four']@1 c.`d'#D[`three']@1 c.`du'#DU[`three'>`four']@1 `*' if `touse' [`weight' `exp'], from(`start') notable `options' ///
			var(A[`three']@a AU[`three'>`four']@a D[`three']@d DU[`three'>`four']@d) ///
			cov(A[`three']*D[`three']@0 AU[`three'>`four']*DU[`three'>`four']@0)
		}
		else {
			gsem `one' <- c.`a'#A[`three']@1 c.`au'#AU[`three'>`four']@1 c.`d'#D[`three']@1 c.`du'#DU[`three'>`four']@1 `*' if `touse' [`weight' `exp'], notable `options' ///
			var(A[`three']@a AU[`three'>`four']@a D[`three']@d DU[`three'>`four']@d) ///
			cov(A[`three']*D[`three']@0 AU[`three'>`four']*DU[`three'>`four']@0)
		}
		local ae = _b[var(A[`three']):_cons]
		...
		local ds = _se[var(D[`three']):_cons]
		if ("`f'" == "gaussian" & "`total'" != "total") | "`f'" == "gaussian ," {
			quietly: lincom _b[var(A[`three']):_cons] + _b[var(D[`three']):_cons] + _b[var(e.`one'):_cons]
			local te = r(estimate)
			local ts = r(se)
		}
		if "`f'" == "bernoulli" | "`f'" == "ordinal" { 
			quietly: lincom _b[var(A[`three']):_cons] + _b[var(D[`three']):_cons] + `ee'
			local te = r(estimate)
			local ts = r(se)
			if "`ese'" == "ese" {
				local es = r(se)
	...
	* define additional locals for quantities estimated
	...
	forvalues x = 1/`k' {
		...
		if "``x''" != "J[`three']" & "``x''" != "IJ[`three'>`ij']" & "``x''" != "c.`a'#A[`three']" & "``x''" != "c.`au'#AU[`three']" & "``x''" != "c.`av'#AV[`three']" & "``x''" != "c.`d'#D[`three']" & "``x''" != "c.`du'#DU[`three']" & "``x''" != "c.`dv'#DV[`three']" & "``x''" != "c.`au'#AU[`three'>`four']" & "``x''" != "c.`du'#DU[`three'>`four']" {
			local vl`l' "``x''"
	...
	* define labels for quantities estimated
	...
	label define `n_val' `=`k'+2' "variance:" `=`k'+3' "A" `=`k'+4' "D" `=`k'+5' "E" `=`k'+6' "A+D+E" `=`k'+7' "A %" `=`k'+8' "D %" `=`k'+9' "E %", add
	if "`tcr'" == "tcr" {
		label define `n_val' `=`k'+10' "obs.cor.:" `=`k'+11' "MZr" `=`k'+12' "DZr", add
	}
	if "`total'" == "total" {
		label define `n_val' `=`k'+7' "total", modify
	}
