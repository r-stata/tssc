 ...
 *
 * April 2017: THis program uses "ado/plus/stata.trk" to figure out where the files were downloaded from.
 * This is needed, for example, when downloading <click to run> examples on the fly from the same original download location.
 * If there the same package has been downloaded multiple times from different download sites, the earlier versions
 * should be uninstalled using "ado uninstall ngram [xx]" where xx is the package number that "ado" indicates
 */
