...
version 11.0
syntax varlist [aw] , nc(str) [WMFile(str) diag LL(real 0) ///
 stand vce(passthru) MFX(str) NOLog INV2 LMHet LMSPac ROBust ///
...
tempvar XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Sig2 Sig2o SSE SST1 SST2
tempname Cov D den DVE DVNE Dx E E1 Eg Eo F
tempname h Hat hjm IPhi J K M P Phi Pm q Q IN xMx v V1 v1 V1s v2 In WXb
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname Y Yh Yi YYm YYv Z Sig2n WS D WW eVec eigw Xo mh TRa2 wWw1 xBx
tempname N DF kx kb NC NT Nmiss llf Sig21 Dim wmat xAx xAx2
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
matrix `In'=I(`N')
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit Spatial Panel Autoregressive Generalized Least Squares}}"
di _dup(78) "{bf:{err:=}}"
qui cap drop Time
ereturn scalar kb=`kb'
ereturn scalar DF=`DF'
ereturn scalar NC=`nc'
 spxttobits `yvar' `SPXvar' `wgt' , ll(`ll') nc(`nc') gmm(`gmm') ///
 `tolog' `noconstant' `vce' `robust' `coll' iter(`iter') tech(`tech')
scalar `llf'=e(llf)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
...
di _dup(78) "-"
tempvar Sig2 SigLR SigLRs SigLM SigLMs SigW SigWs E E2 En cN cT Obs Egh
local idv `idv'
qui tsset `Time'
...
program define spxttobits , eclass 
 version 11.0
syntax varlist [aw iw] , [nc(str) gmm(int 1) NOCONStant vce(passthru) ///
 LL(real 0) coll iter(int 100) tech(str) tolog]
gettoken yvar xvar : varlist
 if "`coll'"=="" {
...
qui regress `yvar' `xvar' `wgt' , noconstant `vce' `coll' 
 }
 else {
qui mkmat `xvar' `_Cons' , matrix(`X')
qui regress `yvar' `xvar' `_Cons' `wgt' , noconstant `vce' `coll' 
 }
matrix `Beta'=e(b)
matrix `Beta'=`Beta'[1,1..`kb']'
matrix `E'=`Y'-`X'*`Beta'
