*! lmoments9 renamed 15 April 2010
...
program lmoments9, rclass sort byable(recall)   
        version 9 
	...
	quietly {
		...
		tempname N d b0 L1 L2 L3 L4 mylbl V
	...
	if "`se'`se2'" != "" { 
		...
		if `ng' > 1 { 
			if `nvars' > 1 local what "``ng''"
			else local what `"`by' = `= `by'[`min']'"'
			di _n as txt "NB: se for `what' only"
		}

		local varname : word `nvars' of `varlist' 
		mata: work("`V'", "`id'", "`varname'", `min', `max')

		// now get the variance matrix of b0 b1 b2 b3 
...
void work(string scalar Vname, string scalar rankname, string scalar vname,
	  real scalar min, real scalar max)
{
	real colvector		rank, v, IK, IL, JL, JK, prod, xixj, range 
	real scalar		nrank, k, l, ikl, i, n, nn 
	real matrix		kl, V 

	rank = st_data(., rankname)
	nrank = rows(rank) 
	v  = st_data(., vname)
	...
	for (ikl = 1; ikl <= rows(kl); ikl++) {
		...
		prod = J(nrank, 1, 0)
		for (i = min; i < max /* NB */; i++) {
			range = (i+1) \ max
			xixj = (v[i] / nn) * v[|range|]
			prod[|range|] = prod[|range|] + 
				(IK[i] * JL[|range|] + IL[i] * JK[|range|]) :* xixj
		}
