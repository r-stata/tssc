{smcl}
{hline}
{cmd:help: {helpb sptobitsarxt}}{space 50} {cmd:dialog:} {bf:{dialog sptobitsarxt}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:sptobitsarxt: Tobit MLE Spatial Lag Panel Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb sptobitsarxt##01:Syntax}{p_end}
{p 5}{helpb sptobitsarxt##02:Description}{p_end}
{p 5}{helpb sptobitsarxt##03:Options}{p_end}
{p 5}{helpb sptobitsarxt##04:Spatial Panel Aautocorrelation Tests}{p_end}
{p 5}{helpb sptobitsarxt##05:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb sptobitsarxt##06:Heteroscedasticity Tests}{p_end}
{p 5}{helpb sptobitsarxt##07:Non Normality Tests}{p_end}
{p 5}{helpb sptobitsarxt##08:Saved Results}{p_end}
{p 5}{helpb sptobitsarxt##09:References}{p_end}

{p 1}*** {helpb sptobitsarxt##10:Examples}{p_end}

{p 5}{helpb sptobitsarxt##11:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt sptobitsarxt} {depvar} {indepvars} {weight} , {bf:{err:nc(#)}} {opt wmf:ile(weight_file)} {opt ll(#)} {p_end} 
{p 3 5 6} 
{err: [}{opt lmsp:ac} {opt lmh:et} {opt lmn:orm} {opt test:s} {opt stand inv inv2 diag} {opt dist(norm|exp|weib)}{p_end} 
{p 5 5 6} 
{opt mfx(lin, log)} {opt mhet(varlist)} {opt nocons:tant} {opt coll zero tolog nolog robust}{p_end} 
{p 5 5 6} 
...
{marker 02}{bf:{err:{dlgtab:Description}}}
 ...
 {cmd:sptobitsarxt} estimates Tobit MLE Spatial Lag Panel Regression

 {cmd:sptobitsarxt} can estimate the following models:
   1- Heteroscedastic Regression Models in disturbance term.
   2- Non Normal Regression Models in disturbance term.

{p 2 4 2}{cmd:sptobitsarxt} can generate:{p_end}
...
{p 2 4 2} {cmd:sptobitsarxt} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:sptobitsarxt} generates some variables names with prefix:
...
{col 20}{cmd:sptobitsarxt} will convert automatically spatial cross section weight matrix
...
{p 2 4 2 }Depending on the model estimated, {cmd:sptobitsarxt} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog sptobitsarxt}.
{bf:Note 4:} xtset is included automatically in {cmd:sptobitsarxt} models.
{hline}

  {stata clear all}

  {stata sysuse sptobitsarxt.dta, clear}

 {stata db sptobitsarxt}

{bf:{err:* (1) Tobit MLE Spatial Panel SAR Normal Regression Model}}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(norm) predict(Yh) resid(Ue) ll(0)}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(norm) mfx(lin) test ll(0)}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(norm) mfx(log) test ll(0) tolog}
{hline}

{bf:{err:* (2) Tobit MLE Spatial Panel SAR Exponential Regression Model}}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(exp) mfx(lin) test ll(0)}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(exp) mfx(log) test ll(0) tolog}
{hline}

{bf:{err:* (3) Tobit MLE Spatial Panel SAR Weibull Regression Model}}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(weib) mfx(lin) test ll(0)}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) dist(weib) mfx(log) test ll(0) tolog}
{hline}

{bf:{err:* (4) Tobit MLE Weighted Spatial Panel SAR Regression Model}}
 {stata sptobitsarxt ys x1 x2  [weight = x1], nc(7) wmfile(SPWxt) mfx(lin) test ll(0)}
 {stata sptobitsarxt ys x1 x2 [aweight = x1], nc(7) wmfile(SPWxt) mfx(lin) test ll(0)}
 {stata sptobitsarxt ys x1 x2 [iweight = x1], nc(7) wmfile(SPWxt) mfx(lin) test ll(0)}
{hline}

{bf:{err:* (5) Tobit MLE Spatial Lag Panel Multiplicative Heteroscedasticity}}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test ll(0) mhet(x1 x2)}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test ll(0) mhet(x1 x2)}
 {stata sptobitsarxt ys x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test ll(0) mhet(x1 x2)}
{hline}
...
. sptobitsarxt y x1 x2 , nc(7) wmfile(SPWxt) dist(norm) mfx(lin) test ll(0)
...
------------------------------------------------------------
==============================================================================
* Tobit MLE Spatial Lag Panel Normal Model (SAR)
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
- Akaike Final Prediction Error AIC               =   380.5802
- Schwarz Criterion             SC                =   386.2557
- Akaike Information Criterion  ln AIC            =     4.2870
- Schwarz Criterion             ln SC             =     4.4029
- Amemiya Prediction Criterion  FPE               =    83.6751
- Hannan-Quinn Criterion        HQ                =    76.0180
- Rice Criterion                Rice              =    73.3467
- Shibata Criterion             Shibata           =    72.2470
- Craven-Wahba Generalized Cross Validation-GCV   =    73.0348
------------------------------------------------------------------------------
...
{bf:{err:{dlgtab:SPTOBITSARXT Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2012)}{p_end}
{p 1 10 1}{cmd:SPTOBITSARXT: "Tobit MLE Spatial Lag Panel Regression"}{p_end}
