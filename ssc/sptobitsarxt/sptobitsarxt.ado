...
program define sptobitsarxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsarxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) LL(str) WMFile(str) [ORDer(int 1) diag ///
 INV mhet(str) vce(passthru) dist(str) stand MFX(str) NOLog ///
 INV2 LMHet LMSPac ROBust iter(int 100) zero coll level(passthru) ///
 PREDict(str) RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp sptobitsarxt
...
tempvar Bw D DE DF1 DumE E dcs XQX_  EE Hat ht idv itv LE LEo P Q Sig2 SSE
tempvar SST Time U U2 Ue wald weit Wi Wio WS X X0 eigw Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE DVNE Dx E E1 F
tempname h Hat IPhi J K M Sig2 Sig2o SSE SST1 SST2 WXb
tempname P Phi Pm Q In IRW IRWR wmat xAx xAx2 TRa2 wWw1 xBx
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V
tempname Y Yh YYm YYv Z X0 WS D WW eVec eigw Xo mh olsin
tempname N DF kx kb NC NT Nmiss llf Sig21 Dim rRo kb 
tempname minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df kbm
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
local initopt init(`olsin', copy)  
 ml model lf sptobitsarxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Panel Normal Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SAR1e"
 matrix `olsin'=0,`Bo'
local initopt init(`olsin', copy)
 ml model lf sptobitsarxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) `wgt' , contin `mlopts' `nolog' `diparm' `initopt' maximize ///
 title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Panel Exponential Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitsarxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Panel Weibull Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local initopt init(`olsin', copy) 
 ml model lf sptobitsarxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Panel Multiplicative Heteroscedasticity}}"
di _dup(78) "{bf:{err:=}}"
 }
...
tempname I m1k m2k m3k m4k MEAN NUM NUM1 NUM2 NUM3 RJ IN xMx
