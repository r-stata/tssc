...
program define spregsarxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsarxt" {
 error 301
...
version 11.0
syntax varlist [aw] , nc(str) [WMFile(str) ORDer(int 1) diag ///
 LL(real 0) INV mhet(str) vce(passthru) dist(str) stand MFX(str) NOLog ///
 tobit INV2 LMHet LMSPac ROBust iter(int 100) zero coll level(passthru) ///
 PREDict(str) RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spregsarxt
...
tempname Xb A B b Beta BetaSP Bx h Hat IPhi J K M
tempname Cov D den DVE DVNE Dx E E1 F wWw1 xBx SSE SST1 SST2 WXb
tempname P Phi Pm Q In IRW IRWR wmat xAx xAx2 TRa2 Sig2 Sig2b Sig2o
...
local initopt init(`olsin', copy)  
 ml model lf spregsarxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Lag Panel Normal Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SAR1e"
 matrix `olsin'=0,`Bo'
local initopt init(`olsin', copy)
 ml model lf spregsarxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Lag Panel Exponential Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spregsarxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Lag Panel Weibull Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local initopt init(`olsin', copy) 
 ml model lf spregsarxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Lag Panel Multiplicative Heteroscedasticity}}"
di _dup(78) "{bf:{err:=}}"
 }
