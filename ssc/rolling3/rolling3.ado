...
"option start() cannot be equal to option end()"
       ...
       capture qui tsset
       capture local time `r(timevar)'

		if "`predict'" != ""  &  "`p'" == ""{
        di in red "varname required for predicted values"
        exit 198
		}
		...
		if "`predict'" != ""  &  "`p'" != "" & {
		local j=`i'- `stepsize'
		capture gen `p'`j'=. if `touse' & `startpost' & `endpost'
		}	
		
		local ++iter
		qui capture predict forecast_rolling if `j'<= `time' & `touse' & `startpost' & `endpost'
		qui capture replace `p'`j'=forecast_rolling if `touse' & `startpost' & `endpost'
		qui capture drop forecast_rolling
		local next = `i' + `leads'
