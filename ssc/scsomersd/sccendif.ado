...
*
 Call cendif.
 THIS PART OF THE PROGRAM IS SPECIFIC TO sccendif
 AND IS DIFFERENT FROM THE CORRESPONDING PARTS OF THE OTHER MODULES.
*;
if "`nscen'"=="" {;local nscen "_scen1";};
qui gene byte `nscen'=`scenseq'-1;
lab var `nscen' "Scenario 1 indicator";
`bybyvars' cendif `nyvar' [`weight'=`nweight'] , by(`nscen') cluster(`cluster') cfweight(`ncfweight') funtype(vonmises) `options';
return add;
