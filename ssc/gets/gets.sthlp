...
select the 'best' model based upon a criteria of relevance and explanatory power. From a user-defined general unrestricted model (GUM), {cmd:gets} 
...
the econometric literature. The user passes the GUM to {cmd:gets} as a {it:{help varnames:depvar}} and a group of {it:{help varnames:indepvars}} which 
...
models, the user written {help xtserial} command is used.  This option does not accept {help fvvarlist:factor variable} operators. If factor variable
...
{phang2}{cmd:. gets ln_w grade age c.age#c.age ttl_exp c.ttl_exp#c.ttl_exp tenure c.tenure#c.tenure 2.race not_smsa south msp nev_mar union, xt(fe) numsearch(2)}{p_end}
