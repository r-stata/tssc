...
program define spregsemxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsemxt" {
 error 301
...
syntax varlist [aw] , nc(str) [WMFile(str) diag LL(real 0) vce(passthru) ///
 dist(str) stand MFX(str) NOLog INV2 LMHet LMSPac iter(int 100) INV zero coll tobit ///
 level(passthru) PREDict(str) RESid(str) tech(str) tolog TESTs NOCONStant LMNorm ]
gettoken yvar xvar : varlist
local sthlp spregsemxt
...
tempvar D DE DF1 DumE E dcs XQX_ EE Hat ht idv itv LE LEo P Q Sig2
tempvar SST Time U U2 wald weit Wi Wio WS X X0 eigw Bo SSE
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE DVNE Dx E F xBx Sig2n
tempname h Hat hjm IPhi J K M P Phi Pm Q Sig2 Sig2o SSE In SST1 SST2 WXb
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname xq Y Yh YYm YYv Z Dim wmat xAx xAx2 TRa2 wWw1
tempname WS D WW eVec eigw Xo mh olsin N DF kx kb NC NT Nmiss llf Sig21 
tempname rLm kb minEig maxEig waldm waldmp waldm_df waldl waldlp waldl_df
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
qui global spat_ARGS "`spat_ARGS' beta0 Lambda Sigma"
 ml model lf spregsemxt1 `MODEL' `wgt' , ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Error Panel Normal Model (SEM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
qui global spat_ARGS "`spat_ARGS' beta0 Lambda"
 ml model lf spregsemxt2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Lambda:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Error Panel Exponential Model (SEM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MODEL "`MODEL'(_cons:) (Lambda:) (Sigma:)"
qui global spat_ARGS "`spat_ARGS' beta0 Lambda Sigma"
 ml model lf spregsemxt3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Error Panel Weibull Model (SEM)}}"
di _dup(78) "{bf:{err:=}}"
 }
scalar `llf'=e(ll)
matrix `BetaSP'=e(b)
