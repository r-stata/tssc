...
program define spreghetxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spreghetxt" {
 error 301
...
version 11.0
syntax varlist , nc(str) Model(str) Run(str) [WMFile(str) diag aux(str) ///
 INV mhet(str) vce(passthru) stand MFX(str) NOLog INV2 ///
 LMHet LMSPac spar(str) iter(int 100) zero coll level(passthru) ///
 PREDict(str) RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spreghetxt
...
di
di as err "{bf:model( )} {cmd:options:} {bf:sar, sdm}" 
di in smcl _c "{cmd: see:} {help `sthlp'##05:Model Options}"
di in gr _c " (spreghetxt Help):"
 exit
 }
 }
if "`run'"!="" {
if !inlist("`run'", "mlen", "mleh") {
di
di as err "{bf:model( )} {cmd:options:} {bf:mlen, mleh}" 
...
local lmnorm "lmnorm"
 ...
 if "`mhet'"!="" {
di 
di as err " {bf:mhet({it:varlist})} {cmd:cannot be used with} {bf:run({it:mlen})}"
di as err " {bf:mhet({it:varlist})} {cmd:works only with} {bf:run({it:mleh})}"
 exit
 }
 }
 if inlist("`run'", "mleh") & "`mhet'"=="" {
local mhet "`xvar'"
qui _rmcoll `mhet' , `noconstant' `coll' forcedrop
...
tempvar D DE DF1 DumE DW E dcs XQX_ EE Hat ht idv itv LE LEo P Q Sig2 SSE
tempvar SST Time U U2 Ue wald waldm waldp weit Wi Wio WS X X0 Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yho Yho2 Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE DVNE Dx E E1 EE1 Eg Eo Ew F olsin
tempname h Hat IPhi J K M mh P Phi Pm q Q In WS D WW eVec Xo kbm
tempname S11 S12 Sig2 Sig2b Sig2o SSE SST1 SST2 TRa2 wWw1 xBx Dim kb
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V Sig21 sigox
tempname Y Yh YYm YYv Z X0 N DF kx kb NC NT Nmiss llf wmat xAx xAx2
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
if inlist("`model'", "sar") {
unab zvar: w1y_`yvar'
local SPXvar `zvar' `xvar' `aux'
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Panel Lag Regression (SAR): {cmd:Model(`model') - Run(`run')}}}"
di _dup(78) "{bf:{err:=}}"
 }
if inlist("`model'", "sdm") {
unab zvar: w1x_*
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Panel Durbin Regression (SDM): {cmd:Model(`model') - Run(`run')}}}"
di _dup(78) "{bf:{err:=}}"
 }
 if "`coll'"=="" {
...
ereturn scalar NC=`nc'
 xi: ml model d0 spreghetxt1 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
 (Sigu:) (Sige:) , `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
 local COLNAME " `COLNAME'`yvar':_cons Sigu:_cons Sige:_cons"
 qui test `SPXvar'
scalar `waldm'=r(chi2)
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Random-Effects Panel Data Regression (Normal Distribution Model)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local initopt init(`olsin', copy) 
ereturn scalar NC=`nc'
 xi: ml model d0 spreghetxt2 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Random-Effects Panel Data Regression (Normal Distribution Model)}}"
di as txt "{bf:{err:* Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
 }
scalar `llf'=e(ll)
matrix `Beta'=e(b)
...
tempname I m1k m2k m3k m4k MEAN NUM NUM1 NUM2 NUM3 RJ WXb IN xMx
