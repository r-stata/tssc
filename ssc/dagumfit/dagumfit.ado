...
program define Estimate, eclass byable(recall)
	...
	local title "ML fit of Dagum distribution"

	local inc "`varlist'"
	
        if "`abp'" ~= "" & (("`avar'"~="")|("`bvar'"~="")|("`pvar'"~="")) {
                di as error "Cannot use abpq(.) option in conjunction with avar(.), bvar(.), pvar(.) options"
                exit 198
        }
	...
	marksample touse 
	markout `touse' `varlist' `avar' `bvar' `pvar' `cluster'
	if "`svy'" ~= "" {
		svymarkout `touse'
	...
	if "`avar'"=="" & "`bvar'"=="" & "`pvar'"=="" {
		...
		if "`cdf'" ~= "" {
		 	qui ge `cdf' = (1 + (`b'/`inc')^`a')^(-`p')  if `touse'
