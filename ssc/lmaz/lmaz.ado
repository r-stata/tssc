...
program define lmaz , eclass
version 11.0
syntax varlist [if] [in] , [NOCONStant LAGs(int 1)]
...
tempvar DE DF1 E SSE Time wald weit Wi
tempvar Wio WS X0 Yb Yh Yh2 Yhb Yho YY YYm YYv Yh_ML Ue_ML TimeN
tempname b B Beta Bx Cov D E E1 F h Hat HT IPhi J K L M P Phi Pm Q 
tempname Sig2 Sig2o Sig2o1 W W1 Wald We Wi Wi1 Wio
tempname X X0 Y Yh Nn N kx DF kb SSEo llf SST1 SST2 V
...
matrix `X0'= J(`N',1,1)
_rmcoll `varlist' , `noconstant' forcedrop
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** OLS Autocorrelation Z Test}}"
di _dup(78) "{bf:{err:=}}"
...
di as txt "- Z Test" _col(35) "AR(" `i' ")=" as res %8.4f `lmaz'`i' _col(56) as txt "P-Value >Chi2(`i')" _col(73) as res %5.4f chi2tail(`i', abs(`lmaz'`i'))
