*! Version 1.0.0 
* NEGATIVE BINOMIAL-CLOGLOG HURDLE REGRESSION :  Joseph Hilbe  :  25Sep2005
program hnbclg, eclass properties(svyb svyj  svyr)
    version 9.1
    ...
    ml model lf jhnb_cln_ll (cloglog: `lhs' =  `rhs', `offset' `exposure')   ///
         (negbinomial: `lhs' = `rhs', `offest' `exposure')   /lnalpha                     ///
        `if' `in' `weight',                     ///
        `mlopts' `robust' `cluster'             ///
        title("Negative Binomial-Cloglog Hurdle Regression")           ///
