*! sptobitmstardxt V1.0 25/05/2013
...
program define sptobitmstardxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstardxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) WMFile(str) LL(str) [dist(str) aux(str) ///
 diag INV INV2 vce(passthru) stand zero ROBust MFX(str) coll NOLog ///
 LMHet LMSPac iter(int 100) level(passthru) PREDict(str) tech(str) ///
 RESid(str) tolog NWmat(int 1) TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp sptobitmstardxt
...
tempvar EE Ew Hat ht idv itv LE LEo P Q SBB Sig2 SSE
tempvar SST Time tm U U2 Ue wald weit Wi Wio WS X X0 eigw Bo
tempvar Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A b B Beta BetaSP Bx Cov Cov1 Cov2s CovC D den DVE DVNE Dx E
tempname h Hat hjm IPhi J K M P Phi Pm Q kbm IN xMx E1 F
tempname Sig2 Sig2o SSE SST1 SST2 v V1 v1 V1s v2 In IRW IRWR WXb
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V
tempname Y Yh Yi YYm YYv Z WS D WW eVec eigw Xo mh olsin Sig2n Rostar rRo kb
tempname N DF kx kb NC NT Nmiss kz llf Sig21 Dim wmat xAx xAx2 TRa2 wWw1 xBx
...
_vce_parse , opt(Robust oim opg) argopt(CLuster): `wgt' , `vce'
 mlopts mlopts , `vce' `coll' iter(`iter') tech(`tech')
matrix `In'=I(`N')
local N = _N
tempname Bo
global spat_kx=`kx'
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstardxt01 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy) 
 ml model lf sptobitmstardxt02 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) `wgt' , contin `mlopts' `nolog' `diparm' `initopt' ///
...
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf sptobitmstardxt03 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardxt04 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR2e"
 matrix `olsin'=`Bo',0,0
local initopt init(`olsin', copy)
 ml model lf sptobitmstardxt05 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
...
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardxt06 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstardxt07 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardxt08 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardxt09 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstardxt10 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)
 ml model lf sptobitmstardxt11 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstardxt12 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
