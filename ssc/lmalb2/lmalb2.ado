...
program define lmalb2 , eclass byable(onecall)
version 11.0
...
tempname Sig2o1 SSE Sig2 F K B Bx Zo V Ko Koi
tempname Nn kx kb DF N ky kyi kzi llf Kz kmelo
...
tempname SSE Rho BB SBBs SRhos lmalb 
local N=`Nn'
qui tsset `Time'
...
qui replace `SBB' = `BB'`i' if `touse'
summ `SBB' if `touse' , meanonly
qui replace `SBB' = r(mean) if `touse'
summ `SBB' if `touse' , meanonly
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Autocorrelation Ljung-Box Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
...
di as txt "- Ljung-Box  LM Test" _col(40) "AR(" `i' ")=" as res %8.4f `lmalb'`i' _col(56) as txt "P-Value >Chi2(`i')" _col(73) as res %5.4f chi2tail(`i', abs(`lmalb'`i'))
