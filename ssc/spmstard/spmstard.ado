...
program define spmstard , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spmstard" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) [NWmat(int 1) coll NOCONStant tolog ///
 INV INV2 vce(passthru) dist(str) stand MFX(str) noLOG tobit ROBust aux(str) ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str) LL(real 0)]
gettoken yvar xvar : varlist
local sthlp spmstard
...
tempname Xb A B b B1 b1 Beta BetaSP Bx WS Cov D E F IPhi J K L kx kb
tempname P Phi Pm Q Sig2 Sig2o SSE SST1 SST2 D WW eVec eigw Xo
tempname In IRW Vec vh W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V Nmiss kz DF
tempname Y Yh Yi YYm YYv Z llf Rostar rRo kb Sig21 Dim olsin N
...
local initopt init(`olsin', copy) 
 ml model lf spmstard1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf spmstard2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy)  
 ml model lf spmstard3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstard4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf spmstard5 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstard6 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf spmstard7 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstard8 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin ///
 `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
