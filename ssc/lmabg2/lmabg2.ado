...
program define lmabg2 , eclass byable(onecall)
version 11.2
...
tempname X X0 Vec J SST1 SST2 Sig2o1 CovC SSE Sig2 lf F K B Bx Zo qhiv V Ko Koi
...
tempname SSE Rho BB SSE lmabgd lmabgk
local N=`Nn'
qui tsset `Time'
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Autocorrelation Breusch-Godfrey Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
