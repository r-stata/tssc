{smcl}
{hline}
{cmd:help: {helpb lmndp2}}{space 50} {cmd:dialog:} {bf:{dialog lmndp2}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:lmndp2: 2SLS-IV Non Normality D'Agostino-Pearson Test}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb lmndp2##01:Syntax}{p_end}
{p 5}{helpb lmndp2##02:Description}{p_end}
{p 5}{helpb lmndp2##03:Model}{p_end}
{p 5}{helpb lmndp2##04:GMM Options}{p_end}
{p 5}{helpb lmndp2##05:Other Options}{p_end}
{p 5}{helpb lmndp2##06:Saved Results}{p_end}
{p 5}{helpb lmndp2##07:References}{p_end}

{p 1}*** {helpb lmndp2##08:Examples}{p_end}

{p 5}{helpb lmndp2##09:Authors}{p_end}

{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 2 4 2} 
{cmd:lmndp2} {depvar} {it:{help varlist:indepvars}} {cmd:({it:{help varlist:endog}} = {it:{help varlist:inst}})} {ifin} , {p_end} 
{p 6 6 2}
...
{pstd}
{cmd:lmndp2} computes 2SLS-IV Non Normality D'Agostino-Pearson Test for instrumental variables regression models, via 2sls, liml, melo, gmm, and kclass.{p_end}
...
{synopt:{bf:noconexog}}Exclude Constant Term from all Equations (both RHS and Instrumental Equations). Results of using {cmd:noconexog} option are identical to Stata {helpb ivregress}.
 The default of {cmd:lmndp2} is including Constant Term in both RHS and Instrumental Equations{p_end}

{marker 06}{bf:{err:{dlgtab:Saved Results}}}

{cmd:lmndp2} saves the following in {cmd:e()}:
...
{marker 08}{bf:{err:{dlgtab:Examples}}}
 ...
 {stata sysuse lmndp2.dta , clear}

 {stata db lmndp2}

 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(2sls)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(melo)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(liml)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(fuller) kf(0.5)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(kclass) kc(0.5)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(white)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(bart)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(dan)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(nwest)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(parzen)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(quad)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(tent)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(trunc)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(tukeym)}
 {stata lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(gmm) hetcov(tukeyn)}
{hline}

. clear all
. sysuse lmndp2.dta , clear
. lmndp2 y1 x1 x2 (y2 = x1 x2 x3 x4) , model(2sls)
...
==============================================================================
*** 2SLS-IV Non Normality D'Agostino-Pearson Test - Model= (2sls)
==============================================================================
 Ho: Normality - Ha: Non Normality
------------------------------------------------------------------------------
- D'Agostino-Pearson LM Test           =   5.0270     P-Value > Chi2(2) 0.0810
------------------------------------------------------------------------------
...
{bf:{err:{dlgtab:LMNDP2 Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2015)}{p_end}
{p 1 10 1}{cmd:LMNDP2: "Stata Module to Compute 2SLS-IV Non Normality D'Agostino-Pearson Test"}{p_end}
