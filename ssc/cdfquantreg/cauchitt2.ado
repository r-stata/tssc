...
quietly {
	...
	gen double `mu' = `Xb'
	replace `lnf' =  log((($ML_y1 == 1/2)*2*sqrt(2) + ($ML_y1 > 1/2)*(2*$ML_y1 ///
	- 1)/sqrt(8*(1 - 2*$ML_y1)^2*((1 - $ML_y1)*$ML_y1)^3) + ($ML_y1 < 1/2)*(1 ///
	- 2*$ML_y1)/sqrt(8*(1 - 2*$ML_y1)^2*((1 - $ML_y1)*$ML_y1)^3))/(_pi*`sigma'*(1 ///
	+ (`mu' - 2*(($ML_y1 > 1/2) - 1/2)*(sqrt((1 - 2*$ML_y1)^2/(2 * (1 ///
	- $ML_y1)*$ML_y1))))^2/`sigma'^2)))
	}
end
/*
