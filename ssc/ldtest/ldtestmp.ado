*! ldtest v1.0 GFBarrett 01Feb2013
program ldtestmp, rclass
 version 11.0
 syntax varlist(min=2 max=3 numeric) [if] [in] [, breps(integer 10) CVM LCE]
...
mata: st_view(x=., ., ("`1'","`3'"),"`3'")
mata: st_view(y=., ., ("`2'","`3'"),"`3'")
...
mata:  maxdf=max(LCd)
mata:  ks  = lam*maxdf
mata: kspv=ldbootpvaluemp(ks, x, y, bootreps, testch)
...
mata: U = lam*lcdarea
mata: kspv=ldbootpvaluemp(U, x, y, bootreps, testch)
...
mata: kspv=ldbootpvaluemp(kslce, x, y, bootreps, testch)
