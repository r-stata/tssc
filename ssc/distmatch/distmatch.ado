*! distmatch 1.0.1 01Sep2009 roywada@hotmail.com
*! distance matching based on latitudes and longitudes
...
syntax [varlist(default=none)], id(string) LATitude(string) /*
	*/ LONgitude(string) [dist(string) NEARest(string) SUFfix(string) MAXimum(numlist max=1 >=0) /*
	*/ MINimum(numlist max=1) count COUNT1(string) PREfix(string) km NOIsily DONor(string) RECipient(string)]
...
* place holders
foreach var in `group' `id' `varlist' `lat' `lon' {
	tempvar `var'New
	gen ``var'New'=`var'
}
foreach var in `dist' {
	...
	*}
	...
	forval place=`donMin'/`donMax' {
		* comarison values
		...
		foreach var in `dist' {
			* Great Circle Distance Formulas (from Wikipedia)
			*replace ``dist'New'= 6367.45 * `convert' * acos(sin(``lat'New'/57.2957795) * sin(`lat'/57.2957795) + cos(``lat'New'/57.2957795) * cos(`lat'/57.2957795) * cos(``lon'New'/57.2957795-`lon'/57.2957795))
			
			* Haversine Aug 31, 2009 (from Wikipedia)
			replace ``dist'New'= (sin(  (``lat'New'/57.2957795-`lat'/57.2957795)/2))^2 + cos(``lat'New'/57.2957795)*cos(`lat'/57.2957795)* (sin( (``lon'New'/57.2957795-`lon'/57.2957795) /2))^2
			replace ``dist'New' = 2 *  6367.45 * `convert' * asin( (``dist'New')^.5)
