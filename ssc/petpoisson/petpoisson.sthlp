...
{p 8 12 2} Endogenous Participation Endogenous Treatment Poisson model by MSL

{title:Basic syntax}

{p 8 12 2}{cmd:petpoisson} ({it:Tvar} = {it:varlist1}) ({it:Pvar} = {it:varlist2}) ({it:cvar} = {it:varlist3}) [{cmd:if}
{it:exp}] [{cmd:in} {it:range}]{cmd:,} {cmd:rep(#)} {break} {cmdab:hvec:tor}(# # #){p_end} 

{title:Full syntax}

{p 8 12 2}{cmd:petpoisson} ({it:Tvar} = {it:varlist1}) ({it:Pvar} = {it:varlist2}) ({it:cvar} = {it:varlist13}) [{cmd:if}
...
{p 4 4 2}
{cmd:petpoisson} fits a Endogenous Participation Endogenous Treatment Poisson model for count data by Maximum Simulated Likelihood. {p_end}

{title:Important notes}

{p 4 4 2} The equation for the treatment variable {it:Tvar} always comes first, then the equation for the participation dummy {it:Pvar}, and finally the equation for the main count outcome variable {it:cvar}. This order should be always observed, 
...
{p 4 8 2} . use petpoisson_sdata.dta {p_end}

{p 4 8 2} . petpoisson (T = x1) (P = T x2) (count = T x3), rep(1600) hvec(1 1 100) {p_end}

{p 4 8 2} . petpoisson (T = x1) (P = T x2) (count = T x3), rep(1600) hvec(1 1 100) hbased {p_end}

{title:Marginal effects after estimation}

{p 4 8 2} /* At the mean of explanatory variables */ {p_end}
{p 4 8 2} . petpoisson_me [{cmd:if} {it:exp}] [{cmd:in} {it:exp}] /* At the mean of explanatory variables */ {p_end}

{p 4 8 2} /* MEs at the mode of explanatory variables */{p_end}
{p 4 8 2} . petpoisson_me, dmode {p_end}

{p 4 8 2} /* MEs at the # quantile of the linear predictor */{p_end}
{p 4 8 2} . petpoisson_me, XBQuantile(0.3)  {p_end}

{p 4 8 2} /* ME for T when count is 2 (v.g. mode of main count) */{p_end}
{p 4 8 2} . petpoisson_me, cmode(T 2)  {p_end}
...
{p 4 4 2} {cmd: petpoisson} is not an official Stata command and is an implementation of the methods discussed in Bratti and Miranda (2011). There is no warranty and the author cannot accept any responsability for the use of this software. 
...
{p 8 13 2} Miranda, A. 2012. petpoisson: Stata module (Mata) to estimate an Endogenous Participation Endogenous Treatment Poisson model by MSL. Available at {browse "http://ideas.repec.org/e/pmi55.html"}.{p_end}
