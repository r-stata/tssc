*! xtnptimevaravecb version 1.0.0
*! The evaluator for the command xtnptimevar with bootstrapping
...
capture program drop xtnptimevaravecb
program xtnptimevaravecb, eclass sortpreserve
        version 12.0
        syntax varlist(ts) [if] [in] , STUB(string) [ bwidth(real 999.999) forcereg alle pvk(integer 1) matsize(integer 800) checkb  ]
        marksample touse
        ...
        tempname matcoefs nnsc ttsc hhsc kksc nnttsc ultmbb
        tempvar Ti
        local touse = e(touse)
        set matsize `matsize'
        ...
        forvalues i = 1(1)`nbcoefs' {
                                     label var `stub'_`i' "Coefficient `i', original"
        }
        mkmat `stub'_1-`stub'_`nbcoefs', nomissing
        forvalues i = 1(1)`nbcoefs' {
                                     matrix define `stub'_`i' = (`stub'_`i')'
        }
        matrix define `ultmbb' = `stub'_`pvk'
        quietly {
                 tsset
                 gen `stub'_tvarkda1 = `timevarkda'  if `touse'
                 levelsof `stub'_tvarkda1 if `touse', local(yearlevkda)
                 gen `stub'_tvpkda = .
                 local i = 1
                 foreach l of local yearlevkda {
                                                replace `stub'_tvpkda = `l' if _n == `i'
                                                local i = `i' + 1
                 }
                 levelsof `stub'_tvpkda, local(levmakda)
                 matrix colnames `ultmbb' = `levmakda'
                 capture drop `stub'_tvarkda1 `stub'_tvpkda
        }
        quietly tsset
        ereturn post `ultmbb', esample(`touse')
        ereturn local depvar "`yyvr'"
        ereturn local ivar "`ivarvr'"
        ereturn local nombreco "`nbcoefs'"
        ereturn local stubloc  "`stub'"
        ...
        if "`checkb'" == "" {
                             ereturn local cmd = "xtnptimevaravecb"
        }
        else {
