...
program define lmfreg2 , eclass byable(onecall)
version 11.0
...
tempvar Yt e LDE DF1 Eo Lambda0 Yh2 LYh2 E3 E4 Yhr YYm ELin ELog
tempvar LnE2 absE LE XQ ht Hat Yhat1 Yho Yh_ML Ue_ML Wio Wi
tempvar DFF DFF1 DFF2 Rx COR xq LVal LVal1 logYh LY Yh LYh  
tempvar LY LOGYX_ Yh LYh ELYh LOGyvar LOGvars LOGexog LOGendog LOGinst
...
local N=`Nn'
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* 2SLS-IV Linear vs Log-Linear Functional Form Tests - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
...
qui tsset `Time'
if inlist("`model'", "2sls", "liml", "gmm") {
qui ivregress `model' `yvar' `exog' (`endog' = `inst') if `touse' , `noconstant'
 }
if inlist("`model'", "melo", "kclass", "fuller") {
qui ivregress liml `yvar' `exog' (`endog' = `inst') if `touse' , `noconstant'
 }
scalar `SSELin'=e(rss)
...
if `r2lin'==. {
qui correlate `Yh' `yvar'
scalar `r2lin'=r(rho)*r(rho)
 }
qui gen `logYh'=ln(`Yh') if `touse'
qui gen `LOGyvar' = ln(`yvar') if `touse'
qui foreach var1 of local endog {
qui gen `LOGendog'`var1' = ln(`var1') if `touse'
 }
local lhsq `"`yvar' `exog'"'
qui foreach var2 of local lhsq {
qui gen `LOGvars'`var2' = ln(`var2') if `touse'
 }
qui foreach var3 of local inst {
qui gen `LOGinst'`var3' = ln(`var3') if `touse'
 }
if inlist("`model'", "2sls", "liml", "gmm") {
qui ivregress `model' `LOGyvar' `exog' (`endog' = `inst') if `touse' , `noconstant'
 }
if inlist("`model'", "melo", "kclass", "fuller") {
qui ivregress liml `LOGyvar' `exog' (`endog' = `inst') if `touse' , `noconstant'
 }
scalar `r2log'=e(r2)
qui predict `LYh' if `touse'
if `r2log'==. {
qui correlate `LYh' `LOGyvar'
