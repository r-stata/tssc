...
program define spmstarh , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spmstarh" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) [NWmat(int 1) coll NOCONStant MHET(str) ///
 INV INV2 vce(passthru) stand MFX(str) noLOG tobit ROBust tolog ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str) LL(real 0)]
gettoken yvar xvar : varlist
local sthlp spmstarh
...
tempname A B b B1 b1 Beta BetaSP Bx WS D WW eVec eigw
tempname Cov D E F IPhi J K L olsin N DF kx kb Nmiss kz llf Xo
tempname P Phi Pm Q Sig2 Sig2b Sig2o Sig2u Sig2w SSE SST1 SST2
tempname In IRW Vec vh W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname xq Xx Y Yh Yi YYm YYv Z Sw Rostar rRo kb Sig21 Dim
...
local initopt init(`olsin', copy)  
 ml model lf spmstarh1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di as txt "{bf:{err:* (m-STAR) Spatial Lag Multiplicative Heteroscedasticity Model}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf spmstarh2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstarh3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstarh4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
