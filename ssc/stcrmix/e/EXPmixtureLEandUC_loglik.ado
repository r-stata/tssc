program define EXPmixtureLEandUC_loglik     /* Exponential mixture model */
		args lnlik logitpi beta1 beta2 /*logitpi= log (pi/(1-pi)) logsigma1=log(sigma1) logsigma2=log(sigma2) */
		...
		quietly {
			 ...
			 gen double `x1entry' = (exp(-(`beta1'))*$entrytime)
			 gen double `x1exit' =  (exp(-(`beta1'))*$exittime)
			 gen double `x1upperexit' = (exp(-(`beta1'))*$upperexittime)
			 gen double `x2entry' = (exp(-(`beta2'))*$entrytime)
			 gen double `x2exit' =  (exp(-(`beta2'))*$exittime)
			 gen double `x2upperexit' = (exp(-(`beta2'))*$upperexittime)
			 
			 gen double `S1entry' = exp(-`x1entry') 
			 gen double `S2entry' = exp(-`x2entry')
			 gen double `logf1exit' = -`beta1'-`x1exit' 
			 gen double `logf2exit' = -`beta2'-`x2exit'
