...
program define lmadurh2 , eclass byable(onecall)
version 11.0
syntax [anything] [if] [in] , Model(string) [NOCONStant NOCONEXOG kc(real 0) ///
 KF(real 0) HETcov(str) coll DLag(int 1) LAGs(int 1)]
...
tempname S2y SSE Rho BB Po lmadho lmadhop lmahho Pa1 
tempname lmadha lmadhap lmahha lmahhap lmahhop SSEa Pa lmawt lmawtp lmawc lmawcp
local N=`Nn'
qui tsset `Time'
...
matrix `Cov'=e(V)
local kylag=`kendog'+`dlag'
matrix `vy1' =`Cov'[`kylag'..`kylag', `kylag'..`kylag']
scalar `S2y'=`vy1'[1,1]
cap drop `LE'*
tempvar LE1 
qui gen `LE1' =L1.`E' if `touse'
qui regress `E' `LE1' if `touse' , noconst
scalar `Po'= _b[`LE1']
...
qui replace `Es'=`Ea' in 1/1
qui gen `Es1'=L1.`Es' if `touse'
qui regress `Es' `Es1' if `touse' , noconst
scalar `Pa1'= _b[`Es1']
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Autocorrelation Dynamic Tests - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
...
if `lmadho' != . {
di as txt "- Durbin  h Test (Lag DepVar)" _col(40) "AR(1)=" as res %8.4f `lmadho' _col(56) as txt "P-Value >Z(0,1)" _col(73) as res %5.4f `lmadhop'
 } 
di as txt "- Durbin  h Test after ALS(1)" _col(40) "AR(1)=" as res %8.4f `lmadha' _col(56) as txt "P-Value >Z(0,1)" _col(73) as res %5.4f `lmadhap'
di _dup(78) "-"
di as txt "- Harvey LM Test (Lag DepVar)" _col(40) "AR(1)=" as res %8.4f `lmahho' _col(56) as txt "P-Value >Chi2(1)" _col(73) as res %5.4f `lmahhop'
di as txt "- Harvey LM Test after ALS(1)" _col(40) "AR(1)=" as res %8.4f `lmahha' _col(56) as txt "P-Value >Chi2(1)" _col(73) as res %5.4f `lmahhap'
di _dup(78) "-"
di as txt "- Wald    T Test" _col(40) "AR(1)=" as res %8.4f `lmawt' _col(56) as txt "P-Value >Z(0,1)" _col(73) as res %5.4f `lmawtp'
di as txt "- Wald Chi2 Test" _col(40) "AR(1)=" as res %8.4f `lmawc' _col(56) as txt "P-Value >Z(0,1)" _col(73) as res %5.4f `lmawcp'
di _dup(78) "-"
