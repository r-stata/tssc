program def hbox
*! NJC 1.2.0 2 April 2002 
* NJC 1.1.0 27 April 2000 
* NJC 1.0.1 26 May 1999 
* NJC 1.0.0 3 May 1999
        version 6.0
        syntax varlist(min=1) [if] [in] [aweight fweight] /*
	*/ [, BY(varname) TOtal TItle(str) Qsort(str) SOrt(str) PEn(str) /*
	*/ shown * ] 

	if "`sort'" != "" { 
		di in r "sort( ) not allowed" 
		exit 198
	}	
	...
	if "`qsort'" != "" {
		if index("`qsort'","-") == 1 { 
			local sign "-" 
			local qsort = substr("`qsort'",2,.) 
		}
		local valid 0 
		if "`qsort'" == "min" { local valid 1 }
		else if "`qsort'" == "p25" { local valid 1 }
		else if "`qsort'" == "p50" { local valid 1 }
		else if "`qsort'" == "med" { 
			local valid 1
			local qsort "p50"
		}	
		else if "`qsort'" == "p75" { local valid 1 }
		else if "`qsort'" == "max" { local valid 1 } 
		else if "`qsort'" == "n" { local valid 1 } 
		else if "`qsort'" == "N" { 
			local qsort "n" 
			local valid 1 
		} 
		if !`valid' { 
			di in r "qsort: invalid syntax"
			exit 198
		}
	}
	...
	qui if "`by'" != "" {
		sort `touse' `by' 
		tempvar group 
		by `touse' `by' : gen byte `group' = _n == 1 & `touse'
		...
		while `i' <= `ngroup' {
			tempvar by`i' 
			gen `by`i'' = `varlist' if `group' == `i' 
			local vlbl = `by'[`j'] 
			if "`vallbl'" != "" { 
				cap local vlbl : label `vallbl' `vlbl' 
			} 
			label var `by`i'' "`vlbl'" 
			local newlist "`newlist' `by`i''" 
			count if `group' == `i' 
			local j = `j' + r(N) 
			local i = `i' + 1
		}
		if "`total'" != "" {
		 	tempvar copy 
			gen `copy' = `varlist'
			label var `copy' "Total" 
			local newlist "`newlist' `copy'" 
		}	
        ...
        tempvar min p25 p50 p75 max n names d1 d2 d3 d4
        tempname stat
        qui {
                univstat `varlist' [`weight' `exp'] if `touse', /* 
		*/ s(min p25 p50 p75 max n) m(`stat')
                svmat2 `stat', n(`min' `p25' `p50' `p75' `max' `n') 
                gen `d1' = `p25' - `min'
                gen `d2' = `p50' - `p25'
                gen `d3' = `p75' - `p50'
                gen `d4' = `max' - `p75'
		...
		while `i' <= `nvars' {
			...
			if "`label'" != "" { local name "`label'" }
			replace `names' = "`name'" in `i'
			local i = `i' + 1 
		}
        }

	if "`qsort'" != "" { local sort "sort(`sign'``qsort'')" }

	if "`pen'" == "" { local pen "pen(22222)" }  
	else { 
		local pen = substr("`pen'",1,1) 
		local pen : di _dup(5) "`pen'" 
		local pen "pen(`pen')" 
	}
	
	if "`shown'" != "" { local options "`options' rl(`n')" } 

        Hbar `min' `d1' `d2' `d3' `d4' in 1/`nvars', `sort' showzero /*
        */ `pen' `options' t2(" ") l(`names') `title' sh(nl..r) 
end

* Hbar 1.0.0 NJC 2 April 2002 
* hbar 1.4.4 NJC 8 November 1999 
program define Hbar
    version 6.0
    ...
    qui {
        tempvar touse order gleg gap dpos dpos2 dmax 
        mark `touse' `if' `in'
	...
	* legend on right
        ...
        * data range
        ...
        loc dmaxmax = r(max)
        loc min = min(`min', `dminmin')
        loc max = max(`max', `dmaxmax')
        loc drange = `max' - `min'
