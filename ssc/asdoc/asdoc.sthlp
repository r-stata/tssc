{smcl}
{right:version:  2.4.1}
{cmd:help asdoc} {right:June 15, 2019}
{hline}
...
{bf:{help asdoc##3Correlation: 3. Correlations}}
    ...
    {help asdoc ##3_3PartialCor:3.3 Partical correlations}
...
{bf:{help asdoc##10means:10. Means ({opt ameans})}}
    {help asdoc##101:10.1 Arithmatic / harmonic / geometric means}
...
To develope a table row by row from different runs of the asdoc, we need to use option {opt rowappend}.
This option can be used with ttests [{help asdoc##example53:see example here}], customized summary statistics [see examples here], 
...
{p 4 8 2}  {opt Write text in bold and italics}{break}  
asdoc, text(\b \i A car is a wheeled motor vehicle used for transportation) {break} 

{p 4 8 2}  {opt Write text as a new paragraph}{break}  
asdoc, text(\par A car is a wheeled motor vehicle used for transportation) {break} 

{p 4 8 2} For more on RTF control words, see {browse "http://www.biblioscape.com/rtf15_spec.htm":this pag}.
...
{p 4 4 2} {cmd: 2.3 Custome summary statistics}: {break}  
...
offers more than 10 options, it is discussed in {help asdoc##WideReg:this seperate section} with relevant examples. The following options are available when exporting 
...
regression coefficient. The default for this option is standard errors. 
...
{help asdoc##wide4:4.3.3.2 Supressing the t-values / se text in the header row ({opt notse})}{break} 
...
The default is to use parenthesis around stanard errors / t-values. However, 
we can use option {opt bracket} to use square brackes around these values.
...
{dlgtab:Additonal regresson statistics to report}
...
a. If option {opt t()} is used, then the text "{hi:{it:t-statistics are in parenthesis}}" will be added. {break}
b. If option {opt se()} is used, then the text "{hi:{it:Standard errors are in parenthesis}}  will be added." {break}
...
Option {opt btp} allows to convert square brackets to parenthesis when they are written to the Word file.
...
for each company. We shall run a regression seperately for each company
...
custom-made routines in asdoc that creates clean tables from tabstat command. 
asdoc fully supports the command structure and options of tabstat. And, yes
...
to a seperate cell in the output table. 
...
no futher cells can be appended to the same row. This means that we need to first collect all
the required bits of information before writting a row. Collecting and holding these bits of information
...
The word {opt accum} is an abbreviation that I use for {opt accumulate}. The syntax of this option is given below:
 ...
 {opt ${accum}}. Once we have accumulated all the needed bits of information in the global macro, then its conents
 can be written to the Word table with option {opt row}. Option {opt show} can be used to show contents of the gloabal
 macro {bf: ${accum}}.
...
6. We again accumulate the needed statisics for our second variable in the sixth line.{break}
7. Since our row of required stistics is now complete, we write the accumulated stistics and the first row label, i.e, {bf:1935-1945} to our Word file.{break}
...
{p 4 4 2} YouTube video : {browse "https://www.youtube.com/watch?v=zdI65G6AhdU&t=" : Creating descriptive statistics with asdoc}{break}
 ...
 Blog post : {browse "https://fintechprofessor.com/2018/12/19/asdoc-cutomizing-the-regression-output-ms-word-from-stata-confidence-interval-adding-stars-etc/" : Cutomizing the regression output Confidence Interval, adding stars, etc.}{break}
...
{p 4 4 2} This is the biggest program I have ever written. asdoc took more than 3 months
to complete (average 14 hours per day). It has more than 30 functions written in Mata language and more than 15 programs
written in the Stata language. All these programming efforts span over 5400 lines of 
codes, 799 IF statements, and 313 ELSE statements. If you like it and find it useful,
