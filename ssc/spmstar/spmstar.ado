...
version 11.0
syntax varlist [aw] , WMFile(str) [NWmat(int 1) coll NOCONStant ///
 INV INV2 vce(passthru) dist(str) stand MFX(str) noLOG tobit ROBust tolog ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str) LL(real 0)]
...
tempvar D DE DF DF1 E P Q Sig2 SSE SST Time U U2 wald weit
tempvar Wi Wio Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx WS Sig2o SSE SST1 SST2 D WW
tempname Cov D E F IPhi J K L olsin N DF kx kb Nmiss P Phi Pm Q Sig2
tempname In IRW Vec vh W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V Xo eigw
tempname Y Yh Yi YYm YYv Z llf kz eVec Rostar rRo kb Sig21 Dim
...
local initopt init(`olsin', copy) 
 ml model lf spmstar1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf spmstar2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy)  
 ml model lf spmstar3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
...
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstar4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf spmstar5 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstar6 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
local initopt init(`olsin', copy) 
 ml model lf spmstar7 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf spmstar8 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin ///
 `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
