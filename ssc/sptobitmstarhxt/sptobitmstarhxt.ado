*! sptobitmstarhxt V1.0 14/09/2013
*!
...
program define sptobitmstarhxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstarhxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) WMFile(str) LL(str) [mhet(str) ///
 vce(passthru) stand zero ROBust MFX(str) NOLog LMHet LMSPac diag ///
 iter(int 100) level(passthru) PREDict(str) INV INV2 coll ///
 RESid(str) tech(str) tolog NWmat(int 1) TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp sptobitmstarhxt
...
tempvar D DE DF1 DumE E dcs XQX_ EE Eo Hat ht idv itv LE P Q Sig2 SSE
tempvar SST Time U U2 Ue wald weit Wi Wio WS X X0 eigw Bo E E1 F
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B Beta BetaSP Bx N DF kx kb NC NT Nmiss kz llf Cov den
tempname h Hat IPhi J K M P Phi Pm Q kbm IN xMx Dx D DVE DVNE
tempname Sig2 Sig2o SSE SST1 SST2 In IRW IRWL WXb
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname Y Yh Yi YYm YYv Z WS D WW eVec eigw Xo mh olsin
tempname Sig21 Dim wmat xAx xAx2 TRa2 wWw1 xBx Sig2n Rostar rRo kb
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarhxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarhxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarhxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarhxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
