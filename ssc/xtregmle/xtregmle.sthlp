{smcl}
{hline}
{cmd:help: {helpb xtregmle}}{space 50} {cmd:dialog:} {bf:{dialog xtregmle}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregmle: Trevor Breusch MLE Random-Effects Panel Data Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregmle##01:Syntax}{p_end}
{p 5}{helpb xtregmle##02:Description}{p_end}
{p 5}{helpb xtregmle##03:Options}{p_end}
{p 5}{helpb xtregmle##04:Ridge Options}{p_end}
{p 5}{helpb xtregmle##05:Weight Options}{p_end}
{p 5}{helpb xtregmle##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregmle##07:Other Options}{p_end}
{p 5}{helpb xtregmle##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregmle##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregmle##10:Saved Results}{p_end}
{p 5}{helpb xtregmle##11:References}{p_end}

{p 1}*** {helpb xtregmle##12:Examples}{p_end}

{p 5}{helpb xtregmle##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregmle} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 2 2 2} {cmd:xtregmle} estimates Trevor Breusch MLE Random-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregmle} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregmle} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregmle} can use large types of weighted regression options.
...
{col 20}{cmd:xtregmle} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregmle} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregmle} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregmle.dta, clear}

  {stata db xtregmle}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregmle y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregmle.dta, clear
. xtregmle y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh

==============================================================================
* Trevor Breusch MLE Random-Effects Panel Data: Ridge and Weighted Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
 ...
 Wald Test          =   1550.1487   |   P-Value > Chi2(2)       =      0.0000
 F-Test             =    775.0743   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.7483   |   Raw Moments R2          =      0.9535
 (Buse 1973) R2 Adj =      0.6979   |   Raw Moments R2 Adj      =      0.9442
  Root MSE (Sigma)  =      8.7025   |   Log Likelihood Function =   -170.5727
------------------------------------------------------------------------------
- R2h= 0.5505   R2h Adj= 0.4606  F-Test =   28.17 P-Value > F(2 , 40)  0.0000
- R2v= 0.2563   R2v Adj= 0.1075  F-Test =    7.93 P-Value > F(2 , 40)  0.0013
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
          x1 |  -.2154672   .0111955   -19.25   0.000    -.2380941   -.1928402
          x2 |  -1.007953   .0383704   -26.27   0.000    -1.085502   -.9304036
       _cons |   55.55142   .9614761    57.78   0.000      53.6082    57.49463
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -170.5727
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     69.8760
- Akaike Information Criterion              (1973) Log AIC =      4.2467
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     78.4568
- Schwarz Criterion                         (1978) Log SC  =      4.3625
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     80.3697
- Hannan-Quinn Criterion                    (1979) HQ      =     73.0151
- Rice Criterion                            (1984) Rice    =     70.4493
- Shibata Criterion                         (1981) Shibata =     69.3930
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     70.1496
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|         x1 |            -0.2155 |            -0.2358 |            38.4362 |
|         x2 |            -1.0080 |            -0.4125 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:XTREGMLE Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGMLE: "Trevor Breusch MLE Random-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457458.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457458.htm"}
