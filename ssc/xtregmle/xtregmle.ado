...
program define xtregmle, eclass 
version 11.0
...
tempvar EE Eo Ev Ew P Q Sig2 SSE SST U U2 Ue wald weit Wi Wio WS Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yho Yho2 Yt YY YYm YYv Yh_ML Ue_ML Z
tempvar absE Bw D DE DF DF1 DW E E2 E3 E4 ht LDE LE LEo SRho X X0
tempname A B b B1 b1 B12 B1b B1t b2 Beta Bm BOLS BOLS1 Bt Bv Bv1 Bx Cov 
tempname D den DF dfab Dim DVE DVNE Dx E E1 EE1 Eg Eo eVec Q Qr
tempname Ew F f1 f13 f13d gam gam2 Go GoRY HQ IDRmk In IPhi J K kb 
tempname kbm Ko Koi Kr kx L lf llf Ls M N NC NE Nn NT Omega P Phi Pm
tempname rid Rmk RX RY s sd Sig2 Sig21 Sig2b Sig2n Sig2o
...
tempname Xx Y Yh Yh_ML YYm YYv Z Z0 Z1 Zo Zr Zz Om Om1 Om2 CovC
...
di 
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Trevor Breusch MLE Random-Effects Panel Data: Ridge and Weighted Regression}}"
di _dup(78) "{bf:{err:=}}"
tempname BKo BKi Bo JNT PHM1 PHM2 VQo E 
...
matrix `Bo'=`B'
tempvar BKo BKi
tempname BsKo PHM VQo VPo BsKi BsKoi
qui svmat `Bo' , name(`BKo')
qui summ `BKo' if `touse' 
scalar `BsKo'=r(sum)
matrix `E'=(`Y'-`X'*`B')
matrix `PHM1'=(`E''*`Q'*`E')
...
matrix `BKi'=`B'
qui svmat `BKi' , name(`BKi')
qui summ `BKi' if `touse' 
scalar `BsKi'=r(sum)
...
if "`lmhet'"!= "" {
tempvar Yh Yh2 E E2 E3 E4 absE time LE ht U2
