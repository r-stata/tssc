...
program define lmavon2 , eclass byable(onecall)
version 11.2
...
tempvar wald SSE SST weit EE DW WS LE DE LEo DFF DFF1 DFF2 xq Yh
tempvar LDE DF1 Eo Lambda0 Yh2 YYm LE XQ ht Hat Yhat1 Yho Yh_ML Ue_ML Wio Wi
...
tempname SSE Rho BB SBBs lmadw
local N=`Nn'
qui tsset `Time'
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Autocorrelation Von Neumann Ratio Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
