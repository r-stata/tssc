...
program define xtregfem, eclass 
version 11.0
...
tempvar EE Eo Ev Ew LYh2 P Q U U2 Ue wald weit Wi Wio WS Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yho Yho2 Yt YY YYm YYv Yh_ML Ue_ML Z
tempvar absE Bw D DE DF DF1 DW E E2 E3 E4 ht LDE LE LEo LnE2 LYh2 X X0 Nn NT CovC
tempname A B b Beta Bm BOLS BOLS1 Bx Cov D den DF Dim Dx E E1 EE1 Eg Eo Eom
tempname Ew F f1 f13 f13d gam gam2 Go GoRY HQ IDRmk In IPhi J K kb
tempname Ko Koi Kr kx L lf llf Ls M N NC Nmiss eVec Sig2n
tempname Om Om1 Om2 Omega Omm Omm1 Omm2 P Phi Pm q Q q1 q2 Qr  
tempname rid Rmk RX RY s sd Sig2 Sig21 Sig2b Sig2o
tempname Sig2o1 Sig2u Sig2w sigox SLS sqN SSE SSEo SST1 SST2 Sw Ue Ue_ML 
tempname v2 VaL VaL1 VaL21 VaLv1 Vec vh VM VN VP VQ Vs W Wald waldm We Wi 
tempname v V v1 V1 V1s Wi1 Wio WMTD WS WW WY X X0 XB Xfe Xo xq
tempname Xx Y Yfe Yh Yh_ML YYm YYv Z Z0 Z1 Zo Zr Zz
...
di 
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Fixed-Effects Panel Data: Ridge and Weighted Regression}}"
di _dup(78) "{bf:{err:=}}"
qui sort `id'
preserve
...
tempvar ys 
qui by `id': gen double `ys' = sum(`var')/_N if `touse'
qui summ `var' if `touse' 
qui by `id': replace `ys' = `var' - `ys'[_N] + r(mean) if `touse'
qui by `id': replace `var' =`ys' if `touse'
 }
tokenize `vlist'
local yvarfe "`1'"
macro shift 1
local xvarfe "`*'"
qui mkmat `yvarfe' if `touse' , matrix(`Yfe')
if "`noconstant'"!="" {
qui mkmat `xvarfe' if `touse' , matrix(`Xfe')
 }
 else { 
qui mkmat `xvarfe' `X0' if `touse' , matrix(`Xfe')
 }
if "`weights'"!="" & "`wvar'"!="" {
tempvar Wis ws
tempname Wis
qui by `id': gen double `ws' = sum(`Wi')/_N if `touse' 
qui summ `Wi' if `touse' 
qui by `id': gen `Wis' = `Wi' - `ws'[_N] + r(mean) if `touse' 
...
matrix Beta=invsym(`Xfe''*`Omega'*`Xfe'+`Zz')*`Xfe''*`Omega'*`Yfe'
matrix `E'=`Q'*(`Yfe'-`Xfe'*Beta)
matrix Sig2=`E''*`E'/(`N'-`kx'-`NC')
matrix E=`Wi'*(`Y'-`X'*Beta)
restore 
if "`ridge'"!="" & `kr' > 0 {
matrix `Xx'=`Xfe''*`Omega'*`Xfe'
matrix `Cov'=Sig2*invsym(`Xx'+`Zz')*`Xx'*invsym(`Xx'+`Zz')
 } 
 else { 
matrix `Cov'=Sig2*invsym(`Xfe''*`Xfe')
 }
matrix `Beta'=Beta
