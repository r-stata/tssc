{smcl}
{hline}
{cmd:help: {helpb xtregfem}}{space 50} {cmd:dialog:} {bf:{dialog xtregfem}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregfem: Fixed-Effects Panel Data: Ridge and Weighted Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregfem##01:Syntax}{p_end}
{p 5}{helpb xtregfem##02:Description}{p_end}
{p 5}{helpb xtregfem##03:Options}{p_end}
{p 5}{helpb xtregfem##04:Ridge Options}{p_end}
{p 5}{helpb xtregfem##05:Weight Options}{p_end}
{p 5}{helpb xtregfem##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregfem##07:Other Options}{p_end}
{p 5}{helpb xtregfem##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregfem##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregfem##10:Saved Results}{p_end}
{p 5}{helpb xtregfem##11:References}{p_end}

{p 1}*** {helpb xtregfem##12:Examples}{p_end}

{p 5}{helpb xtregfem##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregfem} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 2 2 2} {cmd:xtregfem} estimates Fixed-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregfem} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregfem} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregfem} can use large types of weighted regression options.
...
{col 20}{cmd:xtregfem} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregfem} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregfem} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregfem.dta, clear}

  {stata db xtregfem}

  {stata xtset id t}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregfem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregfem.dta, clear
. xtregfem y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh
...
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     69.0162
- Akaike Information Criterion              (1973) Log AIC =      4.2343
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     77.4914
- Schwarz Criterion                         (1978) Log SC  =      4.3502
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     79.3808
- Hannan-Quinn Criterion                    (1979) HQ      =     72.1167
- Rice Criterion                            (1984) Rice    =     69.5825
- Shibata Criterion                         (1981) Shibata =     68.5392
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     69.2865
------------------------------------------------------------------------------
...
{bf:{err:{dlgtab:XTREGFEM Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGFEM: "Fixed-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457457.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457457.htm"}
