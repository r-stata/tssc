...
*******************************************************************************
*type logitfe.ado
*! logitfe v3.0.3 mgonza 24Feb2017

*capture program drop logitfe
program logitfe, eclass byable(recall) sortpreserve
	version 11.2, missing
	local version : di "version " string(_caller()) ", missing:"
	local logitfe_cmd "logitfe"
	local cmdline "`logitfe_cmd' `*'"
...
*Validate (binary) depvar
	...
	if r(r) != 2 {
		display as error "Error: `depvar' is not a binary variable"
		exit 198
	}
...
*Check for collinearities
	...
	`rmcoll' `varlist' if `touse', `fvexp' logit touse(`touse')
	tempvar tousesample
			...
			if `febias' == 1 & `tebias' == 0 {
				...
				if `multiple' == 0 | (`multiple' != 0 & "`individuals'" != "" & "`time'" == "") {
					...
					foreach v of local varlist2 {
						...
						qui `rmcoll' `varlist' if `v', `fvexp' logit touse(`v')
		...
		if "`ss2'" != "" | ("`ss1'" == "" & "`ss2'" == "" & "`js'" == "" & "`sj'" == "" & "`jj'" == "" & "`double'" == "") {
			...
			if `febias' == 1 & `tebias' == 0 {
				...
				if `multiple' == 0 | (`multiple' != 0 & "`individuals'" != "" & "`time'" == "") {
					...
					mat `betas'		= J(2, `k', .)
					mat `betasmfx'	= J(2, `k', .)
					tempvar touse1 touse2 groppvar grouptvar
					qui gen byte `touse1' = `tousesample'
					qui gen byte `touse2' = `tousesample'
					qui egen `grouppvar' = group(`pvar') if `tousesample'
					qui egen `grouptvar' = group(`tvar') if `tousesample'
...
********************************************************************************
	...
	qui mata: logitconstantonly(	"`depvar'",		/*
			*/						"`touse'"	)
	scalar `ll_0'	= r(ll_0)
...
void logitconstantonly(	string	scalar yvar,
						string	scalar touse	)
{
external Y, X
...
S 		= optimize_init()
optimize_init_evaluator(S, &llnlogit())
...
for (i=1; i<=k; i++) {
	...
	if ((min(x2p[.,i]) == 0 & max(x2p[.,i]) == 1) & rows(uniqrows(x2p[.,i])) == 2) {
		...
		X1[., i]	= J(rows(X1), 1, 1)
		X0[., i]	= J(rows(X0), 1, 0)
		bmfx[i]		= sum(logcdf(X1 * beta') - logcdf(X0 * beta'))/n_orig
		index0		= index - beta[i] * X[., i]
		index1		= index + beta[i] * (1 :- X[., i])
		ates		= logcdf(index1) - logcdf(index0)
	}
	
	else {
		bmfx[i]		= beta[i] * sum(logpdf(X * beta'))/n_orig
		ates		= beta[i] * logpdf(index)
	}
	...
	if ((min(x2p[.,i]) == 0 & max(x2p[.,i]) == 1) & rows(uniqrows(x2p[.,i])) == 2) {
		...
		ate			= sum(x)/cols(x)
		temp[.,i]	= colsum(logpdf(index1) :* X1 - logpdf(index0) :* X0)'/n_orig
	}
		...
		select[i]	= 1
		if (beta[i] != 0) {
			temp[., i]	= -beta[i] * colsum(index :* logpdf(index) :* X)'/n_orig + ate * select / beta[i]
		}
		else {
			temp[., i]	= J(cols(X), 1, 0)
		}
...
void logit_no_sd(	string	scalar yvar,
					string	scalar Xvars,
					string	scalar pvar,
					string	scalar tvar,
					real	scalar n_orig,
...
if (febias == 0) {
	...
	B				= (((logcdf(index) :* ws :* resx)' * FE) :/ (ws' * FE))' :/ TNp

	if (lags>0) {

		for (i=1; i<=lags; i++) {
			B		= B - ((((lagspsiNT[.,i] :* lagstouse[.,i] :* ws :* resx)' * FE) :/ (ws' * FE))' :/ TNplags[.,i])
		}
...
if (tebias == 0) {
	...
	D				= mean((((logcdf(index) :* ws :* resx)' * TE) :/ (ws' * TE))')
}
...
index				= X * beta'
ws					= logpdf(index)
date				= logpdf(index) :* (1 :- 2* logcdf(index))
ddate				= date :* (1 :- 2 * logcdf(index)) - 2 * logpdf(index) :^2
XX					= quadcross(X1, ws, X1)
...
if (febias == 0) {
	...
	B				= (((-date :* pdate + ddate)' * FE) :/ (ws' * FE))' :/ TNp
...
if (tebias == 0) {
	...
	D				= (1/2) * mean((((-date :* pdate + ddate)' * TE) :/ (ws' * TE))')
}

bias				= febias*B + tebias*D/ng
bmfx				= b * (sum(logpdf(index))/n_orig - bias)
...
for (i = 1; i <= k; i ++) {
	...
	if ((min(x2p[.,i]) == 0 :& max(x2p[.,i]) == 1) :& rows(uniqrows(x2p[.,i])) == 2) {
		...
		date		= logpdf(index1) - logpdf(index0)
		ddate		= logpdf(index1) :* (1 :- 2 * logcdf(index1)) - logpdf(index0) :* (1 :- 2 * logcdf(index0))
		XX			= quadcross(X1, ws, X1)
		Xy			= quadcross(X1, ws, (date :/ ws))
		delta		= invsym(XX) * Xy
		pdate		= X1 * delta
		...
		if (febias == 0) {
			...
			B		= (((-logpdf(index) :* (1 :- 2 * logcdf(index)) :* pdate + ddate)' * FE) :/ (ws' * FE))' :/ TNp
		...
		if (tebias == 0) {
			...
			D		= (1/2) * mean((((-logpdf(index) :* (1 :- 2 * logcdf(index)) :* pdate + ddate)' * TE) :/ (ws' * TE))')
		}
		bias		= febias*B + tebias*D/ng
		bmfx[i]		= sum(logcdf(index1) - logcdf(index0))/n_orig - bias
	}
...
real matrix logcdf(real matrix X)
{
logcdf = exp(X) :/ (1 :+ exp(X))
return(logcdf)
}

real matrix logpdf(real matrix X)
{
logpdf = invlogit(X) :* (1 :- invlogit(X))
return(logpdf)
}
end
