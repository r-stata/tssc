...
{viewerjumpto "Syntax" "des_gs##syntax"}{...}
{viewerjumpto "Description" "des_gs##description"}{...}
{viewerjumpto "Options" "des_gs##options"}{...}
{viewerjumpto "Remarks" "des_gs##remarks"}{...}
{viewerjumpto "Examples" "des_gs##examples"}{...}
{title:Title}
{phang}
{bf:des_gs} {hline 2} Design a group sequential single-arm trial for a single binary endpoint
...
des_gs supports the determination of a variety of (optimised) group sequential
single-arm clinical trial designs for a single binary primary endpoint. For all
supported designs, the following hypotheses are tested for the response
probability pi
...
Note that when J <= 3, the optimal design is determined by an exhaustive search.
This means that vast speed improvements can be made by carefully choosing the
values of nmin and nmax. In contrast, if J > 3, simulated annealing is employed to
stochastically search for the optimal design, as proposed by Chen and Lee (2013).
