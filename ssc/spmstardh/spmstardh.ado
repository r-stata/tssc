...
program define spmstardh , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spmstardh" {
 error 301
...
syntax varlist [aw] , WMFile(str) [NWmat(int 1) coll NOCONStant MHET(str) ///
 INV INV2 vce(passthru) stand MFX(str) noLOG tobit ROBust aux(str) zero ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str) tolog LL(real 0)]
local sthlp spmstardh
...
tempname A B b Beta BetaSP Bx WS Cov D E F IPhi J K L olsin N DF kx kb
...
tempname Y Yh Yi YYm YYv Z llf Rostar rRo kb Sig21 Dim Nmiss
...
local initopt init(`olsin', copy)  
 ml model lf spmstardh1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstardh2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstardh3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstardh4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
