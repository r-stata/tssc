...
tempvar D DE DF1 E TimeN EE Eo Es Ev Ew ht idv itv LE
tempvar LEo P Q Sig2 SSE SST Time tm U U2 Ue wald weit Wi Wio WS X X0 Bo
tempvar Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z D DE DF DF1
tempvar E Ea Ea1 EE Eo Es Es1 LDE LE LEo P Q Sig2 SSE Yb Yh Yh2 Yhb Yt YY Z 
tempname A B b B1 b1 b2 BB2 Beta Bx Cov Cov1 D den DF Dim Dx E E1 EE1 Eg Eo 
tempname Ew F In IPhi J K kb kx llf N NC Nn NT P Phi
tempname Pm Q Rho S2y Sig2 Sig21 Sig2o Sig2o1 SSE SSEo SST1 SST2 Ue Ue_ML v2 vh
tempname V Wi1 Wio WS X X0 Y Yh Yh_ML YYm YYv W W1 W2 Wald waldm We Wi
qui xtset `id' `it'
...
tempname SSEo Sigo r2bu r2bu_a r2raw r2raw_a f fp wald waldp vy1
...
local NC2=r(imax)
qui tsset `Time'
tempname Rho Rhosq Rw1 Rw2 Po lmadh lmadhp lmahh lmahhp SSEu SSE SSE1
qui tsset `Time'
tempvar Ue_ML1
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Autocorrelation Dynamic Durbin h and Harvey LM Tests}}"
di _dup(78) "{bf:{err:=}}"
...
tempvar Ue_ML1
qui gen `Ue_ML1' =L1.`Ue_ML' if `touse' 
qui regress `Ue_ML' `Ue_ML1' if `touse' , noconst
scalar `Po'= _b[`Ue_ML1']
...
if `lmadh' ==. {
di as txt "- Durbin  h Panel Test cannot be computed"
 }
if `lmadh' != . {
di as txt "- Durbin  h Test (Lag DepVar)" _col(43) "=" as res %9.4f `lmadh' _col(55) as txt "P-Value > Z(0,1)" _col(73) as res %5.4f `lmadhp'
 } 
if `lmahh' > 0 {
di as txt "- Harvey LM Test (Lag DepVar)" _col(43) "=" as res %9.4f `lmahh' _col(55) as txt "P-Value > Chi2(1)" _col(73) as res %5.4f `lmahhp'
 }
di _dup(78) "-"
