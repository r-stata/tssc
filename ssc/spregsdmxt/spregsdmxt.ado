...
program define spregsdmxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsdmxt" {
 error 301
...
version 11.0
syntax varlist [aw] , nc(str) [WMFile(str) aux(str) LL(real 0) ///
 ...
 tobit diag RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spregsdmxt
...
tempname Xb A B b Beta BetaSP Bx Cov D den DVE DVNE Dx E E1 F h Hat IPhi J K L M
tempname P Phi Pm Q Sig2 Sig2o SSE In IRW IRWR kbm Sig2n rRo kb SST1 SST2
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname Y Yh YYm YYv Z Ue WS D WW eVec eigw Xo mh olsin N DF kx kb NC NT
tempname Sig21 dfab Dim wmat xAx xAx2 TRa2 wWw1 xBx Nmiss llf minEig maxEig
...
local initopt init(`olsin', copy) 
 ml model lf spregsdmxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Durbin Panel Normal Model (SDM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SDM1e"
 matrix `olsin'=`Bo',0
 local initopt init(`olsin', copy) 
 ml model lf spregsdmxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) `wgt' , `mlopts' `nolog' `diparm' `initopt' maximize ///
 title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Durbin Panel Exponential Model (SDM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SDM1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf spregsdmxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Durbin Panel Weibull Model (SDM)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
matrix `olsin'=`Bo',`olshet',0,`rmse'
 local initopt init(`olsin', copy) 
 ml model lf spregsdmxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial Durbin Panel Multiplicative Heteroscedasticity}}"
di _dup(78) "{bf:{err:=}}"
 }
