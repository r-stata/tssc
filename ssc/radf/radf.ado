...
tempvar en y trd yboot resid
tempname tstat res allres b
...
// BS logic
if  ("`bs'" != "" | `boot'>0) {
   local bootrepl = cond(`boot'==-1, 199, `boot') 
}

/*
//   di as res _n "Bootstrap replications = `bootrepl'"
}
else {
	local bootrepl = `boot'
}
...
mat `tstat' = __adfstat \ __pwystat \ __psystat
mat `res' = __adfcv[1,2..4] \ __sadfcv[1,2..4] \  __gsadfcv[1,2..4]
...
// BS logic
if ("`bs'" != "" | `boot'>0) {
	qui gen double `yboot' = .
	if `maxlag' == 0 {
	qui regress D.`y' if `touse' 
	}
	else {
	qui regress D.`y' L(1/`maxlag')D.`y' if `touse' 
	}
	matrix `b' = e(b)
	qui predict double `resid' if `touse', residuals
	local maxlag2 = `maxlag'+2
// call original radf routine (renamed to radf0)
	...
	di    as res "      from Vasilopoulos, Pavlidis, Spavound and Martínez-García (2020)"
	if (`wwid' != `wwidauto') {
		loc winwarn "Note: tabulated critical values based on auto window width = `wwidauto', not specified width = `wwid'"
		di as res _n "`winwarn'"
	}
}
// end BS logic
...
return scalar adfstat = `allres'[1,1]
return scalar sadfstat = `allres'[2,1]
return scalar gsadfstat = `allres'[3,1]
return matrix radfstats = `allres'
return scalar ntests = __tests
if (`bootrepl' > 0) {
	return scalar nreps = `bootrepl'
}
if (`seed' != -1) {
...
// -----------------------------------------------------------------------------
void bradf(string scalar yy,
		   string scalar yb,
		   string scalar dyy,
           string scalar bb,
		   real scalar maxlag, 
		   real scalar enobs,
		   real scalar bootrepl,
		   string scalar resid, 
		   string scalar touse,
		   string scalar ldyy,
		   string scalar cmd)
{
		...
		st_view(y=.,.,tokens(yy))
		st_view(yboot=.,.,tokens(yb))
		st_view(ldy=.,.,tokens(ldyy))
		st_view(dy=.,.,tokens(dyy))
		st_view(eps=.,.,tokens(resid),touse)
		b = st_matrix(bb)
		stat = J(bootrepl, 3, .)
/*
		maxlag
...
// lose maxlag+2 to define L(maxlag)D.y
		maxlag2 = maxlag + 2
		nindex = enobs - maxlag2 + 1
		index = J(nindex,1,.)
		e = J(nindex,1,.)
		for(i=1;i<=bootrepl;i++) {
			index = runiformint(nindex, 1, maxlag2, enobs)
			w = rnormal(nindex,1,0,1)
			e = eps[index] :* w
//		maxlag2, enobs
			if (maxlag == 0) {
				xb = e
				...
				xb = ldy[maxlag2..enobs,.] * b[1..maxlag]' + e					
			}
			yboot[.,.] = y
/*
		"yboot"
		...
		xb
*/
			for(j=maxlag2;j<=enobs;j++) yboot[j] = yboot[j-1] + xb[j-maxlag-1]
//		"yboot"
//		yboot[maxlag2..enobs]
			stata(cmd, 1)
			stat[i,.] = adfstat, pwystat, psystat
		}
//		stat
// mm_quantile from moremata
		p90 = mm_quantile(stat, 1, 0.90)
		p95 = mm_quantile(stat, 1, 0.95)
		p99 = mm_quantile(stat, 1, 0.99)
//		"90, 95, 99 percentiles of bootstrap distribution"
		bsres = (p90 \ p95 \ p99)'
//		bsres
		st_matrix("__bsres",bsres)
		st_local("tee",strofreal(nindex))
}
// -----------------------------------------------------------------------------
