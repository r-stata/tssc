...
program define lmadurm2 , eclass byable(onecall)
version 11.2
...
tempvar wald SSE SST weit EE Ea Ea1 DW WS LE DE LEo Yho2
tempvar Yt LDE DF1 Eo Lambda0 Yh2 LYh2 E3 E4 YYm Yh
tempvar LnE2 absE LE xq ht Hat Yhat1 Yho Yh_ML Ue_ML Wio Wi DFF DFF1 DFF2
...
tempname X X0 Vec J Ds SST1 SST2 Sig2o1 CovC SSE Sig2 lf F K B Bx Zo qhiv V
...
tempname SSE Rho BB lmadmk lmadmd
local N=`Nn'
qui tsset `Time'
tempvar Yh Yh2 U2 E E2 E3 E4 Es SBB SRho
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** 2SLS-IV Autocorrelation Dynamic Durbin m Test - Model= ({bf:{err:`model'}})}}"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf: Ho: No Autocorrelation - Ha: Autocorrelation}"
di _dup(78) "-"
if "`lags'"!="" {
forvalue i=1/`lags' {
di as txt "- Rho Value for Order(" `i' ")" _col(35) "AR(" `i' ")=" as res %8.4f `Rho'`i'
ereturn scalar rho`i'=`Rho'`i'
di as txt "- Durbin m Test (drop `i' obs)" _col(35) "AR(" `i' ")=" as res %8.4f `lmadmd'`i' _col(53) as txt "P-Value >Chi2(`i')" _col(70) as res %5.4f chi2tail(`i', abs(`lmadmd'`i'))
ereturn scalar lmadmd`i'=`lmadmd'`i'
ereturn scalar lmadmdp`i'=chi2tail(`i', abs(`lmadmd'`i'))
di as txt "- Durbin m Test (keep `i' obs)" _col(35) "AR(" `i' ")=" as res %8.4f `lmadmk'`i' _col(53) as txt "P-Value >Chi2(`i')" _col(70) as res %5.4f chi2tail(`i', abs(`lmadmk'`i'))
