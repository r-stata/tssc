{smcl}
{hline}
{cmd:help: {helpb spregsacxt}}{space 50} {cmd:dialog:} {bf:{dialog spregsacxt}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:spregsacxt: Maximum Likelihood Estimation Spatial AutoCorrelation (SAC) Panel Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb spregsacxt##01:Syntax}{p_end}
{p 5}{helpb spregsacxt##02:Description}{p_end}
{p 5}{helpb spregsacxt##03:Options}{p_end}
{p 5}{helpb spregsacxt##04:Spatial Panel Aautocorrelation Tests}{p_end}
{p 5}{helpb spregsacxt##05:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb spregsacxt##06:Heteroscedasticity Tests}{p_end}
{p 5}{helpb spregsacxt##07:Non Normality Tests}{p_end}
{p 5}{helpb spregsacxt##08:Saved Results}{p_end}
{p 5}{helpb spregsacxt##09:References}{p_end}

{p 1}*** {helpb spregsacxt##10:Examples}{p_end}

{p 5}{helpb spregsacxt##11:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt spregsacxt} {depvar} {indepvars} {weight} , {bf:{err:nc(#)}} {p_end} 
{p 3 5 6} 
{err: [} {opt wmf:ile(weight_file)} {opt lmsp:ac} {opt lmh:et} {opt lmn:orm} {opt diag} {opt test:s}{p_end} 
{p 5 5 6} 
{opt stand inv inv2} {opt dist(norm|exp|weib)} {opt mfx(lin, log)} {opt spar(rho, lam)}{p_end} 
{p 5 5 6} 
{opt mhet(varlist)} {opt pred:ict(new_var)} {opt res:id(new_var)}{p_end} 
{p 5 5 6} 
{opt iter(#)} {opt tech(name)} {opt ll(real 0)} {opt coll zero tolog nolog} {opt l:evel(#)}{p_end} 
{p 5 5 6} 
...
{marker 02}{bf:{err:{dlgtab:Description}}}
 ...
 {cmd:spregsacxt} estimates Maximum Likelihood Estimation Spatial AutoCorrelation
 (SAC) Panel Regression

 {cmd:spregsacxt} can estimate the following models:
   1- Heteroscedastic Regression Models in disturbance term.
   2- Non Normal Regression Models in disturbance term.

 {cmd:spregsacxt} estimates Continuous and Truncated Dependent Variables models {cmd:tobit}.

{p 2 2 2} {cmd:spregsacxt} deals with data either continuous or truncated dependent variable. If {depvar} has missing values or lower limits,
...
{p 2 4 2}{cmd:spregsacxt} can generate:{p_end}
...
{p 2 4 2} {cmd:spregsacxt} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:spregsacxt} generates some variables names with prefix:
...
{col 20}{cmd:spregsacxt} will convert automatically spatial cross section weight matrix
...
{p 2 4 2 }Depending on the model estimated, {cmd:spregsacxt} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog spregsacxt}.
{bf:Note 4:} xtset is included automatically in {cmd:spregsacxt} models.
{hline}

{stata clear all}

{stata sysuse spregsacxt.dta, clear}

{bf:{err:* (1) MLE Spatial Panel SAC Normal Regression Model}}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test spar(rho)}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test spar(lam)}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(log) test tolog}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) predict(Yh) resid(Ue)}
{hline}

{bf:{err:* (2) MLE Spatial Panel SAC Exponential Regression Model}}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test dist(exp)}
{hline}

{bf:{err:* (3) MLE Spatial Panel SAC Weibull Regression Model}}
 {stata spregsacxt y x1 x2 , nc(7) wmfile(SPWxt) mfx(lin) test dist(weib)}
{hline}

{bf:{err:* (4) MLE Weighted Spatial Panel SAC Regression Model}}
 {stata spregsacxt y x1 x2  [weight = x1] , nc(7) wmfile(SPWxt) mfx(lin) test}
 {stata spregsacxt y x1 x2 [aweight = x1] , nc(7) wmfile(SPWxt) mfx(lin) test}
{hline}

{bf:{err:* (5) MLE Spatial Panel SAC Tobit} - Truncated Dependent Variable (ys)}
 {stata spregsacxt ys x1 x2, nc(7) wmfile(SPWxt) mfx(lin) test tobit ll(0)}
 {stata spregsacxt ys x1 x2, nc(7) wmfile(SPWxt) mfx(lin) test tobit ll(3)}
{hline}
...
Iteration 4:   log likelihood = -186.35717  
==============================================================================
* MLE Spatial AutoCorrelation (SAC) Panel Normal Model
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
  Sample Size       =          49   |   Cross Sections Number   =           7
  Wald Test         =    161.4574   |   P-Value > Chi2(2)       =      0.0000
  F-Test            =     80.7287   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.7783   |   Raw Moments R2          =      0.9597
 (Buse 1973) R2 Adj =      0.7339   |   Raw Moments R2 Adj      =      0.9516
  Root MSE (Sigma)  =      8.6309   |   Log Likelihood Function =   -186.3572
------------------------------------------------------------------------------
- R2h= 0.5182   R2h Adj= 0.4218  F-Test =   24.73 P-Value > F(2 , 40)  0.0000
- R2v= 0.6254   R2v Adj= 0.5505  F-Test =   38.41 P-Value > F(2 , 40)  0.0000
------------------------------------------------------------------------------
             |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
y            |
          x1 |  -.2765983   .1003566    -2.76   0.006    -.4732935    -.079903
          x2 |  -1.552946   .3164991    -4.91   0.000    -2.173273   -.9326194
       _cons |   69.73669   5.197367    13.42   0.000     59.55003    79.92334
-------------+----------------------------------------------------------------
         Rho |   -.094231   .0675474    -1.40   0.163    -.2266216    .0381595
      Lambda |   .0499036   .0319778     1.56   0.119    -.0127717    .1125788
       Sigma |   10.68121    1.10237     9.69   0.000     8.520606    12.84182
------------------------------------------------------------------------------
 LR Test (Rho=0):                      1.9461    P-Value > Chi2(1)   0.1630
 LR Test (Lambda=0):                   2.4354    P-Value > Chi2(1)   0.1186
 LR Test SAC vs. OLS (Rho+Lambda=0):   2.4985    P-Value > Chi2(2)   0.2867
...
==============================================================================
- Log Likelihood Function                   LLF            =   -186.3572
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     68.7311
- Akaike Information Criterion              (1973) Log AIC =      4.2302
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     77.1713
- Schwarz Criterion                         (1978) Log SC  =      4.3460
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     79.0529
- Hannan-Quinn Criterion                    (1979) HQ      =     71.8188
- Rice Criterion                            (1984) Rice    =     69.2950
- Shibata Criterion                         (1981) Shibata =     68.2560
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     69.0003
------------------------------------------------------------------------------
...
- GLOBAL Moran MI            =   0.0280     P-Value > Z( 0.429)   0.6682
- GLOBAL Geary GC            =   0.9289     P-Value > Z(-0.506)   0.6130
- GLOBAL Getis-Ords GO       =  -0.0799     P-Value > Z(-0.429)   0.6682
------------------------------------------------------------------------------
- Moran MI Error Test        =   0.2823     P-Value > Z(2.663)    0.7777
------------------------------------------------------------------------------
- LM Error (Burridge)        =   0.0365     P-Value > Chi2(1)     0.8485
- LM Error (Robust)          =   0.0723     P-Value > Chi2(1)     0.7880
------------------------------------------------------------------------------
  Ho: Spatial Lagged Dependent Variable has No Spatial AutoCorrelation
  Ha: Spatial Lagged Dependent Variable has    Spatial AutoCorrelation

- LM Lag (Anselin)           =   0.0096     P-Value > Chi2(1)     0.9217
- LM Lag (Robust)            =   0.0454     P-Value > Chi2(1)     0.8312
------------------------------------------------------------------------------
  Ho: No General Spatial AutoCorrelation
  Ha:    General Spatial AutoCorrelation

- LM SAC (LMErr+LMLag_R)     =   0.0819     P-Value > Chi2(2)     0.9599
- LM SAC (LMLag+LMErr_R)     =   0.0819     P-Value > Chi2(2)     0.9599
------------------------------------------------------------------------------
...
- Engle LM ARCH Test AR(1): E2 = E2_1   =   0.2189   P-Value > Chi2(1)  0.6399
------------------------------------------------------------------------------
- Hall-Pagan LM Test:   E2 = Yh         =   0.0098   P-Value > Chi2(1)  0.9212
- Hall-Pagan LM Test:   E2 = Yh2        =   0.0112   P-Value > Chi2(1)  0.9158
- Hall-Pagan LM Test:   E2 = LYh2       =   0.0858   P-Value > Chi2(1)  0.7695
------------------------------------------------------------------------------
- Harvey LM Test:    LogE2 = X          =   3.9686   P-Value > Chi2(2)  0.1375
- Wald Test:         LogE2 = X          =   9.7920   P-Value > Chi2(1)  0.0018
- Glejser LM Test:     |E| = X          =   6.3428   P-Value > Chi2(2)  0.0419
- Breusch-Godfrey Test:  E = E_1 X      =   8.2472   P-Value > Chi2(1)  0.0041
------------------------------------------------------------------------------
- Machado-Santos-Silva Test: Ev=Yh Yh2  =   0.0352   P-Value > Chi2(2)  0.9825
- Machado-Santos-Silva Test: Ev=X       =   5.2601   P-Value > Chi2(2)  0.0721
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X      =   7.3085   P-Value > Chi2(2)  0.0259
- White Test - B-P-G (SSR): E2 = X      =  10.2976   P-Value > Chi2(2)  0.0058
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2   =   8.6808   P-Value > Chi2(4)  0.0696
- White Test - B-P-G (SSR): E2 = X X2   =  12.2312   P-Value > Chi2(4)  0.0157
------------------------------------------------------------------------------
- White Test - Koenker(R2): E2 = X X2 XX=  22.4083   P-Value > Chi2(5)  0.0004
- White Test - B-P-G (SSR): E2 = X X2 XX=  31.5728   P-Value > Chi2(5)  0.0000
------------------------------------------------------------------------------
- Cook-Weisberg LM Test: E2/S2n = Yh    =   0.0138   P-Value > Chi2(1)  0.9065
- Cook-Weisberg LM Test: E2/S2n = X     =  10.2976   P-Value > Chi2(2)  0.0058
------------------------------------------------------------------------------
*** Single Variable Tests (E2/Sig2):
- Cook-Weisberg LM Test: x1                =   4.3516 P-Value > Chi2(1) 0.0370
- Cook-Weisberg LM Test: x2                =   1.1431 P-Value > Chi2(1) 0.2850
------------------------------------------------------------------------------
*** Single Variable Tests:
- King LM Test: x1                         =   0.5971 P-Value > Chi2(1) 0.4397
- King LM Test: x2                         =   1.6942 P-Value > Chi2(1) 0.1930
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
*** Non Normality Tests:
- Jarque-Bera LM Test                  =   2.6033     P-Value > Chi2(2) 0.2721
- White IM Test                        =  13.4374     P-Value > Chi2(2) 0.0012
- Doornik-Hansen LM Test               =   3.9631     P-Value > Chi2(2) 0.1379
- Geary LM Test                        =  -0.6552     P-Value > Chi2(2) 0.7207
- Anderson-Darling Z Test              =   0.4160     P > Z( 0.418)     0.6620
- D'Agostino-Pearson LM Test           =   3.6258     P-Value > Chi2(2) 0.1632
------------------------------------------------------------------------------
*** Skewness Tests:
- Srivastava LM Skewness Test          =   1.2373     P-Value > Chi2(1) 0.2660
- Small LM Skewness Test               =   1.4712     P-Value > Chi2(1) 0.2252
- Skewness Z Test                      =  -1.2129     P-Value > Chi2(1) 0.2252
------------------------------------------------------------------------------
*** Kurtosis Tests:
- Srivastava  Z Kurtosis Test          =   1.1688     P-Value > Z(0,1)  0.2425
- Small LM Kurtosis Test               =   2.1546     P-Value > Chi2(1) 0.1421
- Kurtosis Z Test                      =   1.4679     P-Value > Chi2(1) 0.1421
------------------------------------------------------------------------------
    Skewness Coefficient = -0.3892     - Standard Deviation =  0.3398
    Kurtosis Coefficient =  3.8180     - Standard Deviation =  0.6681
------------------------------------------------------------------------------
    Runs Test: (23) Runs -  (27) Positives - (22) Negatives
...
|--------------+------------+------------+------------+------------+------------|
|y             |            |            |            |            |            |
|           x1 |    -0.2766 |    -0.2702 |    -0.3487 |     0.0785 |    38.4362 |
|           x2 |    -1.5529 |    -1.5169 |    -1.9577 |     0.4408 |    14.3749 |
+-------------------------------------------------------------------------------+
...
|--------------+------------+------------+------------+------------+------------|
|           x1 |    -0.3026 |    -0.2956 |    -0.3815 |     0.0859 |    38.4362 |
|           x2 |    -0.6355 |    -0.6207 |    -0.8011 |     0.1804 |    14.3749 |
+-------------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:SPREGSACXT Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2013)}{p_end}
{p 1 10 1}{cmd:SPREGSACXT: "Stata Module to Estimate Maximum Likelihood Estimation Spatial AutoCorrelation (SAC) Panel Regression:}{p_end}
