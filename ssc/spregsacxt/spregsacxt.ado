...
program define spregsacxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsacxt" {
 error 301
...
version 11.0
syntax varlist [aw] , nc(str) [WMFile(str) diag ll(real 0) vce(passthru) ///
 INV dist(str) stand MFX(str) NOLog INV2 LMHet LMSPac spar(str) tolog tobit ///
 iter(int 100) zero coll level(passthru) PREDict(str) RESid(str) tech(str) TESTs DN LMNorm]
gettoken yvar xvar : varlist
local sthlp spregsacxt
...
di in gr _c " (spregsacxt Help):"
 exit
 }
 }
tempvar D DE DF1 DumE E e dcs XQX_ EE Hat ht idv itv LE LEo P Q Sig2 SSE
tempvar SST Time U U2 Ue wald weit Wi Wio WS X X0 eigw Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bm Bx Cov D den DVE DVNE Dx E E1 F SSE
tempname h Hat IPhi J K M P Phi Pm Q Sig2 Sig2o In IRW IRWR IRWL WXb SST1 SST2
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V wWw1
tempname Y Yh YYm YYv Z X0 N DF kx kb NC NT Nmiss llf xBx
tempname WS D WW eVec eigw Xo mh n IN xMx Sig21 Dim wmat xAx xAx2 TRa2
tempname Sig2n rRo rLm kb minEig maxEig waldm waldmp waldm_df waldr waldrp
tempname waldr_df waldl waldlp waldl_df waldj waldjp waldj_df
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
qui _rmcoll `vlistlog' , `coll' forcedrop
...
qui replace `var'=0 if `var'==.
 ...
 if "`coll'"=="" {
_rmcoll `varlist' , `coll' forcedrop
...
qui global spat_ARGS "`spat_ARGS' beta0 Rho Lambda Sigma"
 matrix spat_ols=`Bo',0,0,`rmse'
local initopt init(spat_ols, copy)  
 ml model lf spregsacxt1 `MODEL' `wgt' , ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial AutoCorrelation (SAC) Panel Normal Model}}"
di _dup(78) "{bf:{err:=}}"
 }
if inlist("`dist'", "exp") {
local MName "SAC1e"
...
qui global spat_ARGS "`spat_ARGS' beta0 Rho Lambda"
 matrix spat_ols=`Bo',0,0
local initopt init(spat_ols, copy) 
 ml model lf spregsacxt2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Lambda:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial AutoCorrelation (SAC) Panel Exponential Model}}"
di _dup(78) "{bf:{err:=}}"
 }
if inlist("`dist'", "weib") {
local MName "SAC1w"
...
qui global spat_ARGS "`spat_ARGS' beta0 Rho Lambda Sigma"
 matrix spat_ols=`Bo',0,0,`rmse'
local initopt init(spat_ols, copy)  
 ml model lf spregsacxt3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* MLE Spatial AutoCorrelation (SAC) Panel Weibull Model}}"
di _dup(78) "{bf:{err:=}}"
 }
...
scalar `kxt' = `kx'
qui xtset `idv' `itv'
qui xttobit `yvar' `SPXvar' , nolog ll(`llt') `coll'
qui predict `XBX' , xb
local k = `kxt'
...
qui tobit `yvar' `SPXvar' , nolog ll(`llt') `coll'
...
di as txt " LR Test (Rho=0):" _col(37) %9.4f as res e(waldr) as txt _col(50) "P-Value > Chi2(1)" as res _col(70) %5.4f e(waldrp)
di as txt " LR Test (Lambda=0):" _col(37) %9.4f as res e(waldl) as txt _col(50) "P-Value > Chi2(1)" as res _col(70) %5.4f e(waldlp)
di as txt " LR Test SAC vs. OLS (Rho+Lambda=0):" _col(37) %9.4f as res e(waldj) as txt _col(50) "P-Value > Chi2(2)" as res _col(70) %5.4f e(waldjp) 
di as txt " Acceptable Range for Rho:" _col(37) as res %9.4f e(minEig) " < Rho < " %5.4f e(maxEig)
 }
