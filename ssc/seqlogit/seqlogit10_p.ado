...
/* effect */	
	if "`type'" == "effect" {
		...
		forvalues i = 1/`e(k_eq)' {
			...
			if `"`e(over)'"' != "" {
				foreach var of varlist `e(over)' {
					qui replace  `lodds`i'' = ///
						`lodds`i'' + [#`i']_b[_`e(ofinterest)'_X_`var']*`var'
				}
			}
		}
		tempvar eff
...
program ParseOptions, sclass
	...
	local type `xb' `stdp' `pr' `trpr' `trvar' `tratrisk' `trgain' `trmweight' `trweight' `y' `scores' `score' `residuals' `effect'
