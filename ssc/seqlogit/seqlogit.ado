...
/*------------------------------------------------ playback request */
program seqlogit, eclass byable(onecall) sortpreserve
	version 11
	if replay() {
		if "`e(cmd)'" != "seqlogit" {
...
// start with a clean slate	
	...
	forvalues trans = 1/$S_Ntrans {
		local transvarsopt "`transvarsopt' x`trans'(varlist fv)"
...
// find the estimation sample	
	...
	if "`over'" != "" {
		foreach var of local over {
			...
			if _rc {
				local over2 "`over2' `var'"
			}
			else {
				local over2 "`over2' c.`var'"
			}
		}
		local over "`over2'"
		local inter `"c.`ofinterest'#(`over')"'
	}
...
// estimation	
	if "`sd'" == "" | "`sd'" == "0" {
		`log' ml model lf seqlogit_lf `mod' `wgt' if `touse',          ///
		...
		Parselikelihood
		`log' ml model lf seqlogit_uh_lf `mod'  `wgt' if `touse',          ///
			lf0(`r(df)' `r(ll_0)') maximize init(`init') search(off)  ///
			`robust' `clopt' `level' `mlopts' `stdopts' `modopts' `constraints'
...
program define Chcktree
	...
	if "`intree'" != "" {
		...
		di as error "Value`s' " as result "`intree'" as err " specified in the tree option"
		di as error "`are' not part of variable " as result "`varlist'."
		local ok = 0
	}
	local indata : list datalevels - treelevels
	if "`indata'" != "" {
		local k : word count `indata'
		local s = cond(`k' >= 2, "s", "")
		local are = cond(`k' >= 2, "are", "is")
		di as error "Value`s' " as result "`indata'" as err " of variable " as result "`varlist'" 
	...
	if "`intree'" != "" {
		...
		di as err "the following value`s' `are' missing: " as result "`intree'"
		exit 198
	}
	...
	if "`dups'" != "" {
		...
		local s = cond(`k' >= 2, "s", "")
		di as error "value`s' " as result "`dups'" as err " can be reached through multiple paths"
		local ok = 0
	}
		
	local leftover : list treelevels - final
	if "`leftover'" != "" {
		local k : word count `leftover'
		local s = cond(`k' >= 2, "s", "")
		di as error "no path in the tree leads to value`s' " as result "`leftover'" 
		local ok = 0
	}
	if !`ok' exit 198
...
program define Parsemodel, sclass
	forvalues i = 1/$S_Ntrans {
		local opts "`opts' x`i'(varlist fv)"
	}
	syntax varlist(fv)	, [`opts']
...
program define Starting, rclass
	...
	if $S_Nchoice1 == 2 {
		...
		scalar `ll0' = e(ll_0)
		if "`x'" != "" {
			Chk_dropped, b(`b') x("`x'") eq("`choice'v`ref'")
		}
	}
	else{
		di as txt "Transition " as result "1"
...
program define Chk_dropped
	syntax, b(name) x(string) eq(string)
	fvexpand `x'
	local x `r(varlist)'
	local coln : colnames `b'
	local missed : list x - coln
	local s = cond(`: word count `missed'' > 1, "s", "")
	if "`missed'" != "" {
		di as err "the following parameter`s' could not be estimated in transition `eq'"
		di as err "`missed'"
		exit 198
	}
end
