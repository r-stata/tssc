*! sptobitmstarxt V1.0 14/09/2013
*!
...
program define sptobitmstarxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstarxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) LL(str) WMFile(str) [vce(passthru) ///
 diag INV stand MFX(str) NOLog INV2 LMHet LMSPac ROBust iter(int 100) ///
 zero coll level(passthru) PREDict(str) RESid(str) tech(str) NWmat(int 1) ///
 tolog TESTs NOCONStant LMNorm dist(str)]
gettoken yvar xvar : varlist
local sthlp sptobitmstarxt
...
tempvar Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B Beta BetaSP Bx Cov Cov1 Cov2s CovC D den DVE DVNE Dx E E1 F
tempname h Hat idv itv IPhi J K M Sig2n Rostar rRo kb xBx
tempname P Phi Pm Q kbm IN xMx v V1 v1 V1s v2 In IRW IRWR WXb
tempname Sig2 Sig2o SSE SST1 SST2 Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio
tempname Y Yh Yi YYm YYv Z X0 WY X XB V WS D WW eVec eigw Xo mh olsin
tempname N DF kx kb NC NT Nmiss llf Sig21 Dim wmat xAx xAx2 TRa2 wWw1
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstarxt01 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Normal Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
if inlist("`dist'", "exp") {
local MName "MSTAR1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy)
 ml model lf sptobitmstarxt02 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) `wgt' , contin `mlopts' `nolog' `diparm' `initopt' maximize ///
 title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Exponential Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy) 
 ml model lf sptobitmstarxt03 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Weibull Model (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarxt04 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Normal Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "exp") {
local MName "MSTAR2e"
 matrix `olsin'=`Bo',0,0
local initopt init(`olsin', copy)
 ml model lf sptobitmstarxt05 (`yvar': `yvar' = `SPXvar' , `noconstant') ///
 (Rho1:) (Rho2:) `wgt' , contin `mlopts' `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Exponential Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR2w"
 matrix `olsin'=`Bo',0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarxt06 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Weibull Model (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstarxt07 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin ///
 `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Normal Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "exp") {
local MName "MSTAR3e"
 matrix `olsin'=`Bo',0,0,0
local initopt init(`olsin', copy)
 ml model lf sptobitmstarxt08 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) `wgt' , contin `mlopts' `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Exponential Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR3w"
 matrix `olsin'=`Bo',0,0,0,`rmse'
local initopt init(`olsin', copy)
 ml model lf sptobitmstarxt09 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Weibull Model (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy) 
 ml model lf sptobitmstarxt10 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Normal Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "exp") {
local MName "MSTAR4e"
 matrix `olsin'=`Bo',0,0,0,0
local initopt init(`olsin', copy)
 ml model lf sptobitmstarxt11 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) `wgt' , contin `mlopts' `nolog' `diparm' ///
 `initopt' maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Exponential Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
 if inlist("`dist'", "weib") {
local MName "MSTAR4w"
 matrix `olsin'=`Bo',0,0,0,0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarxt12 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho1:) (Rho2:) (Rho3:) (Rho4:) (Sigma:) `wgt' , `mlopts' contin `nolog' ///
 `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho1:_cons Rho2:_cons Rho3:_cons Rho4:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Panel Weibull Model (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
 }
matrix `BetaSP'=e(b)
...
local PARM1 "Rho1"
di as txt " Wald Test [`PARM1'=0]:" _col(37) %9.4f as res e(waldr) as txt _col(52) "P-Value > Chi2(1)" as res _col(70) %5.4f e(waldrp)
...
di as txt " Wald Test [`PARM2'=0]:" _col(37) %9.4f as res e(waldr) as txt _col(52) "P-Value > Chi2(2)" as res _col(70) %5.4f e(waldrp)
di as txt " Acceptable Range for Rho1: " as res %6.4f minEig1 " < Rho1 < " %6.4f maxEig1 
...
di as txt " Wald Test [`PARM3'=0]:" _col(37) %9.4f as res e(waldr) as txt _col(52) "P-Value > Chi2(3)" as res _col(70) %5.4f e(waldrp)
di as txt " Acceptable Range for Rho1: " as res %6.4f minEig1 " < Rho1 < " %6.4f maxEig1 
di as txt " Acceptable Range for Rho2: " as res %6.4f minEig2 " < Rho2 < " %6.4f maxEig2 
...
di as txt " Wald Test [`PARM4'=0]:" _col(37) %9.4f as res e(waldr) as txt _col(52) "P-Value > Chi2(4)" as res _col(70) %5.4f e(waldrp)
di as txt " Acceptable Range for Rho1: " as res %6.4f minEig1 " < Rho1 < " %6.4f maxEig1 
di as txt " Acceptable Range for Rho2: " as res %6.4f minEig2 " < Rho2 < " %6.4f maxEig2 
di as txt " Acceptable Range for Rho3: " as res %6.4f minEig3 " < Rho3 < " %6.4f maxEig3
