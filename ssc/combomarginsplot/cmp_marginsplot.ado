*! 0.1.1 Nicholas Winter 13mar2014
*! based on official -marginsplot- version 1.0.3  15jun2012
program cmp_marginsplot
	version 11
	tempname results
...
program _marginsplot
	...
	// NJGW default behavior if -using- not specified
	if mi(`"`using'"') {
		tempfile filenm					// Save margins file
		capture _marg_save , saving(`filenm') `eclass' `level' `mcompare'
		if _rc == 301 {
	...
	// NJGW if -using- is specified, just load it...
	else {
		local filenm `using'
	}
...
//di `"MakeDim  `div'var:``dim'var' `dim'lab:``dim'lab' `dim'contx:`dim'contx : `dim' `"`macval(`dim'sep)'"' ``dim'simple' ``dim'nolabels' `"`macval(`dim'labs)'"' `"`macval(`dim'elabs)'"' : ``dim'list'"'
//di " ... `dim'list: ``dim'list'"
