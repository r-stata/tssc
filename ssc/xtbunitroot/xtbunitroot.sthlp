...
If no option is specified, the default will be the model with a single break in the mean, at an unknown date: {cmd:xtbunitroot} {varname}{cmd:,} {cmdab:unk:nown}(1 100).{p_end}
...
{pstd} 
The errors can be nonnormal, cross-sectionally dependent and cross-sectionally heteroskedastic. The tests have power against both homogeneous and heterogeneous alternatives.
...
{p_end}
{phang}
{opt no:bootstrap} stops the command from running the bootstrap. The bootstrap is necessary for calculating critical values and p-values for the test when the dates of the breaks are unknown. 
...
The minZ statistic can then be compared to the approximate critical values which appear in Table 1 of Karavias and Tzavalis (2014). 
...
{synopt:{cmd:r(obs)}}number of total observations{p_end}
{synopt:{cmd:r(breaks)}}number of breaks{p_end}
{synopt:{cmd:r(break1)}}time of first break{p_end}
{synopt:{cmd:r(break2)}}time of second break{p_end}
