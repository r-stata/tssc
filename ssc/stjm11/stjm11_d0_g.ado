...
*quietly{
	...
	/* Mata program to evaluate joint likelihood */
		if ("$rescale"=="0"){
			mata stjm_g_mata_init$quick(st_global("surv_indlab"),y_ij,st_numscalar("`gamma'"),	
		...
		mata stjm_g_mata$quick("`finallnf2'",st_global("surv_indlab"),y_ij,st_numscalar("`gamma'"),
							st_numscalar("`s_e'"),N,Nmeas,nres,st_numscalar("`n_assoc_time'"),
							st_global("touse"),intercept,Z_dm,X_dm_surv,Z_dm_surv,
							diff_X_dm_surv,diff_Z_dm_surv,nodesfinal,weightsfinal,knewnodes,
							kweights,jlnodes,stime,d,betas_fixed,alpha_mat,
	...
	/* Loop over patients */
		...
		for (i=1; i<=rows(info); i++) {
			...
			/* Hazard function at each random effect quadrature points */
				...
				haz = (lambda_mat_i:*exp(gamma:*stime_i):*exp(assoc1)):^d_i
			...
			/* Cumulative hazard */
				...
				for (q=1;q<=nmeas[i,1];q++) {
					...
					//beta_mat = J(cols(nodesfinal),1,betas_fixed_i[q,.])
					//beta_mat[.,rand_ind_gk] = beta_mat[.,rand_ind_gk] :+ nodes_i'				/* adds GH nodes_i to appropriate betas_fixed_i */
...
mata:
mata set matastrict off
			void stjm_g_mata(string scalar finallnf, 				//
								string scalar s_ind, 				//
								numeric matrix y_ij,	 			//
								numeric scalar gamma, 				//
								real scalar sdresidual, 			//
														...
														
{
	...
	for (i=1; i<=rows(info); i++) {
			...
			/* Hazard function at each random effect quadrature points */
				...
				haz = (lambda_mat[i,]:*exp(gamma:*stime[i,]):*exp(assoc1)):^d[i,]
...
mata:
mata set matastrict off
			void stjm_g_mata_quick(string scalar finallnf,	//
						string scalar s_ind, 				//
						numeric matrix y_ij,	 			//
						numeric scalar gamma, 				//
						real scalar sdresidual, 			//
