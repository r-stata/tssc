...
*quietly{
	...
	/* Mata program to evaluate joint likelihood */
		if ("$rescale"=="0"){
			mata stjm_w_mata_init$quick(st_global("surv_indlab"),y_ij,st_numscalar("`gamma'"),	
		...
		mata stjm_w_mata$quick("`finallnf2'",st_global("surv_indlab"),y_ij,st_numscalar("`gamma'"),
							st_numscalar("`s_e'"),N,Nmeas,nres,st_numscalar("`n_assoc_time'"),
							st_global("touse"),intercept,Z_dm,X_dm_surv,Z_dm_surv,
							diff_X_dm_surv,diff_Z_dm_surv,nodesfinal,weightsfinal,knewnodes,
							kweights,jlnodes,stime,d,betas_fixed,alpha_mat,
							aghnodes,aghweights,gknodes,gknodes_deriv,rand_ind_gk,`gen_nodes',
							info,nmeas,ngk,lambda_mat,st_matrix("vcv"),firstit,adaptit)	
						...
						numeric matrix vcv)			
{
	...
	for (i=1; i<=N; i++) {
			...
			/* Hazard function at each random effect quadrature points */
				...
				haz = (lambda_mat_i:*gamma:*stime_i:^(gamma:-1):*exp(assoc1)):^d_i
...
mata:
mata set matastrict off
			void stjm_w_mata(string scalar finallnf, 				//
								string scalar s_ind, 				//
								numeric matrix y_ij,	 			//
								numeric scalar gamma, 				//
								real scalar sdresidual, 			//
														...
														
{
	...
	for (i=1; i<=rows(info); i++) {
			...
			/* Hazard function at each random effect quadrature points */
				...
				haz = (lambda_mat[i,]:*gamma:*stime[i,]:^(gamma:-1):*exp(assoc1)):^d[i,]
...
mata:
mata set matastrict off
			void stjm_w_mata_quick(string scalar finallnf,	//
						string scalar s_ind, 				//
						numeric matrix y_ij,	 			//
						numeric scalar gamma, 				//
						real scalar sdresidual, 			//
