...
program define gs2slsxt, eclass 
 version 11.0
syntax varlist , nc(str) [WMFile(str) ORDer(int 1) tolog diag ///
 INV vce(passthru) stand MFX(str) INV2 LMHet LMSPac BE FE RE zero ///
 coll level(passthru) PREDict(str) RESid(str) TESTs NOCONStant LMNorm]
...
tempvar D DE DF1 DumE E dcs XQX_ Ew Hat ht idv itv LE LEo P Q Sig2 SSE
tempvar SST Time tm U U2 Ue wald weit Wi Wio WS X X0 eigw Bo EE
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE DVNE Dx E E1
tempname h Hat hjm IPhi J K M Sig2o SSE SST1 SST2 F
tempname P Phi Pm q Q IN xMx Sig2 N DF kx kb NC NT Nmiss llf
tempname In WXb Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY
tempname Y Yh Yi YYm YYv Z X0 X XB V Sig2n kb
tempname WS D WW eVec eigw Xo Sig21 Dim wmat xAx xAx2 TRa2 wWw1 xBx
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
matrix `In'=I(`N')
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Generalized Spatial Panel Two Stage Least Squares (GS2SLS)}}"
di _dup(78) "{bf:{err:=}}"
qui xtset `idv' `itv'
