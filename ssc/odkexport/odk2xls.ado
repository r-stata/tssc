...
program odk2xls
	...
	syntax using/, to(string) [keep(string)] [DROPType(string)] [DROPVar(string)] [max(integer 30)] [fill] [DELete(string)] [clean] [mark(string)] [tfmt(string asis)] [qfmt(string asis)] [afmt(string asis)] [replace]
...
append using "`temp_vars'"
	...
	if !ustrregexm(`"`to'"',".xlsx") & !ustrregexm(`"`to'"',".xls") {
		local to `to'.xlsx
	}
export excel using "`to'", firstrow(varl) `replace'

	if `"`tfmt'"'!="" | `"`afmt'"'!="" | `"`qfmt'"'!="" {

		putexcel set "`to'", modify
		local c: word `c(k)' of `c(ALPHA)'
		if `"`tfmt'"'!="" {
			qui putexcel A1:`c'1, overwritefmt `tfmt'
		}
		if `"`afmt'"'!="" {
			qui putexcel (A2:`c'`=_N+1'), overwritefmt `afmt'
		}
		if `"`qfmt'"'!="" {
			local r = 2
			forv n = 1/`=_N' {
				if `question'[`n']!="" {
					local qlist `"`qlist' (A`r':`c'`r')"'
				}	
				local r = `r'+1
			}
			qui putexcel `qlist', overwritefmt `qfmt'
		}
	}
