...
version 11.0
syntax varlist , nc(str) [WMFile(str) diag LL(real 0) ///
 ...
 RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
...
tempvar Bw D DE DF1 E e dcs XQX_ EE Eo Es Ev Ew Hat ht idv itv LE LEo P Q
tempvar SST Time tm U U2 Ue weit Wi Wio WS X X0 eigw Bo SBB Sig2 SSE
tempvar Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B Beta BetaSP Bx Cov Cov1 Cov2s CovC D den DVE
tempname h Hat IPhi J K L M wmat xAx xAx2 WY llf Sig21 Dim
tempname P Phi Pm Q IN xMx v V1 v1 V1s v2 In WXb X XB V Nmiss NC NT
tempname Sig2 Sig2o SSE SST1 SST2 Vec vh VM VP VQ Vs W W1 W2 We Wi Wi1 Wio
tempname Y Yh Yi YYm YYv Z Z0 Z1 Zo X0 Sig2n Sig2o kb F DF kx kb
tempname WS D WW eVec eigw Xo mh n TRa2 wWw1 xBx DVNE Dx E E1 Eo N
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=_N/`NC'
...
qui spgmmxts `yvar' `SPXvar' , nc(`nc') gmm(`gmm') `noconstant' `vce' `robust' `tobit' 
scalar `llf'=e(llf)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
...
program define spgmmxts , eclass 
 version 11.0
syntax varlist , [nc(str) gmm(int 1) NOCONStant vce(passthru) ///
 ROBust tobit coll iter(int 100) tech(str) LL(real 0)]
tempvar `varlist'
gettoken yvar xvar : varlist
 if "`coll'"=="" {
...
qui cap matrix drop `MWT21' `MW1' `MW2' `MW3' `MW4'
 if "`tobit'"!="" {
qui xttobit `yvar' `xvar' , `noconstant' `vce' ///
 ll(`llt') iter(`iter') tech(`tech') `coll' 
...
qui tsset `Time'
qui regress `yvar' `xvar' , `noconstant' `vce' `robust'
tempvar res res2
...
qui xtset `idv' `itv'
qui xttobit `yvar' `xvar' , noconstant `vce' ll(`llt') ///
 iter(`iter') tech(`tech') `coll' 
 }
 else {
qui mkmat `xvar' `_Cons' , matrix(`X')
qui xtset `idv' `itv'
qui xttobit `yvar' `xvar' `_Cons' , noconstant ///
 `vce' ll(`llt') iter(`iter') tech(`tech') `coll' 
 }
matrix `Beta'=e(b)
matrix `Beta'=`Beta'[1,1..`kb']'
matrix `E'=`Y'-`X'*`Beta'
