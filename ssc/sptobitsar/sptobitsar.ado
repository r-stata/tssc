...
program define sptobitsar , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsar" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) LL(str) [vce(passthru) ///
 dist(str) stand MFX(str) NOLog LMHet LMSPac ROBust iter(int 100) ///
 zero level(passthru) coll PREDict(str) RESid(str) tech(str) ///
 INV INV2 tolog TESTs NOCONStant MHET(str) diag LMNorm]
gettoken yvar xvar : varlist
local sthlp sptobitsar
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ Hat ht LE LEo LYh2 P Q Sig2 SSE
tempvar SST Time U U2 wald weit Wi Wio WS X X0 Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY
tempname A B b Beta BetaSP Bx Cov D den DVE Dx E F Yh_ML Ue_ML Z YYm YYv
tempname IPhi J K M mh olsin Sig2n P Phi Pm Q In IRW IRWR kbm
tempname Xb Sig2 Sig2o SSE SST1 SST2 D WW eVec eigw Xo
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V
tempname Y Yh Yi YYm YYv Z llf rRo kb df Dim Sig21 kmhet N DF kx kb Nmiss kz
...
local initopt init(`olsin', copy)  
 ml model lf sptobitsar1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Normal Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SAR1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy)  
 ml model lf sptobitsar2 (`yvar': `yvar' = `SPXvar' , `noconstant') (Rho:) `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Exponential Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local MName "SAR1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy)  
 ml model lf sptobitsar3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
 local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Weibull Model (SAR)}}"
di _dup(78) "{bf:{err:=}}"
 }
...
local initopt init(`olsin', copy) 
 ml model lf sptobitsar4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Spatial Lag Multiplicative Heteroscedasticity}}"
di _dup(78) "{bf:{err:=}}"
 }
