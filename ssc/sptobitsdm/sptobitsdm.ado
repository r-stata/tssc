...
program define sptobitsdm , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitsdm" {
 error 301
...
version 11.0
syntax varlist [aw] , WMFile(str) LL(str) [aux(str) INV INV2 ///
 vce(passthru) dist(str) stand MFX(str) NOLog LMHet LMSPac ROBust ///
 LMNorm zero level(passthru) coll PREDict(str) RESid(str) tech(str) ///
 tolog TESTs NOCONStant MHET(str) diag iter(int 100)]
gettoken yvar xvar : varlist
local sthlp sptobitsdm
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ EE Hat ht LE LEo LYh2 P Q
tempvar SST Time U U2 wald weit Wi Wio WS X X0 Sig2 SSE
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE Dx E F
tempname IPhi J K M mh olsin Sig2n P Phi Pm Q In IRW IRWR kbm eigw Xo
tempname Sig2 Sig2o Sn SSE SST1 SST2 D WW eVec We Wi Wi1 Wio WY X XB V
tempname Vec vh VM VP VQ Vs W W1 W2 Wald Y Yh Yi YYm YYv Z X0 llf rRo kb 
tempname minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
tempname waldx waldxp waldx_df waldj waldjp waldj_df 
tempname N DF kx kb NC NT Nmiss kmhet kz Sig21 Dim
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
local initopt init(`olsin', copy) 
 ml model lf sptobitsdm1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "SDM1e"
 matrix `olsin'=`Bo',0
local initopt init(`olsin', copy)  
ml model lf sptobitsdm2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local MName "SDM1w"
 matrix `olsin'=`Bo',0,`rmse'
local initopt init(`olsin', copy)
 ml model lf sptobitsdm3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
 (Rho:) (Sigma:) `wgt' , `mlopts' contin `nolog' `diparm' `initopt' ///
 maximize title(`MName') search(on) `robust'
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
...
matrix `olsin'=`Bo',`olshet',0,`rmse'
 local initopt init(`olsin', copy) 
 ml model lf sptobitsdm4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
