...
program define lmhwaldxt, eclass 
version 11.0
...
tempvar D DE DF1 DumE E dcs TimeN Yb b A B Beta Bx Cov D den DF Dim Dx E E1
tempvar Hat ht idv itv LE LEo LYh2 P Q Sig2 SSE
tempvar SST Time tm U U2 Ue wald weit Wi Wio WS X X0 Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yho Yt YY YYm YYv Yh_ML Ue_ML Z
tempvar DF E2 EE Hat ht LDE LE LEo LnE2 LYh2 P Q Sig2 SSE
tempname F In IPhi J K kb kx llf lmhs M N NC NE Nn NT P Phi Pm Q Sig2 Sig21
tempname SSE SSEo SST1 SST2 Ue_ML Y Yh Yh_ML YYm YYv Sig2n Sig2o
tempname Vec Vs W W1 W2 Wald We Wi V Wi1 Wio WS WW WY X X0 XB Xg Xo xq
qui xtset `id' `it'
...
qui tsset `Time'
tempvar E2 LnE2 lmhwald lmhwaldp
qui tsset `Time'
qui gen `E2'=`Ue_ML'^2 if `touse'
qui gen `LnE2'=log(`E2') if `touse'
qui regress `LnE2' `xvar' if `touse' , `noconstant'
scalar `lmhwald'=e(mss)/2
scalar `lmhwaldp'=chi2tail(1, abs(`lmhwald'))
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Heteroscedasticity Wald Test}}"
di _dup(78) "{bf:{err:=}}"
