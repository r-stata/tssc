...
program define lmabxt, eclass 
version 11.0
syntax varlist [if] [in] , [id(str) it(str) coll NOCONStant dlag(int 1) cross]
...
tempvar D DE DF1 E TimeN EE Eo ht idv itv LE
tempvar LEo P Q Sig2 SSE SST Time U U2 Ue wald weit Wi Wio WS X X0 Bo
tempvar Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z D DE DF DF1 YY Z
tempvar DW E e E2 EE Eo LDE LE LEo P Q Sig2 SRho SSE X X0 Yb Yh Yh2 Yhb
tempname A B b Beta Bx Cov Cov1 D den DF Dim Dx E E1 Eo 
tempname Ew F In IPhi J K kb kx llf N NC Nn NT P Phi Yh_ML
tempname Pm Q Rho Sig2 Sig21 Sig2o Sig2o1 SSE SSEo YYm YYv
tempname SST1 SST2 Ue Ue_ML vh W W1 W2 Wald waldm We Wi Wi1 Wio WS X X0 Y Yh
qui xtset `id' `it'
...
local NC1=r(imin)
local NC2=r(imax)
tempname Rho lmabpp SSEu SSE SSE1 lmab1 lmab1p lmab2 lmab2p
qui tsset `Time'
tempvar Ue_ML1
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Autocorrelation Baltagi Test}}"
di _dup(78) "{bf:{err:=}}"
...
local idv `idv'
qui sort `idv' `ivt'
qui gen `EE'=`E'*`E' if `touse' 
qui gen `SSE'=sum(`EE') if `touse' 
scalar `SSEo'=`SSE'[_N]
qui gen `EE1'=`E'*`E'[_n-1] if `idv'==`idv'[_n-1]
qui gen `SSE1'=sum(`EE1') if `touse' 
...
di as txt "- Baltagi LM Test" _col(43) "=" as res %9.4f `lmab1' _col(55) as txt "P-Value > Chi2(1)" _col(73) as res %5.4f `lmab1p'
di as txt "- Baltagi  Z Test" _col(43) "=" as res %9.4f `lmab2' _col(55) as txt "P-Value > Z(0,1)" _col(73) as res %5.4f `lmab2p'
di _dup(78) "-"
