{smcl}
{hline}
{cmd:help: {helpb xtregbn}}{space 50} {cmd:dialog:} {bf:{dialog xtregbn}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:xtregbn: Balestra-Nerlove Random-Effects Panel Data: Ridge and Weighted Regression}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb xtregbn##01:Syntax}{p_end}
{p 5}{helpb xtregbn##02:Description}{p_end}
{p 5}{helpb xtregbn##03:Options}{p_end}
{p 5}{helpb xtregbn##04:Ridge Options}{p_end}
{p 5}{helpb xtregbn##05:Weight Options}{p_end}
{p 5}{helpb xtregbn##06:Weighted Variable Type Options}{p_end}
{p 5}{helpb xtregbn##07:Other Options}{p_end}
{p 5}{helpb xtregbn##08:Model Selection Diagnostic Criteria}{p_end}
{p 5}{helpb xtregbn##09:Heteroscedasticity Tests}{p_end}
{p 5}{helpb xtregbn##10:Saved Results}{p_end}
{p 5}{helpb xtregbn##11:References}{p_end}

{p 1}*** {helpb xtregbn##12:Examples}{p_end}

{p 5}{helpb xtregbn##13:Author}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt xtregbn} {depvar} {indepvars} {ifin} , {bf:{err:id(var)}} {bf:{err:it(var)}}{p_end} 
{p 3 5 6} 
...
{p 2 2 2} {cmd:xtregbn} estimates Balestra-Nerlove Random-Effects Panel Data with Ridge and Weighted Regression, and calculate Panel Heteroscedasticity,
...
{p 2 4 2}{cmd:xtregbn} estimates Ordinary Ridge regression as a multicollinearity remediation method.{p_end}
...
{col 10}{cmd:xtregbn} not like official Stata command {helpb xtreg} in weight option,
{col 10}{cmd:xtregbn} can use large types of weighted regression options.
...
{col 20}{cmd:xtregbn} not like official Stata command {helpb xtreg} in constant term option,
{col 20}{cmd:xtregbn} can exclude constant term.
...
{p 2 4 2 }{cmd:xtregbn} saves the following results in {cmd:e()}:
...
{marker 12}{bf:{err:{dlgtab:Examples}}}
  ...
  {stata sysuse xtregbn.dta, clear}

  {stata xtset id t}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(lin) predict(Yh) resid(Eu) diag lmh}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(log) predict(Yh) resid(Eu) diag lmh tolog}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(lin) ridge(orr) kr(0.5)}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1)}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(lin) ridge(grr2)}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(lin) ridge(grr3)}

  {stata xtregbn y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh}
{hline}

. clear all
. sysuse xtregbn.dta, clear
. xtregbn y x1 x2 , id(id) it(t) mfx(lin) ridge(grr1) weight(x) wvar(x1) diag lmh

==============================================================================
* Balestra-Nerlove Random-Effects Panel Data: Ridge and Weighted Regression
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  Wald Test         =   1728.7514   |   P-Value > Chi2(2)       =      0.0000
  F-Test            =    864.3757   |   P-Value > F(2 , 40)     =      0.0000
 (Buse 1973) R2     =      0.7482   |   Raw Moments R2          =      0.9535
 (Buse 1973) R2 Adj =      0.6978   |   Raw Moments R2 Adj      =      0.9442
  Root MSE (Sigma)  =      8.7085   |   Log Likelihood Function =   -170.6066
------------------------------------------------------------------------------
- R2h= 0.5508   R2h Adj= 0.4610  F-Test =   28.20 P-Value > F(2 , 40)  0.0000
- R2v= 0.2546   R2v Adj= 0.1056  F-Test =    7.86 P-Value > F(2 , 40)  0.0013
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
          x1 |  -.2123682   .0105998   -20.04   0.000    -.2337912   -.1909453
          x2 |  -1.011641   .0353039   -28.66   0.000    -1.082993   -.9402888
       _cons |   55.04046   .9846392    55.90   0.000     53.05043    57.03049
------------------------------------------------------------------------------

==============================================================================
* Panel Model Selection Diagnostic Criteria
==============================================================================
- Log Likelihood Function                   LLF            =   -170.6066
---------------------------------------------------------------------------
- Akaike Information Criterion              (1974) AIC     =     69.9728
- Akaike Information Criterion              (1973) Log AIC =      4.2481
---------------------------------------------------------------------------
- Schwarz Criterion                         (1978) SC      =     78.5654
- Schwarz Criterion                         (1978) Log SC  =      4.3639
---------------------------------------------------------------------------
- Amemiya Prediction Criterion              (1969) FPE     =     80.4810
- Hannan-Quinn Criterion                    (1979) HQ      =     73.1162
- Rice Criterion                            (1984) Rice    =     70.5469
- Shibata Criterion                         (1981) Shibata =     69.4891
- Craven-Wahba Generalized Cross Validation (1979) GCV     =     70.2468
------------------------------------------------------------------------------
...
|------------+--------------------+--------------------+--------------------|
|         x1 |            -0.2124 |            -0.2324 |            38.4362 |
|         x2 |            -1.0116 |            -0.4140 |            14.3749 |
+---------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:XTREGBN Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih (2012)}{p_end}
{p 1 10 1}{cmd:XTREGBN: "Balestra-Nerlove Random-Effects Panel Data: Ridge and Weighted Regression"}{p_end}

	{browse "http://ideas.repec.org/c/boc/bocode/s457455.html"}

	{browse "http://econpapers.repec.org/software/bocbocode/s457455.htm"}
