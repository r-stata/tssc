...
program define sptobitmstarh , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="sptobitmstarh" {
 error 301
 }
 Display `0'
 }
...
syntax varlist [aw] , WMFile(str) LL(str) [NWmat(int 1) coll NOCONStant ///
 MHET(str) INV INV2 vce(passthru) stand MFX(str) noLOG ROBust tolog ///
 iter(int 100) level(passthru) PREDict(str) RESid(str) tech(str)]
gettoken yvar xvar : varlist
local sthlp sptobitmstarh
...
tempname A B b Beta BetaSP Bx WS Cov D E F IPhi J K olsin N DF kx kb Nmiss kz llf 
tempname P Phi Pm Q Sig2 Sig2o SSE SST1 SST2 D WW eVec eigw Xo
tempname In IRW Vec vh W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname xq Xx Y Yh Yi YMB YYm YYv Z Rostar rRo kb Sig21 Dim
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarh1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di as txt "{bf:{err:* (m-STAR) Spatial Lag Multiplicative Heteroscedasticity (1 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarh2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Multiplicative Heteroscedasticity (2 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarh3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Multiplicative Heteroscedasticity (3 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
...
local initopt init(`olsin', copy)  
 ml model lf sptobitmstarh4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Tobit MLE Multiparametric Spatio Temporal AutoRegressive Regression}}"
di as txt "{bf:{err:* (m-STAR) Spatial Lag Multiplicative Heteroscedasticity (4 Weight Matrix)}}"
di _dup(78) "{bf:{err:=}}"
matrix `BetaSP'=e(b)
