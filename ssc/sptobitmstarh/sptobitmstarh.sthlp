{smcl}
{hline}
{cmd:help: {helpb sptobitmstarh}}{space 50} {cmd:dialog:} {bf:{dialog sptobitmstarh}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:sptobitmstarh: Tobit (m-STAR) Multiparametric Spatio Temporal AutoRegressive Regression}
{p 9 1 1}{bf:(Spatial Lag Multiplicative Heteroscedasticity Cross Sections Models)}{p_end}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb sptobitmstarh##01:Syntax}{p_end}
{p 5}{helpb sptobitmstarh##02:Description}{p_end}
{p 5}{helpb sptobitmstarh##03:Options}{p_end}
{p 5}{helpb sptobitmstarh##04:Saved Results}{p_end}
{p 5}{helpb sptobitmstarh##05:References}{p_end}

{p 1}*** {helpb sptobitmstarh##06:Examples}{p_end}

{p 5}{helpb sptobitmstarh##07:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt sptobitmstarh} {depvar} {indepvars} {weight} , {opt wmf:ile(weight_file)} {opt mhet(varlist)} {opt ll(#)}{p_end} 
{p 5 5 6} 
{err: [} {opt nw:mat(#)} {opt nocons:tant} {opt stand inv inv2} {opt mfx(lin, log)} {opt coll zero tolog nolog robust}{p_end} 
{p 5 5 6} 
...
{p 2 2 2} {cmd:sptobitmstarh} estimates Tobit Spatial econometric regression (mSTAR) "Multiparametric Spatio Temporal AutoRegressive Regression" for Spatial Lag Multiplicative Heteroscedasticity Cross Sections Models.{p_end}

{p 2 4 2}{cmd:sptobitmstarh} can generate:{p_end}
...
{p 2 4 2} {cmd:sptobitmstarh} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:sptobitmstarh} generates some variables names with prefix:
...
{p 2 2 2}{bf:sptobitmstarh} is used with more than Weight Matrix: (Border, Language, Currency, Trade...){p_end}
...
{p 2 4 2 }{cmd:sptobitmstarh} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog sptobitmstarh}.
{hline}

{stata clear all}

{stata sysuse sptobitmstarh.dta, clear}
...
{bf:* (1) *** m-STAR Model}
{stata sptobitmstarh ys x1 x2 , wmfile(SPWmcs1) nwmat(1) mfx(log) mhet(x1 x2) ll(0) tolog}
{stata sptobitmstarh ys x1 x2 , wmfile(SPWmcs1) nwmat(1) mfx(lin) mhet(x1 x2) ll(0)}
{stata sptobitmstarh ys x1 x2 , wmfile(SPWmcs2) nwmat(2) mfx(lin) mhet(x1 x2) ll(0)}
{stata sptobitmstarh ys x1 x2 , wmfile(SPWmcs3) nwmat(3) mfx(lin) mhet(x1 x2) ll(0)}
{stata sptobitmstarh ys x1 x2 , wmfile(SPWmcs4) nwmat(4) mfx(lin) mhet(x1 x2) ll(0)}
{hline}

{bf:* (2) Weighted mSTAR}
{stata sptobitmstarh ys x1 x2  [weight= x1], wmfile(SPWmcs1) nwmat(1) mfx(lin) mhet(x1 x2) ll(0)}
{stata sptobitmstarh ys x1 x2  [weight= x1], wmfile(SPWmcs2) nwmat(2) mfx(lin) mhet(x1 x2) ll(0)}
{stata sptobitmstarh ys x1 x2  [weight= x1], wmfile(SPWmcs3) nwmat(3) mfx(lin) mhet(x1 x2) ll(0)}
{stata sptobitmstarh ys x1 x2  [weight= x1], wmfile(SPWmcs4) nwmat(4) mfx(lin) mhet(x1 x2) ll(0)}
{hline}

. clear all
. sysuse sptobitmstarh.dta, clear
. sptobitmstarh y x1 x2 , wmfile(SPWmcs1) nwmat(1) mfx(lin) mhet(x1 x2) ll(0)
...
* (m-STAR) Spatial Lag Multiplicative Heteroscedasticity (1 Weight Matrix)
==============================================================================
  y = x1 + x2
------------------------------------------------------------------------------
  Sample Size       =          49
  Wald Test         =     50.6113   |   P-Value > Chi2(2)       =      0.0000
  F-Test            =     25.3056   |   P-Value > F(2 , 47)     =      0.0000
 (Buse 1973) R2     =      0.5239   |   Raw Moments R2          =      0.9134
 (Buse 1973) R2 Adj =      0.5137   |   Raw Moments R2 Adj      =      0.9116
  Root MSE (Sigma)  =     11.6677   |   Log Likelihood Function =   -182.6148
------------------------------------------------------------------------------
- R2h= 0.5327   R2h Adj= 0.5227  F-Test =   26.22 P-Value > F(2 , 47)  0.0000
- R2v= 0.4169   R2v Adj= 0.4044  F-Test =   16.44 P-Value > F(2 , 47)  0.0000
------------------------------------------------------------------------------
- Sum of Rho's =    0.0328516    Sum must be < 1 for Stability Condition
------------------------------------------------------------------------------
           y |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
y            |
          x1 |  -.1717661   .0897863    -1.91   0.056     -.347744    .0042119
          x2 |  -1.432416   .2907052    -4.93   0.000    -2.002188   -.8626442
       _cons |   56.00646   6.300343     8.89   0.000     43.65802    68.35491
-------------+----------------------------------------------------------------
Hetero       |
          x1 |   .0019573   .0058455     0.33   0.738    -.0094997    .0134142
          x2 |  -.0675846   .0239854    -2.82   0.005    -.1145952   -.0205741
-------------+----------------------------------------------------------------
        Rho1 |   .0328516   .0185752     1.77   0.077    -.0035551    .0692584
       Sigma |   24.56815    10.6427     2.31   0.021     3.708846    45.42746
------------------------------------------------------------------------------
 Wald Test [Rho1=0]:                 3.1279        P-Value > Chi2(1) 0.0770
...
|--------------+------------+------------+------------+------------+------------|
|y             |            |            |            |            |            |
|           x1 |    -0.1718 |    -0.1708 |    -0.1438 |    -0.0270 |    38.4362 |
|           x2 |    -1.4324 |    -1.4244 |    -1.1994 |    -0.2250 |    14.3749 |
+-------------------------------------------------------------------------------+
...
|--------------+------------+------------+------------+------------+------------|
|           x1 |    -0.1879 |    -0.1869 |    -0.1574 |    -0.0295 |    38.4362 |
|           x2 |    -0.5862 |    -0.5829 |    -0.4908 |    -0.0921 |    14.3749 |
+-------------------------------------------------------------------------------+
...
{bf:{err:{dlgtab:SPTOBITMSTARH Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2013)}{p_end}
{p 1 10 1}{cmd:SPTOBITMSTARH: "Tobit (m-STAR) Spatial Multiparametric Spatio Temporal AutoRegressive Regression: Spatial Lag Multiplicative Heteroscedasticity Cross Sections Models"}{p_end}
