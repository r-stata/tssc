*! version 1.0.4 08jun2012 Daniel Klein
...
pr labrec
	...
	[ , noCHange noLabel Add SEParate(str) noDELete ///
	...
	foreach lab of loc namelist {
			...
			foreach val of loc numl`j' {
				...
				if (`"`macval(txt)'"' != "") {
					...
					if (`"`macval(label`j')'"' == "") & !(`hasl`j'') {
						cap conf e `text`j''
						if _rc {
							mata : st_local("text`j'", st_local("txt"))
						}
						else mata : st_local("text`j'", ///
			...
			& ("`add'" != "") {
				...
				if ("`macval(oldlbl)'"' != "") {
					...
					if _rc {
						...
						mata : st_local("isin", strofreal(strpos( ///
						st_local("text`j'"), st_local("oldlbl"))))
						if !(`isin') {
							mata : st_local("text`j'", ///
							st_local("oldlbl") + ///
							st_local("separate") + ///
							st_local("text`j'"))
						}
		...
		forv j = 1/`nr' {
			cap conf e `text`j''
			if _rc continue
			mata : st_vlmodify("`lab'", `num2`j'', st_local("text`j'"))
		}
