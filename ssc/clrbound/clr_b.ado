capt program drop clr_b
program define clr_b, eclass
...
syntax [if] [in] [,noTEST NULL(real 0) LEVel(numlist) noAIS MINSmooth(integer 5) MAXSmooth(integer 20) noUNDERSmooth RND(integer 10000) SEed(integer 0) *]
...
quietly clr_s `varl' , lower lev("`con_level'") `ais' mins(`minsmooth') maxs(`maxsmooth') `undersmooth' rnd(`rnd') `options'
...
forval i = 1/`lnum_ineq' {
	tempname ltheta_ineq`i' lse_ineq`i' lais_ineq`i' lnf_x_ineq`i'
	
	local lgrid_ineq`i' `e(grid`i')'
	local lnf_x_ineq`i' `e(nf_x`i')'
...
quietly clr_s `varu' , lev("`con_level'") `ais' mins(`minsmooth') maxs(`maxsmooth') `undersmooth' rnd(`rnd') `options'
...
forval i = 1/`unum_ineq' {
	tempname utheta_ineq`i' use_ineq`i' uais_ineq`i' unf_x_ineq`i'
	
	local ugrid_ineq`i' `e(grid`i')'
	local unf_x_ineq`i' `e(nf_x`i')'
...
if "`test'" != "notest" {
	...
	quietly clrtest `test' , met("series") lev("`level'") `ais' mins(`minsmooth') maxs(`maxsmooth') `undersmooth' rnd(`rnd') `options'
...
ereturn local cmd = "clr_b"
ereturn local title  = "CLR Intersection Bounds (Series)"
...
ereturn matrix l_omega = `lomega'
ereturn matrix u_omega = `uomega'
...
forval i = 1/`lnum_ineq' {
	...
	ereturn scalar l_nf_x`i' = `lnf_x_ineq`i''
	...
	display as text "Numbers of Approximating Functions : " as result e(l_nf_x`i') 	
...
forval i = 1/`unum_ineq' {
	...
	ereturn scalar u_nf_x`i' = `unf_x_ineq`i''
	...
	display as text "Numbers of Approximating Functions : " as result e(u_nf_x`i') 	
...
if "`test'" != "notest" {
	...
	ereturn matrix t_omega = `test_omega'
	...
	forval i = 1/`sum_ineq' {
		...
		ereturn scalar t_nf_x`i' = `test_nf_x_ineq`i''
