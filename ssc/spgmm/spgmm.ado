...
program define spgmm , eclass 
 version 11.0
syntax varlist , WMFile(str) [LL(real 0) vce(passthru) coll ///
 stand MFX(str) tobit LMHet LMSPac LMNorm zero level(passthru) ///
 INV INV2 PREDict(str) RESid(str) tolog TESTs NOCONStant diag]
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ ht LE LEo LYh2 P Q Sig2 SSE
tempvar EE Hat SST Time U U2 wald weit Wi Wio WS X X0
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE Dx E E1 F J K M mh In Sig2 Sig2o
tempname Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V xq Y Yh Yi llf eVec eigw
tempname YYm YYv Z Z0 Z1 Zo X0 Ue sqN Sig2o1 kb kb1 kb2 Xo Vec vh IPhi D
tempname N DF kx kb Nmiss kz Sig21 Dim Sig2n P Phi Pm Q VM VP WW SSE SST1 SST2
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
ereturn scalar Nn=`N'
qui spgmms `yvar' `SPXvar' , `noconstant' ll(`llt') `robust' `tobit' `vce' 
scalar `llf'=e(llf)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
...
program define spgmms , eclass 
 version 11.0
 syntax varlist , [NOCONStant vce(passthru) ROBust tobit coll iter(int 100) ll(real 0)]
tempvar `varlist'
gettoken yvar xvar : varlist
qui {
preserve
...
tempname WYv WX0 XB Z Uh wmat varx vary X0 X B Beta Cov Sig2
tempname Omega Wi Xx Wi Omega Y varx var kb N DF
scalar `kb'=e(kb)
scalar `N'=e(Nn)
scalar `DF'=e(DF)
local llt=spat_llt
...
qui gen `X0'=1
 if "`tobit'"!="" {
qui tobit `yvar' `xvar' , `noconstant' `vce' ll(`llt')
tempvar yhat
qui predict `yhat' , xb
qui gen `E'=`yvar'-`yhat'
 }
else { 
qui regress `yvar' `xvar' , `noconstant' `vce'
 predict `E' , resid
 }
 gen `E2'=`E'^2 
 matrix `wmat'=WMB
