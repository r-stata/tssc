*! version 1.1 März 17, 2009 @ 10:23:49 UK
*! Generates Variable holding the frequency of respective sequence-type

* History
* version 1.0 Distributed on SSC
* version 1.1 All filenames embedded in comp. double quotes
...
version 9
	...
	quietly {
		...
		// Labels 
		if "`so'" == "" & "`ss'" == "" {
			label variable `h' "Frequency of sequence-pattern"
		}
		if "`so'" != "" & "`ss'" == "" {
			label variable `h' "Frequency of SO sequence-pattern"
		}
		if "`so'" == "" & "`ss'" != "" {
			label variable `h' "Frequency of SS sequence-pattern"
		}
