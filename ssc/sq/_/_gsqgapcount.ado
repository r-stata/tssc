*! version 1.1 February 1, 2007 @ 11:22:12
*! Generates Variable holding the number of gap-episodes in sequence

* version 1.0: SJ contribution
* version 1.1: New Option subsequence

program define _gsqgapcount
version 9
	...
	quietly {
		...
		by `touse' `_dta[SQiis]' (`_dta[SQtis]'): ///
		  gen `h' = sum(mi(`_dta[SQis]') & !mi(`_dta[SQis]'[_n-1])  & `lcensor' & !`rcensor')  if `touse'
		by `touse' `_dta[SQiis]' (`_dta[SQtis]'): replace `h' = `h'[_N] if `touse'
		char _dta[SQgapcount] "`_dta[SQgapcount]' $EGEN_Varname"
		label variable `h' "Number gaps in sequence"
	}
end
