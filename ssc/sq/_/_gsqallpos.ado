*! version 1.4 November 27, 2009 @ 15:07:02 UK
*! luniak@wzb.eu
* Returns position on which subsequence is first found in sequence

* 1.0: initial version
* 1.1: program didn't store varname in char. -> Fixed
* 1.2: Subprogram CheckConstant missing -> Fixed
* 1.3: Program didn't used "touse" -> Fixed
*      Option so allowed
* 1.4: Egenerated Vars starting with stups as sqvar can create a error -> fixed
...
program _gsqallpos
	...
	quietly {
		...
		// Mata
		mata: BMAll("`_dta[SQis]'","`vecpat'","`alpha'", "`_dta[SQiis]'")

		// Bring Back Mata-Results to Orignal Data
		keep `_dta[SQiis]' _SQBMAll
		sort `_dta[SQiis]'
		tempfile x
		save `x'
		...
		label variable `h' "Number of `pattern'"
		char _dta[SQallpos] "`_dta[SQallpos]' $EGEN_Varname"
