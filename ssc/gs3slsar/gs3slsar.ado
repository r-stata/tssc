...
program define gs3slsar , eclass 
 version 11.0
syntax varlist [aw] , WMFile(str) var2(str) [aux(str) INV INV2 ///
 vce(passthru) stand MFX(str) LMHet LMSPac iter(int 100) LMNorm ///
 ORDer(int 1) level(passthru) PREDict(str) RESid(str) tolog TESTs ///
 NOCONStant EQ(int 1) coll zero diag ols 2sls 3sls sure mvreg]
gettoken yvar xvar : varlist
local sthlp gs3slsar
...
di as err " {bf:gs3slsar y1 x1 x2 , wmfile(SPWcs) var2(y2 x3 x4)} eq(1)"
di as err " {bf:gs3slsar y1 x1 x2 , wmfile(SPWcs) var2(y2 x3 x4)} eq(2)"
di _dup(78) "{bf:{err:-}}"
exit
 }
...
tempvar absE Bw D DE DF DF1 DumE E XQX_ EE Eo Es Ev Ew Hat ht LE LEo
tempvar Sig2 SSE SST Time U U2 wald weit Wi Wio WS X X0 LYh2 P Q
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b B1 b1 b2 Beta Bm Bt Bv Bx IPhi J K Cov D den DVE Dx
tempname E E1 F M In P Phi Q mh Sig2 Sig2o SSE SST1 SST2 D WW eVec eigw Xo
tempname Vec vh VM VP Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V Sig2n
tempname Y Yh Yi YYm YYv Z kb Pm q df Dim Ko N DF kx kb NC NT Nmiss kz llf
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
di _dup(78) "-" 
 if "`var2'"!="" {
di as txt "{bf:** GS3SLSAR Variables - EQ.(2)}"
...
_rmcoll `varlist2' , `noconstant' `coll' forcedrop
...
gs3slss `yvar' `xvar1' `wgt' , `noconstant' `vce' aux(`aux') `coll' ///
...
di
 ...
 `wgt' , small `ols' `2sls' `3sls' `sure' `mvreg' `vce' exog(`xvar' `zvar') ///
...
reg3 (`yvar'_1 wy11_`yvar' wy21_`endg' `endg'_1 `xvar1' `aux' _cons1 , noconstant) ///
 ...
 `wgt' , small `ols' `2sls' `3sls' `sure' `mvreg' `vce' noconstant ///
