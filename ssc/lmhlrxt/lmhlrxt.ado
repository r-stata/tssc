...
tempvar Sig2 SigLR SigLRs E E2 EE1 En resid cN cT Obs Egh Time
qui gen `Time'=_n
local id `idv'
qui tab `id'
local N_g = r(r)
...
local hdf = `N_g'-1
local TM=`NT'/`N_g'
local lrh=`NT'*ln(`Sig2')- `SigLRs'
local lrhp= chiprob(`hdf', abs(`lrh'))
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Greene Likelihood Ratio Panel Heteroscedasticity Test}}"
di _dup(78) "{bf:{err:=}}"
di as txt _col(2) "{bf: Ho: Panel Homoscedasticity - Ha: Panel Heteroscedasticity}"
di
di as txt _col(5) "Likelihood Ratio LR Test" _col(35) " = " as res %10.5f `lrh'
di as txt _col(5) "Degrees of Freedom" _col(35) " = " as res %10.1f `hdf'
di as txt _col(5) "P-Value > Chi2(" `hdf' ")" _col(35) " = " as res %10.5f `lrhp'
di _dup(78) "{bf:{err:=}}"
qui `cmd'
return scalar lrh = `lrh'
return scalar lrhp= `lrhp'
return scalar lrhdf= `hdf'
end
