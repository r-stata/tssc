...
program define spregsac , eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spregsac" {
 error 301
...
version 11.0
syntax varlist [aw] , [WMFile(str) LL(real 0) INV INV2 ///
 vce(passthru) dist(str) stand MFX(str) NOLog LMHet LMSPac spar(str) ///
 iter(int 100) zero NOCONStant level(passthru) coll PREDict(str) ///
 RESid(str) tech(str) tolog TESTs LMNorm diag tobit]
gettoken yvar xvar : varlist
local sthlp spregsac
...
di in gr _c " (spregsac Help):"
 exit
 }
 }
tempvar absE Bw D DE DF DF1 DumE E XQX_ LYh2 P Q Sig2 SSE
tempvar EE Hat ht LE LEo SST Time U U2 wald weit Wi Wio
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z WS X X0
tempname Xb A B b B1 b1 b2 Beta BetaSP Bx Cov D den DVE Dx E F 
tempname IPhi J K M mh n Sig2n P Phi Pm Q In IRW IRWR IRWL kbm
tempname Sig2 Sig2o Sn SSE SST1 SST2 D WW eVec eigw Xo kbc2 kb2
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X X0 XB V
tempname Y Yh Yi YYm YYv Z Z0 Z1 Zo llf rRo rLm kbc1 kb1 N DF kx kb
tempname minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
tempname waldl waldlp waldl_df waldj waldjp waldj_df Nmiss kz Sig21 Dim 
di
local N=_N
scalar `Dim' = `N'
local MSize= `Dim'
...
local initopt init(spat_ols, copy)  
 ml model lf spregsac1 `MODEL' `wgt' , ///
...
local initopt init(spat_ols, copy) 
 ml model lf spregsac2 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
...
qui global spat_ARGS "`spat_ARGS' beta0 Rho Lambda Sigma"
 matrix spat_ols=`Bo',0,0,`rmse'
local initopt init(spat_ols, copy)  
 ml model lf spregsac3 `MODEL' `wgt' , ///
 `mlopts' contin `nolog' `diparm' `initopt' maximize search(on) title(`MName')
local COLNAME " `COLNAME'`yvar':_cons Rho:_cons Lambda:_cons Sigma:_cons"
di _dup(78) "{bf:{err:=}}"
