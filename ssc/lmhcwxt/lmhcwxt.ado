...
tempvar D DE DF1 DumE E dcs TimeN EE Eo Hat ht idv itv LE LEo P Q Sig2 SSE
tempvar SST Time wald weit Wi Wio WS X X0 Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yho Yt YY YYm YYv Yh_ML Ue_ML Z
tempvar E2 E3 E4 EE Eo Ev Ew Hat ht LDE LE LEo P Q Sig2 SSE
tempvar Yb Yh Yh2 Yhb Yho YY Z F In IPhi J K kb kx llf lmhs M M1
tempname b A B Beta Bx Cov D den DF Dim Dx E E1 eVec 
tempname N NC Nn NT P Phi Pm Q Sig2 Sig21 Sig2n Sig2o WW
tempname SSE SSEo SST1 SST2 Ue_ML Y Yh Yh_ML YYm YYv V V1 Wi1 Wio WS
tempname Vec Vs W W1 W2 Wald waldm We Wi WY X X0 XB Xg Xo xq XQX_ 
qui xtset `id' `it'
...
*qui tsset `Time'
* local N=`N'
tempvar Yh Yh2 E E2 ht U2
tempname mh vh h Q LMh_cwx lmhcw1 cwdf1 lmhcw1p lmhcw2 cwdf2 lmhcw2p h mh vh
qui tsset `Time'
qui gen `U2' =`Ue_ML'^2/`Sig2n' if `touse'
qui gen `Yh'=`Yh_ML' if `touse'
qui gen `Yh2'=`Yh_ML'^2 if `touse'
qui gen `E' =`Ue_ML' if `touse'
qui regress `U2' `Yh' if `touse' , `noconstant'
scalar `lmhcw1'= e(mss)/2
scalar `cwdf1' = e(df_m)
scalar `lmhcw1p'=chi2tail(`cwdf1', abs(`lmhcw1'))
qui regress `U2' `xvar' if `touse' , `noconstant'
scalar `lmhcw2'=e(mss)/2
scalar `cwdf2' =e(df_m)
scalar `lmhcw2p'=chi2tail(`cwdf2', abs(`lmhcw2'))
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:*** Panel Data Heteroscedasticity Cook-Weisberg Test}}"
di _dup(78) "{bf:{err:=}}"
