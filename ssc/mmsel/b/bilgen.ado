...
/*
	...
	while (`i' < `n') {;
		...
		qreg `y' $vars `vars_lambda' if `fulltime' == 1 , q(`hulp2');
	...
	use `hulp_file2', clear;

	if (`method' < 5){;
		quietly keep if `gender' == 1;
	};
	else {;
		quietly keep if `gender' == 0;
	};
	...
	{;
		tempfile hulp_file1;
		quietly save `hulp_file1', replace;
		local n1 = int(`n' / _N);
		local n2 = `n' - `n1' * _N;
		...
		while (`i' < `n1') {;
			local i = `i' + 1;
			use `hulp_file1', clear;
			quietly bsample _N;
			tempfile hulp_file_`i';
			quietly save `hulp_file_`i'', replace;			
		};
		...
		while (`i' < `n1') {;
			local i = `i' + 1;
			quietly append using `hulp_file_`i'';
		};	
	};
	};
	...
	while (`i' < `k') {;
		...
		local x = word("`vars3_hulp'",1);
		local vars3_hulp1 : list local(vars3_hulp) - local(x);
		local vars3_hulp "`vars3_hulp1'";
	...
	while (`i' < `n') {;
		local i = `i' + 1;
		mata: hulp = X[`i',.] * result[`i',.]';
		if (`i' == 1){;
			mata: result1 = hulp;
		};
		else {;
			mata: result1 = result1 \ hulp;
		};	   
	};
	...
	while (`q1' < 1 - (2 * `hulp_increment')) {;
		local q1 = `q1' + `hulp_increment';
		local i = `i' + 1;
		quietly qreg `hulp_var', q(`q1');
		matrix b = e(b);
		if (`i' == 1){;
			matrix result2 = `q1', b;
		};
		else {;
			matrix result2 = result2 \ (`q1', b);
		};		
	};
	...
	if (`method' > 2) {;
		...
		while (`i' < `n') {;
			local i = `i' + 1;
			mata: hulp = X1[`i',.] * result[`i',.]';
			if (`i' == 1){;
				mata: result1_full = hulp;
			};
			else {;
				mata: result1_full = result1_full \ hulp;
			};
		};
		...
		while (`i' < `n') {;
			local i = `i' + 1;
			mata: st_store(`i',1,result1_full[`i',1]);
		};
		...
		while (`q1' < 1 - (2 * `hulp_increment')) {;
			...
			matrix b = e(b);
			if (`i' == 1){;
				matrix result2_full = `q1', b;
			};
			else {;
				matrix result2_full = result2_full \ (`q1', b);
			};		
		};
		matrix result2_1_full = result2_full[1..rowsof(result2_full),1];
		matrix result2_2_full = result2_full[1..rowsof(result2_full),2];
	};
	...
	matrix result2_1 = result2[1..rowsof(result2),1];
	matrix result2_2 = result2[1..rowsof(result2),2];
	mata q =  st_matrix("result2_1");
	mata theta1 = st_matrix("result3");


	if (`variance' == 1) {;
		mata n = `n';
		mata theta = st_matrix("result2_2");
		matrix b1 = gamma[1,1];
		matrix b0 = gamma[1,colsof(gamma)];
		matrix gamma = gamma[1, 2..colsof(gamma)-1]';
		...
		mata beta = result_b3;
		mata y = st_data(.,"`y'");
		mata y1 = st_data(.,"`y'", "`y1'");
		mata yb = st_data(.,"`y'", "`gender'");
		mata cov_step1 = st_matrix("cov_step1");
		mata cov_step1 = cov_step1[2..cols(cov_step1)-1,2..cols(cov_step1)-1];
		...
		while (`i' < `k_totaal'-1) {;
			...
			if (`i' == 1) {;
				mata x1 = st_data(., "`x'", "`fulltime'");
				mata x1a = st_data(., "`x'", "`fulltime1'");
			};
			else{;
				mata x1 = x1, st_data(., "`x'", "`fulltime'");
				mata x1a = x1a, st_data(., "`x'", "`fulltime1'");
			};		
		};

		mata: hulp_mat1 = J(rows(x1),1,1);
		mata: hulp_mat2 = J(rows(x1a),1,1);	
		mata x1 = x1, hulp_mat1;
		mata x1a = x1a, hulp_mat2;

		local i = 0;

		local vars_hulp = "$vars";

		local k_hulp : list sizeof vars_hulp;

		while (`i' < `k_hulp') {;
			local i = `i' + 1;
			local x = word("`vars_hulp'",1);
			local vars_hulp1 : list local(vars_hulp) - local(x);
			local vars_hulp "`vars_hulp1'";
			
			if (`i' == 1) {;
				mata x2 = st_data(., "`x'");
				mata x2a = st_data(., "`x'", "`y1'");
			};
			else{;
				mata x2 = x2, st_data(., "`x'");
				mata x2a = x2a, st_data(., "`x'", "`y1'");
			};
		};

		
		mata: hulp_mat3 = J(rows(x2),1,1);
		mata: hulp_mat4 = J(rows(x2a),1,1);	
		mata x2 = x2, hulp_mat3;
		mata x2a = x2a, hulp_mat4;
		tempvar x;
		tempvar hulp;
		tempvar s;
		quietly g `x' = `z' - `c';
		quietly g `hulp' = `x' / (`ba' - `x');
		quietly g `s' = (1 - exp(-`hulp')) * ((`x' >= 0) * (`x' < `ba')) + (`x' >= `ba');
		
		mata s = st_data(., "`s'", "`fulltime1'");

		if ((`method' ==  2) | (`method' ==  3) | (`method' == 5)) {;
			mata st_matrix("theta1", theta1[.,2]);
			matrix dtheta = theta1 - result2_2;
		};
							
		mata sd = variance_machmat(q, gamma, b0, b1, y, x1, x2, `l', `c', `ba', cov_step1, `n',
						beta, hulp_u, result1, X, theta, y1, x1a, x2a, s);

		
		if ((`method' == 3) | (`method' == 4)) {;
			mata theta_full = st_matrix("result2_2_full");		
			mata sd2 = variance_machmat(q, gamma, b0, b1, y, x1, x2, `l', `c', `ba', cov_step1, `n',
						beta, hulp_u, result1_full, X1, theta_full, y1, x1a, x2a, s);						
		};
		
		if (`method' == 2) {;
			mata var2 = bereken_var2(q, theta, theta1[.,2], yb, result1, n);
			mata sd1 = sqrt(abs(sd :* sd + var2 :/ n));
			mata st_matrix("sd", sd1);
		};
		...
		if (`method' == 1) {;
			mata st_matrix("sd", sd);
			ereturn matrix q = result2_1;
			ereturn matrix theta = result2_2;
		...
		if (`method' == 5) {;
			mata var2 = bereken_var2(q, theta, theta1[.,2], yb_gender0, result1, n);
			mata sd1 = sqrt(abs(sd :* sd + var2 :/ n));
			ereturn matrix q = result2_1;
			ereturn matrix dtheta = dtheta;
			mata st_matrix("sd", sd1);
		...
		if (`method'== 4) {;
			mata st_matrix("theta1", theta1[.,2]);
			matrix dtheta = theta1 - result2_2;
			matrix dtheta1 = theta1 - result2_2_full;
			matrix dtheta2 = result2_2_full - result2_2;
			
			mata var2 = bereken_var2(q, theta, theta1[.,2], yb, result1, n);
			
			mata sd = sqrt(abs(sd :* sd + var2 :/ n));
			mata sd_1 = sqrt(abs(sd2 :* sd2 + var2 :/ n));
			mata sd_2 = sqrt(abs(sd2 :* sd2 + sd :* sd));
			mata st_matrix("sd", sd);
			mata st_matrix("sd_1", sd_1);
			mata st_matrix("sd_2", sd_2);
 
			ereturn matrix q = result2_1;
			ereturn matrix dtheta = dtheta;
			ereturn matrix dtheta_obs = dtheta2;
			ereturn matrix dtheta_unobs = dtheta1;
			ereturn matrix sd = sd;
			ereturn matrix sd_obs = sd_1;
			ereturn matrix sd_unobs = sd_2;
		};
		...
		if (`display' == 1) {;
			...
			if (!(`method' == 4)) {;
				display "";
				display "Estimation of the Machado-Mata method with sample selection correction";
				display "";
				display "------------------------------------------------------------------";
				display "";
				display "   quantile       point        standard       lower         upper";
				display "                estimate       deviation      bound         bound";
				display "";
				display "------------------------------------------------------------------";
				display "";
				...
				while (`i' < `n_q') {;
					...
					if (`method' == 1) {;
						...
						matrix sd = e(sd);
						local q_dis = q[`i',1];
						local theta_dis = theta[`i',1];
						local sd_dis = sd[`i',1];
						local lower = `theta_dis' - 2 * `sd_dis';
						local upper = `theta_dis' + 2 * `sd_dis';
						display _skip(2) %10.4f `q_dis' "   " %10.4f `theta_dis' "   " %10.4f `sd_dis'
										"    " %10.4f `lower' "    " %10.4f `upper';
					};
					else {;
						matrix dtheta = e(dtheta);
						matrix sd = e(sd);
						local q_dis = q[`i',1];
						local dtheta_dis = dtheta[`i',1];
						local sd_dis = sd[`i',1];
						local lower = `dtheta_dis' - 2 * `sd_dis';
						local upper = `dtheta_dis' + 2 * `sd_dis';
						display _skip(2) %10.4f `q_dis' "   " %10.4f `dtheta_dis' "   " %10.4f `sd_dis'
									"    " %10.4f `lower' "    " %10.4f `upper';
					};
				};
			};
				...
				display "";
				display "Estimation of the Machado-Mata method with sample selection correction";
				display "                    Selection on observables    ";
				display "";
				display "------------------------------------------------------------------";
				display "";
				display "   quantile       point        standard       lower         upper";
				...
				while (`i' < `n_q') {;
					local i = `i' + 1;
					matrix dtheta2 = e(dtheta_obs);
					matrix sd2 = e(sd_obs);
					local q_dis = q[`i',1];
					local dtheta_dis2 = dtheta2[`i',1];
					local sd_dis2 = sd2[`i',1];
					local lower2 = `dtheta_dis2' - 2 * `sd_dis2';
					local upper2 = `dtheta_dis2' + 2 * `sd_dis2';
					display _skip(2) %10.4f `q_dis' "   " %10.4f `dtheta_dis2' "   " %10.4f `sd_dis2'
									"    " %10.4f `lower2' "    " %10.4f `upper2';
				};

				display "";
				display "                    Selection on unobservables    ";
				display "";
				display "------------------------------------------------------------------";
				display "";
				display "   quantile       point        standard       lower         upper";
				...
				while (`i' < `n_q') {;
					local i = `i' + 1;
					matrix dtheta1 = e(dtheta_unobs);
					matrix sd1 = e(sd_unobs);
					local q_dis = q[`i',1];
					local dtheta_dis1 = dtheta1[`i',1];
					local sd_dis1 = sd1[`i',1];
					local lower1 = `dtheta_dis1' - 2 * `sd_dis1';
					local upper1 = `dtheta_dis1' + 2 * `sd_dis1';
					display _skip(2) %10.4f `q_dis' "   " %10.4f `dtheta_dis1' "   " %10.4f `sd_dis1'
									"    " %10.4f `lower1' "    " %10.4f `upper1';
				};
			};
		};	
	};
		...
		if (`display' == 1) {;
			matrix q = e(q);
			display "";
			display "Estimation of the Machado-Mata method    ";
			display "";
			display "----------------------------";
			display "";
			display "   quantile       point        ";
			display "                 estimate      ";
			display "";
			display "----------------------------";
			display "";
			...
			while (`i' < `n_q') {;
				...
				if (`method' == 1) {;
					...
					local q_dis = q[`i',1];
					local theta_dis = theta[`i',1];
					display _skip(2) %10.4f `q_dis' "   " %10.4f `theta_dis';
				};
				else {;
					matrix dtheta = e(dtheta);
					matrix sd = e(sd);
					local q_dis = q[`i',1];
					local dtheta_dis = dtheta[`i',1];
					display _skip(2) %10.4f `q_dis' "   " %10.4f `dtheta_dis';
				};
			};
		};			
	};
	
	quietly use `hulp_file_overall', clear;
end;
