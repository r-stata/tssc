...
program define single;
	...
	ml maximize, repeat(50) iterate(100) nonrtolerance /*ltolerance(0.001) tolerance(0.001)*/
							nooutput;
	...
	quietly sum `vars_rh1';/*
	quietly sum `vars_rh2';
				 */
	local i = 0;
	...
	make_dg `knew1' `h' `y' `fh' `rh' `mh' `vars_rh1' `vars_rh2' `vars_dg' `vars_dg1';
	
	bereken_c_d `knew' `vars_dg' `vars_dg1';
