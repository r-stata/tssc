*! Version 1.0.0 
* POISSON-CLOGLOG HURDLE REGRESSION :  Joseph Hilbe  :  26Sep2005
program hpclg, eclass properties(svyb svyj  svyr)
    version    9.1
    ...
    ml model lf jhpoi_cln_ll (cloglog: `lhs' =  `rhs', `offset' `exposure')   ///
         (poisson: `lhs' = `rhs', `offset' `exposure')                   ///
        `if' `in' `weight',                     ///
        `mlopts' `robust' `cluster'             ///
        title("Poisson-Cloglog Hurdle Regression")           ///
