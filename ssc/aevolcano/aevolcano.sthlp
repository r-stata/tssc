{smcl}
{* *! aevolcano version 1.0 18Dec2019}{...}
{cmd:help aevolcano}
{hline}
...
{p2colset 5 14 18 2}{...}
{p2col :{cmd:aevolcano} {hline 2} Volcano plot for adverse event data}{p_end}
{p2colreset}{...}
...
{p 8 15 2}
{cmdab:aevolcano} {varname} {cmd:,}  {opth treat(varname)} {opth id(varname)} {opth n1(integer)} {opth n2(integer)} [{it:options}]


{phang}
{bf:aevolcano} requires data in long format with one row per event per participant, where {varname} indicates the variable that contains the event name/identifier.
...
{synopt:{opt labelyn(#)}}used to turn bubble labels on by specifying {cmd:labelyn(1)}; default is bubble labels off {cmd:labelyn(0)}{p_end}
...
{synopt:{opt xaxismin(#)}} allows the user to extend the x-axis beyond the minimum plotted value; default is 0 so minimum value used{p_end}
{synopt:{opt xaxismax(#)}} allows the user to extend the x-axis beyond the maximum plotted value; default is 0 so maximum value used{p_end}
{synopt:{opt yaxismin(#)}} allows the user to extend the y-axis beyond the minimum value; default is 0 so minimum value used{p_end}
{synopt:{opt yaxismax(#)}} allows the user to extend the y-axis beyond the maximum value; default is 0 so maximum value used{p_end}			
{synopt:{opt xaxisticks(#)}} allows the user to specify how the x-axis ticks are spaced; default is 4{p_end}
{synopt:{opt yaxisticks(#)}} allows the user to specify how the y-axis ticks are spaced; default is 4{p_end}
...
{synopt:{opt legendyn(#)}} specify whether legend appears on the plot; {cmd:legendyn(#)} takes value 0 to indicate legend off or 1 to indicate legend on, default is {cmd:legendyn(0)}{p_end}
...
{pstd}
{cmd:aevolcano} creates a volcano plot for adverse event data from a two-arm clinical trial, 
as proposed by {help aevolcano##R2013:Zink, Wolfinger, and Mann (2013)}. 
...
process lowest coded value first. 
...
The default is {cmd:labang2(90)} to give vertical labels. {cmd:labang2(#)} takes integer values only.
{cmd:labang1(#)} and {cmd:labang2(#)} can be used to differentiate the angle of the labels according to the direction 
of the treatment effect. 
...
{phang2}{help aevolcano##general_remarks:General remarks}{p_end}
...
{pstd}
(4) {cmd: treat(varname)} must be a numerical variable. The command will always
process lowest coded value first. For example:{break}
{it:treat(1)} - {it:treat(2)} or {it:treat(0)} - {it:treat(1)} and {it:treat(1)}/{it:treat(2)} or
{it:treat(0)}/{it:treat(1)}
...
{pstd}
(11) If when running the program user receives error message {it: "postfile ae_volc already exists"} user needs to type {cmd: postclose ae_volc} before rerunning the command.
...
{phang2}{cmd:. use example_volcano.dta}{p_end}

{pstd}
Volcano plot {p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31)}{p_end}
 
{pstd}
Volcano plot using the {cmd:ylineyn} option to include a horizontal line at the default value and using the {cmd:yaxismax} option to extend the y-axis {p_end}
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) ylineyn(1) yaxismax(0.2)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) ylineyn(1) yaxismax(0.2) yline(1 2)}{p_end}

{pstd}
Volcano plot using the {cmd: labelyn} option to include labels on the plot{p_end}
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) yaxismax(0.2) labelyn(1)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) labelyn(1) yaxismax(0.2) label(0.5)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) labelyn(1)  yaxismax(0.2) label(0.5) labcol(red)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) labelyn(1) yaxismax(0.2) label(0.5) labcol1(green)  labcol2(red)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) mfcolor1(orange) mfcolor2(green)}{p_end}

{pstd}
Volcano plot using the {cmd:legendyn} option to include a legend on the plot{p_end}
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) legendyn(1)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) legendyn(1) legend1(Intervention) legend2(Placebo)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) oddsratio(1)}{p_end}
...
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) riskratio(1)}{p_end}

{pstd}
Volcano plot using the {cmd:padj} option to calculate the FDR adjusted p-values{p_end}
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) padj(1) fdrlower(ae_pt) fdrhigher(aebodsys)}{p_end}
...
are flagged {p_end}
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31) padj(1) fdrlower(ae_pt) fdrhigher(aebodsys) fdrval(0.05)}{p_end}

{pstd}
Saving the dataset used to produce the plot with filename {it:volcano_example}{cmd:.dta}{p_end} 
{phang2}{cmd:. use example_volcano.dta, clear}{p_end}
{phang2}{cmd:. aevolcano aebodsys, treat(arm) id(usubjid) n1(30) n2(31)   saving(volcano_example)}{p_end}
