...
program stpm2_prefix, eclass byable(onecall)
	version 11.1
...
program Estimate_prefix, eclass byable(recall)
	st_is 2 analysis	
	...
	[, DF(string) TVC(varlist fv) DFTvc(string) KNOTS(numlist ascending) KNOTSTvc(string) ///
		...
		NOFIRSTDER NOSECONDDER RCSPREFIX(string)] ///
	[                               ///
...
/* Use old estimation commands if Stata version <15 */
	if `c(stata_version)' < 15.0 {
		local oldest oldest
	}
...
/* Drop previous created _rcs and _d_rcs variables */
	capture drop `rcsprefix'_rcs* 
	capture drop `rcsprefix'_d_rcs*    
	capture drop `rcsprefix'_s0_rcs*
	
	/*if "`rcsprefix'" == "" {
		local rcsname _rcs
		capture drop `rcsname'* //added new by Sarwar
		capture drop _d`rcsname'* //added new by Sarwar
		capture drop _s0`rcsname'* //added new by Sarwar
	}
	else {
		local rcsname `rcsprefix' //added new by Sarwar
		capture drop `rcsname'* //added new by Sarwar
	}*/
...
/* Knot placement for baseline hazard (unless cure option is specified) */
	if `nbhknots' == 0 & "`rcsbaseoff'" == "" & "`cure'" == "" {
		if `df' == 1 {
			qui rcsgen `lnt' if `touse2', gen(`rcsprefix'_rcs) dgen(`rcsprefix'_d_rcs) `orthog' `rmatrixopt' `reverse' `nosecondder' `nofirstder'
			if "`orthog'" != "" {
				matrix `R_bh' =  r(R)
			}
		}
		...
		foreach tvcvar in  `tvc' {
			if "`tvcknots_`tvcvar'_user'" == "" {
				if `tvc_`tvcvar'_df' == 1 {
					qui rcsgen `lnt' if `touse2', gen(`rcsprefix'_rcs_`tvcvar') dgen(`rcsprefix'_d_rcs_`tvcvar') `orthog' `reverse' `nosecondder' `nofirstder'
					if "`orthog'" != "" {
...
/* Generate splines for baseline hazard */
	...
	if "`df'" != "1" & "`rcsbaseoff'" == "" {
		...
		qui rcsgen `lnt' if `touse2', knots(`bhknots') gen(`rcsprefix'_rcs) dgen(`rcsprefix'_d_rcs) `orthog'  `rmatrixopt' `reverse' `nosecondder' `nofirstder'
		if "`orthog'" != "" {
			matrix `R_bh' = r(R)
		}
	}
...
/* Generate splines for time-dependent effects */	
	if "`tvc'" != "" {
		foreach tvcvar in  `tvc' {
			if `tvc_`tvcvar'_df' != 1 {
				...
				qui rcsgen `lnt' if `touse2', knots(`tvcknots_`tvcvar'') gen(`rcsprefix'_rcs_`tvcvar') dgen(`rcsprefix'_d_rcs_`tvcvar') `orthog' `reverse' `nosecondder' `nofirstder'
				if "`orthog'" != "" {
					tempname R_`tvcvar' Rinv_`tvcvar'
					matrix `R_`tvcvar'' = r(R)
				}
	...
	if `del_entry' == 1 {
		...
		if "`df'" == "1" & "`rcsbaseoff'" == "" {
			qui rcsgen `lnt0' if `touse2' & _t0>0, gen(`rcsprefix'_s0_rcs) `rmatrixopt' `reverse' `nosecondder' `nofirstder'
		}
		else if "`df'" != "1" & "`rcsbaseoff'" == "" {
			qui rcsgen `lnt0' if `touse2' & _t0>0, knots(`bhknots') gen(`rcsprefix'_s0_rcs) `rmatrixopt' `reverse' `nosecondder' `nofirstder'
		}
		...
		foreach tvcvar in  `tvc' {
			...
			if `tvc_`tvcvar'_df' == 1 {
				qui rcsgen `lnt0' if `touse2' & _t0>0,  gen(`rcsprefix'_s0_rcs_`tvcvar') `rmatrixopt' `reverse' `nosecondder' `nofirstder'
			}
			else if `tvc_`tvcvar'_df' != 1 {
					qui rcsgen `lnt0' if `touse2' & _t0>0, knots(`tvcknots_`tvcvar'') gen(`rcsprefix'_s0_rcs_`tvcvar') `rmatrixopt' `reverse' `nosecondder' `nofirstder'
			}
		}		
	}
...
/* create list of spline terms and their derivatives for use when orthogonalizing and in model equations */
	forvalues i = 1/`df' {
		local rcsterms_base "`rcsterms_base' `rcsprefix'_rcs`i'"
		local drcsterms_base "`drcsterms_base' `rcsprefix'_d_rcs`i'"
	}
	...
	if "`tvc'" != "" {
		...
		foreach tvcvar in  `tvc' {
			forvalues i = 1/`tvc_`tvcvar'_df' {
				local rcsterms_`tvcvar' "`rcsterms_`tvcvar'' `rcsprefix'_rcs_`tvcvar'`i'"
				local drcsterms_`tvcvar' "`drcsterms_`tvcvar'' `rcsprefix'_d_rcs_`tvcvar'`i'"
				local rcsterms "`rcsterms' `rcsprefix'_rcs_`tvcvar'`i'"
				local drcsterms "`drcsterms' `rcsprefix'_d_rcs_`tvcvar'`i'"
			}
		}
	}
	
	local s0_rcsterms : subinstr local rcsterms_base "`rcsprefix'_rcs" "`rcsprefix'_s0_rcs", all 
	local s0_rcsterms_base `s0_rcsterms'
	if "`tvc'" != "" {
		foreach tvcvar in  `tvc' {
			forvalues i = 1/`tvc_`tvcvar'_df' {
				local s0_rcsterms_`tvcvar' `s0_rcsterms_`tvcvar'' `rcsprefix'_s0_rcs_`tvcvar'`i'
			}
...
/* multiply time-dependent _rcs and _drcs terms by time-dependent covariates */
	if "`tvc'" != "" {
		foreach tvcvar in  `tvc' {
			forvalues i = 1/`tvc_`tvcvar'_df' {
				qui replace `rcsprefix'_rcs_`tvcvar'`i' = `rcsprefix'_rcs_`tvcvar'`i'*`tvcvar' if `touse2'
				qui replace `rcsprefix'_d_rcs_`tvcvar'`i' = `rcsprefix'_d_rcs_`tvcvar'`i'*`tvcvar' if `touse2'
				if `del_entry' == 1 {
					qui replace `rcsprefix'_s0_rcs_`tvcvar'`i' = `rcsprefix'_s0_rcs_`tvcvar'`i'*`tvcvar' if `touse2' & _t0>0
				}
			}
		}
	}
...
/* replace missing values for delayed entry with -99 as ml will omit these cases. -99 is not included in the likelihood calculation */
	if `del_entry' == 1 {
		forvalues i = 1/`df' {
			qui replace `rcsprefix'_s0_rcs`i' = -99 if `touse2' & _t0 == 0 & "`rcsbaseoff'" == ""
		}
		foreach tvcvar in `tvc' {
			forvalues i = 1/`tvc_`tvcvar'_df' {
			qui replace `rcsprefix'_s0_rcs_`tvcvar'`i' = -99 if `touse2' & _t0 == 0
			}
		}
	}
...
/* variable labels */
	if "`rcsbaseoff'" == "" {
		forvalues i = 1/`df' {
			label var `rcsprefix'_rcs`i' "restricted cubic spline `i'"
			label var `rcsprefix'_d_rcs`i' "derivative of restricted cubic spline `i'"
			if `del_entry' == 1 {
				label var `rcsprefix'_s0_rcs`i' "restricted cubic spline `i' (delayed entry)"
			}
		}
	}
	...
	if "`tvc'" != "" {
		foreach tvcvar in  `tvc' {
			forvalues i = 1/`tvc_`tvcvar'_df' {
				label var `rcsprefix'_rcs_`tvcvar'`i' "restricted cubic spline `i' for tvc `tvcvar'"
				label var `rcsprefix'_d_rcs_`tvcvar'`i' "derivative of restricted cubic spline `i' for tvc `tvcvar'"
				if `del_entry' == 1 {
					label var `rcsprefix'_s0_rcs_`tvcvar'`i' "restricted cubic spline `i' for tvc `tvcvar' (delayed entry)"
				}
			}	
		}
	}
...
/* Fit linear term to log(time) for initial values. */
		...
		if inlist("`scale'","hazard","odds","normal","log") {
			if "`rcsbaseoff'" == "" {
				local initrcslist `rcsprefix'_rcs1
				local initdrcslist `rcsprefix'_d_rcs1
				constraint free
				constraint `r(free)' [xb][`rcsprefix'_rcs1] = [dxb][`rcsprefix'_d_rcs1]
			}
			local initconslist `r(free)'
			if "`tvc'" != "" {
				foreach tvcvar in `tvc' {
					local initrcslist `initrcslist' `rcsprefix'_rcs_`tvcvar'1
					local initdrcslist `initdrcslist' `rcsprefix'_d_rcs_`tvcvar'1
					constraint free
					constraint `r(free)' [xb][`rcsprefix'_rcs_`tvcvar'1] = [dxb][`rcsprefix'_d_rcs_`tvcvar'1]
			...
			if `del_entry' == 1 {
				...
				if "`rcsbaseoff'" == "" {
					local xb0 `xb0' `rcsprefix'_s0_rcs1 
				}
				if "`tvc'" != "" {
					foreach tvcvar in `tvc' {
						local xb0 `xb0' `rcsprefix'_s0_rcs_`tvcvar'1
					}
				}
				...
				foreach var in `initrcslist' `varlist' `addconstant' {
					...
					if substr("`var'",1,4) == "_rcs" {
						constraint `r(free)' [xb][`var'] = [xb0][`rcsprefix'_s0`var']
					}
					else {
...
/* constraints for baseline */
	forvalues k = 1/`=cond("`cure'"=="",`df',`df'-1)' {
		constraint free
		constraint `r(free)' [xb][`rcsprefix'_rcs`k'] = [dxb][`rcsprefix'_d_rcs`k']
		local conslist `conslist' `r(free)'
	}
	if "`cure'" != "" {
		constraint free
		constraint `r(free)' [dxb][`rcsprefix'_d_rcs`df'] = 0
		local conslist `conslist' `r(free)'
	}
...
/* add constraint for baseline if cure option is specified*/
	if "`cure'" != "" {
		if "`rcsbaseoff'" == "" {
			constraint free
			constraint `r(free)' [xb][`rcsprefix'_rcs`df'] = 0
			local conslist `conslist' `r(free)'
		}
	}
	...
	if "`tvc'" != "" {
		foreach tvcvar in  `tvc' {
			forvalues k = 1/`tvc_`tvcvar'_df' {
				constraint free
				constraint `r(free)' [xb][`rcsprefix'_rcs_`tvcvar'`k'] = [dxb][`rcsprefix'_d_rcs_`tvcvar'`k']
				local conslist `conslist' `r(free)'
			}
		}
	}
...
/* add constraints for time-dependent effects if cure option is specified*/
	if "`tvc'" != "" & "`cure'" != ""{
		foreach tvcvar in  `tvc' {
				constraint free
				constraint `r(free)' [xb][`rcsprefix'_rcs_`tvcvar'`tvc_`tvcvar'_df'] = 0
				local conslist `conslist' `r(free)'
		}
	}
...
//	ereturn scalar nxbterms = e(rank) - ("`scale'" == "theta")
	...
	Replay_prefix, level(`level') `alleq' `eform' `showcons' `diopts'
end

program Replay_prefix
