...
{hline}
{cmd:help metapow}{right:also see: {helpb metasim}, {helpb metapowplot}}
{hline}

{title:Title}

{p2colset 5 16 20 2}{...}
{p2col :{cmd:metapow} {hline 2}}Tool to estimate power through simulation of a 2-arm clinical trial(s) or diagnostic test accuracy study(ies) to change 
inferences when updating an existing meta-analysis including the new study(ies){p_end}
{p2colreset}{...}
...
{phang2}
{cmd: metapow} {it:varlist} {cmd:,} {opt n:(#)} {opt nit:(#)} {opt type:(string)} {opt pow:(numlist)} [{it:options}]
...
{pstd}{cmd:metapow} implements an approach to estimating the power of a newly simulated study(ies), generated using the program metasim. It can also estimate 
the power of the updated meta-analysis including the new study. The power estimates are then stored in a file called temppow2 within the working directory.
...
{pstd}Fixed effect meta-analysis of clinical trials using the odds ratio, assuming 100 patients in both the control group and the treatment group of the new study 
and using a confidence interval width cut-off of 2 on the odds ratio scale to calculate power through 100 iterations (i.e. power is based on the proportion of the simulations
in which the confidence interval for the pooled OR is less than 2).{p_end}

{phang}{cmd:. metapow e_trt ne_trt e_ctrl ne_ctrl, model(fixed) measure(or) n(100) nit(100) type(clinical) pow(2) inference(ciwidth)}{p_end}


{pstd}Random effects meta-analysis of clinical trials using the risk difference, assuming 50 patients in both the control group and the treatment group of the new study 
and using a p-value of 0.05 for the hypothesis test that the pooled treatment effect is different from 0 to calculate power through 1000 iterations (i.e. power is based on the proportion
of simulations in which the p-value for the treatment effect being different from 0 is less then 0.05).{p_end}

{phang}{cmd:. metapow e_trt ne_trt e_ctrl ne_ctrl, model(random) measure(rd) n(50) nit(1000) type(clinical) pow(0.05) inference(pvalue)}{p_end}
...
confidence interval to a reduction in pain of 20 points. We choose a sample size of 250 patients in each arm. We then use {cmd:extfunnel} to intuitively explain the simulated individual 
studies on the extended funnel plot (Crowther et al. 2012).{p_end}

{phang}{cmd:. use http://fmwww.bc.edu/repec/bocode/m/metapow_eg1}{p_end}
{phang}{cmd:. metapow nrxpain rxmeanpain rxsdpain ncomppain compmeanpain compsdpain, n(250) nit(1000) type(clinical) measure(nostandard) model(randomi) pow(-20) inference(lci)}{p_end}
{phang}{cmd:. merge 1:1 _n using "`c(pwd)'\temppow2.dta", nogen noreport}{p_end}
{phang}{cmd:. extfunnel paindiff painse, randomi cpoints(500) null(-20) name(painfunnel20, replace) 	///}{p_end}
{phang}{cmd:> nometan title("PAIN: smallest worthwhile effect = -20", size(medium)) xsc(titlegap(medium))	///}{p_end}
{phang}{cmd:> xrange(-100 75) xlabel(-100(25)75)yrange(0 15) scale(1.2) legend(order(1 "Unclear if worthwhile" 2  	///}{p_end}
{phang}{cmd:> "Clearly not worthwhile" 3 "Clearly worthwhile" 4 "Smallest worthwhile effect" 5 "Current pooled estimate"  	///}{p_end}
{phang}{cmd:> 6 "Current point estimates" 7 "Simulated studies") size(small)) scheme(color) ///}{p_end}
{phang}{cmd:> addplot(scatter indse_es indes, msize(tiny) msym(T) mcol(black))}{p_end}
{phang}{it:({stata "metapow_eg1":click to run})}{p_end}


{pstd}Random effect meta-analysis of diagnostic test accuracy studies using sensitivity and specificity to base inferences on, assuming 1000 patients in the diseased group and 250 patients in the healthy 
group of the new study and using 95% confidence interval widths of 0.2 and 0.1 for sensitivity and specificity respectively to calculate power through 1000 iterations
(i.e. power is estimated as the proportion of simulations in which the updated meta-analysis estimates sensitivity and specificity with 95% confidence interval
widths less than 0.2 and 0.1 respectively).{p_end}

{phang}{cmd:. metapow TP FP FN TN, model(random) measure(ss) n(1000) nit(1000) type(diagnostic) pow(0.2 0.1) inference(ciwidth) r(0.25)}{p_end}


{pstd}Bivariate random effect meta-analysis of diagnostic studies using sensitivity and specificity, assuming 80 patients in both the diseased group and the healthy group of the new study 
and using a lower confidence interval value of 0.25 for sensitivity to calculate power through 50 iterations (i.e. power is estimated as the proportion of simulations in which the updated meta-analysis
estimates the lower confidence interval for sensitivity to be less than 0.25).{p_end}

{phang}{cmd:. metapow TP FP FN TN, model(bivariate) measure(ss) n(80) nit(50) type(bivariate) pow(0.25) inference(lci) sos(sens)}{p_end}
...
{phang}Crowther MJ, Langan D, Sutton AJ. Graphical augmentations to the funnel plot to assess the impact of a new study on an existing meta-analysis. {it: The Stata Journal} 2012; (In Press).{p_end}
...
{psee}
Online: {helpb metasim}, {helpb metapowplot}
{p_end}
