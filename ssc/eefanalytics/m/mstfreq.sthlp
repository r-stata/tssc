...
{synopt :{opt iter:ate(#)}}number of maximum iterations; default is 100. {p_end}
{synopt :{opt *}}maximization options such as {cmd:technique()}, {cmd:difficult}. {p_end}
...
{phang2}
# number of {it:supplementary} permutations/bootstraps are automatically employed when # number of permutated/bootstrapped models have failed to converge. One can specify {cmd:showprogress} to identify how many models (if any) have failed.

{phang2}
Any attached variables from previous use of {cmd:mstfreq} will be replaced. 

{phang}
{opt iterate(#)} Sets number of maximum iterations specifically for permutations/bootstraps; default is set to 100.
...
{pstd}Model using permutations including factor parameters and additional maximization options with base level change{p_end}
{phang2}{cmd:. mstfreq Posttest Prettest i.School, int(ib(#2).Intervention) ran(School) nboot(3000) technique(dfp) difficult}{p_end}
...
{phang2}{cmd:. mstfreq Posttest Prettest, int(Intervention2) ran(School) nperm(3000) nboot(2000) ml show}{p_end}
