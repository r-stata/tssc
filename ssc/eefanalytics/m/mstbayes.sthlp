...
{title:Syntax}
	...
	{cmd:mstbayes} {varlist} {ifin}{cmd:,} {opt int:ervention(interv_var)} {opt ran:dom(clust_var)} [{it:options}]
...
{pstd}
{cmd:mstbayes} performs {cmd:Effect Size} (ES) calculation of multisite randomised educational trials using a multilevel model under a Bayesian setting. This analysis produces ES estimates for both conditional
and unconditional model specifications.
...
{pstd}
{cmd:mstbayes} stores the following in {cmd:r()}:
