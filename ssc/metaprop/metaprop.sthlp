...
multiple studies study and/or displays the results in a forest plot. The confidence 
intervals are based on score(Wilson) (Newcombe, R. G. 1998) or 
exact binomial(Clopper-Pearson) (Newcombe, R. G. 1998) procedures.
A test of whether the summary effect measure is equal to the zero is given,
...
{p 4 4 2}
{cmd:metaprop} requires two variables in the format {n, N} such that p = n/N
to be declared. 

{p 4 4 2}
Note that the {cmd:metaprop} command requires Stata 10.1 or later versions.

{title:Options for metaprop}

{dlgtab:Specifying the measure and model}

{p 4 8 2}
{cmd:ftt} Calculate the pooled estimate after Freeman-Tukey Double Arcsine 
Transformation (Freeman, M. F. , and Tukey, J. W. 1950) to stabilize the variances.

{p 4 8 2}
{cmd:cimethod} Specifies the method to compute the confidence intervals for the individual studies. By default,
the Score(Wilson) confidence intervals are computed. Available option is "exact".
...
{cmd:cc(}{it:#}{cmd:)} defines a fixed continuity correction to add in the case where 
    a study has zero success. By default, {cmd:metaprop}
...
{cmd:wgt(}{it:weightvar}{cmd:)} specifies alternative weighting
    for any data type. The pooled effect size is to be computed by assigning 
...
The x-axis labels (and dp if necessary) should be adjusted accordingly when power(#) is adjusted. 
...
{cmd:sgweight} specifies that the display is to present the percentage
    weights within each subgroup separately. By default {cmd:metaprop} presents
...
{cmd:nosubgroup} indicates that within-group pooled results are to be ommitted when 
    {cmd:by()} is used. By default {cmd:metaprop} presents both within group and overall 
    pooled results.
...
{p 4 8 2}
{cmd:nooverall} prevents display of overall effect size on graph 
...
{p 8 8 2}
{cmd:olineopt()} controls the overall effect line with options for an
...
By default, {cmd:metaprop} adds the following new variables to the dataset:
...
suppressed weights, increased text size, changes on the box effect estimate,
a red diamond for the confidence intervals of the pooled effect estimate,
a vertical line at zero, a red dashed line,
for the pooled effect estimate, e.t.c.
...
{p 8 12 2}
{cmd:. metaprop num denom, random by(tgroup) cimethod(exact)}
{p_end}
{p 12 12 2}
...
{p_end}
{p 12 12 2}
{it:({stata "metaprop_examples metaprop_example_one":click to run})}
...
{p 8 12 2}
{cmd:.	metaprop p16p total, random ftt cimethod(exact)}
{p_end}
{p 12 12 2}
...
{p_end}
{p 12 12 2}
{it:({stata "metaprop_examples metaprop_example_two":click to run})}
...
{p 8 12 2}
{stata "use http://fmwww.bc.edu/repec/bocode/t/tsoumpou2009cancertreatrevfig2HSIL.dta":. use http://fmwww.bc.edu/repec/bocode/t/tsoumpou2009cancertreatrevfig2HSIL.dta.dta}
...
{p_end}
{p 12 12 2}
{it:({stata "metaprop_examples metaprop_example_three":click to run})}
