*! Version 1.0.0 
* GEOMETRIC-LOGIT HURDLE REGRESSION :  Joseph Hilbe  :  30Sep2005
program hglogit, eclass properties(svyb svyj  svyr)
    version    9.1
    ...
    ml model lf jhgeo_logit_ll (logit: `lhs' =  `rhs', `offset' `exposure')   ///
         (geometric: `lhs' = `rhs', `offset' `exposure')                   ///
        `if' `in' `weight',                     ///
        `mlopts' `robust' `cluster'             ///
        title("Geometric-Logit Hurdle Regression")           ///
