{smcl}
{hline}
{cmd:help: {helpb sptobitmstard}}{space 50} {cmd:dialog:} {bf:{dialog sptobitmstard}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:sptobitmstard: Tobit (m-STAR) Multiparametric Spatio Temporal AutoRegressive Regression}
{p 9 1 1}{bf:(Spatial Durbin Cross Sections Models)}{p_end}

{marker 00}{bf:{err:{dlgtab:Table of Contents}}}
{p 4 8 2}

{p 5}{helpb sptobitmstard##01:Syntax}{p_end}
{p 5}{helpb sptobitmstard##02:Description}{p_end}
{p 5}{helpb sptobitmstard##03:Options}{p_end}
{p 5}{helpb sptobitmstard##04:Saved Results}{p_end}
{p 5}{helpb sptobitmstard##05:References}{p_end}

{p 1}*** {helpb sptobitmstard##06:Examples}{p_end}

{p 5}{helpb sptobitmstard##07:Authors}{p_end}

{p2colreset}{...}
{marker 01}{bf:{err:{dlgtab:Syntax}}}

{p 5 5 6}
{opt sptobitmstard} {depvar} {indepvars} {weight} , {opt wmf:ile(weight_file)}  {opt ll(#)}{p_end} 
{p 3 5 6} 
{err: [} {opt nw:mat(#)} {opt nocons:tant} {opt aux(varlist)} {opt stand inv inv2} {opt dist(norm|weib)} {opt mfx(lin, log)} {opt l:evel(#)}{p_end}  
{p 5 5 6} 
{opt coll zero tolog nolog robust} {opt pred:ict(new_var)} {opt res:id(new_var)} {opt iter(#)} {opt tech(name)}{p_end} 
{p 5 5 6} 
...
{p 2 2 2} {cmd:sptobitmstard} estimates Tobit Spatial econometric regression (MSTAR) "Multiparametric Spatio Temporal AutoRegressive Regression" for Spatial Durbin Cross Sections models.{p_end}

{p 2 4 2}{cmd:sptobitmstard} can generate:{p_end}
...
{p 2 4 2} {cmd:sptobitmstard} predicted values are obtained from conditional expectation expression.{p_end}
...
{cmd:sptobitmstard} generates some variables names with prefix:
...
{p 2 2 2}{bf:sptobitmstard} is used with more than Weight Matrix: (Border, Language, Currency, Trade...){p_end}
...
{p 2 4 2 }{cmd:sptobitmstard} saves the following results in {cmd:e()}:
...
{bf:Note 3:} You can use the dialog box for {dialog sptobitmstard}.
{hline}

{stata clear all}

{stata sysuse sptobitmstard.dta, clear}
...
{bf:* (1) *** Normal Distribution}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs2) nwmat(2) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs3) nwmat(3) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs4) nwmat(4) mfx(lin) dist(norm) ll(0)}

 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs4) nwmat(4) mfx(lin) dist(norm) aux(x3 x4) ll(0)}
{hline}

{bf:* (2) *** Weibull Distribution}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs2) nwmat(2) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs3) nwmat(3) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs4) nwmat(4) mfx(lin) dist(weib) ll(0)}

 {stata sptobitmstard ys x1 x2 , wmfile(SPWmcs4) nwmat(4) mfx(lin) dist(weib) aux(x3 x4) ll(0)}
{hline}

{bf:* (3) Weighted mSTAR Normal Distribution:}
 {stata sptobitmstard ys x1 x2  [weight = x1], wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(norm) ll(0)}
 {stata sptobitmstard ys x1 x2 [aweight = x1], wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(norm) ll(0)}
{hline}

{bf:* (4) Weighted mSTAR Weibull Distribution:}
 {stata sptobitmstard ys x1 x2  [weight = x1], wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(weib) ll(0)}
 {stata sptobitmstard ys x1 x2 [aweight = x1], wmfile(SPWmcs1) nwmat(1) mfx(lin) dist(weib) ll(0)}
{hline}

. clear all
. sysuse sptobitmstard.dta, clear
. sptobitmstard ys x1 x2 , wmfile(SPWmcs1) nwmat(1) dist(norm) mfx(lin) ll(0)
...
------------------------------------------------------------------------------
- Sum of Rho's =    0.0146995    Sum must be < 1 for Stability Condition
------------------------------------------------------------------------------
          ys |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
ys           |
          x1 |  -.4287432   .1158967    -3.70   0.000    -.6558965   -.2015899
      ...
      w1x_x2 |   .1317292   .1363179     0.97   0.334    -.1354489    .3989073
       _cons |   70.25771   6.494276    10.82   0.000     57.52916    82.98626
-------------+----------------------------------------------------------------
        Rho1 |   .0146995   .0300642     0.49   0.625    -.0442253    .0736243
       Sigma |   10.45099   1.158755     9.02   0.000      8.17987    12.72211
------------------------------------------------------------------------------
 Wald Test [Rho1=0]:                 0.2391        P-Value > Chi2(1) 0.6249
...
{bf:{err:{dlgtab:SPTOBITMSTARD Citation}}}

{p 1}{cmd:Shehata, Emad Abd Elmessih & Sahra Khaleel A. Mickaiel (2013)}{p_end}
{p 1 12 1}{cmd:SPTOBITMSTARD: "Tobit (m-STAR) Spatial Multiparametric Spatio Temporal AutoRegressive Regression: Spatial Durbin Cross Sections Models"}{p_end}
