...
program define spmstardhxt, eclass 
 version 11.0
 if replay() {
if "`e(cmd)'"!="spmstardhxt" {
 error 301
 }
 Display `0'
 }
...
version 11.0
syntax varlist [aw] , nc(str) WMFile(str) [NWmat(int 1) mhet(str) aux(str) ///
 diag LL(real 0) vce(passthru) stand zero ROBust MFX(str) NOLog LMHet LMSPac ///
 iter(int 100) level(passthru) PREDict(str) INV INV2 tobit ///
 coll RESid(str) tech(str) tolog TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spmstardhxt
...
tempvar D DE DF1 DumE E e dcs XQX_ EE Eo Hat ht idv itv LE P Q Sig2 SSE
tempvar SST Time U U2 Ue wald weit Wi Wio WS X X0 eigw Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE DVNE Dx E E1 F h Hat
tempname IPhi J K L M P Phi Pm Q Sig2 Sig2o In IRW IRWR WXb kbm IN xMx SST1 SST2
tempname Vec vh VM VP VQ Vs W W1 W2 Wald We Wi Wi1 Wio WY X XB V
tempname Y Yh Yi YYm YYv Z X0 Ue Sig2n WS D WW eVec eigw Xo mh olsin SSE
tempname N DF kx kb NC NT Nmiss kz llf Sig21 Dim wmat xAx xAx2 TRa2 wWw1 xBx
tempname Rostar kb minEig maxEig waldm waldmp waldm_df waldr waldrp waldr_df
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
local N=`N'
...
local initopt init(`olsin', copy)  
 ml model lf spmstardhxt1 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstardhxt2 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstardhxt3 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
...
local initopt init(`olsin', copy)  
 ml model lf spmstardhxt4 (`yvar': `yvar' = `SPXvar' , `noconstant' ) ///
