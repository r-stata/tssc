...
*                                                                                       *
*   This is the "predict" subroutine of regoprob.                                       *
*                                                                                       *
* ************************************************************************************* *



program define regoprob_p
	version 8
...
program define Predict_Pr
	// Generates the predicted probabilities for regoprob
	// Adapted from Richard Williams' gologit2_p
	...
	forval j = 1/`Numeqs' {
		...
		quietly _predict double `xb`j'' if `touse', eq(#`j') xb `offset'
			local rho = [rho]_b[_cons]
			quietly gen double `xbr`j'' = `xb`j''*sqrt(1-`rho')
	}
