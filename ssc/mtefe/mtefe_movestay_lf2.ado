...
*! adapted for mtefe by Martin E. Andresen, 19feb2018

#delim ;
program define mtefe_movestay_lf2, eclass;
	args todo b lnf g1 g2 g3 g4 g5 g6 g7 negH;
	...
	loc sig0 	= exp(`lns0');
	loc sig1 	= exp(`lns1');
	loc rho0 	= tanh(`theta0');
	loc rho1 	= tanh(`theta1');
	loc rr0		= 1/sqrt(1-`rho0'^2);
	loc rr1		= 1/sqrt(1-`rho1'^2);

	tempvar eta0 eta1 eps0 eps1;
	...
	loc const0=0.5*ln(2*_pi*`sig0'^2);
	loc const1=0.5*ln(2*_pi*`sig1'^2);

	replace `lnf' = cond($ML_y3==1, ln(normal(  `eta1'))-`const1'-0.5*(`eps1'/`sig1')^2,
				                    ln(normal(- `eta0'))-`const0'-0.5*(`eps0'/`sig0')^2);
	...
	generate double `M'         =  cond($ML_y3==1,normalden(`eta1')/normal(`eta1'),-normalden(`eta0')/normal(-`eta0'));

	loc drho0_dtheta0 	= 4*exp(2*`theta0')/((1+exp(2*`theta0'))^2);	  // derivative of rho w.r.t theta
  	loc drho1_dtheta1 	= 4*exp(2*`theta1')/((1+exp(2*`theta1'))^2);	  // derivative of rho w.r.t theta
	...
	loc d2rho1_dtheta12      = 8*exp(2*`theta1')*(1-exp(2*`theta1'))/(1+exp(2*`theta1'))^3;	  // 2nd derivative of rho w.r.t theta
	loc d2rho0_dtheta02      = 8*exp(2*`theta0')*(1-exp(2*`theta0'))/(1+exp(2*`theta0'))^3;
