...
************************************************
*** ml for joint mmlogit and normal          ***
************************************************

program mtreatreg_normal_lf
	version 10.1
	...
	tempvar lnL gi `L_xbT' xbO `L_lam' lnsigma sigma `L_h' `L_nH'
	...
	mleval `lnsigma' = `b', eq(`=neq+2')

	qui gen double `sigma' = exp(`lnsigma')

	mata: mtreatreg_normal_lf("`lnL'","`G'","`H'" ///
		,"`xbTnames'","`xbO'","`sigma'","`lamnames'" ///
