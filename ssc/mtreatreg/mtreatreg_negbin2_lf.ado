...
************************************************
*** ml for joint mmlogit and negbin2         ***
************************************************

program mtreatreg_negbin2_lf
	version 10.1
	...
	tempvar lnL gi `L_xbT' xbO `L_lam' lnalpha alpha `L_h' `L_nH'
	...
	mleval `lnalpha' = `b', eq(`=neq+2')

	qui gen double `alpha' = exp(`lnalpha')

	mata: mtreatreg_negbin2_lf("`lnL'","`G'","`H'" ///
		,"`xbTnames'","`xbO'","`alpha'","`lamnames'" ///
