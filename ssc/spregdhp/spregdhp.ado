...
program define spregdhp, eclass 
version 11.0
syntax varlist , nc(str) WMFile(str) [Model(str) diag coll INV stand zero ///
 MFX(str) NOLog INV2 LMHet LMSPac ROBust BE FE RE iter(int 100) vce(passthru) ///
 level(passthru) PREDict(str) RESid(str) tolog TESTs NOCONStant LMNorm]
gettoken yvar xvar : varlist
local sthlp spregdhp
...
tempvar D DE DF1 DumE E dcs XQX_ EE Hat wald weit Wi Wio WS
tempvar ht idv itv LE P Q Sig2 SSE SST Time U U2 Ue X X0 Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yt YY YYm YYv Yh_ML Ue_ML Z
tempname Xb A B b Beta BetaSP Bx Cov D den DVE IN xMx DVNE Dx E E1
tempname h Hat IPhi J K M P Phi Pm q Q q1 q2 s kbm Eg In WXb Eo Ew F V1s v2
tempname S11 S12 Sig2 Sig2b Sig2o SSE SST1 SST2 v V1 v1 We Wi Wi1 Wio WY X XB
tempname Vec vh VM VP VQ Vs W W1 W2 Wald V Y Yh Yi YYm YYv Z kb kb2 kbc1 kb1 kbc2
tempname N DF kx kb NC NT Nmiss llf X0 Ue WS D WW eVec Xo mh Zo Sig2n rRo
tempname kxd kbd Sig21 sigox Dim wmat xAx xAx2 TRa2 wWw1 xBx
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=int(_N/`NC')
...
di _dup(78) "{bf:{err:=}}"
qui spdhp `yvar' `SPXvar' , nc(`nc') id(`idv') `fe' `re' `be' `noconstant'
matrix `Beta'=e(b)
...
program define spdhp , eclass byable(recall)
version 11.0
...
qui foreach x of local xvar {
 ...
 qui replace `X0' = 1 - _b[_cons] if `touse'
...
if "`noconstant'"!="" {
 ...
 qui matrix accum `xy2' = `ldy' `ldy2' `X0' if `touse' , `noconstant'
 }
 matrix `By'= `xy2'[2,1]/`xy2'[1,1]
 local  By1 = `By'[1,1]
 qui gen `E'= `ldy2' - `By1' * `ldy' if `touse'
 qui gen `ldye' = `ldy' * `E'
