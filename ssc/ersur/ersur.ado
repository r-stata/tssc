...
global fileref "`c(sysdir_plus)'e/ersur.mtx"

tempname cbar rhobar
tempvar  glsy y trd glstrd glscons sspl
...
// Run the ers regression with no lags (no deterministic components necessary)

quietly regress D.`y'  L1.`y' if `touse', noconstant
...
quietly replace `ers'  = _b[L1.`y']/_se[L1.`y']                            in 1

// Run the ers regression augmented with lags (no deterministic components necessary)

if `maxlag'>0 {

forvalues i=1/`maxlag' {
   quietly regress D.`y'  L1.`y' L(1/`i')D.`y' if `touse', noconstant
   local ii = `i' + 1
   ...
   quietly replace `ers'  = _b[L1.`y']/_se[L1.`y']                                    in `ii'
   }
}
...
local ersm1 = `ers'[`store_fix'+1]
local ersm2 = `ers'[`store_aic'+1]
local ersm3 = `ers'[`store_sic'+1]
local ersm4 = `ers'[`store_gts05'+1]
local ersm5 = `ers'[`store_gts10'+1]

// Compute the 1, 5 and 10% critical values along with the associated p-value of the ers statistic.
...
// Response surface coefficients are retrieved from the Stata file "ersur.mtx"

   loc qhl 1 `t1' `t2' `t3' `t4' `p1t' `p2t' `p3t' `p4t'

   loc ersv `ersm1' `ersm2' `ersm3' `ersm4' `ersm5'
 
   mata: ersur1("`qhl'", "`ersv'")
   mat `res' = ___res
   ...
   foreach w of local lbl  {
		...
		mat `res'[`i',2] = `ers'[`store_`w''+1]
	}

if "`print'" != "noprint" {

display as result _n "Elliott, Rothenberg & Stock (1996) test results for `minp' - `maxp'"
...
display "Criteria{col 12}Lags{col 20}ERS stat.{col 33}p-value{col 45}1% cv{col 57}5% cv{col 66} 10% cv"
display as text "{hline 74}"
...
mat colnames `res' = Lags ERS_stat p-value critval01 critval05 critval10
