...
di
di as err "{bf:r2reg3} {cmd:works only after:} {bf:reg3, sureg}"
 exit
 }

tempvar E`var' Yb_Y`var' R4S SYY
...
scalar `R2_'`i'=e(r2_`i')
 ...
 egen `SYY' = varprod(`Yb_Y'`var'*) if `touse'
 sum `SYY' if `touse' , meanonly
scalar `SY'=r(mean)
matrix `Trm'=trace(inv(`Omega'))*`SY'
scalar `R'5=1-(`Q'/`Trm'[1,1])
 gen double `R4S' = . if `touse'
scalar `R'4 = 0
 forvalues i =1/`Q' {
matrix `YM'=`Y'[1..`N', `Q'..`Q']
...
scalar `R4'`i'=`R2Mat'[1,1]
 replace `R4S' = `R4'`i' if `touse'
 sum `R4S' if `touse' , meanonly
 replace  `R4S' = r(mean) if `touse'
...
di
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:* Simultaneous Equations (3SLS-SUR) - Method = (`e(method)')}"
di as txt "{bf:{err:* (3SLS-SUR) Overall System R2 - Adjusted R2 - F Test - Chi2 Test}}"
di _dup(78) "{bf:{err:=}}"
...
matrix rownames `RSQ' = Berndt McElroy Judge Dhrymes Greene
