...
*! Fit Singh-Maddala distribution by ML to unit record data


/*------------------------------------------------ playback request */
 
program define smfit, eclass byable(onecall)
	version 8.2
	if replay() {
		if "`e(cmd)'" != "smfit" {
			noi di as error "results for smfit not found"
			exit 301
		}
		if _by() { 
			error 190 
...
program define Estimate, eclass byable(recall)
	...
	syntax varlist(max=1) [if] [in] [aw fw pw iw] [,  ///
		Avar(varlist numeric) Bvar(varlist numeric) Qvar(varlist numeric) ///
		ABQ(varlist numeric) CDF(namelist max=1) PDF(namelist max=1) POORfrac(real 0) ///
		Robust Cluster(varname) SVY STats   From(string) ///
		Level(integer $S_level)    ///
		noLOG * ]


	local title "ML fit of Singh-Maddala distribution"

	local inc "`varlist'"

        if "`abq'" ~= "" & (("`avar'"~="")|("`bvar'"~="")|("`qvar'"~="")) {
                di as error "Cannot use abpq(.) option in conjunction with avar(.), bvar(.), qvar(.) options"
                exit 198
        }
	...
	marksample touse 
	markout `touse' `varlist' `avar' `bvar' `qvar' `cluster'
	if "`svy'" ~= "" {
		svymarkout `touse'
	...
	if "`avar'"=="" & "`bvar'"=="" & "`qvar'"=="" {
		...
		if "`cdf'" ~= "" {		 	
			qui ge `cdf' = 1 - ( 1/(1 + (`inc'/`b')^`a')^`q' ) if `touse'
...
program define Display
	...
	if `poorfrac' > 0 & (`e(nocov)' == 1) {
		di " "
		di "Fraction with {res: `e(depvar)'} < `poorfrac' = " as res %9.5f 1 - ( 1/(1 + (`poorfrac'/`e(bb)')^`e(ba)')^`e(bq)' )	
		di " "
	}
