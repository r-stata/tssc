*! Endogenous-treatment Sample Selection Poisson by MSL
*! Version 4.0
*! Author: Alfonso Miranda
*! Date: 06.01.2011

capture program drop setpoisson
program define setpoisson, eclass
version 11
 if replay() {
  if "`e(cmd)'" != "setpoisson" {
   error 301
...
if "`hbased'" == "" {
 if "`evaluate'" != "" {
  #delimit ;
   capture ml model d2 setpoisson_ll $S_eqs
   $S_slambda $S_sigma_u, init(`b0', copy)
   `mlopts'  `l0' search(off) `trace' missing negh
  ...
  #delimit ;
   ml model d2 setpoisson_ll $S_eqs
   $S_slambda $S_sigma_u, init(`b0', copy)
    missing `constraints' `options' negh
   max `trace' search(off) `l0'  
   nopreserve ;
  #delimit cr
 }
}
 ...
 if "`evaluate'" != "" {
  #delimit ;
   capture ml model d1 setpoisson_ll $S_eqs
   $S_slambda $S_sigma_u, init(`b0', copy)
  ...
  #delimit ;
   ml model d1 setpoisson_ll $S_eqs
   $S_slambda $S_sigma_u, init(`b0', copy)
   `options'  `l0' search(off) `trace' missing 
...
ereturn scalar rho_Sy = `rho_2'
ereturn scalar rho_Sy_se = sqrt(`rhoV'[2,2])
ereturn scalar arho_Sy = `arho_2'
ereturn scalar arho_Sy_se = `arho_2_se' 
ereturn scalar rho_TS = `rho_12'
ereturn scalar rho_TS_se = sqrt(`rhoV'[3,3])
ereturn scalar arho_TS = `arho_12'
ereturn scalar arho_TS_se = `arho_12_se'
...
#delimit ;
di as text "Selection Endogenous Treatment Poisson Model by MSL";
...
pr label(rho_TS) ci(atanh);
#delimit cr
di in smcl in gr "{hline 13}{c +}{hline 64}"
_diparm sigma_u, exp prob label(sigma_u) 
di in smcl in gr "{hline 13}{c BT}{hline 64}"
di as text "Wald test for rho_Ty=rho_Sy=rho_TS=0:" _col(40) as txt "chi2(" as res "3" as txt") = " as res `fmt' `Wrho' /*
*/ in gre " Prob > chi2 = "  in ye %5.4f `pWrho'
di in smcl in gr "{hline 13}{hline 65}"
di as text "Note: Endogenous treatment  eqn  reported  in  top panel and selection  dummy"
